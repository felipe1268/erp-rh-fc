...
export async function ensureAlmoxItemForEquipamento(
  ...
  params: {
    ...
    failOnError?: boolean;
    connectionIsTransaction?: boolean;
  },
): Promise<{ itemId: number; acao: "criado" | "transferido" | "nenhum" } | null> {
  if (!params.obraId) return null;
  try {
    const run = async (conn: any) => {
      // O lock vem antes da leitura. Quando recebe a conexão principal, este
      // helper abre uma transação própria para manter o xact lock até o INSERT.
      await conn.execute(sql`SELECT pg_advisory_xact_lock(${params.companyId}::int, 1010::int)`);
      const existing: any = await conn.execute(sql`
      ...
      if (row) {
        const acao: "nenhum" | "transferido" =
          Number(row.obra_id) === Number(params.obraId) ? "nenhum" : "transferido";
        await conn.execute(sql`
          ...
          SET obra_id = ${params.obraId},
              ...
              quantidade_atual = CASE
                WHEN EXISTS (
                  SELECT 1 FROM warehouse_loans wl
                  WHERE wl.item_id = ${Number(row.id)}
                    AND wl.company_id = ${params.companyId}
          ...
          WHERE id = ${row.id}
        `);
        return { itemId: Number(row.id), acao };
      }
      ...
      const inserted: any = await conn.execute(sql`
        WITH base AS (
        ...
        RETURNING id
      `);
      return { itemId: Number(inserted?.rows?.[0]?.id), acao: "criado" as const };
    };
    return params.connectionIsTransaction || typeof db.transaction !== "function"
      ? await run(db)
      : await db.transaction(async (tx: any) => run(tx));
  } catch (e: any) {
...
export async function backfillAlmoxFromEquipamentos(
  ...
  if (present.has("equipamentos_locados")) {
    try {
      ...
      for (const { company_id: companyId } of companyIds?.rows ?? []) {
        const r: any = await db.transaction(async (tx: any) => {
  ...
  if (present.has("equipamentos_proprios")) {
    try {
      // Reativa vínculos existentes sem trocar seu ID. Se houver empréstimo
      // ainda aberto, o saldo permanece zero até a devolução.
      await db.execute(sql`
        UPDATE almoxarifado_itens ai
        SET obra_id = ep.localizacao_atual_obra_id,
            ativo = true,
            quantidade_atual = CASE
              WHEN EXISTS (
                SELECT 1 FROM warehouse_loans wl
                WHERE wl.item_id = ai.id
                  AND wl.company_id = ai.company_id
                  AND wl.status = 'emprestado'
              ) THEN 0
              ELSE 1
            END,
        ...
        WHERE ai.equipamento_vinculado_tipo = 'proprio'
          ...
          AND ai.company_id = ep.company_id
          AND COALESCE(ep.ativo, true) = true
          AND COALESCE(ep.status, 'disponivel') NOT IN ('manutencao', 'baixado')
...
export async function removeAlmoxItemForEquipamento(
  db: any,
  params: { companyId: number; tipo: "proprio" | "locado"; equipamentoId: number; failOnError?: boolean },
): Promise<number> {
  try {
...
/**
 * Preserva o ID do item e o histórico de empréstimos, mas impede novas
 * retiradas enquanto o equipamento não estiver disponível.
 */
export async function disableAlmoxItemForEquipamento(
  db: any,
  params: { companyId: number; tipo: "proprio" | "locado"; equipamentoId: number; failOnError?: boolean },
): Promise<number> {
  try {
    ...
    console.error("[disableAlmoxItemForEquipamento] falhou:", e?.message || e);
    if (params.failOnError) throw e;
    return 0;
  }
}
  ...
  if (present.has("equipamentos_proprios")) {
    try {
      const stale: any = await db.execute(sql`
        ...
        RETURNING id
      `);
      const disabled: any = await db.execute(sql`
        UPDATE almoxarifado_itens ai
        SET ativo = false,
            quantidade_atual = 0,
            atualizado_em = NOW()
        ...
        WHERE ai.equipamento_vinculado_tipo = 'proprio'
          ...
          AND (ai.ativo IS DISTINCT FROM false OR COALESCE(ai.quantidade_atual, 0) <> 0)
        RETURNING ai.id
      `);
      propriosRemovidos = (stale?.rows?.length ?? 0) + (disabled?.rows?.length ?? 0);
    } catch (e: any) {
