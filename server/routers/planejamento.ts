...
export const planejamentoRouter = router({
  ...
  // ── Rev. 1808 — Diagnóstico SOMENTE-LEITURA de avanços ────────────────────
  // Não escreve nada. Mostra: total de avanços por revisão, quantos são órfãos
  // (atividade_id sem correspondência em planejamento_atividades), última
  // semana lançada, último timestamp/usuário e audit log recente do projeto.
  diagnosticarAvancos: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (String(proj.companyId) !== String((ctx as any).user?.companyId))
        throw new Error("Sem permissão para diagnosticar este projeto.");

      // Revisões + contagens de atividades
      const revisoes = await db.select().from(planejamentoRevisoes)
        .where(eq(planejamentoRevisoes.projetoId, input.projetoId))
        .orderBy(asc(planejamentoRevisoes.numero));

      // Estatísticas por revisão (total avanços, órfãos, semanas distintas, último timestamp/usuário)
      const porRevisao: Array<{
        revisaoId: number;
        ...
        primeiraSemana: string | null;
        ultimaSemana: string | null;
        ultimoCriadoEm: string | null;
        ultimoCriadoPor: string | null;
        atividadesNaRevisao: number;
        atividadesIdsRefMin: number | null;
        atividadesIdsRefMax: number | null;
        avancosIdsRefMin: number | null;
        avancosIdsRefMax: number | null;
      }> = [];
      ...
      for (const rev of revisoes) {
        const stats = await db.execute(sql`
          SELECT
            COUNT(*)::int                                 AS total,
            COUNT(DISTINCT semana)::int                   AS semanas_distintas,
            MIN(semana)::text                             AS primeira_semana,
            MAX(semana)::text                             AS ultima_semana,
            MAX(criado_em)::text                          AS ultimo_criado_em,
            MIN(atividade_id)::int                        AS av_min_aid,
            MAX(atividade_id)::int                        AS av_max_aid
          FROM planejamento_avancos
          WHERE projeto_id = ${input.projetoId}
            AND revisao_id = ${rev.id}
        `);
        const s: any = (stats as any).rows?.[0] ?? (stats as any)[0] ?? {};

        const orfaosRes = await db.execute(sql`
          SELECT COUNT(*)::int AS orfaos
          FROM planejamento_avancos av
          WHERE av.projeto_id = ${input.projetoId}
            AND av.revisao_id = ${rev.id}
            AND NOT EXISTS (
              SELECT 1 FROM planejamento_atividades a
              WHERE a.id = av.atividade_id
            )
        `);
        const o: any = (orfaosRes as any).rows?.[0] ?? (orfaosRes as any)[0] ?? {};

        const ultimoUserRes = await db.execute(sql`
          SELECT criado_por
          FROM planejamento_avancos
          WHERE projeto_id = ${input.projetoId}
            AND revisao_id = ${rev.id}
          ...
          LIMIT 1
        `);
        const u: any = (ultimoUserRes as any).rows?.[0] ?? (ultimoUserRes as any)[0] ?? {};

        const ativStats = await db.execute(sql`
          SELECT COUNT(*)::int AS total, MIN(id)::int AS amin, MAX(id)::int AS amax
          FROM planejamento_atividades
          WHERE revisao_id = ${rev.id}
        `);
        const a: any = (ativStats as any).rows?.[0] ?? (ativStats as any)[0] ?? {};

        porRevisao.push({
          revisaoId: rev.id,
          revisaoNumero: rev.numero ?? null,
          revisaoStatus: rev.status ?? null,
          totalAvancos: Number(s.total ?? 0),
          orfaos: Number(o.orfaos ?? 0),
          semanasDistintas: Number(s.semanas_distintas ?? 0),
          primeiraSemana: s.primeira_semana ? String(s.primeira_semana).slice(0, 10) : null,
          ultimaSemana: s.ultima_semana ? String(s.ultima_semana).slice(0, 10) : null,
          ultimoCriadoEm: s.ultimo_criado_em ?? null,
          ultimoCriadoPor: u.criado_por ?? null,
          atividadesNaRevisao: Number(a.total ?? 0),
          atividadesIdsRefMin: a.amin != null ? Number(a.amin) : null,
          atividadesIdsRefMax: a.amax != null ? Number(a.amax) : null,
          avancosIdsRefMin: s.av_min_aid != null ? Number(s.av_min_aid) : null,
          avancosIdsRefMax: s.av_max_aid != null ? Number(s.av_max_aid) : null,
        });
      }

      // Audit log recente do projeto (planejamento) — últimas 72h
      const sinceIso = new Date(Date.now() - 72 * 3600 * 1000).toISOString();
      const auditRaw = await db.execute(sql`
        SELECT id, user_name, action, entity_type, entity_id, details, created_at::text
        FROM audit_logs
        WHERE module = 'planejamento'
          AND created_at >= ${sinceIso}
          AND (
            entity_id = ${input.projetoId}
            OR details ILIKE ${'%projeto ' + input.projetoId + '%'}
            OR details ILIKE ${'%projetoId":' + input.projetoId + '%'}
            OR details ILIKE ${'%projetoId: ' + input.projetoId + '%'}
          )
        ORDER BY created_at DESC
        LIMIT 50
      `);
      const auditRows: any[] = (auditRaw as any).rows ?? (auditRaw as any) ?? [];

      // Top 10 últimos avanços (de qualquer revisão) com seus contextos
      const ultimosAvRaw = await db.execute(sql`
        SELECT id, revisao_id, atividade_id, semana::text, percentual_acumulado,
               criado_em::text, criado_por
        FROM planejamento_avancos
        WHERE projeto_id = ${input.projetoId}
        ORDER BY criado_em DESC NULLS LAST
        LIMIT 10
      `);
      const ultimosAvancos: any[] = (ultimosAvRaw as any).rows ?? (ultimosAvRaw as any) ?? [];

      // Total agregado
      const totalAgregado = porRevisao.reduce((s, r) => s + r.totalAvancos, 0);
      const orfaosAgregado = porRevisao.reduce((s, r) => s + r.orfaos, 0);

      return {
        projeto: { id: proj.id, nome: proj.nome, companyId: proj.companyId },
        totalAvancos: totalAgregado,
        totalOrfaos: orfaosAgregado,
        porRevisao,
        ultimosAvancos: ultimosAvancos.map(r => ({
          ...r,
          semana: r.semana ? String(r.semana).slice(0, 10) : null,
        })),
        auditRecente: auditRows.map(r => ({
          id: r.id,
          userName: r.user_name,
          action: r.action,
          entityType: r.entity_type,
          entityId: r.entity_id,
          details: r.details ? String(r.details).slice(0, 500) : null,
