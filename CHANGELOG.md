...
- **Financeiro › Contas a Receber — matriz agora reflete o mês de RECEBIMENTO (não mais a competência)**: o `getContasReceberMatrix` agora desloca cada valor previsto da medição para o mês em que o cliente efetivamente paga, usando o mesmo cálculo do Planejamento (`competência → data de corte do mês + N dias úteis`). O **SINAL** cai no mês da "Data Prev. Pagto do Sinal" (sem somar prazo, pois é antecipado). A **retenção liberada** também sofre o prazo. Vale para o modo *Por Avanço Físico* (parcela_fixa não é alterado). Resultado: a coluna do mês na matriz mostra o que realmente vai entrar no caixa naquele mês — alinhada com o cronograma exibido em Planejamento › Detalhe.
...
- **Gestão de Documentos — criar várias disciplinas de uma vez**: no modal **Nova Pasta**, os atalhos de disciplina (Arquitetura, Estrutural, Elétrica, etc.) agora suportam **seleção múltipla**. Clique em quantas disciplinas quiser — cada atalho marcado fica com borda azul e um ✓ no canto. O botão **Criar Pasta** vira **"Criar N Pastas"** e cria todas em sequência, todas com as mesmas sub-pastas definidas (DWG/PDF/IFC/DOC por padrão). Mostra o progresso "Criando X/N..." e ignora as que já existem. O modo de criação manual (Nome/Sigla/Cor) continua funcionando quando nenhum atalho está selecionado.
...
- **Fix — Falso aviso "sem orçamento vinculado" durante carregamento**: quando o cliente Vite recompilava antes do servidor reiniciar, a resposta antiga (sem o novo campo `orcamentoIdProj`) fazia a tela exibir incorretamente "⚠ Projeto sem orçamento vinculado" mesmo em projetos que tinham vínculo. Agora o componente diferencia "campo undefined" (API antiga / dados ainda carregando — exibe a mensagem neutra) de "campo === null" (projeto realmente sem vínculo).
- **TOTAL — coluna Retenção mostra apenas o crédito final** e **bate com a linha de Liberação**: antes mostrava `−R$ X / +R$ X` que se anulavam visualmente. Agora mostra só o **valor positivo final** em verde, **incluindo a retenção do Sinal** (quando "Reter também do Sinal" está marcado) — assim o TOTAL bate exatamente com o valor da linha **RETENÇÃO** (Liberação da Retenção). Tooltip: "Crédito a receber: total retido durante a obra (mensal + sinal, quando aplicável) que será liberado após a conclusão".
- **Fix — Retenção do Sinal agora aparece visualmente na tabela**: quando "Reter também do Sinal" está marcado, o valor da retenção (ex.: −R$ 2.115,94) já era abatido corretamente do Líquido da linha SINAL, mas a célula da coluna **− Ret. %** estava fixa em "—", o que dava a impressão de que a retenção não estava sendo aplicada. Agora a célula mostra o valor descontado em vermelho, igual às demais linhas mensais.
- **Fix CRÍTICO — `getConfigMedicao` quebrando para todos os projetos**: a coluna `reter_sinal` (criada na Rev. 1344) **não foi aplicada no banco**, porque o `ColFix` correspondente estava dentro de um bloco condicional que só roda quando há nova versão de schema. Resultado: `SELECT * FROM planejamento_medicao_config` falhava com `column "reter_sinal" does not exist`, fazendo a Configuração de Medição (Sinal, F.D., Retenção, Data, Bloqueio) **resetar para os valores padrão na tela** mesmo com config salva no banco. Coluna criada manualmente no Neon e o `SyncSchema+` (que **sempre** roda no boot) agora também garante `reter_sinal`. A configuração salva (sinal=10, fd_valor=82811.11 do projeto VITRA) volta a ser exibida corretamente.
...
- **Aditivo PJ — Auto-save de rascunho + edição após gerar**: dois reforços no fluxo de criação de aditivos contratuais PJ para evitar perda de trabalho:
  ...
  - **Editar Aditivo após gerar**: na tela de visualização do aditivo (`/contrato-pj/:id/aditivo/:aditivoId`), agora há um botão **"Editar Aditivo"** no cabeçalho. Ele abre um modal que permite: alterar a **data** do aditivo, **editar o texto** de qualquer cláusula já incluída, **remover** uma cláusula e **adicionar** outras cláusulas do contrato original que ficaram de fora. As alterações **atualizam o mesmo aditivo** (não criam um novo) — útil para corrigir erros de digitação ou completar informações antes de imprimir/emitir o documento. Backend ganhou a mutation `pj.aditivos.update`.
