...
export const payrollPeriods = mysqlTable("payroll_periods", {
	...
	status: mysqlEnum(['aberta','ponto_importado','aferida','vale_gerado','pagamento_simulado','consolidada','travada']).default('aberta').notNull(),
	...
	afericaoRealizada: tinyint().default(0).notNull(),
...
export const timecardDaily = mysqlTable("timecard_daily", {
	id: int().autoincrement().notNull(),
	companyId: int().notNull(),
	employeeId: int().notNull(),
	data: date({ mode: 'string' }).notNull(),
	mesCompetencia: varchar({ length: 7 }).notNull(),
	statusDia: mysqlEnum(['registrado','escuro','aferido']).default('registrado').notNull(),
	entrada1: varchar({ length: 10 }),
	saida1: varchar({ length: 10 }),
	entrada2: varchar({ length: 10 }),
	saida2: varchar({ length: 10 }),
	...
	horasExtras: varchar({ length: 10 }),
	horasNoturnas: varchar({ length: 10 }),
	isFalta: tinyint().default(0).notNull(),
	isAtraso: tinyint().default(0).notNull(),
	isSaidaAntecipada: tinyint().default(0).notNull(),
	minutosAtraso: int().default(0),
	minutosSaidaAntecipada: int().default(0),
	tipoDia: mysqlEnum(['util','sabado','domingo','feriado','compensado']).default('util').notNull(),
	timeRecordId: int(),
	obraId: int(),
	origemRegistro: varchar({ length: 20 }).default('dixi').notNull(),
	numBatidas: int().default(0),
	isInconsistente: tinyint().default(0).notNull(),
	...
	atestadoId: int(),
	advertenciaId: int(),
	obraSecundariaId: int(),
	rateioPercentual: int(),
	statusAnterior: mysqlEnum(['registrado','escuro','aferido']),
	afericaoResultado: mysqlEnum(['ok','falta','atraso','saida_antecipada','divergencia']),
	afericaoObs: text(),
	afericaoEm: timestamp({ mode: 'string' }),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
...
export const payrollAdvances = mysqlTable("payroll_advances", {
	...
	bloqueado: tinyint().default(0).notNull(),
	motivoBloqueio: varchar({ length: 255 }),
	faltasNoPeriodo: int().default(0),
	valorHora: varchar({ length: 20 }),
	cargaHorariaDiaria: int().default(8),
	diasUteisNoMes: int(),
	status: mysqlEnum(['calculado','aprovado','pago','cancelado']).default('calculado').notNull(),
	dataPagamento: date({ mode: 'string' }),
	aprovadoPor: varchar({ length: 255 }),
	aprovadoEm: timestamp({ mode: 'string' }),
	bancoDestino: varchar({ length: 100 }),
...
export const payrollPayments = mysqlTable("payroll_payments", {
	...
	status: mysqlEnum(['simulado','consolidado','pago','cancelado']).default('simulado').notNull(),
	dataPagamento: date({ mode: 'string' }),
...
export const payrollAdjustments = mysqlTable("payroll_adjustments", {
	...
	data: date({ mode: 'string' }).notNull(),
	tipo: mysqlEnum(['falta','atraso','saida_antecipada','he_nao_autorizada','outro']).notNull(),
	descricao: text(),
	...
	timecardDailyId: int(),
	paymentId: int(),
	status: mysqlEnum(['pendente','aplicado','abonado','cancelado']).default('pendente').notNull(),
	abonadoPor: varchar({ length: 255 }),
	abonadoEm: timestamp({ mode: 'string' }),
	motivoAbono: text(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
...
export const financialEvents = mysqlTable("financial_events", {
	id: int().autoincrement().notNull(),
	companyId: int().notNull(),
	tipo: mysqlEnum(['saida_vale','saida_pagamento','saida_he','saida_vr','saida_vt','saida_encargos','entrada_desconto','ajuste']).notNull(),
	...
	valor: varchar({ length: 20 }).notNull(),
	status: mysqlEnum(['previsto','consolidado','pago','cancelado']).default('previsto').notNull(),
	employeeId: int(),
	employeeName: varchar({ length: 255 }),
	obraId: int(),
	obraNome: varchar({ length: 255 }),
...
export const payrollAlerts = mysqlTable("payroll_alerts", {
	...
	companyId: int().notNull(),
	mesReferencia: varchar({ length: 7 }).notNull(),
	tipo: mysqlEnum(['ponto_nao_importado','vale_nao_gerado','pagamento_pendente','divergencias_aferidas','funcionario_bloqueado','prazo_proximo']).notNull(),
	titulo: varchar({ length: 255 }).notNull(),
	descricao: text(),
	prioridade: mysqlEnum(['baixa','media','alta','critica']).default('media').notNull(),
	lido: tinyint().default(0).notNull(),
...
export const contractTemplates = mysqlTable("contract_templates", {
	id: int().autoincrement().notNull(),
	companyId: int().notNull(),
	tipo: mysqlEnum(['experiencia', 'indeterminado', 'prorrogacao']).notNull(),
	nome: varchar({ length: 255 }).notNull(),
...
export const employeeContracts = mysqlTable("employee_contracts", {
	...
	templateId: int(),
	tipo: mysqlEnum(['experiencia', 'indeterminado', 'prorrogacao']).notNull(),
	status: mysqlEnum(['vigente', 'prorrogado', 'efetivado', 'encerrado', 'rescindido']).default('vigente').notNull(),
	dataInicio: date({ mode: 'string' }).notNull(),
	dataFim: date({ mode: 'string' }),
