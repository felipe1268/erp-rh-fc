...
- **Rev. 2904** — **INSTALAR NO CELULAR (PWA) — O BANNER "INSTALAR" AGORA APARECE NO iPad/iPhone (SAFARI), NÃO SÓ NO PC.** Pedido: "Não está aparecendo para baixar no celular... só aparece no PC, nem no tablet." O banner dependia 100% do evento `beforeinstallprompt`, EXCLUSIVO de Chromium (Chrome/Edge/Android) — o Safari do iPad/iPhone NUNCA dispara esse evento, então o banner jamais aparecia em iOS/iPadOS (o grosso dos usuários FC usa iPad). SOLUÇÃO (SÓ CLIENT — ZERO schema/ALTER/DROP/DELETE) em `client/src/components/PwaInstallBanner.tsx`: detecta iOS/iPadOS (UA `iPad|iPhone|iPod` + iPadOS 13+ que se identifica como `MacIntel` com `maxTouchPoints>1`) e, sem `beforeinstallprompt`, mostra banner com INSTRUÇÕES manuais (Compartilhar → "Adicionar à Tela de Início"); mantém o botão automático do Android/Chrome; some quando já instalado (`display-mode: standalone`/`navigator.standalone`); "Fechar" persiste em `sessionStorage`. RESSALVA: no iOS a Apple não permite instalar por botão (daí a instrução manual). SÓ CLIENT; ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
