...
- **Rev. 2860** — **COLETA DE CAMPO (RH) — "GERAR TODOS" + "COPIAR TODOS": GERA UM LINK PARA TODAS AS OBRAS ATIVAS DE UMA VEZ E COPIA A LISTA (OBRA → LINK) PRONTA PARA ENVIAR A CADA RESPONSÁVEL.** Pedido (tela `/coleta-campo`): "quero poder gerar link para todas obras ativas de uma vez... depois só copio e envio para cada responsável." Antes era 1 link por vez. BACKEND (`server/routers/coletaRh.ts`): NOVO endpoint protegido `criarSessoesTodas` (mesmo guard anti-IDOR `assertColetaCompanyAccess` + mesmo filtro canônico de obras ativas de `obrasDisponiveis`); IDEMPOTENTE — obra que já tem link ATIVO e não-expirado é REAPROVEITADA (não duplica); retorna `{criadas, reaproveitadas, totalObras}`. FRONTEND (`client/src/pages/ColetaCampo.tsx`): NOVO card "Gerar para todas as obras ativas" com "Gerar todos" (chama `criarSessoesTodas`) + "Copiar todos" (monta `obraNome: link` de todos os links ativos/não-expirados no clipboard). Fluxo 1-a-1 intacto. ZERO ALTER/DROP/DELETE; ZERO schema (reusa tabelas da Rev. 2858). Detalhe: `shared/changelog.ts`.
