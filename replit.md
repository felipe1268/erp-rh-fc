...
- **Rev. 4564** — **FEAT: EQUIPAMENTOS — TAG DE LOCALIZAÇÃO DO FIXO (OBRA × ALMOXARIFADO).** Um fixo pode estar fisicamente no almoxarifado (desmontado). `localizacao` ('em_obra'|'no_almox') derivada em locadosUtilizacao/propriosUtilizacao via LEFT JOIN almoxarifado_itens + LATERAL do último warehouse_loan (empréstimo aberto = em obra; devolvido/nunca saiu = no almox; sem vínculo = em obra); badge indigo "Instalado na obra" × âmbar "No almoxarifado" na seção Instalados. Arquivos: `equipamentos.ts`, `LocadosUtilizacao.tsx`, `PropriosUtilizacao.tsx`. ZERO schema change.
