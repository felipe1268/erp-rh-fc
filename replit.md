...
- **Rev. 3156** — **FINANCEIRO / LANÇAMENTOS · OS LANÇAMENTOS CANCELADOS QUE VIERAM DE OUTRO MÓDULO E NUNCA FORAM PAGOS NO FINANCEIRO SUMIRAM DA LISTA — FICA SÓ O QUE FOI CANCELADO DENTRO DO PRÓPRIO FINANCEIRO OU O QUE TEVE BAIXA EM ALGUM MOMENTO.** PEDIDO (IMG_2094, filtro "Cancelado", iPad): a lista de cancelados vinha poluída por OCs/Compras/Frota canceladas LÁ na origem (nunca pagas) — "deixa só o que foi cancelado DENTRO do financeiro; o resto é poluição visual". REGRA (FRONTEND-ONLY, `FinanceiroLancamentos.tsx`): novo `isCanceladoRuido(l)` = `status==="cancelado"` E origem EXTERNA (`origemModulo` preenchido e fora de `recorrente`/`importacao_excel`; `null`=manual=nativo) E NUNCA teve baixa (`dataPagamento==null` e `valorRealizado` 0/nulo via `teveBaixaFinanceiro`). Descartados SEMPRE (`baseLancamentos`), antes de contagem/exibição — inclusive no filtro explícito "Cancelado" e no contador "Mostrar cancelados (N)". PERMANECEM: cancelados nativos do financeiro + qualquer cancelado com rastro de pagamento/realizado. Totais já ignoravam cancelado → esconder é puramente visual, números batem; mecânica da Rev. 3152/3154/3155 intacta sobre `baseLancamentos`. ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE — rastro segue no banco. Detalhe: `shared/changelog.ts`.
