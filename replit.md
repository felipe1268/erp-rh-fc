...
- **Rev. 3678** — **NFS-e EMITIDAS · CAMPOS COMPLETOS DO XML SIAP GEO: 12 NOVAS COLUNAS + DIALOG REDESENHADO. BACKEND ADITIVO + SCHEMA + FRONTEND · ZERO DROP/DELETE.** 12 novas colunas em `fiscal_notes`: retencao_csll/pis/cofins/outras, data_prestacao, cd_cnae, cd_lista_servico, optante_simples, tributada, tomador_inscricao/email/telefone. SyncSchema+ Rev.3678 faz 12× `ALTER TABLE ADD COLUMN IF NOT EXISTS`. Parser `parseSiapGeoExportXml` expandido + INSERT 28 campos. Dialog redesenhado com 5 seções scrolláveis (Identificação / Serviço / Tomador / Impostos / Vínculos), max-w-xl, max-h-[90svh]. Arquivos: `schema.ts`, `index.ts`, `nfseEmitidas.ts`, `FinanceiroNotasFiscais.tsx`. Detalhe: `shared/changelog.ts`.
...
- **Rev. 3671** — **NFS-e EMITIDAS · PROGRESSO 0–100% NO HISTÓRICO + CRONÔMETRO RATE-LIMIT + BUGFIX HTTP 404 SIAP GEO. BACKEND PONTUAL + FRONTEND · ZERO ALTER/DROP/DELETE.** (1) Progresso real: loop ano a ano 2018→2025 (8× `syncMunicipio`), barra verde avança a cada ano, percentual + contador de notas em tempo real. (2) Cronômetro âmbar "MM:SS p/ próxima consulta" no banner (rate-limit 55min prefeitura). (3) Bugfix 404: ON CONFLICT `getMunicipios` não atualizava `endpoint` → URL antiga pré-Rev.3652 persistia no DB → 404. Fix: `endpoint = EXCLUDED.endpoint`. Arquivo: `nfseEmitidas.ts`, `FinanceiroNotasFiscais.tsx`. Detalhe: `shared/changelog.ts`.
