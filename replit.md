...
- **Rev. 2585** — **PLANEJAMENTO · ABA "EFETIVO × IA" · CORRIGE A "TRAVA EM 95%" DO SIMULADOR DE MÃO DE OBRA NO iPad/iOS — A SIMULAÇÃO AGORA CONCLUI EM VEZ DE FICAR PENDURADA E CAIR EM ERRO.** User (screenshot, obra "HOTEL QIU 2 - 4 FASE", Rev. 0, 47 alocados): "Erro é trava no 95% arrume isso." A Rev. 2584 só TRADUZIU o sintoma — a falha de fundo (requisição não retornava) continuava. CAUSA-RAIZ: o `planoAtaque` (Rev. 2583) tornou a `simularEfetivo` a chamada de IA mais pesada da aba (`maxTokens: 8000` de JSON denso); o `invokeLLM` usa Claude Sonnet 4 NÃO-STREAMING, lento demais pra essa saída → a conexão HTTP única estoura o timeout do proxy/iOS ANTES de retornar → a Promise nunca resolve, a barra simulada satura em 95% e a mutation rejeita. O fallback Gemini também era lento (não desligava o "extended thinking" do 2.5). FIX (ADITIVO, ZERO SCHEMA/ALTER/DROP/DELETE): SERVER (`server/_core/llm.ts`) novo caminho rápido `invokeGeminiFast` (endpoint NATIVO `generateContent` do Gemini 2.5 Flash com `thinkingConfig.thinkingBudget=0` + `responseMimeType:"application/json"`, retornando o MESMO `InvokeResult`); `invokeLLM` ganha flag `fast?: boolean` que, com `GOOGLE_API_KEY`, tenta o caminho rápido PRIMEIRO e cai pro fluxo padrão (Claude→Gemini lento) se falhar — sem regressão pros demais chamadores. SERVER (`iaCronograma.ts`, `simularEfetivo`) passa `fast: true` → saída em segundos, cabe no timeout → CONCLUI. CLIENT (`AnaliseEfetivoIA.tsx`, `useProgressoSimulado`) a barra não para "dura" em 95% — segue crawl lento até 99% e salta pra 100% no sucesso. Validado via esbuild (server+client, exit 0). Detalhe: `shared/changelog.ts`.
