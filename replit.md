...
- **Rev. 4559** — **FIX/UX: ALMOXARIFADO — RENOVAÇÃO USA O FLUXO REAL (COMPRAS → FINANCEIRO) + HEADER 2 LINHAS + NÚMEROS pt-BR + PLURAL.** Causa-raiz do "renovar não funciona": o Almoxarifado ainda usava o modal legado da Rev. 4345 que só mudava `data_vencimento_locacao` via `compras.atualizarItem` (sem OC, sem Contas a Pagar, sem evento — badge ficava "1ª Locação"). Agora os 2 pontos de entrada (catálogo e modal Locações a Vencer) abrem modal novo que chama `equipamentos.locadoRenovar` (mesmo fluxo do Locados/alerta global), com passo-a-passo explicativo (OC no Compras → Contas a Pagar → vencimento/ciclo), vencimento sugerido +30d e valor da nova OC; item sem vínculo com Equipamentos Locados recebe aviso orientando a vincular. Extras: header em 2 linhas (ações | chips de alerta), plural "locações" corrigido e milhar pt-BR nos KPIs/contagens. Arquivo: `almoxarifado/index.tsx`. ZERO schema change.
- **Rev. 4558** — **FEAT: EQUIPAMENTOS LOCADOS — RENOVAÇÃO REAL (NOVA OC NO COMPRAS) + BADGE DE CICLO + REDESIGN DO ALERTA.** Nova mutation `equipamentos.locadoRenovar`: gera nova OC de locação no Compras (tipo `locacao`, auto-aprovada, encadeada via `locacao_oc_anterior_id`, segue fluxo normal até o Contas a Pagar), atualiza vencimento/valor do locado, grava evento RENOVACAO com numeroCiclo e sincroniza o item do almoxarifado — tudo em transação com advisory lock. `locadosListar` e `getItensLocadosVencendo` agora retornam `renovacoesCount` (+ foto/valor no alerta). UI: badge "1ª Locação / Nª Renovação" + badge de urgência nos cards, botão "Renovar" (card, modal de grupo) com dialog próprio, e `AlertaLocacoesVencendo` redesenhado com foto + renovação inline direto do alerta de login. Arquivos: `equipamentos.ts`, `compras.ts`, `Locados.tsx`, `AlertaLocacoesVencendo.tsx`. ZERO schema change (colunas de encadeamento já existiam).
### 5 one-liners
...
- **Rev. 4556** — **FIX: AVISO PRÉVIO — BANNER "CONCLUÍDO INCORRETAMENTE" CONTAVA TODOS OS CONCLUÍDOS.** Banner usa `concluidosSemBaixa` real (sem baixaRescisaoData E sem dataBaixa); `revertAllConcluidos` restringido a isNull(dataBaixa)+isNull(baixaRescisaoData). Arquivos: `AvisoPrevio.tsx`, `avisoPrevioFerias.ts`. ZERO schema change.
