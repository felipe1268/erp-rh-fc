...
- **Rev. 2183** — **NOVA FEATURE · Filtro por OBRA no Relatório de Períodos HE (Select acima dos cards KPI da Rev. 2182).** Lilian: "quero filtro tbm.. por obra para analisar..". **Backend (`server/routers/horasExtras.ts:getDetalhe`):** estendido pra retornar `obrasPorEmp: Array<{employeeId, obraId, obraNome}>` via query extra a `time_records` JOIN `obras` no range `[dataInicio, dataFim]` do período. **Frontend (`client/src/pages/FolhaPagamento.tsx`):** (1) state `heObraFilterMod` + useEffect zera junto com origem ao trocar período; (2) mapas `obrasMap`/`obrasDoPeriodo` + `obrasOptions` (sort pt-BR) alimentam `<select>`; (3) `periodEmpsAllRaw` (sem filtro) vs `periodEmpsAll` (filtrado por obra — alimenta KPIs cards Rev. 2182); (4) `<select>` "🏗️ Filtrar por obra" acima dos cards, com link "Limpar obra" + contador contextual. **Empilhamento:** obra → origem → tabela. Cards KPI agora refletem escopo da obra selecionada. **Limitação:** filtro é "funcionários que tocaram esta obra no período" (HE não tem alocação por obra). **R-001/R-007/R-010:** OK — só SELECT.
