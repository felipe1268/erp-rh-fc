...
- **Rev. 2773** — **EPI · TELA "ESTOQUE POR OBRA": OS CARDS DAS OBRAS E O CARD DO ESCRITÓRIO CENTRAL FICARAM CLICÁVEIS — AO CLICAR, A TABELA DETALHADA ABAIXO FILTRA E MOSTRA, ITEM A ITEM, O QUE ESTÁ NO ESTOQUE DAQUELE LOCAL.** Pedido (usuário): na aba "Estoque por Obra" da tela de EPIs, os cards (um por obra + o card do Escritório Central) só mostravam o RESUMO; queria clicar e ver, na tabela abaixo, CADA item daquele estoque. Fix (SÓ CLIENT; ZERO SCHEMA/SERVER) em `client/src/pages/Epis.tsx` (`viewMode==="estoque_obra"`): (1) novo memo `centralItensList` — o Central NÃO tem lista de itens no servidor (só resumo), então deriva do catálogo `episAllList` (epis c/ `quantidadeEstoque>0`), normalizado no shape das linhas de obra (com `epiId` p/ manter clique→editar); (2) novo memo `tabelaEstoqueList` decide a fonte: "central"→central, número→filtra `estoqueObraList2`, "todas"→todos de obra; (3) cards de obra E central ganharam `onClick` com TOGGLE (clica filtra; clica de novo volta p/ "todas") + `ring` de realce; (4) dropdown ganhou "🏢 Escritório Central"; (5) `showCentral` inclui `filterObraEstoque==="central"`; (6) empty-state e corpo da tabela usam `tabelaEstoqueList` (empty-state contextual Central vs obra); (7) `episAllQ.limit` 200→1000 p/ a lista do Central não ficar incompleta com catálogo grande. Bug corrigido na iteração: os memos referenciavam `filterObraEstoque` antes do `useState` (TDZ) → state movido p/ antes dos memos e duplicata removida. Validação: servidor sobe limpo; HMR ok; architect PASS. Detalhe: `shared/changelog.ts`.
- **Rev. 2772** — **ALMOXARIFADO · NOVA VISÃO "📍 SALDO POR OBRA": MOSTRA, OBRA A OBRA, TODOS OS INSUMOS ALOCADOS NO ESTOQUE — PRA AFERIR DE RELANCE ONDE TEM SALDO E ONDE ESTÁ ZERADO.** Pedido (usuário): faltava ver, por obra, quais insumos estão alocados e em que quantidade, sem abrir item por item. Fix (SÓ CLIENT; ZERO SCHEMA/SERVER) em `client/src/pages/almoxarifado/index.tsx`: o seletor ganhou "📍 Saldo por Obra" (`obraContexto="porObra"`); a query existente `listarItensConsolidado` (já respeita busca/categoria/estoque-baixo/equip via `consListFinal` E o guard de obras no backend) foi habilitada p/ "porObra" — ZERO endpoint novo. Novo bloco INVERTE "item→almoxarifados[]" p/ "obra→insumos", consolidando duplicados por `nome|unidade` (soma quantidade/valorTotal num `itensMap`), com filtro de saldo (todos/com/sem), seções colapsáveis por obra (Central primeiro), tabela Insumo/Categoria/Saldo/Vlr Unit/Vlr Total e destaque vermelho p/ zerados. Hardening tRPC: TODOS os payloads que derivam `obraId` de `obraContexto` passaram a usar guarda `typeof obraContexto === "number"` (nunca a string "porObra") — `listarItens` (+`enabled`), `identificarPorFoto`, `preencherIA`, `salvarItem`. Validação: servidor sobe limpo; HMR ok; architect PASS. Detalhe: `shared/changelog.ts`.
