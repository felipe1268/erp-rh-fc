...
- **Rev. 2895** — **MEDIÇÃO DE CONTRATOS — LEVANTAMENTO DE CAMPO AGORA É OFFLINE-FIRST NO TABLET (PWA): MEDIR/CONTORNAR/CALIBRAR/FOTOGRAFAR SEM INTERNET E SINCRONIZAR SOZINHO AO RECONECTAR, SEM PERDA E SEM DUPLICAR.** Pedido (Task #67, continuação da Rev. 2893): a tela de Levantamento de Campo precisa funcionar EM CAMPO (obra sem internet). SOLUÇÃO (ADITIVA, ZERO destrutivo; reaproveita o schema da Rev. 2893 que já tem `uuid`+`atualizado_em`+`deleted_at`): (1) SHARED `shared/levantamentoConsolidado.ts` — função PURA `consolidarContornos` (contornos → R$) usada no SERVIDOR e no CLIENTE (consolidação offline idêntica). (2) BACKEND `server/routers/medicao.ts` — NOVO `medicao.sincronizarLote`: fila de ≤500 ops (contorno/foto upsert+delete, pdf calibrar) aplicada IDEMPOTENTE (upsert por `uuid` ou `id`) com guard de tenant em CADA op; conflito = LAST-WRITE-WINS por `atualizadoEm` (servidor nunca sobrescreve versão mais nova; status por op `ok`/`conflito`/`erro`); `getConsolidadoCampo` refatorado p/ usar o shared. (3) CLIENTE sem deps: `client/src/lib/offlineDb.ts` (IndexedDB: `campoSnapshots`/`blobs`/`syncQueue` + estimate de espaço), `client/src/lib/levantamentoSync.ts` (fila + pub/sub + dreno automático online/offline + pré-download de PDFs/fotos + tRPC vanilla cookie-based), hook `client/src/hooks/useLevantamentoOffline.ts` (server + snapshot + overlay otimista da fila; resolve blob URLs offline). (4) FRONT `MedicaoLevantamento.tsx` — troca queries/mutations diretas pelo hook + barra de status (online/offline, pendentes, sync, conflitos, "Baixar p/ offline", uso de espaço). (5) PWA: `manifest.json` ganha `icons` 192/512; NOVO `client/public/sw.js` (network-first navegação + app shell offline, cache-first `/assets`, bypass `/api`); registro do SW em `main.tsx` SÓ em produção. Envio/exclusão de PDF e boletim seguem ONLINE. ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
- **Rev. 2894** — **COMPRAS — NUMERAÇÃO DE EXIBIÇÃO DAS OC/OS AGORA É "NÚMERO PRIMEIRO, ANO DEPOIS" (OC-2026-389 → OC-389-2026), IGUAL AO PADRÃO JÁ APLICADO EM SOLICITAÇÕES (SC) E COTAÇÕES (COT).** Pedido (usuário): a numeração de OC/OS aparecia como `OC-AAAA-NNN`; o padrão de leitura do ERP (já feito em SC e Cotações Rev. 2808) é mostrar o nº sequencial primeiro e o ano depois (`OC-389-2026`). SOLUÇÃO (SÓ exibição — ZERO valor canônico, ZERO schema, ZERO backend): (1) NOVO helper `shared/numeroOc.ts` — `formatNumeroOcDisplay(numero)`: regex `^(OC|OS)-(\d{4})-(\d+)$` → `${prefixo}-${seq}-${ano}`; valores não-canônicos ("RASCUNHO-...", null) retornam intactos (espelha `numeroCotacao.ts`/`numeroSc.ts`). (2) FRONT — wrap de TODOS os pontos de EXIBIÇÃO de `numeroOc`: `Ordens.tsx` (toast/lista/título), `Comissoes.tsx`, `Solicitacoes.tsx`, `Painel.tsx`, `PainelFd.tsx`, `Realocacao.tsx`, `Recebimentos.tsx`, `Cotacoes.tsx`, `SmartEntry.tsx`, `FinanceiroContasAPagar.tsx`, `ContratoDetalhe.tsx`, `Locados.tsx`. NÃO MUDA: valor canônico gravado (`OC/OS-AAAA-NNN`), geração/busca/ordenação/filtros (casam o canônico), PDF/comprovante do SERVIDOR + nome do arquivo (documentos formais não invertem — precedente cotações), e notas persistidas (ex.: `observacoes` em `Locados.tsx`). ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
