...
- **Rev. 2811** — **COMPRAS · SOLICITAÇÕES — HOTFIX: NO PDF DA SC ("GERAR PDF") AS QUANTIDADES APARECIAM COM "ESCALA EM MIL" (ex.: QTD 50 SAÍA COMO "50.000", QTD 1 COMO "1.000"). AGORA A COLUNA "QTD" É FORMATADA EM pt-BR ANTES DE IMPRIMIR.** Pedido (usuário, com screenshot da SC-2026-0320 → "Gerar PDF"): "Ao clicar em gerar pdf as unidades de medida estão com escala em mil". CAUSA-RAIZ: a coluna `quantidade` é numeric com escala 3 no banco, então o valor cru chega como string "50.000" (ponto = decimal). O gerador `gerarPdfSC` em `Solicitacoes.tsx` imprimia o valor BRUTO via `esc(it.quantidade)` — e em pt-BR o ponto vira separador de MILHAR, fazendo "50.000" parecer cinquenta mil. A tela de detalhe da SC já formatava certo; só o PDF ficou de fora. O QUE FOI FEITO: novo helper local `fmtQtd` (`parseFloat → toLocaleString("pt-BR", { maximumFractionDigits: 2 })`, guarda `Number.isFinite` → "—"); a célula "Qtd" passou de `esc(it.quantidade)` para `esc(fmtQtd(it.quantidade))` (50.000 → "50", 1.5 → "1,5"). RESSALVA: só o PDF da SC foi tocado; Ordens/Aprovações não tinham o mesmo render cru. ZERO ALTER/DROP/DELETE; ZERO schema novo; ZERO backend. Validação: esbuild OK no arquivo. Detalhe: `shared/changelog.ts`.
- **Rev. 2810** — **CONFIGURAÇÕES · IA — HOTFIX: O LIGA/DESLIGA DO ASSISTENTE DE PERGUNTAS E RESPOSTAS NÃO SALVAVA ("ERRO AO SALVAR CONFIGURAÇÃO DE IA" / O PAINEL VOLTAVA SEMPRE A "7 DE 7 ATIVAS"). O SCHEMA DRIZZLE DA TABELA `ai_module_config` ESTAVA DESALINHADO DA COLUNA REAL DO BANCO.** Pedido (usuário, com screenshot): "Não tá deixando desligar, eu clico e não acontece nada" — toast vermelho "Erro ao salvar configuração de IA". CAUSA-RAIZ: a tabela `ai_module_config` foi CRIADA pelo self-heal (Rev. 2805) com a coluna `company_id` (snake_case), mas no `drizzle/schema.ts` o campo estava `companyId: integer().notNull()` SEM nome explícito — e esta instância do Drizzle NÃO usa casing snake_case global, então gerava a coluna `"companyId"` (camelCase). TODA query à tabela falhava com `column "companyId" does not exist`: na LEITURA (`getQaConfig`) o erro era silencioso → `data` undefined → o front caía no default permissivo do catálogo estático → "7 de 7 ativas"; na ESCRITA (`setQaModulo`/`setQaTodos`) virava TRPCError → `onError` → toast. Por isso a tabela estava VAZIA (nenhuma escrita jamais persistiu). O QUE FOI FEITO: 1 LINHA — `companyId: integer().notNull()` → `companyId: integer("company_id").notNull()`, alinhando o ORM à coluna `company_id` que JÁ existe no Neon (as demais colunas já tinham nome explícito; `modulo`/`enabled`/`id` são idênticas). REGRAS FC: ZERO ALTER/DROP/DELETE — só o mapeamento do ORM foi corrigido. Validação: teste real via Drizzle contra o Neon (INSERT/READ/UPDATE/DELETE OK); esbuild OK no schema; servidor reiniciado (Neon OK). Detalhe: `shared/changelog.ts`.
