...
- **Rev. 3610** — **NF-e RECEBIDAS · BUGFIX BOTÃO "CONSULTAR NO SEFAZ" ABRIA PÁGINA INICIAL EM VEZ DA NOTA. 100% FRONTEND · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** Parâmetro `tipoConteudo=XmlNFe` não é reconhecido pelo portal nfe.fazenda.gov.br → redirecionava para a página inicial. Fix: `tipoConteudo=7PhJ%2BgAVw2g%3D` (base64 URL-encoded, identificador padrão do portal). Agora abre diretamente a página da nota com todos os dados: emitente, destinatário, itens, impostos e protocolo. Detalhe: `shared/changelog.ts`.

- **Rev. 3609** — **NF-e RECEBIDAS · BUGFIX CRÍTICO BACKFILL "115 NOTAS SEM XML" — ESTRATÉGIA NSU RESET + RESYNC EM VEZ DE consChNFe. BACKEND PONTUAL + FRONTEND · ZERO ALTER/DROP/DELETE.** `consChNFe` via DistDFeInt devolvia o mesmo `resNFe` de volta (SEFAZ distribui `resNFe` primeiro + `nfeProc` com NSU maior; se o NSU já avançou além, o `consChNFe` nunca entrega o XML). Fix: `executarSyncNFe` ganha `{ skipTimeGate, forceUltNSU }`; `recuperarXmlsBackfill` encontra o menor `nsu_sefaz` das notas sem XML, subtrai 50 posições de buffer, reseta `ultimo_nsu` no banco e chama o sync sem gate de tempo — o loop Rev.3605 faz UPDATE das notas existentes ao receber `nfeProc`. Detalhe: `shared/changelog.ts`.
