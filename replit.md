...
- **Rev. 3770** — **FINANCEIRO · MAPEAMENTO DE CONTAS FLUTUANTES: BATCH UPDATE EM PRODUÇÃO (7 GRUPOS / ~1.200 LANÇAMENTOS COM `conta_id IS NULL` RECEBERAM CONTA VINCULADA) + MAPA CANÔNICO `CONTA_ID_BY_NOME` + `resolveContaId` EM `financialIntegrationBridge.ts` (TODO NOVO INSERT VIA `insertEntry` RESOLVE `conta_id` AUTOMATICAMENTE) + `financialAutoImport.ts` CORRIGIDO (FOLHA CLT→506, PJ→391). PROJEÇÕES DE CRONOGRAMA MANTIDAS SEM CONTA (ANTI-DUPLA-CONTAGEM). ZERO SCHEMA/ALTER/DROP/DELETE.** Grupos mapeados: SALÁRIO MÃO DE OBRA (id 299→506), FIN/FINANCIAMENTOS/Financiamento→264, SERV→391, MARKETING→9, JURÍDICA→271, Combustíveis→384, Vale Refeição+Alimentação→265. Grupos excluídos intencionalmente: "Custos Diretos de Obra" e "Custos Indiretos" (origem `cronograma_atividade` = projeções que duplicariam o custo real). Detalhe: `shared/changelog.ts`.
