...
- **Rev. 2814** — **COMPRAS · PAINEL FD — HOTFIX: AS OCs DE FATURAMENTO DIRETO CRIADAS DIRETO NA TELA DE ORDENS (SELO "FAT. DIRETO") NÃO APARECIAM NO "PAINEL DE FATURAMENTO DIRETO" DA OBRA. AGORA APARECEM.** Pedido (usuário, screenshot item "8. Visualização de AFD"): "No painel de FD não aparecem as FDs vinculadas à obra; lancei FD da obra REVTE e não aparece" (OC-2026-339, REVTE-CIVIL). CAUSA-RAIZ: a modalidade de FD é persistida com DOIS valores conforme o caminho — criação DIRETA da OC grava `modalidadeFd = 'fd_fc'` (enum do front `["normal","fd_cliente","fd_fc"]`); o caminho via cotação mapeia `fd_fc → fd_terceiro`. O `getSaldoFd` (que alimenta o painel) filtrava só `IN ('fd_cliente', 'fd_terceiro')`, deixando de fora toda OC FD criada direto (`fd_fc`). O QUE FOI FEITO: o filtro do `getSaldoFd` em `server/routers/compras.ts` passou a incluir `'fd_fc'` (`IN ('fd_cliente', 'fd_terceiro', 'fd_fc')`), cobrindo os dois formatos. O front (`PainelFd.tsx`) já trata modalidade ≠ `fd_cliente` como "FD Terceiro", sem mudança. `totalFdComprometido` segue contando só `fd_cliente` (inalterado). RESSALVA: só LEITURA/filtro. ZERO ALTER/DROP/DELETE; ZERO schema novo; ZERO front. Validação: esbuild OK em `compras.ts`. Detalhe: `shared/changelog.ts`.
