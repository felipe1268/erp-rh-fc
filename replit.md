...
- **Rev. 1670 — Fase 1**: **Planejamento — snapshot Texto10/Texto7 por atividade persistido no import (unificação MSP, fase 1/5)**. Primeira fase da unificação Rev. 1670 que tornará o XML do MS Project a fonte única de TEMPO e o módulo Orçamento a fonte única de R$. Esta fase é só persistência (zero mudança visual): o parser XML passou a extrair, por atividade, os ExtendedAttributes Texto10 (FieldID 188743750, %PREVISTO 4 casas — fórmula do Project) e Texto7 (FieldID 188743747, %Reali AUX), gravando em 2 colunas novas (`previsto_msp_pct`, `realizado_msp_pct` NUMERIC(8,4) nullable) em `planejamento_atividades`. Aplicado nos modos SUBSTITUIR (`salvarAtividades`) e MESCLAR (`importarComModo`). XLSX legado / XML sem ExtendedAttributes deixam null e o ERP segue no cálculo dinâmico atual. ColFix garante colunas em todo startup. Próximas fases: F2 (`getCurvaS` lê snapshot quando disponível), F3 (helpers únicos pvAt/evAt), F4 (Curva S Financeira via Orçamento + Texto5), F5 (modal auditoria MSP×ERP).
