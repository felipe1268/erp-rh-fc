...
- **Rev. 2582** — **PLANEJAMENTO · ABA "EFETIVO × IA" · REDESIGN MAIS DINÂMICO E MODERNO: (1) LEGENDA INTERATIVA + ASSISTENTE DE DÚVIDAS (Q&A COM A IA), (2) VÁRIOS GRÁFICOS ILUSTRATIVOS (PANORAMA DO EFETIVO) E (3) CARDS DE INSIGHT.** User: "deixar a aba mais dinâmica/moderna — uma legenda onde tiro dúvidas, vários gráficos ilustrativos e cards com insights." FIX (ADITIVO, ZERO SCHEMA/ALTER/DROP/DELETE): SERVER novo endpoint SOMENTE LEITURA `iaCronograma.perguntarEfetivo({projetoId,companyId,pergunta})` (após `getAnaliseEfetivo`) — reusa `coletarEfetivoCronograma`, mesma tenancy anti-IDOR, `invokeLLM` (maxTokens 1000) respondendo DIDÁTICO e curto SÓ com os dados da obra; não persiste. CLIENT (`AnaliseEfetivoIA.tsx`): `PerguntarIA` (Q&A chat-like com chips de sugestão, input+enviar, auto-scroll, "Pensando…") no fim do `Diagnostico`; `LegendaAjuda` (colapsável "Como ler esta análise"); `PanoramaEfetivo` (3 donuts via `DonutCard` — categoria/disponibilidade/vínculo — + barra Top funções, tudo client-side de `porCargoAtual`, sem IA); `InsightsEfetivo`/`InsightCard` (função mais numerosa, disponibilidade, funções distintas, maior gargalo Δ>0, maior folga Δ<0, resumo de ações). Fiação no `DiagnosticoView` após `erroIa`, guardada por `porCargoAtual.length>0` (vale com e sem análise). Datas via `formatDateTime` iOS-safe. Validado via esbuild (server+client, exit 0). Detalhe: `shared/changelog.ts`.
- **Rev. 2581** — **PLANEJAMENTO · ABA "EFETIVO × IA" · CORRIGE O ERRO "The string did not match the expected pattern" NO iPad/iOS SAFARI AO ABRIR A ABA.** User (screenshot iPad, obra REVTE-CIVIL): banner vermelho de erro na aba. CAUSA-RAIZ: a Rev. 2580 passou a renderizar `criadoEm` (TIMESTAMP do Postgres da nova tabela `planejamento_analises_efetivo`, lido por `listarAnalisesEfetivo`) no histórico via `new Date(a.criadoEm).toLocaleString("pt-BR")`; Drizzle/superjson entrega o timestamp como string crua "YYYY-MM-DD HH:MM:SS" (espaço, sem 'T'), e o iOS Safari 17+ lança "RangeError: The string did not match the expected pattern" que sobe pela error boundary global — mesma classe de bug já corrigida várias vezes no projeto. FIX (SÓ CLIENT, ZERO SERVER/SCHEMA): `AnaliseEfetivoIA.tsx` passa a usar o helper já existente `formatDateTime` (`client/src/lib/dateUtils.ts` — normaliza espaço→'T', trata Invalid Date→"—", fuso Brasília) nas 3 exibições de data (`criadoEm` no histórico + `geradoEm` no diagnóstico e na simulação). REFINO (mesma rev): o erro PERSISTIA porque a fonte real NÃO era a aba "Efetivo × IA" (que ficou comprovadamente limpa — superjson faz Date round-trip por ISO, seguro no Safari), e sim OUTRO "simulador": o **Simulador de Cenários de Compras** em `PlanejamentoDetalhe.tsx`, que renderizava `new Date(c.criadoEm/c.aprovadoEm/revExibida.geradoEm).toLocaleDateString("pt-BR")` direto sobre o TIMESTAMP cru do Postgres (4 pontos). Trocados pelo helper iOS-safe já existente `fmtTimestampBR`. Validado via esbuild (client). Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
