...
- **Rev. 2175** — **MELHORIA UX · Mensagem de conflito de nome no Plano de Contas agora diz EXATAMENTE onde está a conta conflitante (Plano de Contas vs Categorias) e com qual código.** Follow-up direto da Rev. 2174. Lilian: "se existe uma conta com esse nome, nao está aparecendo... onde está ela?". Explicação: a tela "Plano de Contas" filtra `codigo NOT LIKE 'AUTO-%'` (Rev. 2157, `escopo='plano'` em `listAccounts`). Categorias usam código `AUTO-NNNN` e moram na mesma tabela `financial_accounts` — então uma Categoria chamada "Mão de Obra Direta" fica invisível no Plano de Contas, MAS o índice único parcial é compartilhado. **Fix em `server/routers/financial.ts` updateAccount:** no catch do 23505 da Rev. 2174, antes de lançar `TRPCError`, faz SELECT extra `WHERE company_id=$1 AND LOWER(nome)=LOWER($2) AND ativo=1 AND id<>$3`. Se acha, classifica pelo prefixo do código: `AUTO-*` → "em **Categorias** (código `AUTO-0042`, id #N) — não aparece na tela do Plano de Contas"; outro → "no **Plano de Contas** com o código `3.4` (id #N)". SELECT em try/catch separado (fail-open). **R-001/R-007/R-010:** OK.
- **Rev. 2174** — **HOTFIX UX · Erro cru do Postgres (23505 / `uq_fa_company_lower_nome_ativo`) vazava no toast ao editar conta do Plano de Contas — traduzido pra mensagem amigável.** Print Lilian: editando "3.1 TESTE" pra "Mão de Obra Direta" (sob "3 CUSTOS DIRETOS"), toast mostrava `DB: code=23505 | constraint=uq_fa_company_lower_nome_ativo | ... | msg=duplicate key value violates unique constraint`. Causa real: já existe outra conta ATIVA com nome "Mão de Obra Direta" no company 60002 (índice único parcial `(company_id, lower(nome)) WHERE ativo=1`). **Fix em `server/routers/financial.ts` updateAccount:** envolver `dbExecute(UPDATE)` em try/catch que detecta `code=23505` (via `e.cause.code` enriquecido pela Rev. 2170 ou `e.code` direto), constraint `uq_fa_*`, ou mensagem `duplicate key`. Para `uq_fa_company_lower_nome_ativo`, lança `TRPCError BAD_REQUEST` amigável. `createAccount` já tinha tratamento próprio (L194-202: SELECT-by-name + idempotente alreadyExists=true). **Out of scope:** auto-resolver conflito. **R-001/R-007/R-010:** OK.
...
- ~~Rev. 2173~~ — HOTFIX BLOQUEANTE · Edição de código contábil no Plano de Contas era silenciosamente ignorada (zod do `updateAccount` não aceitava `codigo`; cliente também não enviava em edição). Fix: backend aceita `codigo` c/ validação do create; `onPickParent` sempre sugere próximo; `handleSave` envia. Campo Código deixou de ser `disabled` em edição. Ver `shared/changelog.ts`.
...
- ~~Rev. 2170~~ — DIAGNÓSTICO · `dbExecute` do Financeiro agora propaga causa real do PG (code/constraint/column/detail/hint) via try/catch + log `[dbExecute][PG ERROR]` + re-throw `Error("DB: <diag>")` com `.cause` preservada. Ver `shared/changelog.ts`.
