...
- **Rev. 3151** — **FINANCEIRO / CUSTO TOTAL (Análise de Custos · Detalhe) · OS 5 CARDS DE KPI ("Custo do recorte", "Pago", "Em aberto", "Vencido", "Lançamentos") VIRARAM FILTRO CLICÁVEL — TOCAR NUM CARD RESTRINGE OS GRÁFICOS E A TABELA ÀQUELE STATUS.** PEDIDO (IMG_2085/IMG_2086): "clicar no card e ver as informações, como um filtro". COMPORTAMENTO: os cards seguem mostrando o RESUMO COMPLETO do recorte (não zeram entre si); clicar em Pago/Em aberto/Vencido aplica filtro de status com toggle (clicar de novo limpa); "Custo do recorte" e "Lançamentos" limpam (mostrar tudo). O filtro só restringe a seção de baixo — "Distribuição por Mês", "Por Fornecedor" e a tabela "Lançamentos detalhados" — preservando os KPIs de topo como visão-resumo. CLASSIFICAÇÃO espelha os kpis: pago=`status==="pago"`, aberto=`status!=="pago"`, vencido=`isVencido` (subconjunto). Card ativo ganha anel + selo "• filtrando"; header da tabela mostra chip "Filtrando: <status> ✕"; rodapé soma a VISÃO exibida (`viewTotal`); lista vazia sob filtro mostra "Mostrar tudo". FRONTEND-ONLY (`FinanceiroAnaliseCustosDetalhe.tsx`: estado `cardFiltro` + `rowsView` alimentando `porMes`/`breakdown`/`lancamentos`/`viewTotal`; cards `role=button`+`aria-pressed`+teclado). ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
