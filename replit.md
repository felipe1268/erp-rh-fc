...
- **Rev. 4561** — **UX: CONTAS A PAGAR — REDESIGN LÚDICO DO MODAL "DETALHE DO TÍTULO".** Só apresentação (zero mudança de dados): hero gradiente por status (verde pago / vermelho vencido / laranja a vencer) com chips e valor grande; tabs em pills coloridas; aba Geral agrupada em 4 cartões temáticos via novo `DetSection` (Valores, Datas, Classificação, Pessoas); Origem/Itens/Parcelas em cartões tintados; Histórico vira timeline com bolinhas coloridas por ação + empty-state amigável; rodapé com botões arredondados e "Validar e Pagar" gradiente. Arquivo: `FinanceiroContasAPagar.tsx`. ZERO schema/server change.
