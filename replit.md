...
- **Rev. 2451** — **[BUG GRAVE] ALMOXARIFADO · tela quebrava com `ReferenceError: Can't find variable: consListFinal` (print user 23:36 iPad).** Causa: Rev. 2406 declarou `consListFinal` DENTRO do IIFE da visão consolidada (`{obraContexto === "todos" && (() => { ... })()}` em `index.tsx` L1727), mas Rev. 2442 adicionou a barra inferior `modoClassificarEquip` (L4441+) referenciando essa var FORA do IIFE — escopo lexical não alcançava. Em iOS Safari o TDZ explodia fatal. **Fix:** hoistei `consListFinal` pra escopo do componente como `useMemo` logo após `consolidado` (`pages/almoxarifado/index.tsx` L443-464), deps `[consolidado, busca, filtroCateg, apenasAbaixo, filtroEquip]`. Removi a declaração local do IIFE — closure mantém as ~10 refs internas funcionando sem mudança. R-001/R-007/R-010 OK (zero backend). Detalhe: `shared/changelog.ts`.
