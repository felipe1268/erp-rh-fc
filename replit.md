...
- **Rev. 4617** — **FIX: CRACHÁS — PILLS COM TODOS OS TREINAMENTOS FEITOS (REGRA DE OURO).** Regra de ouro do usuário: o crachá classifica TODOS os treinamentos FEITOS, não só vigentes (a Rev. 4614 filtrava `dataValidade >= hoje`, sumindo com vencidos e sem-validade). `badgeStatus` passa a montar as pills a partir de todos os registros do funcionário (dedup canônico mantido); selos NR-35/NR-10 e pendência "Nenhum treinamento vigente" continuam só com vigentes (selo = aptidão atual, pill = histórico de formação). Vale pra lista, preview e PNG. Arquivo: `server/routers/sprint1Foundation.ts`. ZERO schema change.
