...
- **Rev. 3138** — **PORTAL DO CLIENTE / AVALIAÇÃO (NPS) · O LINK DE PESQUISA JÁ RESPONDIDO NÃO É MAIS "DESATIVADO": AO REABRIR, O CLIENTE VÊ UMA MENSAGEM AMIGÁVEL DE "PESQUISA JÁ CONCLUÍDA · OBRIGADO!" EM VEZ DO TEXTO DE LIMITE MENSAL / "MÓDULO DESATIVADO".** PEDIDO (iPad, print de "Acessos do Portal" com 3 links "Disponível" + 1 resposta · NPS 100): "o link que foi preenchido não deve ser desativado, e [mostrar] uma mensagem que esta pesquisa foi concluída." CAUSA: o link público é de USO ÚNICO (nonce `linkId` consumido em `cliente_avaliacao_link_uso`, Rev. 2973) e NÃO é tecnicamente desativado — `resolverLinkAvaliacao` (short-link `/a/<codigo>`) e o long-link `/portal/avaliacao/<token>` seguem devolvendo o token e renderizando `PortalDashboardCliente` em modo público. Era um problema de TEXTO: ao reabrir um link já respondido, a aba "Avaliação" caía no card do PORTAL LOGADO (limite por janela), exibindo "Avaliação deste mês já registrada" + "O módulo de Avaliação está DESATIVADO até <próxima janela>". CORREÇÃO (FRONTEND + i18n; ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE): (1) `shared/portalAvaliacaoI18n.ts` ganhou `concluidaTitulo`/`concluidaTexto` nos 3 idiomas (pt/en/zh); (2) `PortalDashboardCliente.tsx` — no branch `jaAvaliouEsteMes && !avaliado`, quando `isPublic` renderiza o card NOVO de "pesquisa concluída" (`CheckCircle2` + `T.concluidaTitulo`/`T.concluidaTexto`), sem o texto de limite mensal; o caso do PORTAL LOGADO mantém o card de janela mensal intacto. RESSALVA: nada muda no backend nem no claim de uso único — o link segue 1-resposta-por-link e permanece "Disponível" na listagem admin; vale para os dois pontos de entrada públicos. Detalhe: `shared/changelog.ts`.
