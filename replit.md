...
- **Rev. 2890** — **PORTAL DO CLIENTE — LINK PÚBLICO DE AVALIAÇÃO (NPS) PARA ENVIAR DIRETO AO CLIENTE, SEM LOGIN/ACESSO AO PORTAL.** Pedido: "Além do portal quero ter a opção de gerar link para enviar ao cliente fazer a avaliação." Reaproveita 100% o formulário NPS e os endpoints públicos: (1) BACKEND `server/routers/portalExterno.ts` — nova `admin.gerarLinkAvaliacao` (admin/admin_master) assina JWT "link aberto" `{tipo:"cliente", companyId, linkAberto:true}` SEM `portalId` (180d, STATELESS); como `criarAvaliacao`/`podeAvaliarEsteMes` só limitam por período quando há `credId`, o token sem credencial pula o limite (anônimo/reutilizável). (2) `client/src/pages/portal/PortalDashboardCliente.tsx` — prop `publicToken` (modo público): lê token da URL, pula guard de login, desabilita queries que exigem credencial (`meusDados`/`minhasObras`/`listarComentarios`), força aba avaliação, oculta "Sair"/abas. (3) NOVA página `client/src/pages/portal/AvaliacaoPublica.tsx` + rota pública `/portal/avaliacao/:token` em `App.tsx`. (4) `ClientesPortalAdmin.tsx` (aba Avaliações) — botão "Gerar link" (copia URL + botões Copiar/Abrir). Ressalva: avaliação geral (obraId null), sem limite por período (por design). ZERO schema; ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
