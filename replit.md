...
- **Rev. 3355** — **FOLHA / FERIADOS · NOVO "BAIXAR FERIADOS {ANO}": BAIXA AUTOMATICAMENTE OS FERIADOS NACIONAIS (FIXOS + MÓVEIS) E, NUM DIÁLOGO, DEIXA O USUÁRIO ESCOLHER AS UFs PARA BAIXAR OS FERIADOS ESTADUAIS (PRÉ-MARCANDO AS UFs DAS OBRAS ATIVAS, QUANDO PREENCHIDAS). MUNICIPAIS NÃO SÃO BAIXADOS (SEM BASE PÚBLICA CONFIÁVEL) → CADASTRO MANUAL. 1 CONST CURADA (25 UFs) + 1 MUTATION + 1 QUERY + 1 FRONT · ZERO SCHEMA/ALTER/DROP/DELETE.** Pedido (usuário): baixar automaticamente nacionais + estaduais e deixar municipais p/ cadastro manual ("baixe o que vc tem e os que não tiver o usuário cadastra manualmente"). DESCOBERTA na validação (Neon): as 55 obras ativas estão TODAS com `estado`/`cidade` NULL → auto-detectar a UF da obra seria no-op silencioso; e o motor de HE da Rev. 3352 só trata estadual como feriado (HE 100%) quando `obra.estado === UF`. Logo, o caminho honesto é o USUÁRIO ESCOLHER as UFs (pré-marcando as das obras quando houver). CONST `FERIADOS_ESTADUAIS` (`server/routers/feriados.ts`): Record<UF,{nome,data MM-DD}[]> com 25 UFs (SP 09/07; RJ 23/04 São Jorge + 20/11 Consciência Negra; Consciência Negra fica ESTADUAL em RJ/AL/AP/AM/MT, não nacional). MUTATION `baixarFeriados({companyId,companyIds?,ano,ufs?})`: `ensureUserOwnsCompanies`; semeia nacionais fixos (companyId=null, dedup por data) + móveis (Sexta Santa observado=1; Carnaval/Corpus observado=0) + estaduais das UFs ESCOLHIDAS na empresa selecionada (tipo='estadual', estado=UF, recorrente=1, observado=1, dedup por (companyId|null)+data+nome). Nova query `ufsEstaduaisDisponiveis` alimenta o seletor. FRONT (`Feriados.tsx`): botão abre DIÁLOGO com card "nacionais (sempre)" + grade de UFs (chips) pré-marcadas por `ufsDasObras` + nota sobre HE/UF e municipais manuais; `seedNacionais`/"Carregar Nacionais" aposentada do front (mutation mantida no backend). VALIDAÇÃO: tsc limpo nos arquivos tocados; obras conferidas no Neon. Detalhe: `shared/changelog.ts`.
