...
- **Rev. 2441** — **ALMOXARIFADO · Combobox FILTRÁVEL de categoria (digite pra achar) no cadastro/edição de item + nos 2 modais "Alterar categoria em lote".** Pedido user (print iPad 22:17): `<select>` nativo abria roleta iOS com 30+ categorias sem busca — pra achar "Andaimes" ou "Equipamento Locado" era rolar tudo a dedo. **Fix (UI-only):** novo componente local `CategoriaCombobox` em `client/src/pages/almoxarifado/index.tsx` L50-145 (input com filtro on-type sem acento + dropdown; Enter aceita 1º match; click-fora fecha). Usado em 3 pontos: modal Editar/Cadastrar item (L2766-2780, `allowFree=true` — permite criar categoria nova), modal "Alterar categoria em lote" CONSOLIDADO (L4377-4388, `allowFree=false`+autoFocus), modal "Alterar categoria em lote" por linhas (L4469-4480, idem). Tooltip de estado vazio diferenciado conforme `allowFree`. Sem mudança de schema/mutation. R-001/R-007/R-010 OK. Detalhe: `shared/changelog.ts`.
- **Rev. 2440** — **ALMOXARIFADO · VISÃO GERAL · card limpo (3 badges + "+N locais") e card/linha clicáveis abrem modal completo de edição (foto, qtd, preço, locação etc.).** Pedido user (prints iPad 22:08, ANDAIME E ESCORAMENTO): itens com 30+ vínculos viravam colunas verticais GIGANTES de badges, quebrando o grid; pra editar foto/preço o user precisava de outro botão. **Fix (UI-only, zero backend):** `client/src/pages/almoxarifado/index.tsx` L1877-1976 (cards) + L1999-2065 (tabela) — card e `<tr>` agora chamam `abrirEditar(item)` (modal completo já existente, com foto via `handleFotoChange` + IA, auditoria de qtd preservada). Clique na foto mantém preview ampliado via `stopPropagation`. Badges limitados (3 cards / 4 tabela) + chip "+N locais" com `title` listando TODOS os vínculos pra long-press no iPad. Modo "Classificar Equipamentos" intacto. R-001/R-007/R-010 OK. Detalhe: `shared/changelog.ts`.
