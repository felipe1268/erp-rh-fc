...
- **Rev. 3617** — **NF-e RECEBIDAS · DIALOG "ESPELHO FIEL" COMPLETO — SEÇÃO "DADOS DA NF-e" + "EVENTOS E SERVIÇOS" + DigestValue. BACKEND PONTUAL + FRONTEND · ZERO SCHEMA/ALTER/DROP/DELETE.** Referência: portal nfe.fazenda.gov.br. Adicionada seção "Dados da NF-e" (natureza op, tipo "1-Saída", chave, modelo, série, nº, data/hora HH:mm:ss) antes de Emitente/Dest. Adicionada seção "Eventos e Serviços" (tabela evento/protocolo/data autorização + DigestValue + versão aplic SEFAZ). Backend: `parseNFeXml` extrai `digVal`/`verAplic`/`tpAmb` do infProt. Detalhe: `shared/changelog.ts`.
