...
- **Rev. 3619** — **NFS-e EMITIDAS · PORTAL NACIONAL REST+mTLS+NSU (sefin.nfse.gov.br) + ESPELHO FIEL ABRASF + XML SALVO. BACKEND ADITIVO + FRONTEND · ZERO ALTER DESTRUTIVO/DROP/DELETE.** Provider `nfse_nacional` reescrito para API REST NSU (migração 01/01/2026): GET `/DFe/{NSU}` com mTLS A1, GZip+Base64 por lote de 50, NSU persistido em nova coluna `ultimo_nsu` (SyncSchema+ ALWAYS-RUN). XML completo salvo em `fiscal_notes.xml_payload` no INSERT. Nova função `parseSefinNfseXmlFull` extrai todos os campos ABRASF (prestador, tomador, endereços, discriminação, ISS/INSS/IRRF/CSLL/PIS/COFINS, valor líquido, competência, RPS, órgão gerador). Endpoint `getDetalhesNFse` + dialog "Espelho Fiel NFS-e" no frontend (seções: Identificação, Prestador, Tomador, Serviço, Discriminação, Valores, Informações Complementares; fallback para notas sem XML). Detalhe: `shared/changelog.ts`.

- **Rev. 3618** — **NF-e RECEBIDAS · BUGFIX NF# EM FLOAT — "JOGO DA VELHA" SUMIU DA LISTA. BACKEND PONTUAL (SyncSchema+) + FRONTEND · ZERO ALTER/DROP.** Rev. 3600 (ColFix, 1x) limpava chaves `e+` na aplicação; NF-es que chegaram depois ficavam com `numero_nf="3.5260304"` visível. Rev. 3618 no bloco ALWAYS-RUN: (a) DELETE dos que têm cópia limpa, (b) `status='duplicata'` nos restantes → somem da lista. 26 entradas corrompidas limpas na 1ª execução. Frontend: `resolveNumeroNf` retorna `""` (→ "—") quando chave também corrompida. Detalhe: `shared/changelog.ts`.
