...
- **Rev. 2924** — **INTEGRAÇÃO DE SEGURANÇA (SST) · ABA "VÍDEOS" (ADMIN) — O VÍDEO DE TREINAMENTO VOLTOU A APARECER NO CARD, CENTRALIZADO E COM FALLBACK QUANDO NÃO CARREGA (FIM DO "BURACO PRETO" SEM FEEDBACK).** Problema (usuário, com print de "Vídeos da Integração"): o card do vídeo de upload mostrava só caixa preta com controles em "0:00" — "não carrega" e "não centralizado". O player do card (`isFileVideo`) era um `<video preload="metadata">` cru, sem estado de carregando/erro: se a metadata não chegasse (arquivo grande, `moov` no fim do mp4, ou o cap de 8MB/chunk do proxy do deploy — Rev. 2917, só em PROD republicado), ficava o buraco preto eterno, sem spinner e sem alternativa. SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE): novo `VideoCardPlayer` em `client/src/pages/sst/IntegracaoSST.tsx`, espelhando o portal público (Rev. 2920) — vídeo `object-contain` centralizado na caixa `aspect-video` (`playsInline`+`key={url}`); overlay com spinner (`Loader2`) até `onLoadedMetadata`/`onCanPlay`; `onError` mostra "Não foi possível exibir o vídeo aqui." + "Abrir em nova aba"; fallback temporal de 12s ("Demorando? Abrir em nova aba"); links com `stopPropagation`. Substitui o `<video>` inline antigo. Backend de serving e schema intactos. Detalhe: `shared/changelog.ts`.
- **Rev. 2923** — **DASHBOARD FINANCEIRO · CARDS "A PAGAR"/"A RECEBER" — FIM DOS VALORES INFLADOS (R$ 25 MI "A PAGAR" QUE NÃO EXISTIAM): SÓ CONTAS REAIS + CORREÇÃO DO BUG QUE DUPLICAVA AS PROJEÇÕES DO CRONOGRAMA A CADA REIMPORTAÇÃO DO MS PROJECT.** Problema (usuário, com print): "A Pagar" = R$ 25,2M e "A Receber" desalinhado ("não temos 25 milhões para pagar"). Auditoria no Neon achou DOIS problemas (empresa 60002): (1) CONCEITUAL — o card somava PROJEÇÕES (`origem_modulo='cronograma_atividade'` = custo planejado do MSP mês a mês) junto com contas reais; idem "A Receber" com faturamento previsto (`revenue`). Decisão do usuário: cards só com contas reais comprometidas. (2) DUPLICAÇÃO — entradas do cronograma repetidas até 18× (4011 linhas-lixo); causa-raiz em `server/services/financialIntegrationBridge.ts` (`importAtividadesCronogramaToFinancial`): a dedup montava a chave `origem_id|data` com `String(e.data_competencia).substring(0,10)`, mas a coluna é `date` → o driver devolve objeto Date → `"Sun Jun 01 2026…"` ≠ `"2026-06-01"` → chave nunca casava → toda reimportação reinseria tudo; `ON CONFLICT DO NOTHING` não protegia (sem índice UNIQUE). SOLUÇÃO (ZERO ALTER/DROP/DELETE): CARDS — `server/routers/financial.ts` `getDashboardExecutivo` ganhou `exclProjPagar`/`exclProjReceber` aplicados em A Pagar, A Receber, vencidos (ambos) e lista "próximos vencimentos"; séries realizadas intactas. GERADOR — a pré-busca passou a usar `TO_CHAR(data_competencia,'YYYY-MM-DD')` (string casável) → reimport idempotente. LIMPEZA — UPDATE no Neon (NUNCA DELETE): partição (company_id,origem_id,data_competencia), mantém MIN(id), resto vira `status='cancelado'` (2450 linhas). Efeito (60002): A Pagar R$ 25,2M→R$ 5,44M; A Receber R$ 4,19M→R$ 555,8 mil. Detalhe: `shared/changelog.ts`.
