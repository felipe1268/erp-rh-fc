...
- **Rev. 3090** — **MEDIÇÃO DE TERCEIROS · O LEVANTAMENTO DE CAMPO (PROJETO/PLANTAS EM PDF + ESCALA + CONTORNOS + FOTOS POR AMBIENTE + MEMÓRIA DE CÁLCULO) DEIXA DE SER UM BECO SEM SAÍDA: PASSA A SER CRIÁVEL E VINCULÁVEL DIRETO DE CADA MEDIÇÃO DE TERCEIROS, REAPROVEITANDO A MESMA ENGINE DA MEDIÇÃO DE CLIENTE.** PEDIDO (usuário frustrado, prints iPad): "preciso de uma lógica em algum local para cadastrar os projetos (plantas/PDF) e fazer o levantamento — tá confuso, arrume de vez". A fundação (schema `levantamento_campo_id`, `vincularLevantamentoMedicao`, 3 níveis, FD) já existia desde Rev. 3078/3079, mas NÃO HAVIA como CRIAR o `medicao_campo` `origem='terceiro'` que o vínculo EXIGE — o `criarCampo` só validava cliente (`medicaoContratos`) e a tela `MedicaoLevantamento` só abria cliente. CAUSA-RAIZ: engine 100% acoplada à Medição de Cliente; como `terceiro_contratos` também tem `orcamentoId`, é reaproveitável — `origem` desambigua os IDs colidentes. SOLUÇÃO (BACKEND ADITIVO READ-PATH + FRONTEND, ZERO ALTER/DROP/DELETE/SCHEMA): (1) `medicao.ts` `criarCampo` aceita `origem` (enum, default cliente), valida na tabela correta com tenant guard, numera escopado por (contrato,origem) e grava `origem`; `listarCampos` ganha filtro `origem`; (2) `MedicaoLevantamento.tsx` fica ORIGEM-AWARE (`?origem=terceiro`, busca `terceiroContratos.getContrato`, `voltarHref`/subtítulo, esconde "Gerar boletim"); (3) `ContratoDetalhe.tsx` aba Medições: ação "Fazer levantamento de campo" por medição (gateada em modoEdicao + status≠aprovada/paga) cria+vincula+abre; badge "vinculado"+Abrir se já existe. Cliente e gestão Terceiros intactos. RESSALVA: rota sob `route="/medicao"` (Admin Master ok; não-master Terceiros pode precisar da permissão — candidato a `sharedPaths`). Detalhe: `shared/changelog.ts`.
