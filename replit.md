...
- **Rev. 2744** — COMPRAS · COTAÇÃO · APROVAÇÃO ("APROVAR E GERAR OC"): o aviso de "acima da meta / déficit" só aparece quando há DÉBITO real — antes estourava falso déficit em cotação POR PACOTE mesmo com crédito (ex.: COT-2026-0283). Causa (`client/src/pages/compras/Cotacoes.tsx`): os 2 gatilhos do popup recomputavam a meta somando os insumos crus de `mapa.itens`, que não representam a meta do pacote. Fix (SÓ CLIENT/UI): passam a usar o `deficit`/`metaGrandTotal` do escopo (pacote-aware); alerta só quando `deficit > 0`. ZERO SERVER/SCHEMA. `vitest` 41/41 verde. Detalhe: `shared/changelog.ts`.
