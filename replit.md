...
- **Rev. 4534** — **FIX: PLANEJAMENTO — PREVISTO (SEMANA) 100% EM TODAS AS SEMANAS FUTURAS (BASELINE DEFASADA).** Revisão ativa importada sem baseline + MSP re-baselined → fallback Rev. 4179 usava a baseline INICIAL (fim 23/07), curva do motor chegava a 100% cedo demais e todas as semanas futuras clampavam em 100% (`raizAt`). Fix em `regenerarPrevistoSemanasCaminhoB`: guard no fallback — se o cronograma ativo termina > 7 dias além do envelope da baseline inicial, usa as datas ATUAIS da revisão ativa como baseline-proxy. Complemento: rollup da raiz com 2 casas decimais e clamp <100 até o fim real do cronograma (99,805% arredondava p/ 100% uma semana antes). Validado no projeto 54: 16/07 = 79,6% (MSP 79%), futuras 83→89→96→99,76→100% em 20/08. ZERO schema change.
- **Rev. 4533** — **FIX: PLANEJAMENTO — REALIZADO ACUMULADO RETROATIVO AO ENVIAR NOVO XML (REFIS/CURVA S).** Após enviar XML da semana 8, o REFIS da semana 7 mostrava % diferente do impresso (25%→20,36%). Causa: `statusDateSnapshot` avança para a semana nova e o check `semanaFim === statusDateSnapshot` falha para semanas históricas → cai no EVM ponderado com pesos atuais. Fix: os 3 caminhos de leitura (`avancoAtual`, `realizadoAcum`, `realizadoComInd`) agora consultam o mapa `realizadoSemanas` (gravado desde Rev. 2781) antes de cair no EVM. Curva S backend também injeta pontos históricos do mapa. ZERO schema change.
