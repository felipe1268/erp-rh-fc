...
- **Rev. 4596** — **FEAT: CHEQUES — REGRA DE Nº DUPLICADO TAMBÉM NA IMPORTAÇÃO E NO PAGAMENTO PELO CONTAS A PAGAR.** Importação (planilha e PDFs por IA) agora aplica a MESMA regra Poka-Yoke da Rev. 4595: `carregarNumerosExistentes` + `numeroColide` (nº normalizado + mesma conta OU fornecedor) classificam a linha como `DUP_NUMERO` — aparece em vermelho na prévia (card/chip/badge "Nº duplicado") e NÃO grava (pulado, sem abortar o lote). Ordem: JA_EXISTE (re-upload idempotente segue OK) > DUP_ARQUIVO > DUP_NUMERO > NOVO, espelhada em prévia e gravação. Pagamento consolidado com cheque próprio no Contas a Pagar (financial.ts) também valida via `assertNumeroChequeDisponivel` (exportado) + anti-repetição dentro do pagamento. ZERO schema change. Arquivos: `cheques.ts`, `financial.ts`, `FinanceiroCheques.tsx`.
...
Ver `replit-history.md` para revisões Rev. 4589 e anteriores.
