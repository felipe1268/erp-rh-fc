...
- **Rev. 2227** — **FIX/UX · Tela "Contas a Pagar" cortava coluna "Ações" (botão Pagar invisível).** Lilian: "a tela de resumo do contas a pagar tambem está cortando, redistribua tela". A Rev. 2215 tinha subido cap de `max-w-7xl` pra `max-w-[1600px]`, ainda insuficiente em alguns viewports. Fix em `FinanceiroContasAPagar.tsx`: (1) L477 `max-w-[1600px] mx-auto` → `w-full mx-auto` (usa 100% da área útil); (2) L809 `<th>` Ações vira `sticky right-0 bg-gray-50` + sombra suave + `w-24`→`w-32`; (3) L1162 `<td>` Ações com mesma stickiness + `bg-white group-hover:bg-slate-50`. Tabela já estava em `overflow-x-auto` então sticky funciona. Resultado: coluna Ações sempre visível mesmo em viewport estreito (congela à direita) e em viewport largo aparece naturalmente. **R-001/R-007/R-010:** N/A (só CSS).
