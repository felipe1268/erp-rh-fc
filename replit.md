...
- **Rev. 3771** — **FINANCEIRO · DUPLICATA DE CATEGORIA ELIMINADA: "MÃO DE OBRA TERCEIRIZADA - OBRA" (1.119 LANÇAMENTOS) + "Subempreiteiros" FLUTUANTE (28) UNIFICADOS EM "MÃO DE OBRA TERCEIRIZADA / SUBEMPREITEIRO" (conta_id=23). DASHBOARD PASSA A MOSTRAR UMA SÓ LINHA. MAPA CANÔNICO + BRIDGE + AUTOIMPORT ATUALIZADOS. ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.

- **Rev. 3770** — **FINANCEIRO · MAPEAMENTO DE CONTAS FLUTUANTES: BATCH UPDATE EM PRODUÇÃO (7 GRUPOS / ~1.200 LANÇAMENTOS COM `conta_id IS NULL` RECEBERAM CONTA VINCULADA) + MAPA CANÔNICO `CONTA_ID_BY_NOME` + `resolveContaId` EM `financialIntegrationBridge.ts` + `financialAutoImport.ts` CORRIGIDO. PROJEÇÕES DE CRONOGRAMA MANTIDAS SEM CONTA. ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
