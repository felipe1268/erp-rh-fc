...
- **Rev. 2907** — **RECONTRATAÇÃO · SUPLENTES DE APROVAÇÃO — A TELA AGORA MOSTRA OS DEMAIS SÓCIOS (ADMIN MASTER) COMO APROVADORES TITULARES, PRA OUTRO SÓCIO PODER LIBERAR/RECUSAR QUANDO O TITULAR NÃO PUDER.** Pedido: "Precisa aparecer os demais sócios também, para aprovação, caso eu não possa fazer a liberação" (card "Recontratação · Suplentes de Aprovação" em Configurações → Critérios do Sistema). CONTEXTO: o backend `server/routers/recontratacao.ts` JÁ deixa QUALQUER `admin_master` liberar/recusar (`assertPodeAprovar`) e `getSuplentes` JÁ retorna todos os usuários (inclusive admin_masters); o gap era SÓ de visibilidade — o card filtrava os admin_masters pra fora da lista, parecendo haver 1 só titular. SOLUÇÃO (FRONTEND-ONLY — ZERO BACKEND/SCHEMA/DADOS, ZERO ALTER/DROP/DELETE): `client/src/pages/Configuracoes.tsx` (`RecontratacaoAprovadoresSection`) deriva `titulares = usuarios.filter(role==="admin_master")` e renderiza, acima da busca, um bloco só-leitura âmbar "Aprovadores titulares (sócios)" com badge "TITULAR" por sócio (nome+e-mail); lista de suplentes selecionáveis abaixo inalterada (segue excluindo admin_masters); `CardDescription` atualizado pra plural ("inclusive um na ausência do outro"). RESSALVA: nenhuma mudança de permissão — só torna os sócios VISÍVEIS. ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
