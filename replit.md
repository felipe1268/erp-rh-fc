...
- **Rev. 2803** — **COMPRAS · COTAÇÕES — A LISTA E O DETALHE DA COTAÇÃO PASSAM A MOSTRAR O NÚMERO **REAL** DA SC VINCULADA (`numero_sc`, EXIBIDO COMO `SC-NNNN-AAAA`) EM VEZ DO ID INTERNO `SC #<id>`; E O NÚMERO VIROU UM LINK CLICÁVEL QUE ABRE A SOLICITAÇÃO.** Pedido (usuário, com screenshot da lista "Cotações" mostrando "SC #617"): "que em cada cotação apareça o número correto da solicitação, garanta que o número será real, e quando eu clicar na solicitação seja possível abrir ela". O "SC #617" era o ID interno (`comprasCotacoes.solicitacaoId`), NÃO o `numero_sc` (counters distintos). Fix: `server/routers/compras.ts` (`listarCotacoes`) — `scMap` passou a trazer `numeroSc` (devolvido como `numeroSc: sc?.numeroSc ?? null`; só +1 coluna num `select` que já existia). `client/src/pages/compras/Cotacoes.tsx` — na LISTA a linha "SC #<id>" virou `<button>` com `formatNumeroScDisplay(cot.numeroSc)` que navega (wouter, `stopPropagation`) p/ `/compras/solicitacoes?destaque=<solicitacaoId>` (fallback `SC #<id>` se faltar); no DETALHE fullscreen o card "SC Vinculada" usa `scInfo.numeroSc` (já vinha do `getCotacao`) + mesmo link. A tela de Solicitações já suportava `?destaque=<id>` — reusada, ZERO mudança lá. ZERO ALTER/DROP/DELETE; ZERO schema novo. Validação: esbuild OK em `compras.ts`+`Cotacoes.tsx`; servidor reiniciado e reconectado ao Neon. Detalhe: `shared/changelog.ts`.
