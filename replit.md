...
- **Rev. 3066** — **AVISO PRÉVIO · CIPEIRO (ESTABILIDADE PROVISÓRIA): O CARD DA "INDENIZAÇÃO DO PERÍODO DE ESTABILIDADE" GANHA UMA BARRA "TOTAL GERAL (RESCISÃO + INDENIZAÇÃO ESTABILIDADE)" SOMANDO O TOTAL LÍQUIDO DA RESCISÃO COM A INDENIZAÇÃO — ANTES SÓ MOSTRAVA OS DOIS TOTAIS SEPARADOS.** PEDIDO (print do "Novo Aviso Prévio" no iPad, colaborador cipeiro): "Coloque o total tbm" — via-se o "TOTAL LÍQUIDO RESCISÃO" e, separado, o "TOTAL DA INDENIZAÇÃO DE ESTABILIDADE", mas não a SOMA dos dois (o gestor somava de cabeça). CONTEXTO: a indenização de estabilidade (Súmula 396 TST) é verba SEPARADA e NÃO entra no Total Líquido — então não é caso de somar no líquido, e sim oferecer um total gerencial à parte. SOLUÇÃO (FRONTEND-ONLY, ZERO ALTER/DROP/DELETE, ZERO schema/backend) em `client/src/pages/AvisoPrevio.tsx`, dentro do card de estabilidade (`indenizacaoEstabilidade.aplicavel`): após o aviso jurídico, barra escura (mesmo estilo do "TOTAL GERAL (Oficial + Complementar)") com rótulo "TOTAL GERAL (Rescisão + Indenização Estabilidade)" = `parseFloat(totalLiquido) + parseFloat(ie.total)`, formatado via `formatMoeda(x.toFixed(2))` (BRL pt-BR), + os dois componentes em fonte pequena p/ rastreabilidade. Só aparece p/ cipeiro com indenização aplicável. RESSALVA: é só EXIBIÇÃO no preview; não altera valores persistidos, TRCT nem o documento gerado. Detalhe: `shared/changelog.ts`.
