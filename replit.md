...
- **Rev. 2215** — **UX/LAYOUT · Tela Contas a Pagar usa largura estendida (1600px) — coluna "Ações" não corta mais.** Lilian: "redistribua o tamanho da tela para que nao corte a lateral". Print mostrava botão verde "Pagar" parcialmente cortado: 8 colunas (checkbox, data, Nº OC/OS, descrição, categoria, valor, status, ações) espremidas em container `max-w-7xl` (1280px). Fix: `client/src/pages/financeiro/FinanceiroContasAPagar.tsx:480` — trocado `max-w-7xl mx-auto p-6` → `max-w-[1600px] mx-auto px-4 py-6`. `mx-auto` mantém centralização em telas menores, `overflow-x-auto` da tabela continua como fallback mobile. **R-001/R-007/R-010:** OK — visual only.
- **Rev. 2214** — **FIX/UX · Lançamentos recorrentes agora aparecem automaticamente em Contas a Pagar.** Lilian: "os lançamentos de contas recorrentes ainda não estão aparecendo no CONTAS A PAGAR". Print Jun/2026 tinha 71 contas, nenhuma das 4 recorrências (Celular, Energia, Seguro Vida, Título de Capitalização). Causa dupla: `generateRecurringEntries` só gerava quando `proximo_vencimento <= hoje` (recorrências futuras nunca materializavam mesmo clicando "Gerar Pendentes") **+** nenhum trigger automático ligando recorrência → conta a pagar. Fix: novo helper `materializeRecorrentes(db, companyId, horizonteMeses)` (`server/routers/financial.ts:94-187`) idempotente que projeta todas recorrências ativas até o horizonte, e `getContasAPagarByYear` agora chama esse helper ANTES do SELECT com horizonte = meses até fim do ano consultado (capado em 13). Resultado: abrir /contas-a-pagar já popula as recorrências sem clique. **R-001/R-007/R-010:** OK — INSERTs idempotentes, sem DELETE/ALTER.
