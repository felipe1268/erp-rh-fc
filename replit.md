...
- **Rev. 2305** — **FEAT · Seleção múltipla + ESTORNO em lote de movimentações do Almoxarifado (soft-delete auditável).** Pedido user (23/05/2026): "Quero múltipla seleção" → após clarificação, era pra APAGAR várias movimentações erradas. Convertido em ESTORNO (preserva auditoria + devolve qtd ao estoque). **UX** (`client/src/pages/almoxarifado/Movimentacoes.tsx`): toggle "Selecionar" no header (só admin/admin_master via `useAuth`); cards viram clicáveis com checkbox (`Square/CheckSquare`) + ring emerald; estornadas mostram `Ban` e não são selecionáveis; sticky bottom-bar "X selecionada(s) | Cancelar | Estornar"; modal âmbar exige textarea motivo (mín 3 chars). Cards estornados: opacity-60 + line-through + badge cinza "ESTORNADA" (tooltip motivo) + "Estornada por NOME". `e.stopPropagation()` no botão obraNome p/ não disparar seleção. Toast resumido (full/parcial/total failure). **Backend** (`server/routers/warehouse.ts`): novo `reverseMovements` mutation `{companyId, movementIds[1..200], motivo[3..500]}`, authz `isAdmin` ou `FORBIDDEN`. Loop por ID c/ erros individuais: bloqueia mov de OC (regex `/\boc[\s-]/i` ou "Recebimento inteligente" — devem ser estornadas pela tela Recebimentos), bloqueia entrada quando estoque atual < qtd (material consumido), entrada→subtrai/saída→soma. UPDATE seta `estornadaEm/PorId/PorNome/estornoMotivo` (NÃO DELETE). `listMovements` select+3 campos de estorno. **Schema** (`drizzle/schema.ts`): 4 cols novas em `almoxarifado_movimentacoes`. **SyncSchema+ Rev. 2305** (`server/_core/index.ts`): 4× `ADD COLUMN IF NOT EXISTS` aditivos. **R-001/R-007/R-010:** OK — aditivo + soft-delete (sem DROP/DELETE).
