...
- **Rev. 2727** — **RH · RAIO-X DO FUNCIONÁRIO / TIMELINE CRONOLÓGICA (`controleDocumentos.raioX`) · A MUDANÇA DE OBRA DO FUNCIONÁRIO AGORA FICA REGISTRADA NA TIMELINE (antes nunca aparecia).** Pedido do usuário: "quando o funcionário mudar de obra, fique registrado". Causa-raiz: a alocação/transferência/saída de obra grava em `employee_site_history` (`allocateEmployeeToObra`/`removeEmployeeFromObra`), mas a timeline do Raio-X só agregava `employee_history` (Histórico Funcional) — TABELA DIFERENTE; nenhum fluxo de troca de obra escreve em `employee_history`, então a mudança NUNCA aparecia. Solução (SÓ SERVER; ZERO SCHEMA — R-001/R-007/R-010): `server/routers/controleDocumentos.ts` (`raioX`) lê também `employee_site_history` como fonte da timeline (canônica, sem duplicar dados, com histórico retroativo). Eventos: `transferencia`→"Mudança de Obra: Transferido de {origem} para {destino}" (indigo); `alocacao`→"Alocação em Obra" (green); `saida` avulsa→"Saída de Obra" (gray). Dedup: a `saida` que é o par da transferência do mesmo dia é suprimida; `gestor_obra` ignorado. Nomes via mapa id→nome (`inArray` em `obras`). Front já cobre cor/ícone (zero mudança). Validação: query Neon (184 alocacao/43 transferencia/84 saida); esbuild server EXIT 0; `vitest server/rescisao.test.ts` 41/41 verde. Detalhe: `shared/changelog.ts`.
