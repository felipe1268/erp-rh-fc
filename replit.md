...
- **Rev. 1967**: **RH · Dashboard Aviso Prévio · CDM ganha modal "Detalhe do Cálculo" (clique no nome) + seleção em massa via checkbox + modal "Combo de Demissões" com fluxo de caixa consolidado**. User (16/05/2026, IMG_0820): "Quero poder clicar no nome do funcionário e ver todo cálculo pertinente ao aviso, quero poder selecionar vários e fazer um combo de demissões para ver o fluxo de caixa que vai acontecer". **Mudança** em 1 arquivo (`client/src/pages/dashboards/DashAvisoPrevio.tsx`): (1) novos states `detalheCalc`/`selecionados:Set<number>`/`comboOpen`; (2) helpers `computeDataPagamento` (Art. 477 §6: indenizado=+10d corridos, trabalhado=+diasAviso+1d aprox), `toggleSelecionado`, `toggleSelecionarTodos`, memos `linhasSelecionadas` + `comboAgregado` (somatórios de todas as verbas) + `cronogramaPagamentos` (agrupa por data prevista); (3) coluna checkbox como 1ª col + select-all no header (indeterminate quando parcial) + row destacada azul quando selecionada + tfoot colSpan 11→12; (4) clique no nome agora abre modal "Detalhe do Cálculo do Aviso" (saldo+aviso of/compl+13º+férias prop+vencidas+multa 40% = bruto; − descontos = líquido; + compl = custo total; data prevista pagto + FGTS informativo; botão "Abrir Raio-X"); raio-x movido pra ícone Stethoscope ao lado do nome; (5) barra flutuante azul sticky quando há seleção (contagem+total+botões Limpar/Gerar Combo); (6) modal "Combo de Demissões" com 4 KPIs (qtd/folha/bruto/total) + quebra por verba consolidada + cronograma `<details>` agrupado por data com lista de funcionários expandível. version → 1967. **Resultado**: clique no Anderson IMG_0820 abre detalhe R$ 73.902,97 quebrado verba-por-verba; selecionando 5 mestres abre Combo com cronograma "26/05/2026 · 5 pagamentos · R$ X" (indenizado) ou datas escalonadas (trabalhado). **Preservado**: Rev. 1966/1965/1964/1935 INTACTAS. Backend payload INTACTO (já tinha todos os campos). Schema INTACTO. Zero tRPC novo. Zero ALTER/DROP/DELETE. R-001/R-007/R-010 OK.
- **Rev. 1966**: **RH · Dashboard de Férias · Badge de período simplificado para "1º" vs "2º+" SOMENTE no drill Vencidas**. User (16/05/2026, pós-Rev. 1965): "na verdade nesta tela só importa se é 1 ou 2 período, quantas férias o funcionário já teve não importa, deixa que essa info estará no raio-x dele". **Reorientação semântica**: o que pesa no dashboard é o GATILHO DA DOBRA (Art. 137 CLT — 2º período sem gozo = pagamento em dobro), não a contagem sequencial. **Mudança** em 1 arquivo (`client/src/pages/dashboards/DashFerias.tsx` L856-874): (1) Demais drills (Finalizando/Iniciando/Concluídas/Em Gozo/financeiros) → `return null` (badge sumiu — era ruído, info no raio-x). (2) Drill Vencidas → badge binário: `np <= 1` → "1º período" cinza neutro (passivo simples); `np >= 2` → "2º+ período" vermelho (passivo dobra Art. 137 CLT). Removida a escala intermediária laranja/vermelho — binário reflete que juridicamente só existem 2 estados. version → 1966. **Resultado**: drill "Finalizando em Mar 2026" sem o ruído "9º ano" / "13º ano"; drill "Vencidas" com triagem visual instantânea do passivo de dobra (cinza=ok, vermelho=dobra). Detalhe sequencial fica no raio-x do colaborador. **Preservado**: Rev. 1965 (calc fix), Rev. 1964 (CDM descontos), Rev. 1963 (badge sempre visível Vencidas — agora simplificado) e anteriores INTACTAS. Backend `f.numeroPeriodo` INTACTO. Outros badges (RH/Abono) INTACTOS. Zero schema, zero backend, zero tRPC. R-001/R-007/R-010 OK. Reversível em 1 hunk.
