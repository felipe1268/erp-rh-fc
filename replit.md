...
- ~~Rev. 2011~~ — ver `shared/changelog.ts`. **SST · Integração · Modal Vídeo · Largura e distribuição em 2 colunas no tablet/PC**. Pedido direto do usuário (img IMG_0854_1779031556595): "Melhore a tela, está quero ela mais distribuída na tela". Após Rev. 2009 modal estava na regra de ouro mas em tablet/PC mantinha `max-w-2xl` (~672px) com 4 seções empilhadas — desperdiçava 50%+ do espaço lateral. **Mudança** em 1 arquivo (`client/src/pages/sst/IntegracaoSST.tsx`, 2 hunks CSS-only, ZERO lógica): (1) DialogContent: `max-w-2xl` → `max-w-2xl lg:max-w-4xl xl:max-w-5xl`. (2) Container interno ganha grid `grid-cols-1 lg:grid-cols-2 gap-4` envolvendo as 4 seções: mobile/iPad portrait empilha (1 col), tablet landscape/PC distribui em 2 colunas — linha 1 (Onde encaixa | Conteúdo) + linha 2 (Mídia | Configurações). (3) `shared/version.ts` → 2011. **Resultado**: PC widescreen mostra modal compacto em 2 colunas sem scroll vertical na maioria dos casos; preview do YouTube cabe inteiro na coluna direita; tablet portrait/mobile inalterados. **Preservado**: estrutura interna das 4 seções (chips/labels/inputs/preview/toggle) INTACTA; header gradient + footer + CTA INTACTOS; mutations/states/handleSave/resetForm INTACTOS; outros modais do arquivo INTACTOS. Rev. 2010 INTACTA. Schema INTACTO. R-001/R-007/R-010 OK (CSS-only). Reversível em 2 hunks. **Follow-up**: propagar 2-col aos modais ConfigTab/SessaoTab; faxina do replit.md (avisos recorrentes do sistema).
