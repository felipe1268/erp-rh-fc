...
- **Rev. 2236** — **FIX/IMPORTER (continuação 2235) · Avanço ainda não aparecia para terceiros (Rohr, Lotus, Friul, Santuário).** User: "nao esta aparecendo o avanço das atividades pq?". A Rev. 2235 removeu o descarte por Item ausente e adicionou matching por `mspUid`, mas iterava `folhas` (`PlanejamentoDetalhe.tsx:6252`) que exclui `isIndireta=true`. As 22 atividades de terceiros costumam estar marcadas como indireta via Padrão FC — saíam do `forEach` e `newLocal[a.id]` nunca era setado. Fix em `PlanejamentoDetalhe.tsx:6840`: troca `folhas.forEach(...)` por `folhasComInd.forEach(...)` (`!a.isGrupo && !a.disabled` — inclui indiretas). Status "X de Y" agora reflete `folhasComInd.length`. **R-001/R-007/R-010:** N/A.
- **Rev. 2235** — **FIX/IMPORTER · Importar MS Project pulava 20-30% das atividades silenciosamente.** User: "TODA VEZ QUE FAÇO A IMPORTAÇÃO ELE NÃO TRAZ O VALOR DO REALIZADO... ALGUNS ITENS FORAM IMPORTADOS OUTROS NÃO". Diagnóstico real parseando o XML do user (`REVTE-CIVIL_SEMANA_03.xml`): 128 tasks → 48 summary → 80 folhas → **22 sem campo "Item" (Texto1)** porque são terceiros. `importarDoMSProject` tinha `if (!wbs) return` que pulava silenciosamente. Ex.: UID=5655 "Montagem do andaime - Rohr" PercentComplete=22% nunca importava. Fix em `PlanejamentoDetalhe.tsx:6717-6859`: (1) DOIS maps — `percentByUid` (UID MSP, 100% das tasks) + `percentByEap` (Item, fallback legado); (2) removida guarda `if (!wbs) return`; (3) matching cliente prioriza `mspUid` (Rev. 1829) com fallback `eapCodigo`; (4) status "X de Y preenchidas (N via UID · M via Item · K sem correspondência)". **R-001/R-007/R-010:** N/A.
