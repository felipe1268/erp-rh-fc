...
- **Rev. 2662** — **CONTAS A PAGAR (`/financeiro/contas-a-pagar`) · A TABELA É REDISTRIBUÍDA PARA QUE OS VALORES (R$) E O STATUS VOLTEM A APARECER — ESTAVAM ESCONDIDOS À DIREITA, ATRÁS DA COLUNA FIXA "AÇÕES".** Pedido (usuário, print): os valores não aparecem ("estão escondidos") → "redistribua a tela para que eles apareçam". Causa-raiz (SÓ LAYOUT/UI): `<table className="w-full">` dentro de `overflow-x-auto` com colunas largas demais — "Descrição" `max-w-md` (até 448px) + "Ações" `sticky right-0` com 5 controles/linha (Visualizar+Editar+Anexar+Pagar[texto]+Excluir ≈192px); a soma estourava a largura e, sem rolagem horizontal, "Valor" e "Status" caíam fora da viewport / sob a coluna fixa Ações. Fix (SÓ CLIENT/UI; ZERO SCHEMA; ZERO SERVER; R-001/R-007/R-010): CLIENT `FinanceiroContasAPagar.tsx` — Descrição `max-w-md`→`max-w-[190px]` (linhas individuais E filhas; segue `truncate`+`title`); Categoria pill `max-w-[160px]`→`max-w-[110px]`; Ações individuais `gap-1`→`gap-0.5` e botão "Pagar" `px-2`/`mr-1`→`px-1.5`/`mr-0.5`; cabeçalho de grupo "Expandir/Recolher" (texto) vira botão-ícone `ChevronRight` (rotaciona 90°); padding `px-3`→`px-2` nos cabeçalhos e células Valor/Status/Descrição-filha. Nenhuma coluna removida, nada perdido (texto longo no `title`). Validado (estático): esbuild EXIT 0 (`pnpm build`/`tsc` estouram OOM no container). Detalhe: `shared/changelog.ts`.
