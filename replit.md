...
- **Rev. 4550** — **REMOVE: ALMOXARIFADO — VISÃO "📍 SALDO POR OBRA" RETIRADA DO SELETOR.** A pedido do usuário ("tire isso, não precisa ter"), removida a visão obra→insumos criada na Rev. 2772: opção do seletor, bloco de render (~150 linhas), estados órfãos `porObraSaldo`/`porObraColapsadas` e o sentinela "porObra" do tipo de `obraContexto` + guards das queries. Consolidado, Central e obra específica intactos. Arquivo: `client/src/pages/almoxarifado/index.tsx`. ZERO schema/server change.
- **Rev. 4549** — **FIX: TERCEIROS — FILTRO DE OBRA EM FUNCIONÁRIOS TERCEIROS NÃO RETORNAVA NADA (+ TOLERÂNCIA NO DETECTOR DE BASELINE).** (1) Obras existem duplicadas entre empresas do grupo (mesmo nome, ids diferentes: HOTEL DO PAPA = 90004 × 270003); o funcionário terceiro guarda o obraId da empresa de cadastro e o dropdown lista as obras da empresa ativa → filtro por id nunca casava (fallback comparava nome contra id-string). Fix: casa por id OU nome normalizado (trim/case/acento) da obra selecionada; `obras` nas deps do useMemo. (2) Falso positivo do detector da Rev. 4548 no POITA: 2 tarefas de 1085 com baseline 3 dias além disparavam o alerta (igualdade estrita no envelope); agora envelope só diverge com deslocamento >7 dias; alerta falso limpo no Neon. Arquivos: `FuncionariosTerceiros.tsx`, `planejamento.ts`. ZERO schema change.
### 5 one-liners
...
Ver `replit-history.md` para revisões Rev. 4543 e anteriores.
