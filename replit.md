...
- **Rev. 2527** — **FOLHA DE PAGAMENTO · COMPARATIVO FOLHA × ERP (verba por verba, 1 linha por funcionário com expand).** User: "Quero uma tela comparativa Folha × ERP" → escolheu Visão COMPLETA. Nova ViewMode `comparativo_completo` + handler dispatch + banner azul (gradient blue/sky/cyan, ícone Scale) embaixo do banner vermelho do Relatório Consolidado no card Conferência. Componente `ComparativoFolhaErpView` (~370 linhas) com 5 KPIs (Funcionários / Líq.Folha / Líq.ERP parcial* / Soma diferenças / Com divergência), toolbar (busca + checkbox "Só com divergência" + ordenação nome/diferença/líquido), tabela de 10 colunas (Func, Sal.Base Folha/ERP, HE Folha/ERP, Desc.Folha/ERP, Líq.Folha, Diferença) com cell vermelha quando diff > R$1, click expande pra `DetalhamentoVerbasFuncionario` (2 cols Proventos × Descontos com TODAS verbas do PDF + INSS/IRRF/FGTS fixos + rodapé Líq.oficial), export CSV (BOM+`;` BR), print PDF. Reusa 100% backend: `listarItens` + `comparativoDescontos` + `cruzamentoHE`. Honesto: "Líquido ERP parcial" = Sal.Base + HE − Desc.Operacionais (sem INSS/IRRF/FGTS — explicito na legenda); HE ERP é proxy `(salário ÷ 220) × 1,5/h` fallback R$50/h. Arquivos: `client/src/pages/FolhaPagamento.tsx` L74/L2255/L7196/L9117. Zero backend tocado. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
