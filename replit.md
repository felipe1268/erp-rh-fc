...
- **Rev. 3027** — **FINANCEIRO → "ANÁLISE DE CUSTOS": (1) "CUSTO POR CATEGORIA" E "CUSTO POR CENTRO DE CUSTO" GANHAM VISUAL EMPILHADO DE PAGO (VERDE) × PREVISÃO (ÂMBAR); (2) NOVA TABELA NO FIM, COMPARATIVA MÊS A MÊS POR CATEGORIA, COM Δ (SUBIU/CAIU) ENTRE OS DOIS ÚLTIMOS MESES; (3) CATEGORIAS PADRONIZADAS NUMA TAXONOMIA CANÔNICA (ACABAM OS DUPLICADOS DE "ENCARGOS" ETC.) COM OS BUCKETS DA LITERATURA EXPLÍCITOS — FÉRIAS, 13º, RESCISÕES/DEMISSÕES, SEGURO DE VIDA, IMPOSTOS/TRIBUTOS, AÇÕES & ACORDOS TRABALHISTAS, PRÓ-LABORE, BENEFÍCIOS, TERCEIROS/PJ, FROTA, MATERIAL, ALUGUÉIS, DESP. FINANCEIRAS/ADMIN.** PEDIDO (print IMG_1913): "gráficos pago vs previsão + tabela mês a mês das categorias (subiu/desceu) + garantir férias/demissões + tirar duplicados + mapear seguro de vida/impostos/acordos pra saber onde cortar". SOLUÇÃO (FRONT-only + 1 módulo shared puro; ZERO ALTER/DROP/DELETE, ZERO backend/schema; 100% client-side sobre `financial.getContasAPagarByYear`). NOVO `shared/custosCategorias.ts`: `GRUPOS_CUSTO_ORDEM` (17 grupos) + `classificarGrupoCusto(contaNome, origemModulo)` — ORIGEM tem precedência (`ORIGEM_GRUPO`), senão keywords por PRECEDÊNCIA (`norm()` tira acento/caixa); mapeado contra dados REAIS do Neon; grupo só aparece se houver lançamento real. FRONT `FinanceiroAnaliseCustos.tsx`: helpers `pagoDe`/`previsaoDe`/`valorEfetivo`/`mesNumDe`; `porCategoria` agrupa por `classificarGrupoCusto` ({name,pago,previsao,value}); cards Categoria/Centro viram `<Bar stackId>` verde+âmbar com `<LabelList dataKey="value">` (total); Pareto opera sobre grupos; NOVO memo `tabelaMensal` (grupo×12 meses, lastIdx/prevIdx, totaisMes) + card-tabela (célula por mês BRLk com ▲/▼ vs mês anterior, coluna Δ% penúltimo→último, tfoot total geral, linha clicável → drill `grupo`); removidos imports órfãos `Cell`/`PIE_COLORS`. FRONT `FinanceiroAnaliseCustosDetalhe.tsx`: `keyOf.grupo`/`DIM_META.grupo`/`rotuloDim.grupo`/`case "grupo"` reaproveitam o motor de drill da Rev. 3024. REPUBLICAR (só front). Detalhe: `shared/changelog.ts`.
- **Rev. 3026** — **EQUIPAMENTOS PRÓPRIOS → BOTÃO "GERAR PREÇOS" (IA): A GERAÇÃO PASSA A MOSTRAR UMA EVOLUÇÃO DE 0 A 100% FASE A FASE (BARRA DE PROGRESSO + LISTA DE FASES + CONTAGEM VIVA DE EQUIPAMENTOS), EM VEZ DO ANTIGO "GERANDO PREÇOS…" ESTÁTICO SEM FEEDBACK.** PEDIDO (prints IMG_1910/IMG_1911): "ver a evolução de 0 a 100% fase a fase enquanto gera". CAUSA-RAIZ: a estimativa rodava numa ÚNICA chamada tRPC (SELECT combos pendentes → 1 `invokeLLM` de até 400 combos → loop de UPDATE) e só retornava no fim, sem progresso intermediário. SOLUÇÃO — PROCESSAMENTO POR LOTE (client-driven batching, 100% stateless: sem job em memória, sem tabela nova, robusto multi-instância; ZERO ALTER/DROP/DELETE). BACKEND (`server/routers/equipamentos.ts` · `propriosGerarPrecosComIA`): input ganha `offset`(0) + `loteMax`(30, cap 400); cada call processa UM lote e retorna `totalCombos`/`combosAnalisados`/`proximoOffset`/`haMaisLotes`; PAGINAÇÃO assimétrica — `sobrescrever=true` AVANÇA offset (condPreco=TRUE nunca encolhe → senão loop infinito), modo "só sem valor" fica em offset 0 (os já precificados saem do filtro, o conjunto encolhe sozinho); tenant guard + UPDATE idempotente por tríade descricao+marca+modelo mantidos. FRONT (`Proprios.tsx`): NOVO estado `precoRun` + driver `rodarGerarPrecos` (LOOP de `mutateAsync`, LOTE=30, captura `totalCombos` da 1ª resposta como denominador, acumula entre lotes, guard `MAX_ITER=500`); NOVO `Dialog` de progresso (`<Progress>` 0→100%, % grande, "X de Y combinações", "Lote N", lista das 3 fases ListChecks/Sparkles/Database com check, contagem viva, estados concluído/erro + "Fechar"; trava fechamento enquanto roda); botão do header troca Sparkles→Loader2 girando. REPUBLICAR (front+backend). Detalhe: `shared/changelog.ts`.
