...
- **Rev. 2867** — **COLETA DE CAMPO (RH) — NA PÁGINA PÚBLICA, QUEM JÁ FOI COLETADO SAI DA LISTA; FICAM SÓ OS QUE FALTAM.** Pedido: "quando coletar precisa sair da lista, ficando pendente somente os que falta fazer" (IMG_1685 = lista pública com TODOS, sem distinção do que já foi feito). FEITO (SÓ frontend `ColetaCampoPublica.tsx`): "coletado" = resposta `pendente` (na fila) OU `aprovada` → SAEM da lista; ficam só os que FALTAM (sem envio OU `rejeitada`/refazer). Filtro 100% client-side sobre o `jaEnviado` que o backend `dadosSessao` JÁ devolve; o `refetch` pós-envio (já existente) faz o card sumir sozinho. NOVO contador "N já coletado(s) · faltam M"; badge simplificado (rejeitados → rosa "↺ refazer"); empty-state contextual (obra vazia / tudo coletado / busca sem resultado). ZERO ALTER/DROP/DELETE; ZERO schema; ZERO backend. Detalhe: `shared/changelog.ts`.
