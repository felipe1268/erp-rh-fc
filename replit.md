...
- **Rev. 2786** — **PLANEJAMENTO · REFIS (RELATÓRIO DE EVOLUÇÃO FÍSICA DA OBRA): REPAGINAÇÃO DA IMPRESSÃO — A MARGEM PADRÃO PASSOU A SER 3cm EM TODOS OS LADOS (LATERAIS + SUPERIOR + INFERIOR); ACABOU O RELATÓRIO "COLADO NA BORDA".** Pedido (usuário): "faz uma repaginação completa do layout de impressão, quero margem lateral de 3cm e superior e inferior de 3cm tbm.. esta colada na porta não pode" (anexou PDFs paisagem/retrato com o relatório encostado nas bordas). A Rev. 2785 corrigira o deslocamento fazendo o relatório OCUPAR A FOLHA INTEIRA — mas o deixou colado nas margens. Causa: a margem vem do `@page { margin: ${refisMargemMm}mm }` (controle da Rev. 2784), com default 8mm (~0.8cm) e valor antigo preso no `localStorage`. Fix (SÓ CLIENT; ZERO SCHEMA/SERVER) em `client/src/pages/planejamento/PlanejamentoDetalhe.tsx` (componente `Refis`): (1) DEFAULT da margem = 30mm (3cm) — o `#refis-print-area` (`position:absolute`) ancora na área útil DENTRO das margens do `@page` (graças à neutralização do shell da Rev. 2785), então a margem do `@page` vira margem REAL e uniforme nos 4 lados, em todas as páginas; (2) MIGRAÇÃO da chave do `localStorage` (`refisMargemMm`→`refisMargemMmV2`) p/ o 3cm valer já, sem ficar preso ao 8mm salvo; (3) RANGE do controle ampliado 0-25mm→0-40mm (input+clamp) e "reset" agora volta p/ 30mm (era 8mm). Mantido o motor de impressão da Rev. 2785 + Zoom (%) da Rev. 2784 (default 100%). ZERO ALTER/DROP/DELETE. Validação: esbuild OK; HMR; architect. Detalhe: `shared/changelog.ts`.
