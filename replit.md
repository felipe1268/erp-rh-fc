...
- **Rev. 2533** — **PLANEJAMENTO · CAMINHO B (FONTE ÚNICA = `PercentComplete`).** User: "sempre iremos ler a mesma coluna para não ter inconsistência MSP × ERP". 1 upload no cadastro → ERP grava `baseline_start`/`baseline_finish` por atividade E EXPANDE o PREVISTO semana-a-semana pela fórmula nativa do MSP `floor(((cutoff − BL_Start)/(BL_Finish − BL_Start))*100)` (snapshot `planejamento_projetos.previsto_semanas_json`). Avanço semanal continua lendo `PercentComplete` por atividade. Parser (`ImportarCronograma.tsx` L52-60 interface, L470-490 lê `<Baseline Number=0>`) trata `NA`/`2049` como ausente → fallback Start/Finish vigente. Server helper `regenerarPrevistoSemanasCaminhoB` (`server/routers/planejamento.ts` L96-203) roda pós-transaction em `salvarAtividades`; falha silenciosa. Schema: 3 colunas novas via SyncSchema+ (`server/_core/index.ts` L724-732) — `ADD COLUMN IF NOT EXISTS`, zero destrutivo. UI consumption (cards/Curva S leem snapshot) FICA pra Rev. 2534. Validável agora via log `[Caminho B] Projeto X rev Y: snapshot gerado (N semanas × M folhas)`. Detalhe: `shared/changelog.ts`.
