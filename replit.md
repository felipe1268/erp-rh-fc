...
- **Rev. 4614** — **FEAT: CRACHÁS — TODOS OS TREINAMENTOS VIGENTES NA FRENTE (PILLS SUTIS).** Pedido do usuário: crachá deve indicar todos os treinamentos de forma sutil. `badgeStatus` devolve `treinamentos: string[]` (vigentes, dedup por tipo canônico da Rev. 4613, rótulo padronizado "NR-XX"); frente do crachá ganha fileira de pills navy-outline após os selos NR-35/NR-10 (excluídos das pills p/ não duplicar; cap 8 + "+N"; entra no cálculo compacto/denso da Rev. 4612); lista de crachás mostra as mesmas pills (cap 4). Vale p/ o PNG baixado. Arquivos: `server/routers/sprint1Foundation.ts`, `client/src/pages/terceiros/Crachas.tsx`. ZERO schema change.
### 5 one-liners
