...
- **Rev. 2376** — **UX/ALERTA · Botão "ENTRADA" do Almoxarifado também PISCA com badge vermelho mostrando quantas OCs de MATERIAL estão pendentes de recebimento (complementa Rev. 2375 que tratou só LOCAÇÃO).** Pedido user (IMG_1177, 24/05/2026): "Tem várias OCs para receber, mas não tem alerta nem informativo de alerta" — o modal "Receber Material" mostrava 37 pendentes mas o botão ENTRADA estava mudo (Rev. 2375 só cobriu OCs flagged `isLocacao=true`, deixando as compras normais que entram via SmartEntry sem indicador). **Implementação** (`client/src/pages/almoxarifado/index.tsx`, UI-only): nova `trpc.warehouse.listPendingOCs.useQuery` (mesma query do SmartEntry) com refetchInterval=60s + refetchOnWindowFocus; botão ENTRADA recebe o mesmo tratamento da Rev. 2375 quando `qtdMaterialPendente > 0` (ring-4 ring-amber-300 animate-pulse + badge vermelho redondo -top-2 -right-2 animate-bounce + overlay animate-ping + sublabel "{N} pra receber" + tooltip). **R-001/R-007/R-010 OK:** UI-only, zero backend novo, zero DDL.
- **Rev. 2375** — **UX/ALERTA · Botão "RECEBER LOCAÇÃO" do Almoxarifado agora PISCA com badge vermelho mostrando quantos equipamentos estão pra chegar (OCs de locação aguardando recebimento).** Pedido user (IMG_1176, 24/05/2026): "Quero que fique um alerta, com quantidade e alerta visual, piscando para que saibam que tem produto para chegar… quero que o alerta indique a quantidade de os que iram chegar". **Implementação** (`client/src/pages/almoxarifado/index.tsx`, UI-only): nova `trpc.equipamentos.ocsLocacaoPendentes.useQuery` no topo do componente (refetchInterval=60s + refetchOnWindowFocus) — reusa endpoint da Rev. 2371, zero backend novo. Quando `qtdLocacaoPendente > 0` o botão ganha: (1) `ring-4 ring-amber-300 ring-offset-2 animate-pulse` no botão inteiro, (2) badge vermelho `absolute -top-2 -right-2` redondo com o N + `animate-bounce`, (3) overlay `absolute inset-0 animate-ping bg-amber-400/30` (efeito de "está chegando"), (4) sublabel branca "{N} pra chegar" embaixo do "RECEBER LOCAÇÃO", (5) tooltip "{N} equipamento(s) pra chegar — toque pra receber". Quando não há pendência, visual volta ao normal. **R-001/R-007/R-010 OK:** UI-only, zero DDL, zero novas rotas tRPC.
