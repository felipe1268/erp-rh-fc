...
- **Rev. 2984** — **PLANEJAMENTO — USUÁRIO RESTRITO POR OBRA (ex.: engenheiro de campo "Mateus") NÃO VIA AS ATIVIDADES/DADOS DO PROJETO ("Nenhuma atividade cadastrada" / "0 atividades") MESMO COM O PROJETO LISTADO; CORRIGIDO PARA TODOS OS USUÁRIOS RESTRITOS.** PEDIDO: Mateus via o projeto e o avanço (Previsto 36% / Realizado 34%) na tela de detalhe, mas "0 atividades"; arrumar p/ todos os restritos verem o que têm permissão. CAUSA-RAIZ: `listarProjetos`/`dashboardGeral` resolvem permissão por OBRA (`users.allowed_obra_ids` ou e-mail→`employees`→`obra_funcionarios`), então o projeto APARECE; mas as queries de leitura do detalhe (`listarAtividades`, `getDataCorte`, `kpiResponsavelPorProjeto`, `diagnosticoEapOrcVsCron`) usavam compare ESTRITO `projeto.companyId === ctx.user.companyId` → usuário multi-empresa (empresa-default ≠ empresa do projeto da obra permitida) passava no catálogo mas era bloqueado no detalhe. SOLUÇÃO (BACKEND-only, ZERO ALTER/DROP/DELETE): helper ÚNICO `resolvePlanAllowedObraIds` (replica a régua do catálogo; admin→null) em `server/routers/planejamento.ts`; `listarProjetos`/`dashboardGeral` refatorados p/ chamá-lo (puro refactor); as 4 queries de detalhe passam a ler o `obraId` do projeto e validar via helper (restrito → obra do projeto ∈ obras permitidas). RESSALVA DELIBERADA: as MUTATIONS mantêm o guard estrito por `companyId` (tarefa é de VISUALIZAÇÃO, não ampliar escrita). Detalhe: `shared/changelog.ts`.
