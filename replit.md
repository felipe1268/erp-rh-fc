...
- **Rev. 3319** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · AO SELECIONAR UM MÊS SEM ESCOLHER UMA CONTA, A TELA SÓ MOSTRAVA "SELECIONE UMA CONTA BANCÁRIA"; AGORA EXIBE O "PANORAMA GERAL DO MÊS": TOTAIS AGREGADOS (CONCILIADOS / NO EXTRATO SEM LANÇAMENTO / NO ERP SEM EXTRATO / % CONCILIADO) DE TODAS AS CONTAS COM EXTRATO + UM BLOCO EXPANSÍVEL POR CONTA (LISTAS EXTRATO×ERP, CHEQUES DEVOLVIDOS, JÁ CONCILIADOS) E DRILL-IN. O USUÁRIO PODE CONCILIAR 1-A-1 DENTRO DO PANORAMA (BARRA INFERIOR + DIÁLOGO DE CONFIRMAÇÃO) SEM ENTRAR EM CADA CONTA. 100% FINANCEIRO (1 BACKEND READ-ONLY + GUARDS NAS 2 MUTATIONS + 1 FRONT) · ADITIVO · REGRA DE OURO MANTIDA (NADA CONCILIA SEM CONFIRMAÇÃO EXPLÍCITA; ZERO BULK ONE-CLICK) · ZERO SCHEMA/ALTER/DROP/DELETE.** BACKEND (`server/routers/financial.ts`): (a) o motor de `getConciliacaoReport` foi EXTRAÍDO p/ helper module-level `_computeConciliacaoReport(db, companyId, contaBancariaId, dataInicio, dataFim)` (refactor puro — a procedure só faz auth + delega); (b) nova query read-only `getConciliacaoReportGeral({companyId, dataInicio, dataFim})` lista as contas COM extrato no período (JOIN `company_bank_accounts` ⋈ `bank_statement_lines` `excluido_em IS NULL`), roda o MESMO motor por conta, tagueia cada linha com a conta de origem, agrega `totais` e inclui `lancamentosSemConta` (company-wide) UMA vez — ids sintéticos de grupo (`grp:…`) são re-chaveados por conta (`#cNN`) p/ não colidir no array concatenado (`itensIds` reais intactos); (c) GUARD DE CONTA server-side nas mutations: `conciliarLancamento` pré-checa que a conta da linha e a do lançamento batem (lançamento sem conta casa com qualquer); `conciliarGrupoLancamentos` filtra membros pela conta da linha e reaponta o `entry_id` p/ o 1º membro realmente baixado (evita vínculo cross-account). FRONT (`FinanceiroConciliacao.tsx`): query habilitada só com `mesSel != null && !contaBancariaId`; empty-state virou painel "Panorama geral do mês" (KPIs + blocos de conta expansíveis com `setContaBancariaId` p/ drill-in + bloco "sem conta"); barra inferior + `AlertDialog` → `conciliarMut`/`conciliarGrupoMut` (guard que bloqueia quando as contas diferem); `useEffect` limpa a seleção ao trocar panorama↔conta. Todas as rotas mantêm `_assertFinanceiroCompanyAccess`. Detalhe: `shared/changelog.ts`.

- **Rev. 3318** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · O BOTÃO "DESCONSOLIDAR MÊS" QUEBRAVA COM "ERRO AO DESCONSOLIDAR — DB: code=25P02 | msg=current transaction is aborted, commands ignored until end of transaction block" EM EMPRESAS QUE NUNCA USARAM CONCILIAÇÃO EM GRUPO; QUEM CONSOLIDOU UM MÊS POR ENGANO NÃO CONSEGUIA REVERTER. AGORA DESCONSOLIDA NORMAL. 100% BACKEND · BUGFIX · ZERO SCHEMA/ALTER/DROP/DELETE.** RAIZ: `desconsolidarMes` (`server/routers/financial.ts`) roda 3 etapas em UMA transação; o passo 1b reverte os MEMBROS de conciliações EM GRUPO via a tabela-link `financial_conciliacao_grupo`, que é AUTO-CRIADA só no 1º uso desse recurso (logo NÃO EXISTE em empresas que nunca usaram grupo). O passo 1b tinha um `try/catch` que tentava "tolerar" a ausência engolindo `relation ... does not exist` — mas no Postgres QUALQUER statement que falha ABORTA a transação inteira, e engolir o erro no JS NÃO desfaz o aborto, então o passo 2 (UPDATE em `bank_statement_lines`) estourava com `25P02`, mascarando a causa real. CORREÇÃO: checar a existência da tabela ANTES da transação (`SELECT to_regclass('public.financial_conciliacao_grupo')`, FORA do bloco) e só rodar o passo 1b quando `temGrupo`; sem `try/catch` interno → a transação nunca é envenenada. Empresas que usam grupo seguem idênticas; erros REAIS de SQL voltam a propagar/reverter. LIÇÃO: nunca tolerar tabela/coluna inexistente com try/catch DENTRO de transação Postgres — cheque com to_regclass FORA dela ou use SAVEPOINT. Detalhe: `shared/changelog.ts`.
