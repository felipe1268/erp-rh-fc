...
- **Rev. 3259** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · NO PAINEL "CHEQUES DEVOLVIDOS NO BANCO", A LEGENDA DO MOTIVO DA DEVOLUÇÃO PASSOU A MOSTRAR A DESCRIÇÃO REAL (BACEN/COMPE) AO LADO DO CÓDIGO — INCLUSIVE O MOTIVO 39, QUE ANTES CAÍA NO GENÉRICO "DEVOLUÇÃO DE CHEQUE (MOTIVO 39)". AGORA O 39 APARECE COMO "MOTIVO 39 · IMAGEM DO CHEQUE FORA DOS PADRÕES TÉCNICOS DA COMPE (TRUNCAGEM)", E TAMBÉM FORAM ADICIONADOS OS MOTIVOS 26 (INOPERÂNCIA TEMPORÁRIA DE TRANSPORTE) E 27 (FERIADO MUNICIPAL NÃO PREVISTO), COMPLETANDO A TABELA OFICIAL. SÓ CONSULTA.** PEDIDO (piloto FC): "Coloca a legenda de cada motivo possível que o cheque voltou, exemplo qual é o motivo 39? Coloque ao lado" (print IMG_2186). CAUSA: a biblioteca `shared/chequeMotivos.ts` (`MOTIVOS_DEVOLUCAO_CHEQUE`) não tinha o código 39 → `getMotivoDevolucao` caía no fallback genérico; o badge da tela JÁ mostra `motivoTexto` ao lado do código (Rev. 3244), então bastava preencher o dicionário. SOLUÇÃO (SÓ DADOS, `shared/chequeMotivos.ts`): adicionados os motivos oficiais que faltavam, confirmados contra o Manual Operacional da COMPE/FEBRABAN e a regulamentação BACEN (Circ. 3.535 / Res. 1.682): `39`="Imagem do cheque fora dos padrões técnicos da COMPE (truncagem)" (operacional, reapresentável), `26`="Inoperância temporária de transporte" e `27`="Feriado municipal não previsto no calendário" (ambos impedimento, reapresentável). O `33` ("Divergência de endosso") foi conferido contra a tabela oficial e MANTIDO (fontes secundárias o confundem com "numérico×extenso", mas a Res. 1.682 define 33 = endosso). Efeito vale p/ a tela E p/ o relatório PDF (Rev. 3252), pois leem o mesmo `motivoTexto`. ZERO BACKEND NOVO · ZERO SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
