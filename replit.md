...
- **Rev. 2401** — **CONFIGURAÇÕES/UX · Barra de abas com scroll horizontal em vez de quebrar texto em várias linhas.** Pedido user (IMG_image_1779710232514, 25/05/2026): print mostrou abas exibindo "Regras / de / Ouro", "Critérios / do / Sistema", "Templates / de / Documentos" — cada label estourando em 3 linhas verticais. Causa-raiz: container `flex gap-1` sem `overflow` nem `whitespace-nowrap` + 14 abas forçando o flex a squeezar cada botão até quebrar nos espaços. **Fix** (`client/src/pages/Configuracoes.tsx` L425-447): wrapper externo `overflow-x-auto` + container interno `inline-flex min-w-full` (preserva bg cinza full-width quando há espaço sobrando) + botões com `flex-shrink-0 whitespace-nowrap` (mantêm tamanho intrínseco; quando passa do viewport, vira scroll horizontal). Ícone também `flex-shrink-0`. Zero mudança em lógica/estado/rotas. R-001/R-007/R-010 OK (frontend-only). _(Aposentada pela Rev. 2402 — flex-wrap no lugar de scroll-x.)_
