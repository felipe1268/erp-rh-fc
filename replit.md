...
- **Rev. 2499** — **AVISO PRÉVIO · UX · Botão do modal agora mostra "Salvar Alterações" no modo edição.** User: "está dando erro ao salvar o aviso ... o aviso da kellen ja foi criado, porem nao havia selecionado o tipo de redução de jornada, eu quero atualizar essa informação, mas nao tem botao SALVAR, entao quando eu clico em CRIAR AVISO PREVIO ele dá [Unexpected end of JSON input]". Causa raiz UX: botão primário do modal (`AvisoPrevio.tsx` L3212) sempre exibia "Criar Aviso Prévio" mesmo com `editingItem` setado — `handleSubmit` (L449) já roteava certo (`updateAviso` se editando), mas o LABEL enganoso fazia a usuária abrir o "+" novo achando ser a única forma de mudar a redução, e o backend rejeitava a duplicata sem JSON limpo. Fix em L3215-3221: label condicional (`isPending → "Salvando..."` / `editingItem → "Salvar Alterações"` / `isPedidoDemissao → "Registrar Pedido de Demissão"` / default → "Criar Aviso Prévio") + `disabled` agora respeita `updateAviso.isPending` também. Frontend-only, sem schema change. Detalhe: `shared/changelog.ts`.
