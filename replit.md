...
- **Rev. 2352** — **CLEANUP/UX · Removida a subpágina "Parâmetros CAPEX" da UI** (sidebar + card no hub + rota + page component + mapeamento de módulo). Pedido user (24/05/2026, IMG_1148): "Retire esta função de capex não precisa mais aqui". **Escopo**: `client/src/pages/equipamentos/ParametrosCapex.tsx` deletado; `App.tsx` (lazy import + rota `/equipamentos/parametros-capex` removidos); `ModuleContext.tsx` (entrada removida); `DashboardLayout.tsx` (item removido do grupo "Controle de Equipamentos"); `pages/equipamentos/index.tsx` (card removido, import `Settings` não usado removido, grid `sm:grid-cols-3` → `sm:grid-cols-2`). **Preservado no backend**: procedures `parametrosCapexListar`/`parametrosCapexAtualizar`, `seedParametrosCapexIfNeeded` e `PARAMETROS_CAPEX_DEFAULTS` continuam intocados em `server/routers/equipamentos.ts` (sem UI chamando, o seed nem roda — zero efeito colateral). Tabela `parametros_capex` no DB também preservada (R-001 proíbe DROP). **NÃO confundir com CAPEX financeiro**: o cálculo Caixa Livre (FCL = FCO − CAPEX) em `financialKpiService.ts` é INDEPENDENTE e está preservado. **R-001/R-007/R-010:** N/A — só remoção de UI client-side.
