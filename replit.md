...
- **Rev. 2447** — **ALMOXARIFADO · INVENTÁRIO VISUAL · banner "Rotina diária" fixo no rodapé reforça que a aferição é uma tarefa de TODO DIA.** User (print 22:49) pediu pra deixar a mensagem "feita por obra, todo dia" visível no final da página em qualquer estado (não só no placeholder vazio). **Fix CSS-only:** `client/src/pages/almoxarifado/InventarioVisual.tsx` L825-840 — bloco amber discreto (`mt-6 rounded-2xl border-amber-200 bg-amber-50/70`) com ícone `ClipboardList` em círculo, título "Rotina diária" e copy curto explicando que a aferição mantém o saldo fiel ao campo e dispara alertas de consumo anormal. Visível em TODOS os estados (sem obra, obra sem baia, sessão em andamento/concluída, modo Todas as obras). Não é sticky — rodapé natural, pra não competir com o CTA dos cards no iPad. R-001/R-007/R-010 OK. Detalhe: `shared/changelog.ts`.
