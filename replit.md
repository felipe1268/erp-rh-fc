...
- **Rev. 1798**: **R-013 · Auto-sincronização de NOMES com o orçamento (sem botão) + bloqueio de EAP inexistente**. User: "nome E código devem ser EXATAMENTE idênticos ao orçamento" / "não quero botão, quero que seja automático". Em `salvarAtividades` (planejamento.ts L969-1052), quando o projeto tem orçamento vinculado: para cada folha cujo eapCodigo casa com item do orçamento, **sobrescreve `r.nome` com `orcIt.descricao`** (texto EXATO) sempre que houver qualquer diferença (caixa/acento/espaço). Determinístico, sem fuzzy match, sem perguntar. Quando EAP NÃO existe no orçamento: aborta com TRPCError BAD_REQUEST listando até 20 EAPs problemáticos — nunca renumeração silenciosa. Indiretas/Marcos/Grupos/Disabled isentos. Em `DiagnosticoEapOrcCron.tsx` removido botão de sync; aviso de descrição divergente apenas informa que será corrigido auto no próximo save. Removido proc tRPC `sincronizarNomesComOrcamento` (não usado).
