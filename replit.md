...
- **Rev. 2530** — **INVENTÁRIO SEMANAL · BUSCA + LEITOR DE CÓDIGO DE BARRAS (scan → baixa BATE automática).** User com prints: "Preciso de um campo de busca que leia código de barras pra facilitar a busca e baixa do material". Server `warehouse.getInventorySessionItems` (~L1086-1088) passou a retornar `itemCodigoBarras` + `itemCodigoInterno` (LEFT JOIN já existente, Rev. 2439). Client `almoxarifado/Inventario.tsx` (~L452-520) novo input emerald com ícone `ScanLine` embaixo da barra de progresso: filtra `pendentes`/`finalizados` por nome/cód. interno/cód. barras em tempo real; `autoFocus`+`autoComplete=off` scanner-friendly. AUTO-CONFIRMAÇÃO no Enter: match EXATO por código (prioridade) OU único pendente filtrado → `confirmItem.mutate({ quantidadeFisica: sistemaQtd })` (status=`conferido`, diff=0), limpa input e refoca. Hint "Enter confirma BATE quando 1 só item bate". Botão X pra limpar. Arquivos: `server/routers/warehouse.ts` ~L1086-1088 + `client/src/pages/almoxarifado/Inventario.tsx` L2/L12/L205-207/L268-293/L452-520. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
