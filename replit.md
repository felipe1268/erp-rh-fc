...
- **Rev. 4494** — **FIX: "INCLUIR DA EAP" NA COTAÇÃO — 3 BUGS (VÍNCULO, PREÇO META E SINCRONIA SC).** `adicionarItensEAPCotacao` descartava `orcamentoItemId`/`eapCodigo`/`precoMeta` no frontend → SC entrava sem preço nem vínculo ao orçamento. Picker não sinalizava itens já na cotação. Fix: (1) frontend passa os 3 campos, backend os grava; (2) `getItensEAPParaCotacao` retorna `jaEmCotacao:boolean` + badge âmbar; (3) exclusão cascateia para SC. ZERO schema change.
