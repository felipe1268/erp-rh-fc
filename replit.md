...
- **Rev. 2358** — **FEATURE/UX · Import PDF de locação ganha campo "Fornecedor (locadora) deste PDF" + botão "Aplicar a todos" pra padronizar o fornecedor em todos os contratos do mesmo PDF de uma vez.** Pedido user (24/05/2026, IMG_1154 + JALVES_*.pdf): "Preciso ter uma forma de indicar o fornecedor... todos itens e da empresa JALVES locações". O print mostrava coluna Fornecedor preenchida com "6716-FC ENGENHARIA E CONSTRUÇÃO LTDA" (locatário do cabeçalho F051/R051) em vez de "JALVES" (locadora real). **Causa raiz**: cabeçalho do F051/R051 emitido pela JALVES traz o nome do CLIENTE (FC) — o nome JALVES está no rodapé/logo, frequentemente perdido pelo Gemini Vision. Parser então usa o primeiro nome de empresa que vê (o errado). **Fix sem mexer no parser** (`client/src/pages/equipamentos/Locados.tsx`): (1) Input no topo do preview (faixa amber) com `datalist` populado por `trpc.compras.listarFornecedores` (autocomplete). (2) Botão "Aplicar a todos" que faz `importPreview.map(c => ({...c, fornecedorNome: nome}))`. (3) Diagnóstico inline: ⚠ amarelo se nenhum preenchido, ✅ verde se todos iguais, ⚠ lista N diferentes detectados. Campos por contrato continuam editáveis individualmente — feature é puramente UX (encurta de N edits pra 1 clique no caso comum). **R-001/R-007/R-010:** N/A — só UI client-side, zero DDL, zero mutations novas.
