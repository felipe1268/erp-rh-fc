...
- **Rev. 2433** — **ALMOXARIFADO · INVENTÁRIO VISUAL · fix de layout: foto da baia vazando sobre o nome + mini-cards (Chegou hoje / Restante / Consumo) no Safari iPad.** Print anexado pelo user (QIU 2 - FASE 4): card da baia "areia" renderizava a foto cobrindo TODO o card, deixando nome+badge+linha "Saldo no sistema"+grid de mini-cards escondidos atrás da imagem. **Causa-raiz:** div da foto era `relative h-32` SEM `overflow-hidden` próprio + img `w-full h-full object-cover` — em Safari iPad com imagem de proporção muito vertical, `h-full` na img filha falha e a imagem transborda. **Fix (arquivo único, zero backend):** `client/src/pages/almoxarifado/InventarioVisual.tsx` L327-345 — wrapper passa a ter `overflow-hidden flex-shrink-0` + img vira `absolute inset-0 w-full h-full object-cover block` (pattern bulletproof: img fica posicionada dentro do retângulo de 128px independente da proporção natural; `block` mata o gap inline); placeholder do ícone Camera idem (`absolute inset-0`). + `loading="lazy"` de bônus. R-001/R-007/R-010 OK. Detalhe: `shared/changelog.ts`.
- **Rev. 2432** — **ALMOXARIFADO · INVENTÁRIO SEMANAL · `window.confirm` nativo substituído por AlertDialog estilizado ao cancelar a sessão.** Print anexado pelo user (iPad Safari): ao clicar em "Cancelar" no header da sessão ativa, abria o confirm nativo do iOS mostrando "b41aedae-...replit.dev diz" (URL crua) + 3 botões OK/Cancelar/**Bloquear caixas de diálogo** (vermelho — risco do user clicar e perder TODOS os confirms da app). **Arquivo único (zero backend):** `client/src/pages/almoxarifado/Inventario.tsx` — (a) L9-12: imports `AlertDialog/Action/Cancel/Description/Footer/Header/Title` do shadcn; (b) L142-144: estado `confirmCancelar`; (c) L260: onClick passa a abrir o modal; (d) L270-304: bloco AlertDialog novo logo após o header com ícone `AlertTriangle` vermelho, texto destacando "apagados", linha de contexto "Semana XXX · {obra}", botão "Manter inventário" + "Sim, cancelar" vermelho com `Loader2` enquanto a mutation `warehouse.cancelInventorySession` roda (intocada). Mesmo padrão da Rev. 2424 em `PlanejamentoLista`. R-001/R-007/R-010 OK. Detalhe: `shared/changelog.ts`.
