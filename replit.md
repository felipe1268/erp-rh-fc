...
7. **Financeiro** (Rev.1009) — 18 DB tables, 30+ tRPC endpoints, 13 React pages. Fase 1 "Onde está meu dinheiro HOJE": Dashboard Executivo (saldo consolidado, KPIs mês com variação vs anterior, evolução 30d, top despesas, resultado por obra, alertas vencidos), Lançamentos Recorrentes (CRUD completo, geração automática com idempotência, pausar/retomar, 5 frequências), Importação OFX/CSV (parsing, detecção duplicatas, validação ownership), Conciliação Bancária semi-automática. Tabelas: `financial_entries`, `company_bank_accounts` (camelCase: companyId, tipoConta), `bank_statement_lines`, `bank_daily_balance`, `cash_flow_forecast`, `financial_approvals`, `financial_budget`, `financial_recurring_entries`, `financial_opening_balances`. DRE, fluxo de caixa, obrigações fiscais, plano de contas, centros de custo
...
11. **Frotas** — Controle de Frotas e Veículos. 10 submódulos: Dashboard (KPIs consolidados, patrimônio, alertas, custo/km, composição por tipo/marca, gráficos mensais interativos), Veículos (CRUD expandido com consulta FIPE automática via parallelum API, depreciação, RENAVAM, chassi, cor, responsável), Manutenções (preventiva/corretiva, custo, km, fornecedor, próxima manutenção, alertas de atraso, importação OS com IA), Combustível (abastecimentos, importação CSV/PDF, consumo km/l, preço por litro, posto, motorista, gestão de aliases motoristas, limpar mês/tudo), Pedágios e Sem Parar (CRUD, seletor de meses, KPIs, importação PDF com IA — analisa faturas Sem Parar/ConectCar/Veloe, seleção individual de itens, categorias: pedágio/sem_parar/estacionamento/recarga_tag), Multas (auto de infração, gravidade, pontos, valor com desconto, motorista, local, status pendente/paga/recorrida), IPVA (ano referência, parcelas, status, alertas de vencimento), Licenciamento (ano exercício, CRLV emitido, status), Seguros (seguradora, apólice, cobertura, prêmio, franquia, vigência, análise de apólice com IA via invokeLLM), Rastreamento (mapa Leaflet com trajetos por veículo, importação CSV, cores por veículo, pontos interativos). Tabelas: `fleet_maintenances`, `fleet_fuel_records`, `fleet_toll_records`, `fleet_tracking_points`, `fleet_documents`, `fleet_fines`, `fleet_ipva`, `fleet_licensing`, `fleet_insurance`, `fleet_driver_aliases` + campos expandidos em `vehicles`. Router: `server/routers/frotas.ts`. Frontend: `client/src/pages/frotas/`. Rotas: `/frotas/painel`, `/frotas/veiculos`, `/frotas/manutencoes`, `/frotas/combustivel`, `/frotas/pedagios`, `/frotas/rastreamento`, `/frotas/multas`, `/frotas/ipva`, `/frotas/licenciamento`, `/frotas/seguros`. Tema: cyan. Ícone: Truck.
