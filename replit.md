...
- **Rev. 4581** — **FIX+FEAT: FLUXO DE CAIXA — TRANSFERÊNCIAS AO PRÓPRIO GRUPO FORA DAS SAÍDAS + CONFERÊNCIA DE DUPLICIDADES.** Continuação da auditoria do déficit: 36 despesas (R$ 660.524) que eram PIX/TED para empresas do próprio grupo (CNPJ 29.353.906 / FELIPE COSTA ALVES ME) reclassificadas no Neon como `origem_modulo='transferencia_interna'` (reversível); client exclui das Saídas (igual 'aplicacao_financeira') e a linha azul do server passa a contar as duas origens. FEAT: conferência de possíveis duplicidades com humano no circuito — `getPossiveisDuplicidades` (pares mesmo valor >R$ 3 mil, ≤10 dias, mesmo token de texto; 67 pares/R$ 598 mil), card rosa na tela com botões por par "Cancelar este (duplicado)" / "Não é duplicidade" (`confirmarDuplicidade` reversível via status='cancelado'+motivo; `descartarDuplicidade` via tag `[dup-ok:<id>]` nas observações). NADA automático (pedido do usuário). Restante do déficit (~1 mi) é operacional real. Arquivos: `financial.ts`, `FinanceiroFluxoCaixa.tsx`. ZERO schema change.
- **Rev. 4580** — **FIX+UX: FLUXO DE CAIXA — APLICAÇÕES FINANCEIRAS FORA DAS SAÍDAS + MODO SIMPLES.** Causa-raiz do déficit fantasma: 66 lançamentos "APLICACAO CONTAMAX" (R$ 3.204.643) criados via conciliação com origem NULL estavam como DESPESA — mas aplicação é o próprio dinheiro indo pro investimento; os resgates (~3,18 mi) voltavam só como "outras movimentações", nunca como receita → a matriz via a ida e não a volta. Fix: (1) UPDATE reversível no Neon (origem_modulo='aplicacao_financeira' nos 66); (2) client exclui essa origem de despBuckets/despSplit; (3) endpoint de movimentações passa a contar essas linhas na linha informativa azul (simetria ida+volta). UX: novo modo **Simples (padrão)** × Detalhado — Poka-Yoke nível 3: 3 cartões grandes (Entrou/Saiu/No banco hoje com saldo real do extrato), matriz enxuta (5 linhas), 3 insights; expandir no Simples leva ao Detalhado (tela 4578/4579 intacta). Verificado: "FOL PAGTO" 963 mil NÃO duplica com folha. Arquivos: `FinanceiroFluxoCaixa.tsx`, `financial.ts` (1 condição). ZERO schema change.
### 5 one-liners
...
- **Rev. 4576** — **UX: CONTAS A PAGAR — CARDS DE KPI VIRAM FILTROS CLICÁVEIS.** 5 cards viram botões-toggle (`filtroKpi` com precedência sobre pills; "Em Aberto Acum." usa ano inteiro; ring ativo + "Filtrando · toque p/ limpar"). Detalhe em `shared/changelog.ts`. ZERO schema/server change.
...
Ver `replit-history.md` para revisões Rev. 4574 e anteriores.
