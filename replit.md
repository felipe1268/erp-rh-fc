...
- **Rev. 3327** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · NO "PANORAMA GERAL DO MÊS" OS 7 CARDS DE CIMA (TOTAL DE ENTRADAS, TOTAL DE SAÍDAS, SALDO DO MÊS, CONCILIADOS, NO EXTRATO SEM LANÇAMENTO, NO ERP SEM EXTRATO, % CONCILIADO) ERAM SÓ NÚMEROS INERTES; AGORA CADA CARD É CLICÁVEL E ABRE UM DIÁLOGO COM TODAS AS LINHAS/INFORMAÇÕES QUE COMPÕEM AQUELE NÚMERO (LISTA DETALHADA OU RESUMO POR CONTA). 100% FRONT · UX/ADITIVO · REGRA DE OURO MANTIDA (SÓ LEITURA) · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** Pedido (print: "os cards não estão responsivos, quero clicar neles e abrir todas informações pertinentes"). `client/src/pages/financeiro/FinanceiroConciliacao.tsx`: os 7 cards viraram `<button>` (hover + ícone `Eye`) que setam o estado `panoramaDrill` ("entradas"|"saidas"|"saldo"|"conciliados"|"extratoSemLanc"|"lancSemExtrato"|"pct"); um `Dialog` (full-screen no mobile, amplo no desktop) renderiza o conteúdo. SEM backend novo — `getConciliacaoReportGeral` já devolve por conta `conciliados`/`extratoSemLancamento`/`lancamentosSemExtrato`; memo `drill` ACHATA pra empresa toda (filtra pares de estorno, mesma regra) e deriva entradas (valor>0)/saídas (valor<0) de `[conciliados+extratoSemLancamento]`. Cards de lista → linhas (data, descrição/fornecedor, conta, valor BRL colorido) + olho que abre o detalhe consultivo (`setDetalheEntryId`, só ids reais); "Saldo" → resumo + saldo por conta; "% conciliado" → barra + fração por conta. Rodapé reforça "só leitura". Moeda `formatBRL`. Sem rota/permissão nova. Detalhe: `shared/changelog.ts`.
