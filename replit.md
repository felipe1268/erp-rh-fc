...
- **Rev. 3288** — **PLANEJAMENTO / PORTAL DO CLIENTE · O "% PREVISTO" E A CURVA S DO PORTAL DIVERGIAM DO MÓDULO PLANEJAMENTO (FONTE DA VERDADE): NA OBRA REVTE-CIVIL O PORTAL MOSTRAVA PREVISTO 8% (DESVIO +1%) ENQUANTO O MÓDULO MOSTRAVA 9% (DESVIO 0) NO CUTOFF 11/06/2026. AGORA O PORTAL LÊ O "% PREVISTO" DA MESMA CURVA ÚNICA DO ENGENHEIRO (`previsto_semanas_json` + OVERRIDE LITERAL `previsto_literal_json`) E A CURVA S DE TRABALHO REUSA O NÚCLEO COMPARTILHADO `computeCurvaSData` — IDÊNTICA AO MÓDULO. 100% BACKEND · READ-ONLY · ZERO SCHEMA/ALTER/DROP/DELETE.** DIAGNÓSTICO (Neon, proj id=43 REVTE, cutoff 11/06): coexistiam 3 valores de "% Previsto" — `previstoMspSnapshot`=8 (o que o Portal lia), curva `.raiz`=10 (motor minuto-a-minuto) e `previsto_literal_json`=9 (Texto10 capturado no upload). O módulo usa `previstoCurva.raizAt()`, que aplica o OVERRIDE LITERAL primeiro → 9 (= o que o engenheiro vê); o Portal usava o snapshot 8 → divergência de 1% + Curva S com distribuição LINEAR própria. FIX: novo helper puro `shared/previstoCurva.ts` (`parsePrevistoCurva().raizAt(cutoff)` = réplica fiel do hook `previstoCurva`/`raizAt`); `portalExterno.planejamentoObra.pctTotalPrevisto` usa-o como fonte primária (fallback snapshot/linear só se curva ausente/stale). CURVA S: o corpo de `planejamento.getCurvaS` foi extraído VERBATIM p/ `export async function computeCurvaSData(input)` (getCurvaS virou wrapper); o Portal chama `computeCurvaSData({…, usarPesoPorDuracao:true})` em try/catch, trazendo o snapshot-baseline + override de realizado idênticos ao módulo (import dinâmico, sem ciclo). VALIDADO no Neon: `raizAt(11/06)=9`, `raizAt(18/06)=11` (== módulo); previsto 8→9, desvio +1→0. INALTERADO: módulo interno (getCurvaS devolve o MESMO objeto), realizado do Portal (Rev. 3286), `% Previsto` financeiro. REGRA DE OURO: o Portal só REPLICA, NUNCA recalcula. Detalhe: `shared/changelog.ts`.
