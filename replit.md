...
- **Rev. 2927** — **REGISTRAR ENTREGA DE EPI — CONSERTADA A LISTA QUE NÃO MOSTRAVA OS EPIs ("NENHUM EPI ENCONTRADO"), O ESTOQUE EXIBIDO PASSA A BATER COM A ORIGEM (CENTRAL × OBRA) E AO SELECIONAR O FUNCIONÁRIO O ERP JÁ MOSTRA OS TAMANHOS DE CAMISA/CALÇA/CALÇADO.** Pedido (usuário, 2 prints da tela "Registrar Entrega de EPI"): (1) os EPIs não apareciam ("Nenhum EPI encontrado" no seletor); (2) quantidades de estoque "não batiam"; (3) mostrar TODOS os insumos de forma clara; (4) ao selecionar o funcionário, já fazer o LINK dos tamanhos (bota/calçado, camisa, calça). CAUSA-RAIZ (1/3): o seletor é alimentado por `episAllQ` com `limit:1000`, mas `epis.list` (`server/routers/epis.ts`) validava `.max(200)` → tRPC REJEITAVA a query → `episAllQ.data=undefined` → picker caía em lista vazia/parcial. SOLUÇÃO: teto do `epis.list` elevado a `.max(2000)` (zod) + `episAllQ` pede `limit:2000` (catálogo completo volta). CAUSA-RAIZ (2): o subtítulo do EPI mostrava SEMPRE `quantidadeEstoque` (central), mesmo com origem=Obra → habilitei `estoqueObraList` na tela de entrega + memo `estoqueObraMap` (epiId→qtd da obra selecionada); subtítulo agora diz "Estoque na obra: N" (origem=obra) ou "Estoque central: N" (origem=central). MELHORIA (4): card do funcionário ganhou chips Camisa/Calça/Calçado lidos de `employees.tamanho*` (calça com letra+número via `labelTamanhoCalca`, ex. "42 (G)"); sem tamanhos → aviso âmbar. Picker e lista de itens usam `labelTamanhoEpi`. Tudo em `client/src/pages/Epis.tsx`. FRONT+validação — ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
