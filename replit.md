...
- **Rev. 3313** — **RH & DP / FOLHA DE VALE · O "BOTÃO" DE EXCLUIR O PAGAMENTO DO VALE (POR FUNCIONÁRIO) NÃO FUNCIONAVA. NA TABELA DE FUNCIONÁRIOS SEM ALERTA, O "⊘ EXCLUIR"/"✓ OK" DA LINHA ERA UM BADGE DECORATIVO NÃO-CLICÁVEL — A ÚNICA AÇÃO REAL ERA MARCAR O CHECKBOX E CLICAR NO BOTÃO DE LOTE "NÃO PAGAR SELECIONADOS" (NÃO ÓBVIO). E A EXCLUSÃO (`decidirVale` PAGAR:FALSE) ATUALIZAVA SÓ `payroll_advances`, NÃO O SNAPSHOT `valeResultJson` QUE O CARD/FOLHA LÊ — REVERTIA P/ 'CALCULADO' NO RELOAD. AGORA O "EXCLUIR" DA LINHA É UM BOTÃO QUE EXCLUI AQUELE FUNCIONÁRIO E A DECISÃO PERSISTE. 100% BUGFIX (1 BACKEND + 1 FRONT) · ZERO SCHEMA/ALTER/DROP/DELETE.** UX: em `FolhaPagamento.tsx`, o branch não-rejeitado do cell de ação virou `<button>` "Excluir" (XCircle) → `decidirValeMut.mutate({decisoes:[{employeeId,pagar:false}]})` com confirm + limpa `valeExcluirSel`; rejeitado mantém "Reverter"; checkbox/lote intactos. PERSISTÊNCIA: `decidirVale` (`payrollEngine.ts`) passou a chamar `sincronizarValeJson(db, companyId, mesReferencia)` após o loop (igual `reverterVale`), reescrevendo `funcionarios[].status`/`totalVale` a partir de `payroll_advances`. NOTA: a Rev. 3312 já some com desligados (Elizeu) da leitura; este fix vale p/ exclusões MANUAIS de elegíveis. App publicado precisa re-publicar p/ herdar 3312+3313. Detalhe: `shared/changelog.ts`.

- **Rev. 3312** — **RH & DP / FOLHA DE VALE · UM FUNCIONÁRIO DESLIGADO (ELIZEU, SAÍDA EFETIVA 15/05/2026) CONTINUAVA APARECENDO E "RECEBENDO" VALE EM JUNHO/2026. A GERAÇÃO DO VALE (`gerarVale`) JÁ EXCLUÍA QUEM SAIU ANTES DO MÊS, MAS O SNAPSHOT (`valeResultJson`), GERADO QUANDO ELE AINDA ERA ATIVO, FICAVA CONGELADO — E A SANITIZAÇÃO DE LEITURA (REV. 3292) SÓ REMOVIA QUEM HOJE É PJ/SÓCIO/EXCLUÍDO, NÃO DESLIGADOS. AGORA A SANITIZAÇÃO TAMBÉM REMOVE DA LEITURA (E BLOQUEIA NA APROVAÇÃO/REVERSÃO) DESLIGADOS CUJA SAÍDA EFETIVA É ANTERIOR AO 1º DIA DO MÊS. 100% BACKEND · READ-ONLY (NÃO REGRAVA SNAPSHOT) · ZERO SCHEMA/ALTER/DROP/DELETE.** CAUSA: Elizeu (status="Desligado", `dataDesligamentoEfetiva`=2026-05-15, aviso prévio concluído 02–31/03) estava nos snapshots de vale de maio E junho (`status:"calculado"`); `getIdsInelegiveisVale` só checava `deletedAt`/PJ/Socio. CORREÇÃO (`payrollEngine.ts`): `getIdsInelegiveisVale(db, ids, mesReferencia?)` ganhou o mês e marca inelegível quem está em `EMPLOYEE_STATUS_DESLIGADOS` com `(dataDesligamentoEfetiva ?? dataDemissao) < ${mes}-01` (comparação `YYYY-MM-DD`); desligado em aviso que ainda cobre o mês (saída ≥ 1º dia) permanece. `sanitizarValeSnapshotNaoClt`/`getPeriod` repassam `mesReferencia`; guardas de `decidirVale`/`reverterVale` idem (+ mensagem generalizada). Read-only: a linha em `payroll_advances` persiste mas deixa de ser exibida/contada/aprovável. Detalhe: `shared/changelog.ts`.
