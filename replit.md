...
- **Rev. 3206** — **FINANCEIRO / CONTROLE DE CHEQUES · O MODAL "IMPORTAR CONTROLE DE CHEQUES" ESTAVA APERTADO (TUDO EMPILHADO NUMA COLUNA ESTREITA `max-w-2xl`); AGORA ABRE EM TELA CHEIA (96vw × 94vh) COM LAYOUT EM DUAS COLUNAS E A AMOSTRA EM LARGURA TOTAL.** PEDIDO (piloto FC): "melhore este layout está péssimo, deixa ele full screen" + print do modal espremido. SOLUÇÃO — FRONT (`client/src/pages/financeiro/FinanceiroCheques.tsx`): `DialogContent` passou de `max-w-2xl max-h-[88vh]` p/ `max-w-[96vw] w-[96vw] h-[94vh] flex flex-col` (cabeçalho/rodapé `shrink-0`, corpo `flex-1 min-h-0 overflow-y-auto`). Corpo virou grid `lg:grid-cols-2`: ESQUERDA = dropzone maior (`py-16`) + botão "Analisar planilha" `size="lg"`; DIREITA = resumo (placeholder sem preview; com preview = 6 KPIs maiores `text-2xl` + chips de abas). A AMOSTRA foi pra baixo em largura total (`text-sm`, `max-h-[42vh]`, header sticky). ZERO lógica/estado/mutação alterados. ZERO BACKEND · ZERO SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
