...
- **Rev. 3049** — **CONFIGURAÇÕES · CRITÉRIO EXCLUSIVO DE SÓCIOS: DEFINIR O "SÓCIO ADMINISTRADOR ATUAL" QUE ASSINA TODOS OS CONTRATOS/DOCUMENTOS ONLINE (FCSIGN); FELIPE COSTA ALVES DEIXADO COMO SÓCIO ADMINISTRADOR.** PEDIDO: critério exclusivo de sócios em Configurações que LISTA todos os sócios cadastrados e permite DEFINIR o "sócio administrador atual" — responsável por assinar todos contratos/documentos online (IntegraSign/FCSign) — e deixar Felipe Costa Alves como tal. DADOS (Neon): sócios = employees `tipoContrato='Socio'` (tabela `company_partners` vazia); employees é camelCase, então `listSociosFromEmployees` estava QUEBRADO (usava `e.nome_completo`/`e.tipo_contrato`/`e.company_id`) — CORRIGIDO. SOLUÇÃO (ZERO ALTER/DROP/DELETE — só INSERT/UPDATE/leitura): BACKEND `financial.ts` corrige `listSociosFromEmployees` + NOVOS `getSocioAdministrador`/`setSocioAdministrador` (admin-only, valida sócio da empresa, UPSERT em `system_criteria` categoria 'societario' chave 'socio_administrador_employee_id'); `compras.ts` NOVO helper `resolveSocioAdministradorSigner` faz o signatário "diretor" do envelope automático da OC usar nome+CPF do sócio administrador (fallback "Diretor"); FRONT NOVO `SociosAdministradorSection.tsx` + aba "Sócios / Administrador" em `Configuracoes.tsx`. SEED: company 60002 → Felipe Costa Alves (employee id 6). Detalhe: `shared/changelog.ts`.
