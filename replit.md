...
11. **Frotas** — Controle de Frotas e Veículos. 9 submódulos: Dashboard (KPIs consolidados, patrimônio, alertas, custo/km, composição por tipo/marca, gráficos mensais), Veículos (CRUD expandido com consulta FIPE automática via parallelum API, depreciação, RENAVAM, chassi, cor, responsável), Manutenções (preventiva/corretiva, custo, km, fornecedor, próxima manutenção, alertas de atraso), Combustível (abastecimentos, importação CSV, consumo km/l, preço por litro, posto, motorista), Multas (auto de infração, gravidade, pontos, valor com desconto, motorista, local, status pendente/paga/recorrida), IPVA (ano referência, parcelas, status, alertas de vencimento), Licenciamento (ano exercício, CRLV emitido, status), Seguros (seguradora, apólice, cobertura, prêmio, franquia, vigência, análise de apólice com IA via invokeLLM), Rastreamento (mapa Leaflet com trajetos por veículo, importação CSV, cores por veículo, pontos interativos). Tabelas: `fleet_maintenances`, `fleet_fuel_records`, `fleet_tracking_points`, `fleet_documents`, `fleet_fines`, `fleet_ipva`, `fleet_licensing`, `fleet_insurance` + campos expandidos em `vehicles`. Router: `server/routers/frotas.ts`. Frontend: `client/src/pages/frotas/`. Rotas: `/frotas/painel`, `/frotas/veiculos`, `/frotas/manutencoes`, `/frotas/combustivel`, `/frotas/rastreamento`, `/frotas/multas`, `/frotas/ipva`, `/frotas/licenciamento`, `/frotas/seguros`. Tema: cyan. Ícone: Truck.
