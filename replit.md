...
- **Rev. 2272** — **FIX · REFIS "Realizado Acumulado" passa a ESPELHAR a barra do topo (`avancoAtual`) — antes recalculava via ponderação local (3,75 %) divergindo do topo (8,48 %).** Pedido user (23/05/2026, VITRA, 3ª Sem): "o refis previsto e realizado igual a barra de comando superior". Diagnóstico: top bar (Rev. 2262/2269) lê `realizadoMspSnapshot` raiz UID=0 do MSP (snapshot oficial AD/(AD+RD) MSP-nativo), enquanto REFIS "Realizado Acumulado" usava `avancoRealAtual` (ponderação ad-hoc Σ(peso × % acumulado)) — divergência de ~5pp. "Previsto Acumulado" já estava OK desde Rev. 1825. Fix em `PlanejamentoDetalhe.tsx::Refis` L13390-13395: `rReal` agora prioriza a prop `avancoAtual` (número do topo), caindo no `avancoRealAtual` só se o prop não vier como number (defensive). Modo "Global c/ Indiretas" continua usando `refisRealComInd` (snapshot é só MSP puro / diretas — indiretas são do ERP). **R-001/R-007/R-010:** N/A (client-only).
