...
- **Rev. 2164** — **MELHORIA UX · AlertDialog de excluir Centro de Custo mostra vínculos detalhados antes da exclusão.** Sequência da Rev. 2163 — user reportou "17 categoria(s) vinculadas" e perguntou se era real. Era, mas o alerta antigo não mostrava QUAIS. **Backend:** novo procedure `financial.getCostCenterLinks({id, companyId})` → `{categorias[{id,codigo,nome,tipo,ativo}], nEntries, nRecurring, total}`, com mesma estratégia defensiva da 2163 (cada SELECT em try/catch próprio). **Frontend** (`FinanceiroCentrosCusto.tsx`): novo componente `DeleteCostCenterDialog` substitui o AlertDialog inline; ao abrir dispara `getCostCenterLinks.useQuery`; estado vazio mostra badge verde + botão liberado; estado com vínculos mostra card vermelho com contadores + tabela rolável (Código/Nome/Tipo/Status) das categorias e BLOQUEIA o botão de excluir até o user reapontar ou inativar. **R-001/R-007/R-010:** OK — query SELECT-only, sem mudança no DELETE.
- **Rev. 2163** — **HOTFIX · "Excluir Centro de Custo" devolvia `Unexpected end of JSON input`.** User (print): toast "Não foi possível excluir / Failed to execute 'json' on 'Response': Unexpected end of JSON input" ao clicar na lixeira de qualquer CC. **Causa:** `deleteCostCenter` (Rev. 2156) consultava `financial_recurring_entries.centro_custo_id`, mas essa coluna nunca foi criada (SyncSchema+ Rev. 2082 só garantiu em `financial_accounts`); o Postgres devolvia `column does not exist`, e a resposta da tRPC chegava no browser com body vazio quebrando o `.json()`. **Fix em `financial.ts deleteCostCenter`:** cada uma das 3 checagens (accounts, entries via JOIN, recurring_entries) agora vive em `try/catch` próprio — coluna ausente vira `console.warn` + 0 refs em vez de derrubar a request; mensagem de erro reescrita pra "X categoria(s), Y lançamento(s), Z recorrência(s)"; adicionada checagem extra de `financial_entries.conta_id` via subselect. **R-001/R-007/R-010:** OK — zero ALTER/DROP.
...
- ~~Rev. 2159~~ — DATA-FIX · 219 lançamentos migrados de `3.3 DESPESAS COM MATERIAIS` (órfã) → `3.2 Materiais de Construção` (padrão); órfã soft-deletada (`ativo=0`). Aprovado pelo user. Ver `shared/changelog.ts`.
