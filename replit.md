...
- **Rev. 2743** — **CONFIGURAÇÕES · BACKUP & SINCRONIZAÇÃO · HISTÓRICO DE BACKUPS: PERCENTUAL DE PROGRESSO (0–100%) PARA SABER QUANTO FALTA PARA FINALIZAR.** Pedido (Felipe): no "Histórico de Backups" os backups apareciam travados em "Em andamento" com 0 tabelas/0 registros porque o serviço só gravava `tabelas_exportadas` no FINAL da rotina — durante a execução nada era persistido; pedido: mostrar um percentual de 0 a 100% para acompanhar o progresso. Implementação (SERVER + CLIENT; +1 coluna via self-heal `IF NOT EXISTS`; ZERO ALTER destrutivo/DROP/DELETE — R-001/R-007/R-010): SCHEMA `drizzle/schema.ts` ganha coluna `tabelasTotal` em `backups`, garantida em runtime pelo `[SyncSchema+]` (`server/_core/index.ts`: `ALTER TABLE backups ADD COLUMN IF NOT EXISTS tabelas_total INTEGER DEFAULT 0 NOT NULL`, idempotente). SERVER `backupService.ts`: após `descobrirTabelas` grava `tabelas_total = allTables.length` e zera `tabelas_exportadas`; dentro do loop, a cada 10 tabelas faz UPDATE de `tabelas_exportadas` (best-effort em try/catch — nunca interrompe o backup). CLIENT `Configuracoes.tsx` (`BackupTab`): query `backup.listar` ganha `refetchInterval` de 3s ENQUANTO houver backup `em_andamento` (volta a `false` ao terminar); o badge de Status passa a exibir o percentual ("Concluído 100%", "Em andamento 42%", "Erro X%") + barra de progresso (largura = %, tooltip "feitas/total tabelas") para backups não concluídos. Ressalva: backups antigos travados em "em_andamento" têm `tabelas_total = 0` → mostram 0% (rodada morta, sem como recuperar o avanço); o contador avança em passos de 10 tabelas. Validação: esbuild parse EXIT 0 (4 arquivos); `vitest server/rescisao.test.ts` 41/41 verde; app rodando, console limpo. Detalhe: `shared/changelog.ts`.
- **Rev. 2742** — **COMPRAS · MAPA DE COTAÇÃO · "FORNECEDORES PARTICIPANTES": CADASTRAR FORNECEDOR NOVO POR POPUP, SEM SAIR DA COTAÇÃO.** Pedido (Felipe): no seletor "Fornecedores Participantes" da tela de Cotação, quando o fornecedor não existe (ex.: "Nenhum fornecedor encontrado"), poder cadastrar um fornecedor novo ali mesmo via popup/modal, sem ir até a tela de Fornecedores. Implementação (SÓ CLIENT/UI; reusa endpoints existentes; ZERO SERVER NOVO; ZERO SCHEMA): em `client/src/pages/compras/Cotacoes.tsx`, dentro do `PopoverContent` do seletor (abaixo da lista e do "Nenhum fornecedor encontrado"), novo link "Cadastrar novo fornecedor" (UserPlus) que fecha o popover e abre um `Dialog` de cadastro rápido — se já havia texto na busca, vira a Razão Social inicial. Form mínimo: CNPJ (botão "Buscar" auto-preenche via `trpc.compras.buscarCNPJ` — BrasilAPI/ReceitaWS), Razão Social (obrigatória), Nome Fantasia, Telefone, E-mail, Cidade, UF. Salvar usa `trpc.compras.criarFornecedor` (anti-duplicidade de CNPJ já no server); no sucesso: toast, fecha, `fornQ.refetch()` e PRÉ-SELECIONA o novo fornecedor (`mapaFornSelectId`), deixando a um clique de "Adicionar" ao mapa. Ressalva: cadastro rápido grava só o essencial; cadastro completo (banco/sócios/CNAEs/categorias) segue na tela de Fornecedores. ZERO ALTER/DROP/DELETE (R-001/R-007/R-010). Validação: esbuild parse EXIT 0; `vitest server/rescisao.test.ts` 41/41 verde; app rodando, console limpo. Detalhe: `shared/changelog.ts`.
