...
- **Rev. 2993** — **CONTROLE DE EPIs → "ESTOQUE POR OBRA" → A TELA INTEIRA AGORA ROLA JUNTO (REMOVIDO O "CONGELAMENTO" DOS CARDS DE OBRA NO TOPO).** PEDIDO: "Quero um layout mais fácil de organizar — quando rolar, a tela inteira deve subir, não congelar as obras e subir só os itens." Na aba "Estoque por Obra" o painel de resumo + os cards de obra estavam FIXOS (sticky, Rev. 2779) e o grid de cards ainda tinha scroll PRÓPRIO (`max-h-[42vh] overflow-y-auto`); no iPad as obras ficavam congeladas no topo e só a tabela deslizava por baixo. SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE, ZERO backend/schema): em `client/src/pages/Epis.tsx` (view `estoque_obra`) removido `sticky top-0 z-20 bg-background` do wrapper (vira bloco normal) e removido `max-h-[42vh] overflow-y-auto pr-1` do grid de cards; resumo → cards → tabela rolam como documento único. Filtro por card (Rev. 2780) intacto. NOTA (correção de DADOS aplicada após "sim" do usuário, SEM mudança de código): os 2 cards "QIU 2 - FASE 4" eram problema de DADOS — 2 linhas de `epi_estoque_obra` da empresa 60002 (ids 60001/90002, 10 un "Capa de Chuva" + 0 "Óculos") apontavam para a obra `270001` (de outra empresa, 60005); foram REATRIBUÍDAS via `UPDATE ... SET "obraId"=90001` (Neon, SEM DELETE/DROP/ALTER), eliminando o card fantasma. Requer REPUBLICAR. Detalhe: `shared/changelog.ts`.
