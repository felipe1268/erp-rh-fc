...
- **Rev. 2544** — **RAIO-X DO COLABORADOR · TIMELINE · MODAL DE DETALHE · REDESIGN MODERNO (SEM ROLAGEM HORIZONTAL) + FIX DE DATA/HORA.** User: o modal de detalhe do evento (Rev. 2543) exigia barra de rolagem horizontal pra ler os dados e pediu layout mais moderno; o print (DDS #32) mostrava o campo "Assinado Em" embaralhado ("28 00:28:36.47334/05/2026"). CAUSA do bug de data: o `formatDate` LOCAL de `RaioXFuncionario.tsx` (L30) faz `split("-")` ingênuo — num TIMESTAMP gera `["2026","05","28 00:28:36.47334"]` → remontagem quebrada; o `fmtVal` do modal o chamava pra qualquer string com prefixo de data ISO. FIX (client, IIFE do modal): novo `fmtDateSmart` — data-only→`formatDate` (DD/MM/AAAA), timestamp→regex de captura "DD/MM/AAAA HH:mm" (descarta seg/micros), senão string crua; `fmtVal` e o cabeçalho passaram a usá-lo. REDESIGN: `DialogContent` `w-[95vw] max-w-3xl overflow-x-hidden p-0 rounded-2xl` (nunca rola horizontal); cabeçalho com gradiente + ícone pill na cor do evento + título/data/badge refTipo; pares chave/valor viraram TILES responsivos (`grid sm:grid-cols-2`, cada card `bg-gray-50/70` com `min-w-0`+`truncate`+`break-words`) → elimina overflow; aviso LGPD em chip âmbar; rodapé separado. Mantida toda a lógica PII/LGPD da Rev. 2543. Não toquei no `formatDate` local global (afeta ~40 usos de relatório/PDF). Zero schema. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
- **Rev. 2543** — **RAIO-X DO COLABORADOR · TIMELINE · (1) BUG da CONFIRMAÇÃO DE HE FANTASMA + (2) RASTREABILIDADE (timeline clicável → modal com TODOS os dados).** User: a timeline do dossiê mostrava um evento "HE — Assinatura Confirmação" que não deveria existir (confirmação fantasma), e pediu que cada item seja clicável abrindo modal com todas as infos + garantir que o ERP não gere lançamentos equivocados. CAUSA do bug: `heSolicitacoes.editar` (heSolicitacoes.ts ~L804-815) apaga/reinsere `he_solicitacao_funcionarios` mas NUNCA remove `he_solicitacao_confirmacoes` → confirmação órfã quando o colaborador sai da solicitação ou ela é cancelada/rejeitada; a query `empHeConfirmacoes` (controleDocumentos.ts) lia por `employeeId` SEM filtrar status da solicitação-mãe e SEM checar se ainda é participante. FIX read-side NÃO-destrutivo (preserva auditoria): `innerJoin` com `he_solicitacao_funcionarios` (solicitacaoId+employeeId) + `ne(status,'cancelada')`/`ne(status,'rejeitada')`; import de `heSolicitacaoFuncionarios`. Decisão consciente: SEM cascade-delete no `editar` (não apaga trilha; o fix de leitura também blinda órfãs já existentes sem rodar DELETE). FEATURE: os 40 `timeline.push` ganharam `refTipo`/`refId`/`meta` (objeto-fonte completo); no client `RaioXFuncionario.tsx` cada item virou `<button>` com hover → abre `Dialog` (estado `timelineEvt`) com cabeçalho + grade key-value de TODOS os campos do meta (camelCase→Título, ISO→formatDate, bool→Sim/Não, objeto→JSON), RESPEITANDO LGPD (`hidePersonal` oculta cpf/rg/pis/salário/endereço/conta + aviso) e omitindo chaves internas. Zero schema. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
