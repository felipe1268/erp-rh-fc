...
- **Rev. 1774**: **Gestão de Documentos · 2 acervos por obra (Projetos Técnicos + Documentos da Obra) + Catálogo central de Categorias Administrativas**. Schema: `gd_disciplinas` ganha `tipoAcervo`/`categoriaChave`/`ordem`; nova tabela `gd_categorias_admin_padrao`. 9 seeds padrão, 5 procedures admin + CRUD em Configurações. ColFix v1774.
