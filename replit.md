...
- **Rev. 3240** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · DEMONSTRATIVOS · A LISTA "TUDO QUE A IA LEU" (PIX + BOLETOS LIDOS POR IA) GANHOU UMA VISÃO EM TELA CHEIA COM LAYOUT MODERNO E CARDS DE TOTAL (GERAL/PIX/BOLETOS) NO TOPO — ANTES OS VALORES APARECIAM CORTADOS NUM MODAL ESTREITO. A TABELA OCUPA A LARGURA TODA (SEM TRUNCAR VALOR), COM BUSCA LIVRE E CHIPS DE TIPO. DÁ PRA ABRIR A TELA CHEIA JÁ FILTRADA POR PIX OU POR BOLETOS (AS "DUAS TELAS") DIRETO DOS SLOTS DE ANEXO OU PELO BOTÃO "TELA CHEIA" DA LISTA INLINE. SÓ CONSULTA — NÃO CONCILIA NEM BAIXA NADA.** PEDIDO (piloto FC): "quero as duas telas (leitura de PIX e de boletos) em tela cheia, com layout moderno e cards com valores totais para fácil análise" (os modais antigos eram estreitos e cortavam valores no iPad). SOLUÇÃO (FRONT, `FinanceiroConciliacao.tsx`): a computação da leitura combinada (PIX+boletos, filtro+busca) foi extraída p/ um `useMemo` ÚNICO `leituraIA` (`{todos,porFiltro,lista,pixVis,bolVis,somaPix,somaBol,total,termo,chips}`) que alimenta TANTO a lista inline QUANTO o novo diálogo full-screen — sem deriva de números entre as duas. Novo estado `leituraFull` + handler `abrirLeituraFull(kind)` (pré-filtra `demoFiltro` e abre). O diálogo reaproveita o padrão do `expandedList` (`DialogContent resizable=false w-[98vw] h-[96vh] flex-col p-0`): header com gradiente violeta + mês/ano, 3 cards de total grandes (Landmark/ArrowDownCircle/FileText), chips de tipo + busca, tabela ampla (px-4 py-2.5, sem truncar valor) e rodapé "Mostrando X de Y". A lista inline ganhou botão "Tela cheia" (Maximize2) e cada slot de anexo (PIX/Boletos) ganhou link "Ver em tela cheia" que abre já filtrado. ZERO backend · ZERO SCHEMA · read-only. Detalhe: `shared/changelog.ts`.

- **Rev. 3239** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · UNIFICAÇÃO DE LANÇAMENTOS QUE NO EXTRATO APARECEM COMO UM ÚNICO VALOR · O VALE REFEIÇÃO (RH), PULVERIZADO EM CENTENAS DE LINHAS (UMA POR FUNCIONÁRIO), AGORA APARECE COMO 1 LINHA POR MÊS (TOTAL); COMBUSTÍVEL E MANUTENÇÃO (FROTA) SÃO AGRUPADOS PELO NOME DO FORNECEDOR/POSTO (O EXTRATO SÓ MOSTRA O TOTAL PAGO). CADA GRUPO É UMA LINHA SINTÉTICA COM SELO DO TIPO, CONTAGEM DE ITENS E BOTÃO P/ EXPANDIR; CASAR O GRUPO COM A LINHA DO EXTRATO BAIXA TODOS OS MEMBROS DE UMA VEZ (N:1). A SOMA TOTAL NÃO MUDA — SÓ A QUANTIDADE DE LINHAS CAI.** PEDIDO (piloto FC): "VR deveria ser 1 lançamento total no mês; combustível e manutenção agrupados por fornecedor, porque no extrato aparece só o total — não dá pra casar centenas de linhas contra uma só". SOLUÇÃO BACK (`financial.ts`): helpers `_normNomeConc` (NFD+UPPER+collapse, une variações de caixa/acento do mesmo fornecedor) e `_agruparConciliacao` (READ-ONLY); os 2 SELECTs de `getConciliacaoReport` trazem `origem_modulo` + fornecedor REAL da Frota via LEFT JOIN em `fleet_fuel_records`(posto)/`fleet_maintenances`(fornecedor) por `origem_id`. Agrupa VR (`beneficio_vr`) por MÊS, combustível/manutenção por fornecedor normalizado; soma preservada; cada grupo vira `{id:"grp:…",agrupado:true,grupoTipo,qtd,valor=Σ,itensIds[],itens[]}`. Nova procedure `conciliarGrupoLancamentos` (RESERVA ATÔMICA da linha guard `conciliado=0` → entry_id=representante; BAIXA membros conciliado/data/status/valor por COALESCE; registra vínculos em `financial_conciliacao_grupo` CREATE IF NOT EXISTS + `revertido_em` p/ UNDO sem DELETE); `desconsolidarMes` ganhou passo "1b" que reverte os membros via tabela-link. FRONT (`FinanceiroConciliacao.tsx`): `renderEntryRow` trata `e.agrupado` (selo por tipo, badge "N itens", chevron expande membros inline); `selectedEntry` aceita `number|string`; painel manual chama `conciliarGrupoMut` (envia `itensIds`) quando `lan.agrupado`. ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
