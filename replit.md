...
- **Rev. 4560** — **FIX: RENOVAÇÃO DE LOCAÇÃO — PARCELA DO CONTAS A PAGAR VENCIA NA DATA ERRADA (SUMIA DA TELA) + MÁSCARA R$ NO MODAL.** "Não foi pro contas a pagar": a parcela FOI criada, mas `locadoRenovar` usava `inicioISO` (dia seguinte ao fim do ciclo ANTERIOR) como vencimento — locação atrasada → vencimento no passado (ex.: 28/05) → some da tela do mês corrente. Fix: `dataBase = novaDataFim` (fim do NOVO ciclo); entries das OC-919/920 reparadas direto no Neon (30/08 e 31/07). Extra: campo "Valor da nova OC" com máscara pt-BR (R$ + centavos → 1.234,56). Arquivos: `equipamentos.ts`, `almoxarifado/index.tsx`. ZERO schema change.
