...
- **Rev. 3608** — **SEFAZ NF-e · FREQUÊNCIA DE CONSULTA CONFIGURÁVEL (1h → 24h). BACKEND ADITIVO + SYNCSCHEMA+ + FRONTEND · ZERO ALTER DESTRUTIVO/DROP/DELETE.** Nova coluna `sync_intervalo_horas` em `company_nfe_config` (SyncSchema+ Rev.3608, padrão=1h). Configurações → Financeiro → Integração SEFAZ: selector "Frequência de consulta à SEFAZ" com opções 1h, 2h, 3h, 4h, 6h, 8h, 12h, 24h. Cron e gate interno respeitam o intervalo configurado por empresa. Detalhe: `shared/changelog.ts`.
