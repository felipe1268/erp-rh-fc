...
- **Rev. 3342** — **FINANCEIRO / DASHBOARD DE CHEQUES · CORREÇÃO: CHEQUES JÁ COMPENSADOS (COM DATA DE COMPENSAÇÃO, MAS COLUNA `status` AINDA "PENDENTE") NÃO APARECIAM COMO "COMPENSADO" NOS GRÁFICOS — CAÍAM EM "PENDENTE" NA PIZZA E NO EMPILHADO. AGORA O DASHBOARD USA O MESMO STATUS EFETIVO DA TELA "CONTROLE DE CHEQUES" (status="compensado" OU dataCompensacao preenchida ⇒ COMPENSADO). 100% FRONT · BUGFIX · READ-ONLY · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** Pedido (usuário, no iPad, com 2 prints): "Tem vários cheque que já foram compensados mas não tá mostrando nos gráficos". RAIZ: a tela `FinanceiroCheques` deriva o selo via `jaCompensado = status==="compensado" || !!dataCompensacao` (data de compensação preenchida ⇒ compensado mesmo com `status` ainda "pendente"), mas o Dashboard agregava por status CRU — a pizza via `cheques.resumo` (backend `GROUP BY status`) e o empilhado lia `c.status` direto → compensado-por-data caía em "Pendente". CORREÇÃO (`client/src/pages/financeiro/dashboards/DashCheques.tsx`, 100% front): (1) helper `statusEf(c)` espelha o `jaCompensado`; (2) `porStatus` (pizza) deixou de vir do `resumo` e passou a ser agregado client-side a partir da lista `cheques` (que traz `dataCompensacao`) por `statusEf`; (3) `statusKeys`/`evolStatus` (empilhado) classificam por `statusEf`; (4) filtros de clique casam por `statusEf`. KPI "Cheques no ano" segue do `resumo`. FRONT-ONLY p/ não mexer no `resumo` (também alimenta a prévia de limpeza da tela de cheques). RESSALVA: `listar` tem limit:2000 (ok ~492/ano); re-publicar. Detalhe: `shared/changelog.ts`.
