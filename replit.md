...
- **Rev. 2690** — **FROTA · VEÍCULOS (`/frotas/veiculos`) · NOVO MODO "SELECIONAR" PARA MARCAR VÁRIOS VEÍCULOS DE UMA VEZ (CHECKBOX EM CADA CARD + "SELECIONAR TODOS") E ALTERAR O STATUS DE TODOS OS SELECIONADOS EM LOTE (ATIVO / EM MANUTENÇÃO / INATIVO / VENDIDO) NUMA ÚNICA AÇÃO.** Pedido (usuário, print da tela Veículos): "preciso ter múltipla seleção para poder alterar vários status de uma vez". Fix (SÓ CLIENT/UI; ZERO SCHEMA — reusa `frotas.updateVehicle`; R-001/R-007/R-010): `client/src/pages/frotas/Veiculos.tsx` — botão "Selecionar" no cabeçalho (toggle `selectMode`); estados `selectMode`/`selectedIds:Set`/`bulkStatus`/`bulkSaving`; cada card vira clicável c/ `Checkbox` + `ring` ciano (botões de ação usam `stopPropagation`); barra sticky com "Selecionar todos" + contador + `Select` de status + "Aplicar"; `applyBulkStatus()` itera os IDs chamando `updateVehicle` (mutation `bulkStatusMut` sem efeitos colaterais) — guard de obra por veículo respeitado — depois `refetch` + toast e limpa seleção. Detalhe: `shared/changelog.ts`.
