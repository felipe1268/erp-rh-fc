...
- **Rev. 4503** — **FIX: DDS ZIP "LOAD FAILED".** `archiver` fazia `pipe(res)` antes de processar tudo; erro no meio dropava conexão (iOS: "Load failed"). Fix: PassThrough + Buffer em memória → `res.send()` só após `finalize()`. Frontend: catch trata "Load failed"/"Failed to fetch" com mensagem amigável; sucesso mostra toast verde. ZERO schema change.
- **Rev. 4502** — **REDESIGN: DDS SESSÕES — TIMELINE + KPIs + BARRA FLUTUANTE.** Redesign completo do `SessoesList`: toolbar com busca pill (rounded-full + X) e filtro de obra, 4 KPI cards (sessões/finalizadas/presenças/assinaturas), timeline lateral com linha gradiente e ponto colorido por status, cards `rounded-2xl border-l-4` com bloco de data à esquerda (dia grande + dia semana + hora), barra de progresso de presença, categorias e PDF em pills, checkbox no canto. Barra flutuante `fixed bottom-6 backdrop-blur-xl` (dark pill) com Limpar/ZIP/Excluir ao selecionar sessões. ZERO schema change.
- **Rev. 4501** — **FEAT: DDS SESSÕES — CARDS RESPONSIVOS + PDF NA LISTA + ZIP EM LOTE.** Tabela substituída por `SessoesList` com cards agrupados por semana; botão PDF direto; batch ZIP via `/api/dds-ata-lote`. ZERO schema change.
