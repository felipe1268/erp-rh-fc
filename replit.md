...
- **Rev. 2723** — **RH · RESCISÃO (Painel RH → funcionário em aviso → "Cálculos da Rescisão" → aba "Detalhes da Rescisão") · NOVO BLOCO "COMPOSIÇÃO DO CUSTO — PROVISIONADO x ADICIONAL DA DEMISSÃO": SEPARA AS VERBAS EM 🟦 GRUPO A (JÁ ERA CUSTO DA EMPRESA / COMPETÊNCIA) x 🟥 GRUPO B (CUSTO ADICIONAL DA DEMISSÃO), COM O "INCREMENTO DA PROJEÇÃO DO AVISO" (AVOS EXTRAS DE FÉRIAS/13º) DESTACADO NO GRUPO B.** Pedido (direção confirmada via user_query): deixar claro quanto a empresa já provisionava ao longo do contrato vs. custo exclusivo da dispensa sem justa causa; modo PRECISO (destaca o incremento da projeção); novo bloco na própria aba "Detalhes". Modelo de datas (router `avisoPrevioFerias.ts`): `dataDesligamento`=início do aviso (baseline SEM projeção); `dataProjecao`=fim do mês do fim do aviso (COM projeção). Solução (SERVER + CLIENT; ZERO SCHEMA — R-001/R-007/R-010): `server/utils/rescisaoCalc.ts` (`calcularRescisaoCompleta`) calcula e expõe `incAvosFeriasProjecao`/`incAvos13Projecao` + `feriasProporcionalProjecao`/`tercoConstitucionalProjecao`/`decimoTerceiroProjecao`; o incremento de FÉRIAS usa contagem CRUA de avos do período corrente (`mesesServico%12` + regra 15 dias, cap 12) SEM o atalho "ano completo→12/12" (que tornaria a subtração projetado−base negativa quando o início do aviso cai no aniversário — caso Myriélle: base 0 avos, projetado 2 → incremento 2/12); 13º usa `calcularMeses13o` direto; clamps `max(0,…)`/`min(exibido,…)` garantem Grupo A ≥ 0. `getById`/`calcular` RECOMPUTAM → campos aparecem automaticamente. `client/src/pages/PainelRH.tsx` monta `grupoA`/`grupoB` e renderiza 2 cards (azul/vermelho) + faixa-destaque com total e % do Grupo B + nota do ★; reconciliação: A+B = subtotal de proventos. GATING (pós-review): o incremento da projeção só vai pro Grupo B quando `aviso.tipo` começa com `empregador` (dispensa); em pedido de demissão (`empregado_*`) as férias/13º ficam integralmente no Grupo A e o Grupo B fica vazio. Nenhum total existente muda (só re-apresentação). Validação: esbuild server + TSX EXIT 0; `vitest` 41/41 verde. Detalhe: `shared/changelog.ts`.
