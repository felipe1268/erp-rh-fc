...
- ~~Rev. 2230/2231~~ — FIX/PARSER (iterações 1+2, superseded pela 2232) · Importar Cronograma MS Project XLSX com linhas de título acima dos headers. 2230 introduziu auto-detecção de header row (`header: 1` + scan 30 linhas) mas usava `includes` (capturou "Atividade: ..." errado). 2231 endureceu pra match exato + score >= 2 (mas restritivo demais). Ver Rev. 2232 e `shared/changelog.ts`.
