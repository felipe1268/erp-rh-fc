...
- **Rev. 3624** — **NF-e RECEBIDAS · LOG DE AUDITORIA DE SINCRONIZAÇÃO SEFAZ — DATA/HORA BRASÍLIA, NSU, STATUS POR CICLO. BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** Nova tabela `nfe_sync_log` (SyncSchema+ Rev.3624) registra cada execução de sync com `iniciado_em`/`finalizado_em` (gravados UTC, exibidos BRT via `AT TIME ZONE 'America/Sao_Paulo'`), `nsu_inicial`, `nsu_final`, `importadas`, `ignoradas`, `paginas`, `cstat`, `status` (ok/rate_limit/erro/rodando). Helpers `insertSyncLog` + `finalizeSyncLog` chamados em `executarSyncNFe`. Endpoint `sefaz.getSyncLog` (tRPC protected). Painel colapsível `<details>` "🕵️ Log de Sincronizações SEFAZ" na aba NF-e Recebidas com tabela de auditoria + badge de status colorido. Bugfix Rev.3623: `buildResumo` tipo circular removido; `alert()` → toast shadcn. Detalhe: `shared/changelog.ts`.

- **Rev. 3623** — **PANORAMA FISCAL · BOTÃO "PACOTE CONTADOR" — ZIP ORGANIZADO COM NFS-e, NF-e, EXTRATO, OCs + CHECKLIST MENSAL/ANUAL. BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** Nova rota `GET /api/download/pacote-contador` gera um ZIP com 7 arquivos organizados: `00_CHECKLIST.txt` (pendências + lista documentos mensais/trimestrais/anuais), `01_Resumo.csv`, `02_NFS-e_Emitidas.csv` (com ISS/INSS/IRRF/PIS-COFINS), `03_NF-e_Recebidas_SEFAZ.csv`, `04_OCs_x_NF-e.csv`, `05_Extrato_Entradas.csv`, `06_Extrato_Saidas.csv`. Modo mensal = 1 pasta; modo anual (mes=0) = subpastas por mês com dados. CSVs com BOM UTF-8 para Excel BR. Botão violeta "Pacote Contador" no Panorama Fiscal ao lado de Excel/PDF. Detalhe: `shared/changelog.ts`.
