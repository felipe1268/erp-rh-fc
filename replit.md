...
- **Rev. 3320** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · O "PANORAMA GERAL DO MÊS" (Rev. 3319) ESTOURAVA "NÃO CONSEGUI MONTAR O PANORAMA — DB: code=42703 | msg=column cba.company_id does not exist" AO SELECIONAR QUALQUER MÊS; NENHUM PANORAMA CARREGAVA. AGORA MONTA NORMAL. 100% BACKEND · BUGFIX (HOTFIX da Rev. 3319) · ZERO SCHEMA/ALTER/DROP/DELETE.** RAIZ: a query de descoberta de contas em `getConciliacaoReportGeral` (`server/routers/financial.ts`) referenciava `cba.company_id` (JOIN + WHERE) e `cba.descricao`, mas `company_bank_accounts` é camelCase (`"companyId"`) e NÃO tem coluna `descricao` (o rótulo vem de `apelido`). A tabela self-heal `bank_statement_lines` é snake_case (`b.company_id` correto) — a mistura de convenções induziu o erro; Postgres aborta na 1ª coluna inexistente (`42703`). CORREÇÃO: `cba.company_id`→`cba."companyId"` e `cba.descricao`→`cba.apelido AS "descricao"` (alias de saída mantido → front intacto). LIÇÃO: ao fazer JOIN entre tabela Drizzle (camelCase) e self-heal (snake_case), conferir/aspar a convenção de CADA lado — SQL cru não é validado pelo tsc, só estoura em runtime. Detalhe: `shared/changelog.ts`.
