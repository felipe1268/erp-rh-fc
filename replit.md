...
- **Rev. 3148** — **FINANCEIRO / LANÇAMENTOS · NOVO BOTÃO "ZERAR MÊS" (SÓ ADMIN MASTER, COM A SENHA DELE CONFERIDA NO BACKEND) QUE APAGA TODOS OS LANÇAMENTOS DO MÊS ANALISADO — DEIXANDO O MÊS COMPLETAMENTE ZERADO.** PEDIDO (iPad, empresa "FC ENGENHARIA PROJETC", Abr/2026, 118 lançamentos): "botão que SÓ o adm master, com a senha dele, apaga TODOS os lançamentos do mês p/ deixar zerado". DIAGNÓSTICO: o usuário estava em OUTRA empresa (NÃO a FC=60002) — por isso o "Excluir" dava "Lançamento não encontrado" antes (aqueles cancelados de origem Compras não existem na 60002 no Neon); em vez de caçar 1 a 1, ele quer zerar o mês. CORREÇÃO (BACKEND+FRONTEND; HARD-DELETE EXPLICITAMENTE AUTORIZADO, no espírito do deleteEntry/bulkDelete já existentes): NOVA `financial.wipeMonthEntries` ({companyId, dataInicio, dataFim, password, motivo}) com blindagem em 5 camadas — (1) role==='admin_master', (2) SENHA conferida no servidor via `bcrypt.compareSync` contra `users.password` (OAuth sem senha local cai na sessão, igual `_assertMasterComSenha`), (3) tenant-guard `_assertFinanceiroCompanyAccess`, (4) `motivo` obrigatório, (5) auditoria `financial_month_wiped` com snapshot (contagem+total) ANTES de apagar. ESCOPO = MESMO conjunto da tela (período por SOBREPOSIÇÃO competência↔vencimento↔criação + exclui projeções via `sqlNotProjecao`, que se regeneram) e apaga TODAS as situações reais (inclusive pago/recebido). Frontend `FinanceiroLancamentos.tsx`: botão "Zerar mês" (vermelho) visível SÓ p/ admin_master (via `useAuth`) e SÓ com mês selecionado; diálogo com SENHA+MOTIVO+aviso de irreversibilidade; gate de role DUPLO (client esconde + server recusa). ZERO ALTER/DROP/SCHEMA. Detalhe: `shared/changelog.ts`.
