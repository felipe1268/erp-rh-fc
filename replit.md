...
- **Rev. 2589** — **PLANEJAMENTO · ABA "EFETIVO × IA" · (1) CORRIGE O "TIRE SUAS DÚVIDAS COM A IA" QUE NÃO RESPONDIA NO iPad/SAFARI (TIMEOUT); (2) TODA PERGUNTA DIGITADA PASSA A SER REGISTRADA E FICA VISÍVEL EM "TELEMETRIA & ANALYTICS › ANALYTICS DA IA".** User (screenshots): "as perguntas não estão funcionando" + "quero todas as perguntas salvas na telemetria, ver tudo que cada usuário digitar". DIAGNÓSTICO: (a) `perguntarEfetivo` chamava `invokeLLM` SEM `fast` → Claude não-streaming estourava o timeout do proxy/iOS; (b) `perguntarEfetivo` não gravava em `ia_modulo_conversas` (tabela que alimenta a Telemetria) — só `iaModulos.chat` gravava. FIX (ADITIVO, ZERO SCHEMA/ALTER/DROP/DELETE; só leitura + INSERT) em `server/routers/iaCronograma.ts` `perguntarEfetivo`: (1) `invokeLLM` ganha `fast: true` (Gemini 2.5 Flash, responde antes do timeout; Claude fallback); (2) após a resposta, INSERT best-effort em `ia_modulo_conversas` (modulo "planejamento", pergunta, resposta, user, projeto) — try/catch isola falha, só grava com resposta efetiva. CLIENT: nenhuma mudança — `Telemetria.tsx` já renderiza `ultimasPerguntas` (últimas 100, busca + expandir). Validado via esbuild server (exit 0). Detalhe: `shared/changelog.ts`.
- **Rev. 2588** — **PLANEJAMENTO · ABA "EFETIVO × IA" · (1) A ANÁLISE/SIMULAÇÃO AGORA FICA SALVA E É RESTAURADA AO REABRIR A TELA (ANTES SE PERDIA AO SAIR); (2) O "PLANO DE ATAQUE" GANHA UMA "MESA DE GUERRA" QUE ALOCA OS FUNCIONÁRIOS NAS FRENTES, MAIS DETALHADA E DIDÁTICA.** User (screenshots da obra QIU 2 - FASE 4): "a análise deve ficar salva, hoje se perde ao sair da tela"; "quero o plano de ataque mais detalhado, uma mesa de guerra, alocando os funcionários nas frentes"; "tudo mais didático". DIAGNÓSTICO (1): o resultado JÁ persistia no banco (aba Histórico) mas as views só guardavam em `useState` local — ao desmontar, sumia; faltava RESTAURAR a última salva. FIX (ADITIVO, ZERO SCHEMA/ALTER/DROP/DELETE; SOMENTE LEITURA da tabela já existente): SERVER (`server/routers/iaCronograma.ts`) novo endpoint query `ultimaAnaliseEfetivo` ({projetoId,companyId,tipo:diagnostico|simulacao}; anti-IDOR; orderBy criadoEm desc limit 1; aplica `brDatasDeep`; retorna {id,criadoEm,criadoPor,resultado}|null); `simularEfetivo` ganha array `alocacaoFrentes` no `planoAtaque` [{frente,local,objetivo,equipe[{cargo,qtd,papel}],totalPessoas,ritmo,duracao,dependeDe,risco}] + instrução "MESA DE GUERRA" (distribuir FISICAMENTE o efetivo SIMULADO pelas frentes sem estourar por função, descontando férias, com ritmo/takt e sequenciamento). CLIENT (`AnaliseEfetivoIA.tsx`): `Diagnostico`/`Simulador` restauram a última análise salva (banner discreto "Análise salva exibida (gerada em …)"); `PlanoAtaque` ganha seção "Mesa de guerra — alocação nas frentes" (intro didático + cards por frente). Validado via esbuild server+client (exit 0). Detalhe: `shared/changelog.ts`.
