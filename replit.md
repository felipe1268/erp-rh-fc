...
- **Rev. 2968** — **PORTAL DO CLIENTE → PESQUISA DE SATISFAÇÃO (NPS) PÚBLICA — A "NOTA GERAL" DEIXA DE SER DIGITADA/SELECIONADA MANUALMENTE E PASSA A SER CALCULADA AUTOMATICAMENTE, AO VIVO, COMO MÉDIA PONDERADA DAS RESPOSTAS.** Pedido (usuário): a "Nota geral" não deve mais ser preenchida à mão — calcular automaticamente ao final do preenchimento, com peso proporcional de cada pergunta e ponderação enxuta. SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE): novo `useMemo notaGeralAuto` em `PortalDashboardCliente.tsx` = média ponderada de blocos, cada bloco entrando com sua média (só itens respondidos) e peso fixo — Gestor 25%, Equipe direta 25%, Obra/Execução (andamento+prazo+qualidade) 25%, Encarregado 10%, Escritório Central 10%, Empresa FC 5%. Bloco só conta com ≥1 item respondido; pesos RENORMALIZADOS sobre os blocos ativos (preenchimento parcial = nota coerente); resultado arredondado/clamp p/ inteiro 0–10 (backend exige `notaGeral` int). UI: card âmbar do topo perdeu o `<NotaSelector>` e mostra a nota calculada read-only ("Calculada automaticamente…", "—" sem dados); validação/`disabled`/submit usam `notaGeralAuto`. Detalhe: `shared/changelog.ts`.
