...
- **Rev. 3112** — **MEDIÇÃO / LEVANTAMENTO DE CAMPO · RASTREIO POR CONTORNO: CADA CONTORNO PODE RECEBER UM NOME/LOCAL (EX.: "APARTAMENTO 1402") E TER FOTOS VINCULADAS DIRETAMENTE A ELE — NÃO MAIS SÓ FOTOS SOLTAS DO LEVANTAMENTO. NOME E FOTO APARECEM NA LISTA "CONTORNOS DESTA PÁGINA", NA GALERIA (COM ETIQUETA DO CONTORNO) E NA MEMÓRIA DE CÁLCULO.** PEDIDO: rastrear cada medição (nomear contorno + anexar foto do local exato). SOLUÇÃO (FRONTEND-ONLY, ZERO ALTER/DROP/DELETE/SCHEMA/BACKEND — schema/back JÁ tinham `rotulo` e `contorno_id`): (1) NOME: componente `RotuloInput` (grava no blur/Enter) por linha; `salvarRotulo()` via `off.saveContorno` (UPDATE). CRÍTICO: o upsert do servidor reescreve `rotulo` em TODA gravação → `rotulo: c.rotulo ?? null` adicionado a `bindContornoItem`/`recolorContorno`/`salvarGeometriaContorno` (senão vincular/recolorir/redimensionar apagava o nome); (2) FOTO: botão "Foto" por linha (`addFotoContorno` → input dedicado) anexa com `contornoId`; miniaturas inline; galeria geral ganha etiqueta `#NNN · rótulo`. `useLevantamentoOffline.saveFoto` passou a aceitar `contornoId` no `meta`→`data` da op (sync já envia) e no push de `applyOps`; (3) MEMÓRIA: tabela separou "Local / Nome" de "Item vinculado", textos via novo `escHtml`. Próxima: categorias editáveis com cor + tipo automático. Detalhe: `shared/changelog.ts`.
