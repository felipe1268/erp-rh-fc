...
- ~~Rev. 2010~~ — ver `shared/changelog.ts`. **DP · Fechamento de Ponto · BUG-FIX · Discrepância % Presença tabela vs. drill-down**. Reportado pelo usuário (img IMG_0852+IMG_0853): "veja o caso da Isabela, veio todos os dias basicamente, só tem 1 falta e tem um % de 43%, não entendi o pq". Isabela aparecia na tabela com 10 dias / 45% mas drill-down mostrava 21 dias / 1 falta / 95%. **Causa-raiz**: `getSummary` (tabela) e `getDiasEmployee` (drill-down) usam filtros divergentes — getSummary filtra por `mesReferencia` (mês calendário) quando `dataInicio/dataFim` chegam undefined; getDiasEmployee SEMPRE filtra por range. No cliente, o range só era passado a getSummary quando `cicloInicio/cicloFim` estavam preenchidos pela consolidação. Mês não-consolidado (ciclo aberto) → range undefined → backend filtrava por mês calendário "2026-05" → perdia os 11 dias de abril de Isabela (tagged `mesReferencia="2026-04"`). **Mudança** em 1 arquivo (`client/src/pages/FechamentoPonto.tsx`, 1 hunk): nova useMemo `cicloRangeFallback` calcula fallback 16→15 a partir de `mesAno` quando consolidação não tem cicloInicio/Fim; chamada de `getSummary` agora sempre envia range real. `shared/version.ts` → 2010. **Resultado**: tabela e drill-down consultam mesmo range, Isabela bate 21d/95% nos dois lugares, presença média do grupo deixa de ficar artificialmente baixa. **Preservado**: endpoints backend INTACTOS (cliente é que mudou); `periodoIni/periodoFim` INTACTOS; outras queries com `mesReferencia` (stats/inconsistencies/atestadosMes) INTACTAS — elas SÃO por mês calendário. Rev. 2009 INTACTA. Schema INTACTO. R-001/R-007/R-010 OK (SELECT-only). Reversível em 1 hunk. **Follow-up**: auditar `inconsistencies` e `getStats` pra mesma causa; integrar feriados.
