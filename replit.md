...
- **Rev. 3097** — **MEDIÇÃO / LEVANTAMENTO DE CAMPO · A TELA DE DESENHO SOBRE A PLANTA (PDF) VIRA TÁTIL E "FLUIDA" ESTILO AUTOCAD: PLANTA EM PRETO-E-BRANCO (ALTO CONTRASTE) POR PADRÃO, ZOOM POR PINÇA + PAN COM 1 DEDO, FERRAMENTAS NOVAS RETÂNGULO (ARRASTAR) E DESENHO LIVRE (TRAÇO), FERRAMENTA PAREDE (LINHA × ALTURA → m²), TOOLBAR FIXA QUE MANTÉM A FERRAMENTA ATIVA, DESFAZER ÚLTIMO PONTO E ENTRADA DE ALTURA/ESCALA POR CAMPO NO APP (FIM DO `window.prompt`).** PEDIDO (Task #89 "Desenho Fluido"): Levantamento de Campo usável em tablet — desenho direto sobre a planta sem ponto-a-ponto obrigatório, plantas legíveis (P&B), zoom/pan por toque e entrada de números sem popup nativo. SOLUÇÃO: (1) GEO/BACKEND ADITIVO — `shared/levantamentoGeo.ts` ganha tipo "parede" (área = comprimento da polilinha × altura, unidade m²) + helper `simplificarPontos` (RDP); enum `tipo` da proc `salvarContorno` em `server/routers/medicao.ts` aceita "parede" (coluna `varchar(20)` livre, sem enum PG). (2) TELA `client/src/pages/medicao/MedicaoLevantamento.tsx` — filtro CSS `grayscale(1) contrast(1.25)` SÓ no `<Page>` (default ON, botão P&B/Cor); gestos via Pointer Events num overlay `touchAction:"none"`: 2 ponteiros = pinça-zoom focal (`useLayoutEffect` ajusta scroll) + pan, 1 ponteiro = toque marca ponto / arrasto = pan / retângulo+livre desenham; RETÂNGULO (2 cantos) e DESENHO LIVRE (traço simplificado) persistem como tipo "area" (zero backend); ferramenta PERMANECE ativa após finalizar; botão "Desfazer"; `window.prompt` (escala/altura) → modal in-app `NumberPromptDialog` (input pt-BR vírgula decimal). Engine compartilhada Cliente/Terceiros (`?origem=`); offline-first preservado. RESSALVA/DRIFT: retângulo/livre gravam como "area" (sem tipo dedicado); livre é simplificado (RDP ε 0.004). FRONTEND + BACKEND ADITIVO, ZERO ALTER/DROP/DELETE/SCHEMA. Detalhe: `shared/changelog.ts`.

- **Rev. 3096** — **MEDIÇÃO DE TERCEIROS · NA "CONFIGURAÇÃO DE RETENÇÕES DO CONTRATO (%)" O USUÁRIO VOLTA A CONSEGUIR APAGAR O "0" DOS CAMPOS (ISS/INSS/IRRF/OUTRAS/RET. TÉCNICA) PARA DIGITAR O PERCENTUAL — ANTES O CAMPO RESSURGIA COM "0" A CADA TECLA.** PEDIDO: "Não consigo apagar o número 0" nos inputs de percentual de retenção (aba de medição → "RETENÇÕES E DESCONTOS" → "Configuração de Retenções do Contrato (%)"). CAUSA-RAIZ: em `client/src/pages/terceiros/contratos/ContratoDetalhe.tsx` os inputs eram controlados por estado NUMÉRICO (`percConfig`) e o `onChange` fazia `parseFloat(e.target.value) || 0` → ao apagar, `""` vira `NaN` → `|| 0` força "0" de volta. SOLUÇÃO (FRONTEND-ONLY, ZERO BACKEND/ALTER/DROP/DELETE/SCHEMA): `percConfig` passa a guardar TEXTO; `onChange` grava o valor cru (permite vazio durante digitação); a conversão p/ número (`parseFloat(...) || 0`) acontece SÓ no "Salvar Config", no payload da `salvarRetencaoConfig`. Backend segue recebendo números. RESSALVA/DRIFT: nenhuma — cálculo/persistência idênticos; campo vazio ao salvar = 0. Detalhe: `shared/changelog.ts`.
