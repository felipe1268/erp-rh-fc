...
- **Rev. 2642** — **PAINEL RH & DP · A SEÇÃO "QUADRO DE PESSOAL" GANHA O CARD "NA EMPRESA" COM A QUANTIDADE DE PESSOAS QUE AINDA TÊM VÍNCULO HOJE (EXCLUI DESLIGADOS E BLACKLIST), EM PARIDADE EXATA COM O BADGE "NA EMPRESA" DA TELA COLABORADORES.** Pedido (usuário): "coloque a quantidade de pessoas na empresa hoje... igual vc chamou de NA EMPRESA da tela de colaboradores". CAUSA-RAIZ (ausência de feature): o stats do `home.getData` (`server/routers/homeData.ts`) NÃO expunha `naEmpresa` — só Total/Ativos/Férias/Afastados/Licença/Desligados; o conceito "Na Empresa" (total − desligados − blacklist) existia só em `getEmployeeStats`/`employees.stats` (tela Colaboradores). Fix (ADITIVO, SOMENTE SELECT; R-001/R-007/R-010): `server/routers/homeData.ts` — `statsConsolidados` ganha `naEmpresa` = `allEmps` sem `status="Desligado"`/`"Lista_Negra"` e sem `listaNegra` truthy (mesma régua canônica → 123); `client/src/pages/PainelRH.tsx` — novo `KpiCard "Na Empresa"` (ícone `UsersRound`, cor teal) após "Total", `s.naEmpresa` com fallback `total − desligados`, clique → `/colaboradores?status=NaEmpresa`, grade `lg:grid-cols-6`→`7`. Validado: esbuild client+server exit 0. Detalhe: `shared/changelog.ts`.
