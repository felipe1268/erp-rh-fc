...
- **Rev. 2657** — **CONTAS A PAGAR (`/financeiro/contas-a-pagar`) · (1) CADA LINHA GANHA OS BOTÕES EDITAR (✏️) E ANEXAR (📎) — ALÉM DO VISUALIZAR (👁) E EXCLUIR (🗑) QUE JÁ EXISTIAM; (2) O FORNECEDOR/CLIENTE PASSA A APARECER NA LISTA (LINHA DA DESCRIÇÃO) E NO DETALHE (ABA GERAL); (3) O DOCUMENTO ANEXADO (BOLETO/NF/FOTO) GANHA LINK NO DETALHE.** Pedido (usuário, prints image_1780278786477 + image_1780278898496): espelhar no Contas a Pagar o que foi feito em Lançamentos — botões EDITAR e ANEXAR por linha + mostrar o nome do fornecedor/cliente (servidor já retornava `fornecedorNome` mas a UI não exibia). Fix (CLIENT/UI + SERVER aditivo; R-001/R-007/R-010 — só ADD COLUMN IF NOT EXISTS): SCHEMA `drizzle/schema.ts` (`financialEntries`) — +`anexo_url`/`anexo_nome` (documento de ORIGEM do título, distinto do `comprovante_url` da baixa); self-heal `[SyncSchema+]` em `server/_core/index.ts`. SERVER `server/routers/financial.ts` — `getContasAPagarByYear`/`getEntryDetalhe` passam a SELECT `anexo_url`/`anexo_nome`; `updateEntry` (já existia) edita lançamento manual (BLOQUEIA pago/recebido/cancelado e origem ≠ recorrente, com audit log); NOVO `anexarDocumento` (grava anexo_url/nome, NÃO bloqueia por status). CLIENT `client/src/pages/financeiro/FinanceiroContasAPagar.tsx` — linha mostra `🏢 fornecedorNome`; ações ganham EDITAR (Pencil→`openEdit`) + ANEXAR (Paperclip→`openAnexo`), ocultos no modo projeção; aba GERAL ganha KV "Fornecedor / Cliente" + link "Ver documento anexado"; modais EDITAR (com datalists de fornecedores via `compras.listarFornecedores` e categorias via `financial.getAccounts`) e ANEXAR (`<input type=file>`→base64→`uploadComprovante`). Validado (estático): esbuild dos 3 arquivos EXIT 0 (`pnpm build`/`tsc` estouram OOM no container). Detalhe: `shared/changelog.ts`.
- **Rev. 2656** — **LANÇAMENTOS (`/financeiro/lancamentos`) · (1) LANÇAMENTOS REALIZADOS QUE APARECEM NO CONTAS A PAGAR VOLTAM A APARECER NA TELA DE LANÇAMENTOS; (2) O FILTRO DE PERÍODO VIRA UM CALENDÁRIO ABERTO (RANGE "DE … ATÉ …", 2 MESES); (3) CADA LINHA GANHA OS BOTÕES VISUALIZAR + EDITAR + EXCLUIR; (4) EDIÇÃO/EXCLUSÃO REFLETE NO CONTAS A PAGAR/RECEBER E VICE-VERSA.** Pedido (usuário, print image_1780277786287): lançamentos realizados sumiam da tela de Lançamentos; calendário aberto p/ selecionar período; botões EDITAR/VISUALIZAR/EXCLUIR; link automático bidirecional. Causa-raiz #1: `financial.getEntries` filtrava o período SÓ por `data_competencia` (range SQL exclui NULL) → linhas sem competência ou com vencimento em outro mês sumiam, embora aparecessem no Contas a Pagar (que filtra por vencimento). Fix (SÓ CLIENT/UI + 1 ajuste de leitura no SERVER; R-001/R-007/R-010 — ZERO SCHEMA): SERVER `server/routers/financial.ts` (`getEntries`) — filtro de período vira de SOBREPOSIÇÃO: entra se competência OU vencimento cai no intervalo, ou (ambos NULL) a criação — `((data_competencia IS NOT NULL AND BETWEEN) OR (data_vencimento IS NOT NULL AND BETWEEN) OR (ambos NULL AND created_at::date BETWEEN))`; `ORDER BY COALESCE(...)` p/ ordenar as linhas recém-visíveis. `COALESCE` puro NÃO bastava (escolhe competência sempre que existe). `dbExecute` liga por APARIÇÃO — cada uma das 3 aparições empurra seus valores. CLIENT `client/src/pages/financeiro/FinanceiroLancamentos.tsx` — os 2 `<input type=date>` viram Popover + `Calendar mode="range" numberOfMonths={2}` (Limpar/Aplicar, helpers `strToDate`/`dateToStr` sem fuso); botões por linha VISUALIZAR (Eye → modal read-only via `getEntryDetalhe`) + EDITAR + EXCLUIR sempre visíveis (com toast de motivo quando bloqueado); `invalidarContas()` via `trpc.useUtils()` invalida `getContasAPagarByYear`/`getContasAReceber`/`getEntries`/`getEntryDetalhe` no onSuccess das mutations. Validado (estático): esbuild dos 2 arquivos EXIT 0 (`pnpm build`/`tsc` estouram OOM no container). Detalhe: `shared/changelog.ts`.
