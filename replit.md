...
- **Rev. 2322** — **HOTFIX/UX · Botão "Confirmar e cadastrar" da importação PDF parecia não responder no iPad — diagnóstico + diálogo de erro substitui toast invisível.** Pedido user (23/05/2026, screenshot do preview com 46 contratos): "Tô clicando em cadastrar e ele não faz nada". **Diagnóstico**: o `confirmarImport` em `Locados.tsx` filtrava silenciosamente contratos sem `numeroContrato`/`periodoInicio`/`periodoFim`/`itens` e, se TODOS caíssem (cenário comum quando IA retorna alguns contratos incompletos OU quando o regex `/^\d{4}-\d{2}-\d{2}$/` do server rejeita uma data malformada), disparava `toast.error("Nenhum contrato válido após revisão.")` — toast some em ~3s no Safari iOS e o user não vê. Mesma coisa se o tRPC retornar ZodError (mensagem JSON enorme não cabia no toast). **Implementação** (`client/src/pages/equipamentos/Locados.tsx`): (1) Novo state `importErroDetalhe: string | null` + diálogo modal vermelho (z-60, acima do modal de importação) que persiste até o user clicar "Entendi" — usado em vez de toast. (2) Filtro de contratos refatorado pra CONTAR motivos de rejeição (`semNumero`, `semData`, `dataInvalida`, `semItens`) e exibir mensagem específica ("Nenhum contrato válido. Motivos: 3 sem nº de contrato, 1 com data em formato inválido, 2 sem itens. Edite e tente de novo."). (3) `toIsoDate(s)` defensivo: aceita ISO ou DD/MM/AAAA, retorna "" se inválido (server já normaliza no parse, mas blinda contra edição manual no preview). (4) `onError` da mutation tenta parsear ZodError (`JSON.parse(err.message)`) e formata as 5 primeiras issues como `• path: message`; fallback pra mensagem crua se não for JSON. (5) `console.log` diagnóstico ao enviar (visível no devtools mobile/desktop). (6) Guard `if (!companyId)` adicionado. **0 mudança server, 0 mudança schema.**
