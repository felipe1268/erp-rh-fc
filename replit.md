...
- **Rev. 3672** — **NFS-e EMITIDAS · BUGFIX PARSER SIAP GEO 0 NOTAS — PATH .NET ASMX + CDATA INNER XML + LOG DIAGNÓSTICO. BACKEND PONTUAL · ZERO ALTER/DROP/DELETE.** `parseAbrasrResponse` só conhecia paths ABRASF direto (SIL/TINUS/GIAP); SIAP GEO (.NET ASMX) retorna `ConsultarNfseResponse.ConsultarNfseResult` (não `ConsultarNfseResposta`). Fix: 3 camadas — (1) path .NET ASMX + re-parse de CDATA inner XML; (2) path curto `ConsultarNfseResult`; (3) `tryDeep()` varre `Body` até profundidade 4. Log diagnóstico: quando 0 notas, imprime body keys + subchaves para futuro debug. Arquivo: `server/routers/nfseEmitidas.ts`. Detalhe: `shared/changelog.ts`.
