...
- **Rev. 2558** — **OBRAS · EFETIVO · FIX "Unexpected end of JSON input" AO REMOVER FUNCIONÁRIO DA OBRA (dialog "Equipe — POS OBRA", rota `/obras/efetivo`).** User: clicar "Remover" num funcionário dava o toast "Unexpected end of JSON input" — às vezes a remoção até acontecia no banco. DIAGNÓSTICO: essa msg é `JSON.parse('')` do `httpBatchLink` = CORPO de resposta VAZIO. Código da procedure (`obras.removeEmployee`) e da função (`removeEmployeeFromObra`) corretos; schema/constraints de `employee_site_history` OK (todas as colunas do INSERT existem, nullability bate); `superjson.serialize(undefined)` gera JSON válido (retorno `void` não é a causa); NENHUM `[tRPC Error]` nem stack de crash nos logs; authz da Rev. 2480 é pulado p/ admin_master. Conclusão: corpo vazio + zero log = resposta CORTADA (request atingiu worker reiniciando — dev usa `tsx watch`, vários restarts na sessão — ou blip de rede), não bug de lógica/SQL. FIX (não-destrutivo): SERVER `removeEmployeeFromObra` passa a `return { success: true }` (payload garantido, alinha convenção das outras mutations). CLIENT `ObraEfetivo.tsx` `removeMut` ganhou `retry` IDEMPOTENTE (até 2x, 800ms) SÓ em mensagens transitórias ("Unexpected end of JSON input"/"Failed to fetch"/"Load failed"/"NetworkError") — seguro porque a função é idempotente (`WHERE isActive=1`: 2ª chamada vira no-op, sem duplicar histórico). Erros reais (FORBIDDEN etc.) não reexecutam. Zero schema. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
