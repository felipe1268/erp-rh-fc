...
- **Rev. 2952** — **ATESTADOS & ACIDENTES (SST) — AS LISTAS "ÚLTIMOS ATESTADOS" E "ÚLTIMOS ACIDENTES" (ABA VISÃO GERAL) AGORA MOSTRAM A FOTO DO FUNCIONÁRIO NO LUGAR DO ÍCONE GENÉRICO (ESTETOSCÓPIO / TRIÂNGULO).** Pedido (usuário, print do iPad): "quero as fotos dos funcionários aqui também" nas duas listas de últimos eventos. SOLUÇÃO (BACK read-only + FRONT, ZERO ALTER/DROP/DELETE): `server/routers/sstAnalytics.ts` — as queries `atRows`/`acRows` já traziam `employeeFotoUrl`; bastou repassar `fotoUrl: r.employeeFotoUrl || null` nos `.map()` de `ultimosAtestados`/`ultimosAcidentes` (sem query nova); `client/src/pages/sst/DashboardAtestadosAcidentes.tsx` — nas duas listas o `<div>` do ícone redondo virou `<PersonPhoto src={a.fotoUrl} size="sm" caption={a.funcao}>` (lightbox + fallback de iniciais) num `<span>` com `stopPropagation` p/ não disparar o clique da linha (Raio-X via `setSelectedEmployeeId`). Detalhe: `shared/changelog.ts`.
- **Rev. 2951** — **CONTROLE DE DOCUMENTOS (DASHBOARD) — A TABELA "DOCUMENTAÇÃO INCOMPLETA" AGORA MOSTRA A FOTO DO FUNCIONÁRIO NA COLUNA "FUNCIONÁRIO", AO LADO DO NOME (CLICÁVEL P/ RAIO-X) E DO CPF.** Pedido (usuário, print do iPad): "quero a foto dos funcionários aparecendo aqui também" na aba "Documentação Incompleta" — a tabela mostrava só nome + CPF. SOLUÇÃO (BACK read-only + FRONT, ZERO ALTER/DROP/DELETE): `server/routers/dashboards.ts` (`getDashControleDocumentos`) — o SELECT de `ativosRows` passou a trazer `employees.fotoUrl` e o tipo/`push` de `funcIncompletos` ganharam `fotoUrl: emp.fotoUrl ?? null` (nenhuma query nova, só uma coluna a mais na que já roda); `client/src/pages/dashboards/DashControleDocumentos.tsx` — a célula "Funcionário" virou flex com `<PersonPhoto src={f.fotoUrl} size="sm">` (lightbox + fallback de iniciais) ao lado do bloco nome (botão Raio-X) + CPF, com `stopPropagation` no clique da foto/nome p/ não disparar o `setDetalhe(f)` da linha. Aba "Alertas de Vencimento" inalterada. Detalhe: `shared/changelog.ts`.
