...
- **Rev. 2192** — **MELHORIA UX · Nome do funcionário e do responsável agora aparecem em destaque abaixo de cada assinatura na Ficha de Entrega de EPI.** Lilian: "precisa aparecer o nome do Usuário responsavel que fez a entrega o EPI, alem a assintura, para facilitar o entendimento, o nome das assinatura as vezes fica inelegivel". **Schema aditivo (ADD COLUMN IF NOT EXISTS, R-001 OK):** `epi_deliveries.assinatura_responsavel_nome VARCHAR(255)` + `assinatura_responsavel_em TIMESTAMP` (bootstrap `_core/index.ts:1762-1772` em bloco isolado padrão Rev. 2180; drizzle `schema.ts:1121-1122`). **Server `epiAvancado.ts:salvarAssinatura`:** quando `tipoAssinante==='responsavel'`, `updateData` grava `ctx.user.name` + timestamp. **Server `epis.ts:list`:** SELECT inclui os 2 campos novos. **Client `Epis.tsx` L1828-1872:** nome em font-semibold #1B2A4A 14px ABAIXO da linha; label descritivo em 10px gray-600 embaixo. Funcionário usa `nomeFunc || emp?.nomeCompleto`; responsável usa `assinaturaResponsavelNome || user?.name`.
