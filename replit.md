...
- **Rev. 4023** — **MEDIÇÃO DE CONTRATOS: DROPDOWN "PROJETO / OBRA" CORTAVA NOMES LONGOS NO DIÁLOGO "NOVO CONTRATO DE MEDIÇÃO".** Usuário reportou que nomes de obra longos (ex.: "HOTEL DO PAPA...") apareciam cortados (ex.: "OTEL DO PAPA...") no seletor, mais visível em mobile. Causa-raiz: `SelectContent` compartilhado (Radix `position="popper"`) só define `min-width` (ligado ao trigger), sem `max-width` de viewport — em telas estreitas o popover cresce além da tela e o colision-handling do Radix desloca/corta o texto. Fix escopado só no seletor de `MedicaoContratos.tsx` (sem tocar o `select.tsx` compartilhado): `SelectContent` ganhou `max-w-[min(28rem,calc(100vw-2rem))]`, cada `SelectItem` ganhou `whitespace-normal break-words leading-snug py-2` (quebra em várias linhas em vez de cortar). ZERO DELETE · ZERO ALTER destrutivo.

- **Rev. 4022** — **FINANCEIRO/DRE: OPÇÃO DE CONSOLIDAR O MÊS MANUALMENTE.** Pedido do usuário: poder forçar o status "Consolidado" (bolinha verde no seletor de mês) independente do percentual automático de lançamentos realizados vs. previstos, análogo ao que já existe no Fechamento de Ponto. Nova tabela `financial_dre_consolidacoes` (company_id+mes_referencia único); `dreDisponibilidade()` passa a expor `consolidadoManual`/`consolidadoPor`/`consolidadoEm` por mês, sobrepondo o cálculo automático. Novas procedures `getDREConsolidacaoStatus`/`consolidarDRE` (qualquer usuário da empresa)/`desconsolidarDRE` (admin/admin_master). Tela "Financeiro > DRE": cartão com botão "Consolidar Mês" (observações opcionais) ao selecionar um mês, ou selo "consolidado manualmente por/em" + botão "Desconsolidar" (só admin) se já consolidado. ZERO DELETE · ZERO ALTER destrutivo.
