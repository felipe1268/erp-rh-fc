...
- **Rev. 1802**: **Programação Semanal Lotus · troca da posição dos 3 logos para bater com a planilha modelo do cliente**. User (screenshots 14/05/2026 PLANILHA MODELO × NOSSA): no modelo, Santuário fica à ESQUERDA (cols B-D), FC ao CENTRO (cols I-L) e LOTUS à DIREITA (cols N-P); no nosso export estava invertido (LOTUS esquerda / Santuário centro / FC direita). Causa: as 3 imagens do template `programacao_semanal_lotus.xlsx` foram capturadas em ordem de inserção — `tplImgs[0]=B-D`, `tplImgs[1]=N-P`, `tplImgs[2]=I-L` — mas o código atribuía `imgGer→[0]`, `imgEmp→[1]`, `imgCli→[2]`, gerando layout errado. Fix em `ProgramacaoSemanalLotus.tsx` L817-854: renomeei as constantes para `POS_ESQUERDA/POS_CENTRO/POS_DIREITA` e fiz o pareamento correto `imgCli→ESQUERDA`, `imgEmp→CENTRO`, `imgGer→DIREITA`. Não toquei estilos, células, linhas ou outros fills — só a atribuição das imagens.
