...
- **Rev. 2308** — **FEAT · Importação em lote de contratos de locação de equipamentos via PDF da locadora (IA Gemini detecta layout).** MVP/Fase 1 do projeto "Locações com alertas em Compras" (Rev. 2308–2311). Pedido user (23/05/2026, PDF Jalves F051/R051 com 80+ contratos): cada locadora tem layout próprio, IA precisa detectar pra cadastro inicial. Decisões via user_query: Gemini (já tem chave), alertas in-app+email (próxima rev), workflow completo de aprovação de renovação (Rev. 2310). **Schema** (`drizzle/schema.ts`, aditivo em `equipamentos_locados`): +`numero_contrato_fornecedor` (50), +`atendente_responsavel` (255), +`arquivo_origem_url` (TEXT), +`valor_subtotal_contrato` (NUMERIC), +index `(company_id, numero_contrato_fornecedor)`. **SyncSchema+ Rev. 2308** em `server/_core/index.ts` (4×ADD COL IF NOT EXISTS + CREATE INDEX IF NOT EXISTS, com guarda `to_regclass` pra pular se tabela inexistente em dev). **Helper IA** (`server/_core/llm.ts`): novo `invokeGeminiVision` usando endpoint nativo `/v1beta/models/{model}:generateContent`, `inline_data` PDF/imagem + `responseSchema` JSON mode + retry 429. **Backend** (`server/routers/equipamentos.ts`, 2 procedures): `parsearContratoLocacaoPdf` (Gemini c/ schema rígido + datas BR→ISO + **hard limit 25MB base64** pós-review); `importarContratosLocacaoLote` (cada unidade física = 1 linha em `equipamentos_locados`, valor mensal=subtotal/qtde, evento `RECEBIMENTO` por linha, **100% transacional via `db.transaction` pós-review**). **Frontend** (`Locados.tsx`): botão "✨ Importar PDF (IA)" + modal drag&drop + preview editável (cards/contrato, tabela nested de itens). **R-001/R-007/R-010:** OK — 100% aditivo, sem DROP/DELETE.
