...
- **Rev. 2397** — **FINANCEIRO/LANÇAMENTOS · Natureza (Fixo/Variável) é herdada automaticamente da categoria selecionada.** Pedido user (IMG_image_1779707852143, 25/05/2026): se a categoria já tem natureza cadastrada (ex: "Mão de Obra Indireta" = Variável no Plano de Contas), o lançamento devia puxar isso ao invés de forçar escolha manual. Causa-raiz: `getAccounts` já devolvia `natureza` por categoria (`financial_accounts.natureza`), mas `categoriasFiltradas` no componente descartava o campo (mapeava só `{id, nome, centroCustoId}`). **Frontend** (`client/src/pages/financeiro/FinanceiroLancamentos.tsx`): (1) `categoriasFiltradas` (L191-208) propaga `natureza: a.natureza ?? null`. (2) Derivados `categoriaSelecionada` (match exato case-insensitive de `form.contaNome` contra lista filtrada pelo tipo) + `naturezaHerdada` (só "fixo"/"variavel"). (3) `useEffect` (L221-228) sincroniza `form.natureza` quando categoria muda. (4) Select de Natureza (L871-908): `disabled` quando há herança, label com chip "DA CATEGORIA" (azul), legenda dinâmica abaixo ("Definida no cadastro da categoria." vs "Categoria não cadastrada — defina manualmente."). Import `useEffect` adicionado. Zero mudança em schema/backend (campo `financial_accounts.natureza` e endpoint `getAccounts` pré-existentes). R-001/R-007/R-010 OK.
