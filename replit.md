...
- **Rev. 3171** — **CADASTRO · CONTAS BANCÁRIAS · OS NÚMEROS DE AGÊNCIA E CONTA PASSARAM A SER EXIBIDOS NO PADRÃO BANCÁRIO BRASILEIRO — CORPO AGRUPADO POR PONTOS E DÍGITO VERIFICADOR SEPARADO POR TRAÇO (EX: "130026093" → "13.002.609-3"); MESMO PADRÃO REPLICADO NOS CARDS DE CONTA DA CONCILIAÇÃO BANCÁRIA.** PEDIDO (tela "Editar Conta Bancária"): "melhore o layout e os números, quero separado por ponto e traço conforme o padrão contas". FORMATTERS (`client/src/lib/formatters.ts`, idempotentes): `formatConta` agrupa o corpo com pontos + traço no verificador ("130026093"→"13.002.609-3"); `formatAgencia` só dígitos, traço no verificador com 5+ dígitos. TELAS: `ContasBancarias.tsx` cards + inputs do diálogo (`font-mono`, normaliza no `onBlur`, placeholders novos); `FinanceiroConciliacao.tsx` cards `Ag./Conta` no mesmo padrão. Nada no backend casa por número de conta (conciliação liga por `conta_bancaria_id`) → formatar é seguro. ZERO SCHEMA/ALTER/DROP/DELETE · ZERO BACKEND. Detalhe: `shared/changelog.ts`.

- **Rev. 3170** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · CADA CARD DE CONTA PASSOU A MOSTRAR O STATUS DE CONCILIAÇÃO DO PERÍODO — VERDE "CONCILIADO" QUANDO O EXTRATO JÁ FOI SUBIDO E ESTÁ 100% CONCILIADO, AZUL "A CONCILIAR" QUANDO TEM EXTRATO COM PENDÊNCIA, CINZA "SEM EXTRATO" QUANDO NÃO HÁ LINHAS NO PERÍODO.** PEDIDO: "as contas que já tiverem o extrato upado e 100% conciliado deveria aparecer um card verde... algo que mostre que já foi feito". BACKEND (`server/routers/financial.ts`, 1 query READ-ONLY tenant-safe): `getBankAccountsConciliacaoStatus({companyId,dataInicio,dataFim})` agrega `bank_statement_lines` POR conta no período (`COUNT(*)` + `SUM(conciliado=1)`) → `{contaBancariaId,total,conciliadas,status}` (consolidado/lancamento/vazio — mesma semântica das bolinhas de mês, mas por conta). FRONTEND (`FinanceiroConciliacao.tsx`): `accStatusMap` pinta cada card — verde+pill "Conciliado", azul+pill "A conciliar", neutro+pill "Sem extrato"; seleção ganha ring (verde se consolidado) + check sobre o ícone; import/conciliar/consolidar chamam `refetchAccStatus`. ZERO SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
