...
- **Rev. 3052** — **CONFIGURAÇÕES · "SÓCIOS": INDICAR O SÓCIO ADMINISTRADOR FICOU ÓBVIO — BOTÃO "DEFINIR COMO ADMINISTRADOR" DIRETO EM CADA CARD DE SÓCIO.** PEDIDO (print iPad): "Preciso ter a opção de indicar o sócio administrador, quero esta opção." A opção JÁ EXISTIA (rádio por sócio + botão inferior único), mas aparecia CINZA/DESABILITADA → usuário entendeu que NÃO TINHA a opção. CAUSA-RAIZ (FRONT/UX, sem bug de backend): o botão inferior usava `disabled={!dirty}` com `dirty = selected !== currentId`; como o admin atual já vinha SELECIONADO por padrão, `dirty` nascia FALSE e o botão só habilitava ao clicar no rádio de OUTRO sócio — pré-requisito nada óbvio, ainda mais no toque do iPad. SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE — reusa `setSocioAdministrador` da Rev. 3049/3051) em `client/src/pages/configuracoes/SociosAdministradorSection.tsx`: APOSENTADO o padrão "rádio de seleção + botão único embaixo + estado `selected`/`dirty`/Cancelar" (removidos estado, useEffect que o semeava e bloco inferior). Agora CADA card traz no rodapé (ao lado de "Salvar dados financeiros") OU o selo "Sócio administrador atual" (se já é), OU o botão verde "Definir como administrador" (se não é) que chama `setMut.mutate({companyId, employeeId})` na hora — sempre habilitado, com spinner "Definindo…" no card-alvo. O círculo do cabeçalho virou indicador VISUAL do admin (Crown branca), não mais seletor. Gated por `isAdmin`; backend mantém tenant guard + admin. RESSALVA: nada muda no schema nem no backend — pura clareza de UX. Detalhe: `shared/changelog.ts`.
