...
- **Rev. 3153** — **FINANCEIRO / LANÇAMENTOS · CORRIGIDO O ERRO "The string did not match the expected pattern" AO CANCELAR BAIXA EM LOTE NO iPad — A AÇÃO VOLTOU A FUNCIONAR E A LISTA ATUALIZA SOZINHA MESMO QUANDO O iOS DERRUBA A REQUEST.** PEDIDO (IMG_2092, iPad): "Cancelar baixa (45)" jogava o toast vermelho "The string did not match the expected pattern". DIAGNÓSTICO: NÃO é backend (`bulkEstornar` está correto); é DOMException CRUA do iOS Safari quando a request da mutation é DERRUBADA no transporte. Como as 3 ações em lote (baixa/estorno/exclusão) são IDEMPOTENTES no servidor, a operação frequentemente JÁ foi aplicada mesmo com o iOS reportando erro. CORREÇÃO (FRONTEND-ONLY, `FinanceiroLancamentos.tsx`): helper `isTransportErr` + `retry: bulkRetry` (só re-tenta em erro de transporte, idempotente) + `onError` resiliente que, no caso de transporte, fecha o diálogo, limpa a seleção, faz `refetch()`+`invalidarContas()` e mostra aviso BRANDO ("Conexão instável — a ação pode ter sido aplicada; atualizamos a lista") em vez da DOMException. ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.

- **Rev. 3152** — **FINANCEIRO / LANÇAMENTOS · OS LANÇAMENTOS CANCELADOS PASSARAM A FICAR OCULTOS POR PADRÃO NA LISTA — UM BOTÃO "Mostrar cancelados (N)" NO CABEÇALHO REVELA/OCULTA SOB DEMANDA; O RASTRO NUNCA É APAGADO, SÓ ESCONDIDO.** PEDIDO (OPÇÃO 2 escolhida pelo usuário): a lista misturava cancelados com lançamentos vivos, poluindo o dia a dia — esconder por padrão preservando 100% do histórico. COMPORTAMENTO: lista some `status==="cancelado"` por padrão; o cabeçalho do card ganha botão "Mostrar cancelados (N)"/"Ocultar cancelados (N)" quando há ≥1 cancelado no recorte (respeita a busca); se o usuário escolher EXPLICITAMENTE o status "Cancelado" no filtro, eles aparecem sempre (ocultamento só vale com status="all"). PARIDADE: os agregados de topo já ignoram cancelado no servidor (`getEntriesTotais`/`getEntriesResumoMensal`) e a soma da lista também → esconder é puramente visual, totais batem, sem regressão. FRONTEND-ONLY (`FinanceiroLancamentos.tsx`: estado `showCancelados`, helper `matchBusca`, `canceladosCount`, `ocultandoCancelados`, derivação `lancamentos` + botão ghost no `CardTitle`). ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
