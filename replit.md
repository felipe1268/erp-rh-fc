...
- **Rev. 4564** — **FEAT: EQUIPAMENTOS — TAG DE LOCALIZAÇÃO DO FIXO (OBRA × ALMOXARIFADO) + MARCAÇÃO EM LOTE.** `localizacao` ('em_obra'|'no_almox') derivada em locadosUtilizacao/propriosUtilizacao: empréstimo aberto OU obra vinculada no cadastro OU sem vínculo com almox = em obra; senão no almox. Badge indigo "Instalado na obra" × âmbar "No almoxarifado" na seção Instalados. Dados: 125 equipamentos ativos (60 locados + 65 próprios) com keywords de uso contínuo marcados como fixo no Neon (lista aprovada pelo usuário). Arquivos: `equipamentos.ts`, `LocadosUtilizacao.tsx`, `PropriosUtilizacao.tsx`. ZERO schema change.
### 5 one-liners
