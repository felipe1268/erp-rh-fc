...
- **Rev. 2217** — **UX/HE · Alerta "HE aprovada SEM ponto batido" na aba Aprovações.** Lilian: "pode acontecer de ter solicitação de HE aprovada mas o funcionário não bateu o ponto.. precisa colocar um alerta pra o RH analisar se paga ou não". Cenário: funcionário esquece de bater ponto facial → folha gera "sem solicitação" e RH não consegue separar casos legítimos (HE retroativa manual) dos que de fato não fizeram. Fix: nova procedure `heSolicitacoes.aprovadasSemPonto` (`server/routers/heSolicitacoes.ts:184-264`) com raw SQL `JOIN he_solicitacao_funcionarios + LEFT JOIN employees/obras` + `NOT EXISTS` contra `time_records` (filtra `horasTrabalhadas IN ('','0:00','00:00','0:0')`), respeitando `getEffectiveAllowedObraIds` (R-007). Frontend `SolicitacaoHE.tsx` ganhou badge laranja no mini-resumo + card laranja no topo da aba Aprovações agrupando por solicitação, com pills clicáveis abrindo Raio-X e botão "Ver solicitação". Só aparece quando há ≥1 caso. **R-001/R-007/R-010:** OK — apenas SELECT.
