...
- **Rev. 4486** — **FIX: BUG DE CONTAGEM E FILTRO DE STATUS DAS SCs.** `statusEfetivoSC` (Rev. 1693) derivava o badge "Aguardando Pagamento" corretamente, mas `statusBreakdown` e `breakdownPredicates` usavam `r.status` RAW — contavam essas SCs em "Em Cotação"/"Pendente"/"Em Andamento" e o filtro por "Em Cotação" exibia SCs com badge "Aguardando Pagamento". Correção: todos os três contadores excluem `_ocsEntregues=true`; novo contador + predicado `aguardandoPagamento`; novo bloco violeta no painel de status; grid de 9 → 10 colunas. ZERO schema change.
- **Rev. 4485** — **FIX: PLACEHOLDERS DE PRAZO DE NF NO TEMPLATE ISO DE CONTRATO PJ.** Template real = ISO (systemDocumentTemplates), não hardcode. Novos placeholders `[TEXTO_DIA_FECHAMENTO]`, `[PRAZO_NOTA_ADIANTAMENTO]`, `[PRAZO_NOTA_FECHAMENTO]` adicionados aos 4 paths de substituição: `contratoPjDocument.ts`, `ContratoPJView.tsx`, `TemplatesDocsTab.tsx` (FINANCIAL_RE bold) e sidebar de `ModuloPJ.tsx`. `[DIA_FECHAMENTO]=31` agora exibe "último dia do mês". Default 6.2 atualizado com parágrafo Único de prazo NF. ZERO schema change.
