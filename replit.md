...
- **Rev. 2694** — **ALMOXARIFADO · EMPRÉSTIMO DE FERRAMENTAS/EQUIPAMENTOS (`/almoxarifado` → "Emprestar Ferramenta" → "CONFIRMAR EMPRÉSTIMO") · CORRIGIDO O ERRO `DB error: column "foto_devolucao_url" of relation "warehouse_loans" does not exist` QUE IMPEDIA REGISTRAR EMPRÉSTIMOS (E QUEBRAVA A LISTA DE EMPRÉSTIMOS EM ABERTO) EM PRODUÇÃO.** Pedido (usuário, print "Falha após 0 de 1 ferramentas: Failed query: insert into warehouse_loans …"): "está dando erro no empréstimo de ferramentas/Equipamentos no almoxarifado, corrija isso". Causa-raiz: as colunas de rastreio da Rev. 2256 (`foto_devolucao_url`, `equipamento_proprio_id`, `equipamento_locado_id`) entraram no `drizzle/schema.ts` (`warehouseLoans`) mas NUNCA ganharam guard explícito `ADD COLUMN IF NOT EXISTS` no `[SyncSchema+]`. O Drizzle `db.insert().values()` gera INSERT listando TODAS as colunas do schema (não informadas = `default`), inclusive as 3 da Rev. 2256; o banco Neon de PRODUÇÃO (que só roda guards explícitos) nunca as recebeu → 42703. Fix (SÓ SELF-HEAL; ZERO ALTER/DROP/DELETE DESTRUTIVO — R-001/R-007/R-010): `server/_core/index.ts` — bloco `Rev. 2694` com 3× `ALTER TABLE warehouse_loans ADD COLUMN IF NOT EXISTS` (idempotente, cura no próximo deploy). NECESSÁRIO PUBLICAR para criar as colunas em produção. esbuild EXIT 0. Detalhe: `shared/changelog.ts`.
- **Rev. 2693** — **FINANCEIRO · NOVO LANÇAMENTO (`/financeiro/lancamentos` → "Novo Lançamento" → tipo "Transferência") · A ABA "TRANSFERÊNCIA" GANHOU FLUXO PRÓPRIO E ENXUTO PARA REGISTRAR TRANSFERÊNCIAS ENTRE CONTAS BANCÁRIAS, LANÇANDO AS DUAS PERNAS (SAÍDA NA ORIGEM + ENTRADA NO DESTINO) DE UMA VEZ, JÁ COMO PAGAS, E CONCILIANDO AS DUAS JUNTAS AUTOMATICAMENTE.** Pedido (usuário): transferências dentro do modal "Novo Lançamento" (aba Transferência), só daqui pra frente, conciliando as 2 pernas juntas; form só com Valor | Data | Conta Origem | Conta Destino | Tipo(forma) | Observação; NÃO gerar Contas a Pagar/Receber. Comportamento: grava 2 linhas `financial_entries` `tipo='transferencia'`, `status='pago'`, `conciliado=0`, ligadas por `transferencia_grupo_id` (UUID); como não é receita/despesa, fica fora de Contas a Pagar/Receber/Fluxo/DRE/KPIs (líquido zero). Conciliar QUALQUER perna (manual ou OFX em lote) marca a irmã junto via UPDATE self-join. Fix (1 COLUNA READ-ONLY NOVA VIA SELF-HEAL + SERVER + CLIENT; ZERO ALTER/DROP/DELETE — R-001/R-007/R-010): `drizzle/schema.ts` (col `transferenciaGrupoId` varchar(36)) + self-heal `server/_core/index.ts`; `server/routers/financial.ts` (`createEntry` inputs `contaBancariaOrigem/DestinoId` + ramo 2 pernas; `conciliarLancamento` self-join irmã); `server/services/cfoPhase2.ts` (`applyReconciliation` self-join irmã); `client/src/pages/financeiro/FinanceiroLancamentos.tsx` (bloco enxuto + esconde despesa/recorrência + ramo no `handleSave`). FIX ADICIONAL (mesma tela): `getEntries` quebrava com `42601 | syntax error at end of input` pois o COUNT usava `conds.slice(0, -0)` (= `slice(0,0)` → array vazio → `WHERE ` sem condição) — trocado por `conds.join(" AND ")`. esbuild EXIT 0 · vite EXIT 0. Detalhe: `shared/changelog.ts`.
