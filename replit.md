...
- **Rev. 3262** — **MÓDULO PJ / FOLHA PJ (PAGAMENTOS) · A ABA "FOLHA PJ" GANHOU DUAS LEITURAS NOVAS (100% READ-ONLY) P/ AMARRAR CADA PRESTADOR AO CADASTRO DE FORNECEDORES: (1) COLUNA "FORNECEDOR CADASTRADO" COM BADGE — VERDE QUANDO O CNPJ DO CONTRATO PJ CASA EXATO COM UM FORNECEDOR (MOSTRA A RAZÃO SOCIAL), ÂMBAR QUANDO HÁ SUGESTÃO POR NOME A CONFIRMAR, CINZA "NÃO CADASTRADO" COM ATALHO "CADASTRAR" QUE ABRE A TELA DE FORNECEDORES JÁ COMO PRESTADOR DE SERVIÇO E COM O CNPJ PREENCHIDO; (2) PAINEL "RANKING POR FORNECEDOR" COM O SOMATÓRIO HISTÓRICO EM BRL POR PRESTADOR (RECEBIDO/PAGO EM DESTAQUE, TOTAL LANÇADO, E O QUANTO CAI NO MÊS SELECIONADO SINALIZADO EM ROXO), ORDENADO DO MAIOR P/ O MENOR. ZERO ESCRITA.** PEDIDO (piloto FC): saber, dentro da própria Folha PJ, quais prestadores já estão cadastrados como Fornecedores e quanto cada um já recebeu no histórico. SOLUÇÃO: BACK (`server/routers/pjContracts.ts`) — importou `fornecedores`; helpers `normCnpj`/`normNomeEmpresa`/`carregarFornecedorIndex`/`matchFornecedor` (CNPJ exato→"match"; nome→"sugestao"; senão "ausente")/`carregarCnpjPorEmployee`; `pagamentos.list` ganhou LEFT JOIN em `pj_contracts` p/ `cnpjPrestador`/`razaoSocialPrestador` + campos `fornecedorStatus`/`fornecedorId`/`fornecedorNome`/`cnpjPrestador`; nova query `pagamentos.rankingFornecedores` (`SUM(NULLIF(valor,'')::numeric)` por employee: totalHistorico/totalRecebido(pago)/totalMes/qtd + match, desc), ambas tenant-bound. FRONT (`ModuloPJ.tsx`) — componente `FornecedorCadastroBadge` (verde/âmbar/cinza), coluna nova na tabela de pagamentos, painel "Ranking por fornecedor" (tabela + tfoot de totais, ícone `Award`). FRONT (`compras/Fornecedores.tsx`) — handler de URL `?novo=<cnpj>` que abre o cadastro de nova empresa `isPrestadorServico=true` com CNPJ preenchido (espelha o `?edit=`). ZERO BACKEND DE ESCRITA · ZERO SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
