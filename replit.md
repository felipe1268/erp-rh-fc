...
- **Rev. 2855** — **CADASTRO DO COLABORADOR — TAMANHOS DE EPI/UNIFORME VIRAM ABA PRÓPRIA COM LISTAS PRONTAS (DROPDOWN), SEM DIGITAÇÃO LIVRE.** Pedido (follow-up Rev. 2854): "não quero que digite, quero lista pronta — usuário só informa qual é — e o cadastro com uma aba nova para os tamanhos dos pés e do uniforme". FEITO (`Colaboradores.tsx`, SÓ frontend): NOVA ABA "🦺 Uniforme / EPI" (TabsTrigger/TabsContent `value="uniforme"`, entre Profissional e Bancário); a subseção antiga dentro de "Profissional" foi REMOVIDA. Os 3 inputs de texto viraram `Select` com LISTAS PRONTAS module-level (`TAMANHOS_CALCADO` 33–48, `TAMANHOS_CAMISA` PP…EXG, `TAMANHOS_CALCA` 36–58 par) + opção "— Não informado —" (sentinel `TAMANHO_NONE`→string vazia, Radix não aceita value=""). SEM schema/backend: colunas e whitelist da Rev. 2854 intactos; Grade de Tamanhos segue agregando (já fazia `.toUpperCase()`). ZERO ALTER/DROP/DELETE; ZERO schema; só UI. Detalhe: `shared/changelog.ts`.
