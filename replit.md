...
- **Rev. 2460** — **EQUIPAMENTO LOCADO · botão "Desfazer devolução" no Raio-X com senha + motivo obrigatórios (auditado).** User (print IMG_1252, equip "VIGA P/ 3 MTS" travado em status `devolvido`): "preciso reverter a devolução, registrei por engano. Senha e motivo claro pra ficar tudo auditado". **Gap:** não havia caminho reverso de `locadoDevolverEmLote` → equipamento ficava preso em `devolvido`, forçando SQL manual sem rastro. **Backend:** nova mutation `locadoDesfazerDevolucao({companyId,id,senha?,motivo})` em `server/routers/equipamentos.ts` — tenant isolation + `getEffectiveAllowedObraIds` + valida `status==="devolvido"`. Lê `companies.almoxarifadoExigeSenha/exigeJustificativa` e replica inline os helpers de senha (bcrypt.compareSync) e justificativa ≥10 chars (evita cross-import entre routers). UPDATE: `status="em_uso"`, `dataFimReal=null`, `fotosDevolucaoJson=null`. INSERT evento `REVERSAO_DEVOLUCAO` na timeline + INSERT `almoxarifadoAuditoria` com `acao="desfazer_devolucao_locacao"`, dadosAntes/Depois e IP. **Decisão:** NÃO re-cria item no almox central (geraria duplicata sem o contrato original — user repõe manual se quiser). **Frontend (`Locados.tsx`):** novo TIPO_META `REVERSAO_DEVOLUCAO` (ícone `Undo2` laranja). Queries `auth.me` + `compras.getAuditoriaConfig` derivam `requerSenhaAud`/`requerJustAud`. Botão laranja "Desfazer devolução" no footer do modal Raio-X (só se `l.status==="devolvido"`) + reuso de `ModalConfirmacaoAuditoria` com descrição explicitando consequências. R-001/R-007/R-010 OK. Arquivos: `server/routers/equipamentos.ts`, `client/src/pages/equipamentos/Locados.tsx`. Detalhe: `shared/changelog.ts`.
