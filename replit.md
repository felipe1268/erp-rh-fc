...
- **Rev. 2942** — **ATESTADOS & ACIDENTES (SST) — O BOTÃO "VISUALIZAR DOCUMENTO" AGORA MOSTRA O ANEXO NA PRÓPRIA TELA (IMAGEM/PDF), EM VEZ DE FORÇAR O DOWNLOAD DO ARQUIVO NO iPad/CELULAR.** Pedido (usuário, print do iPad): ao clicar em "Documento" em "Atestados & Acidentes", a modal "Visualizar documento" aparecia EM BRANCO e o iOS disparava o download do `.jpeg` ("não quero download, quero ver na tela"). CAUSA: o `EmployeeDetailDialog` (SST) renderizava o anexo num `<iframe>` p/ qualquer tipo — no iOS Safari uma URL de imagem (ou com `Content-Disposition: attachment`) dentro de iframe NÃO renderiza inline, vira download. SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE): `client/src/components/sst/EmployeeDetailDialog.tsx` trocou a modal caseira (Dialog+iframe) pelo componente já existente `DocumentPreviewDialog`, que detecta o tipo e renderiza IMAGEM via `<img>` (zoom/girar/arrastar) e PDF via iframe (+ "Baixar"/"Abrir em nova aba"). `<img>` renderiza inline em todo navegador independente do header. Helper novo `fileNameFromUrl()` deriva o nome p/ inferir o tipo. Detalhe: `shared/changelog.ts`.
