...
- **Rev. 2161** — **HOTFIX BUILD · Syntax error em `FinanceiroCategorias.tsx` (vírgula dupla no `createMut.mutate`).** User: "está dando erro em categorias" (Babel: Unexpected token 154:50). Regressão da Rev. 2157 que adicionou `escopo: "categoria"` no mutate sem quebrar linha, deixando `,,`. Fix: quebrar objeto em múltiplas linhas e remover vírgula extra. Zero mudança em backend ou dados. **R-001/R-007/R-010:** OK.
