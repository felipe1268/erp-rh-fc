...
- **Rev. 3358** — **FOLHA / FERIADOS · O DIÁLOGO "BAIXAR FERIADOS {ANO}" GANHOU UM LAYOUT MODERNO COM AS CORES INSTITUCIONAIS DA FC (FAIXA AZUL NO TOPO COM ÍCONE, CARD VERDE DOS NACIONAIS, CHIPS DE UF COM SELO/CHECK E PONTINHO ÂMBAR P/ AS UFs DAS OBRAS) + ATALHOS "TODAS"/"LIMPAR" E CONTADOR DE UFs SELECIONADAS. 100% FRONT · UX · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** Pedido (usuário): "melhore o layout usando nossas cores e facilidade e modernidade" (print do diálogo "Baixar Feriados 2026" da Rev. 3355). CORES FC: `--primary` `oklch(0.35 0.12 260)` (azul ~#1B2A4A) + acento âmbar/dourado (`--sidebar-primary` `oklch(0.78 0.15 80)`). FRONT (`client/src/pages/Feriados.tsx`, só o diálogo "Baixar Feriados"): `DialogContent` vira `p-0 gap-0 overflow-hidden max-w-xl`; CABEÇALHO = faixa `bg-gradient-to-br from-primary to-primary/85` com ícone Download em badge translúcido + título/subtítulo; card "Feriados nacionais incluídos" redesenhado (esmeralda + ícone em badge); seção "Feriados estaduais" com contador `{n} selec.` (badge azul) e atalhos `Button ghost` "Todas"/"Limpar" (novos handlers `selecionarTodasUfs`/`limparUfs`), aviso das UFs pré-marcadas com pontinho âmbar; CHIPS de UF reestilizados (sigla em quadradinho mono azul quando selecionado, nome, pontinho âmbar p/ UF de obra, CheckCircle quando selecionado, hover com borda azul); rodapé `border-t bg-muted/30`. SEM mudança de comportamento: mutation `baixarFeriados` e query `ufsEstaduaisDisponiveis` idênticas. VALIDAÇÃO: tsc limpo no arquivo tocado. Detalhe: `shared/changelog.ts`.
