...
- **Rev. 2673** — **GESTÃO DE DOCUMENTOS (`/gestao-documentos` → lista de documentos de uma pasta) · O LAYOUT DA COLUNA "TÍTULO / CÓDIGO" PASSA A MOSTRAR O TÍTULO DIGITADO PELO USUÁRIO EM DESTAQUE E O NÚMERO/CÓDIGO DO ARQUIVO ABAIXO — ACABA A REDUNDÂNCIA "CÓDIGO + NOME .dwg" (REFINA A Rev. 2672).** Pedido (usuário, print image_1780332956204): após a Rev. 2672 a célula mostrava CÓDIGO em destaque + NOME ".dwg" logo abaixo (quase idêntico = redundante) + descrição; o usuário pediu "nome em destaque = título do arquivo; abaixo = número do arquivo". Causa: semântica dos campos difere por fluxo — no UPLOAD EM LOTE o "título" digitado vai pra `descricao` e `titulo`/`codigo` recebem o nome/código (iguais); na CRIAÇÃO MANUAL `titulo` é o título real. Fix (SÓ CLIENT/UI; ZERO SCHEMA; ZERO SERVER): `client/src/pages/gestaodocumentos/index.tsx` — por linha do `filteredDocs.map`, heurística robusta `veioDoLote = normTxt(titulo)===normTxt(codigo)` (trim+lowercase) E (sem `arquivoNome` OU `stem(arquivoNome)===codigo`); `tituloIsCode = veioDoLote && descricao não-vazia`; `tituloDestaque = tituloIsCode ? descricao : titulo` (fallbacks codigo/arquivoNome/"—"); `numeroArquivo = codigo||arquivoNome`; `descricaoExtra = !tituloIsCode ? descricao : ""`. Render: 1ª linha `font-semibold` c/ `tituloDestaque`; 2ª linha (número) só se `!== tituloDestaque`; 3ª linha `descricaoExtra` (só docs manuais). Normalização cobre legacy caixa/espaço; stem evita falso-positivo manual. Badges/clipe/`docParsed`/busca inalterados. Sem SQL crua/`ALTER`/`DROP` (R-001/R-007/R-010). esbuild `index.tsx` EXIT 0 (`pnpm build`/`tsc` estouram OOM). Detalhe: `shared/changelog.ts`.
- **Rev. 2672** — **GESTÃO DE DOCUMENTOS (`/gestao-documentos` → lista de documentos de uma pasta) · A COLUNA "TÍTULO / CÓDIGO" PASSA A MOSTRAR O TÍTULO (PREENCHIDO PELO USUÁRIO) EM DESTAQUE E O NOME DO ARQUIVO LOGO ABAIXO, PARA FACILITAR A BUSCA.** Pedido (usuário, prints image_1780331772644 + image_1780331779987): na lista de documentos, a célula "Título / Código" mostrava só o `titulo` (fonte mono pequena cinza, parecendo código) + descrição; o usuário quer o TÍTULO que ele preenche em destaque E o NOME DO ARQUIVO (ex.: "POITA-ARQ-000-PE-PL-GRAL-IMP-R02.dwg") logo abaixo. Fix (SÓ CLIENT/UI; ZERO SCHEMA; ZERO SERVER): `client/src/pages/gestaodocumentos/index.tsx` — na `TableCell` "Título / Código": (a) `doc.titulo` vira `text-sm font-medium text-gray-900` (título legível); (b) nova linha com `doc.arquivoNome` em `text-[11px] font-mono text-gray-500 truncate` (só com arquivo); (c) `descricao` em `text-gray-400` abaixo. Badges/ícone de clipe inalterados; busca já indexava `arquivoNome`. Sem SQL crua/`ALTER`/`DROP` (R-001/R-007/R-010). esbuild `index.tsx` EXIT 0 (`pnpm build`/`tsc` estouram OOM). Detalhe: `shared/changelog.ts`.
