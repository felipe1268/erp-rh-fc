...
- **Rev. 3351** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · A "MOVIMENTAÇÃO INTERNA" (DINHEIRO QUE SÓ GIRA ENTRE AS CONTAS/EMPRESAS DA PRÓPRIA FC) AGORA TEM UMA BASE DE CNPJs/CPFs CADASTRÁVEL (NOVA ABA EM CONFIGURAÇÕES FINANCEIRAS), ALÉM DA HEURÍSTICA POR TEXTO DA Rev. 3349. CLASSIFICAÇÃO SIMÉTRICA (ENTRADA E SAÍDA) E CADA LANÇAMENTO PODE TER EXCEÇÃO MANUAL ("MARCAR COMO EFETIVO: EMPRÉSTIMO/CAPITALIZAÇÃO" / "MARCAR COMO INTERNO") COM MOTIVO, REVISÁVEL PELO DRILL-IN. 2 TABELAS NOVAS (SELF-HEAL) + BACKEND READ-ONLY/CRUD + 3 FRONTS · ZERO ALTER/DROP/DELETE EM DADOS EXISTENTES.** Pedido (usuário): transferências entre empresas do grupo (CNPJs distintos) não caem na regra de texto; quer lista cadastrável revisável, classificação nos 2 sentidos e exceção por lançamento (crédito de empréstimo/capitalização que ENTRA de verdade). MODELAGEM (self-heal `[SyncSchema+]` + `drizzle/schema.ts`, só `CREATE TABLE IF NOT EXISTS`): `financial_internal_cnpjs` (company_id, cnpj[dígitos], nome, observacao, ativo, criado_por, timestamps) + `financial_internal_overrides` (company_id, line_id→`bank_statement_lines.id`, natureza['efetivo'|'interno'|'auto'], motivo, UNIQUE(company_id,line_id)). BACKEND (`financial.ts`, FONTE ÚNICA): `_loadInternoConfig` carrega dígitos ativos + mapa de overrides num load; predicado = regex base Rev. 3349 OR `regexp_replace(descricao,'[^0-9]','','g') LIKE '%<cnpj>%'`, helper JS `_isLancInterno` espelha; override por id sobrepõe (efetivo→nunca interno; interno→sempre; auto→regra); 3 endpoints READ-ONLY consomem a config + `getConciliacaoReportGeral` devolve `overrideNatureza`. CRUD `listInternalCnpjs`/`create`/`update`/`delete`(soft `ativo=0`) + `setLancamentoNatureza` (upsert; `auto`=limpar; motivo obrigatório), todos com `_assertFinanceiroCompanyAccess`. FRONTS: aba "Movimentação Interna" em `FinanceiroConfiguracoes.tsx`; componente compartilhado `_NaturezaOverride.tsx` (`NaturezaBadge`+`NaturezaOverrideDialog`); drill-ins de `DashConciliacao.tsx`/`FinanceiroConciliacao.tsx` ganham selo + botão "Classificar". SEED (60002): 8 CNPJs/CPF do grupo (`ativo=1`, rótulo "validar"); PIX 00360305/90400888/60701190 NÃO semeados. VALIDAÇÃO (Neon 60002/2026): interno sobe de ~31,8% → ~43,7% do giro (394/2.841); caixa real Entradas externas R$ 4.362.797,59, Saídas externas R$ 12.465.225,53. tsc limpo. Detalhe: `shared/changelog.ts`.
