...
- **Rev. 2612** — **COLABORADORES · O CARD "NA EMPRESA" PASSA A SER CLICÁVEL E FILTRA A LISTA (TODOS COM VÍNCULO = TOTAL − DESLIGADOS − BLACKLIST) — ANTES NÃO ACONTECIA NADA AO CLICAR (ERA O ÚNICO CARD INERTE).** Pedido (usuário, screenshot): "quero poder clicar no card e ele filtrar tudo… o card NA EMPRESA não está responsivo, quando eu clico não acontece nada". Causa-raiz: o card (Rev. 2608) tinha `filter: null`, então o `onClick` era no-op. Fix (SÓ CLIENT — `Colaboradores.tsx`; ZERO SERVER/SCHEMA/ALTER/DROP/DELETE): card ganha `filter: "NaEmpresa"` (valor sintético de `statusFilter`); `serverStatus` trata "NaEmpresa" como `undefined` (busca tudo, filtra no client); `displayEmployees` aplica `list.filter(e => !isInativo(e))` (= Ativos + Férias + Afastados + Licença + Aviso + Reclusos, mesma def. do contador da Rev. 2608); dropdown ganha `<SelectItem value="NaEmpresa">`. Validado: esbuild client (exit 0). Detalhe: `shared/changelog.ts`.
- **Rev. 2611** — **GERENCIADORAS · AO DIGITAR O CNPJ OS DADOS SÃO PUXADOS AUTOMATICAMENTE DA RECEITA FEDERAL (RAZÃO SOCIAL, NOME FANTASIA, ENDEREÇO, MUNICÍPIO/UF/CEP, SITUAÇÃO CADASTRAL, TELEFONE, E-MAIL E QUADRO SOCIETÁRIO/SÓCIOS) + MODAL REDESENHADO EM 2 COLUNAS PARA CABER SEM BARRA DE ROLAGEM.** Pedido (usuário, no form Editar Gerenciadora): "quando digitar o CNPJ deve puxar todos os dados automaticamente, endereço, sócios.. etc... ajuste tbm o layout da página.. não quero precisar de barra de rolagem". Implementação: SCHEMA (`drizzle/schema.ts` tabela `gerenciadoras`) ganha colunas ADITIVAS `razao_social/nome_fantasia/endereco/bairro/municipio/uf/cep/situacao_cadastral` + `socios` (json) — R-001/R-007/R-010 (ZERO ALTER destrutivo/DROP/DELETE); SELF-HEAL `[SyncSchema+] Rev. 2611` em `server/_core/index.ts` com `ADD COLUMN IF NOT EXISTS` por coluna (mesmo padrão da Rev. 2607 — o auto `syncSchema()` não pegou as colunas de forma confiável no restart, guard explícito garante dev+prod); ROUTER `gerenciadoras` (criar/atualizar) aceita os novos campos; CLIENT `Gerenciadoras.tsx` faz `fetchCnpjData` à BrasilAPI (mesmo padrão de Empresas/Clientes) ao completar 14 dígitos, modal `max-w-3xl` em grid 2 colunas com CNPJ em destaque (spinner+badge de situação) e quadro societário em lista compacta. Colunas confirmadas no Neon via information_schema. Validado: esbuild client+server+schema (exit 0) + workflow reiniciado. Detalhe: `shared/changelog.ts`.
