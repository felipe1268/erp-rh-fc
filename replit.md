...
- **Rev. 2969** — **PORTAL DO CLIENTE → ADMINISTRAÇÃO → AVALIAÇÕES (NPS) → "LINK DE AVALIAÇÃO (SEM LOGIN)" — NOVO BOTÃO "WHATSAPP" QUE ABRE O WHATSAPP COM UMA MENSAGEM CORDIAL JÁ PRONTA, CONVIDANDO O CLIENTE A RESERVAR ALGUNS MINUTOS PARA AVALIAR A EQUIPE (MELHORIA CONTÍNUA), COM O LINK EMBUTIDO.** Pedido (usuário): ao enviar o link via WhatsApp deveria aparecer uma mensagem cordial pedindo um tempo ao cliente para avaliar a equipe (melhoria contínua). SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE, `client/src/pages/ClientesPortalAdmin.tsx`): ao lado de "Copiar"/"Abrir", botão "WhatsApp" (ícone `MessageSquare`, verde) que monta mensagem pt-BR cordial ("Olá! Tudo bem? Aqui é da FC Engenharia…", convite rápido/anônimo + melhoria contínua) com o `linkAvaliacao` embutido e personaliza "…na obra {linkObraNome}" quando vinculado; abre `https://wa.me/?text=<encoded>` em nova aba (padrão de Locados/IntegracaoSST). Usa o link já gerado; sem backend/schema. Detalhe: `shared/changelog.ts`.
