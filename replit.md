...
- **Rev. 3319** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · AO SELECIONAR UM MÊS SEM ESCOLHER UMA CONTA, A TELA SÓ MOSTRAVA "SELECIONE UMA CONTA BANCÁRIA"; AGORA EXIBE O "PANORAMA GERAL DO MÊS": TOTAIS AGREGADOS (CONCILIADOS / NO EXTRATO SEM LANÇAMENTO / NO ERP SEM EXTRATO / % CONCILIADO) DE TODAS AS CONTAS COM EXTRATO + UM CARD POR CONTA COM SEUS NÚMEROS E DRILL-IN (CLIQUE ABRE A CONTA P/ CONCILIAR). CADA LANÇAMENTO CONTINUA VINCULADO À SUA CONTA. 100% FINANCEIRO (1 BACKEND READ-ONLY + 1 FRONT) · ADITIVO · READ-ONLY (NADA CONCILIA NO PANORAMA) · ZERO SCHEMA/ALTER/DROP/DELETE.** BACKEND (`server/routers/financial.ts`): (a) o motor de `getConciliacaoReport` foi EXTRAÍDO p/ helper module-level `_computeConciliacaoReport(db, companyId, contaBancariaId, dataInicio, dataFim)` (refactor puro — a procedure só faz auth + delega); (b) nova query read-only `getConciliacaoReportGeral({companyId, dataInicio, dataFim})` lista as contas COM extrato no período (JOIN `company_bank_accounts` ⋈ `bank_statement_lines` `excluido_em IS NULL`), roda o MESMO motor por conta, tagueia cada linha com a conta de origem, agrega `totais` e inclui `lancamentosSemConta` (company-wide) UMA vez. FRONT (`FinanceiroConciliacao.tsx`): query habilitada só com `mesSel != null && !contaBancariaId`; empty-state virou painel "Panorama geral do mês" (KPIs + cards por conta com status via `accStatusMap` + botão `setContaBancariaId` p/ drill-in + bloco "sem conta"). Ambas as rotas mantêm `_assertFinanceiroCompanyAccess`. Detalhe: `shared/changelog.ts`.
