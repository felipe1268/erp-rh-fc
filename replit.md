...
- **Rev. 2396** — **FINANCEIRO/CONTAS A PAGAR · Nome do fornecedor visível na lista e no detalhe do título.** Pedido user (IMG_image_1779706619633 + IMG_image_1779706702673, 25/05/2026): nome do fornecedor digitado no lançamento não aparecia em Contas a Pagar (descrição "PGTO FORNECEDOR" sem identificador) nem ao abrir o detalhe. Causa-raiz: tabela `financial_entries` não tinha coluna `fornecedor_nome`; form non-recorrente nem mandava o campo; `materializeRecorrentes` não copiava de `financial_recurring_entries` pra `financial_entries`. **Schema** (`server/_core/index.ts` L633-648): `ALTER TABLE financial_entries ADD COLUMN IF NOT EXISTS fornecedor_nome VARCHAR(255)` (padrão SyncSchema+) + backfill 1-shot `UPDATE … SET fornecedor_nome = rec.fornecedor_nome FROM financial_recurring_entries rec WHERE origem_modulo='recorrente' AND origem_id=rec.id AND fe.fornecedor_nome IS NULL` — popula automaticamente as rows recorrentes já materializadas. **Backend** (`server/routers/financial.ts`): `createEntry` aceita+grava `fornecedorNome`; `materializeRecorrentes` (L145-155) copia o nome no INSERT mensal; `getContasAPagarByYear` (L3196) + `getEntryDetalhe` (L660) selecionam o campo. `drizzle/schema.ts` L6545 atualizado. **Frontend** (`client/src/pages/financeiro/`): `FinanceiroLancamentos.tsx` (L299) caminho non-recorrente envia `fornecedorNome`; `FinanceiroContasAPagar.tsx` `describeEntry` (L116-137) concatena `" — {fornecedor}"` na descrição da lista (evita duplicar se já constar no texto base); header do modal de detalhe (L1342) ganha linha "🏢 {fornecedorNome}" acima do 📍 obra. Política: ADD COLUMN IF NOT EXISTS + UPDATE em coluna não-key idempotente. R-001/R-007/R-010 OK.
- **Rev. 2395** — **ALMOXARIFADO/CONFIG · Limpeza de categorias ÓRFÃS no card de Configurações.** Pedido user (IMG_1204, 24/05/2026 21:30): após apagar "Compras" pelo fluxo da Rev. 2394, os cards continuaram mostrando "Compras" como categoria — causa: a "Compras" tinha sido apagada ANTES da Rev. 2394 entrar (ou direto no banco), então o UPDATE atômico do `excluirCategoria` reescrito não rodou e os itens ficaram com `categoria='Compras'` órfã. **Backend** (`server/routers/compras.ts` L2909-2950): nova mutation `limparCategoriasOrfas({ companyId })` — SELECT TRIM(categoria), COUNT(*) das órfãs (`NOT IN (SELECT nome FROM almoxarifado_categorias)`) pra visibilidade, UPDATE `almoxarifado_itens SET categoria=NULL` com mesmo predicado, `ativo=true` filtro. Idempotente, retorna `{itensMigrados, categoriasOrfas: [{categoria, total}]}`. **Frontend** (`AlmoxarifadoConfigSection.tsx`): calcula `orfas` no client (`Object.entries(contagens).filter(k => k !== '__sem__' && !Set(categorias.nome).has(k))`) — sem nova query. Banner ambar (`border-t bg-amber-50` + `AlertTriangle`) só aparece se houver órfãs, lista até 5 nomes ("Compras" (193)…) + botão `bg-amber-600` "Mover para Sem categoria". Toast `onSuccess` mostra count + lista; invalida `contarItensPorCategoria` + `listarCategoriasAlmoxarifado` + `listarItens` + `listarItensConsolidado` — cards do Almoxarifado atualizam imediatamente. Zero ALTER/DROP/DELETE; UPDATE escopado por companyId, idempotente. R-001/R-007/R-010 OK.
