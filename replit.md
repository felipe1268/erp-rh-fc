...
- **Rev. 2779** — **EPI · TELA "ESTOQUE POR OBRA": OS CARDS (RESUMO + ALMOXARIFADO CENTRAL + OBRAS) FICARAM FIXOS (STICKY) NO TOPO — SÓ A TABELA DE INSUMOS ABAIXO ROLA; E O CARD CLICADO GANHOU UMA COR DE DESTAQUE DISTINTA (ÂMBAR).** Pedido (usuário, 2 prints): "quero que fique fixo os cards mude somente os insumos abaixo... e o card que estiver clicado deve ficar com uma cor diferente de destaque". Fix (SÓ CLIENT; ZERO SCHEMA/SERVER) em `client/src/pages/Epis.tsx`: card-resumo + grade de cards envolvidos em `sticky top-0 z-20 bg-background border-b` (grudam no topo enquanto a tabela rola); grade ganhou `max-h-[42vh] overflow-y-auto` (cards rolam DENTRO do painel fixo); realce do selecionado mudou de `ring esmeralda/azul` (igual à cor do card) para `ring-2 ring-amber-500 bg-amber-100/70 shadow-md` nos DOIS tipos — âmbar contrasta e deixa ÓBVIO qual está selecionado. ZERO ALTER/DROP/DELETE. Validação: HMR ok; architect. Detalhe: `shared/changelog.ts`.
- **Rev. 2778** — **EPI · OS NÚMEROS DA TELA "ESTOQUE POR OBRA" AGORA SÃO FORMATADOS COM SEPARADOR DE MILHAR (PADRÃO pt-BR) — TOTAL DE UNIDADES E QUANTIDADES POR LOCAL FICAM LEGÍVEIS (ex.: "2.102 unid.").** Pedido (usuário, print): "separe tudo com ponto e vírgula certinho" — após a Rev. 2777 (que corrigiu a concatenação no total), pediu que TODOS os números da tela usem o separador de milhar brasileiro. Fix (SÓ CLIENT; ZERO SCHEMA/SERVER) em `client/src/pages/Epis.tsx`: `.toLocaleString('pt-BR')` em TODOS os contadores inteiros do resumo/cards — total de unidades (`unidTotal`), unidades e tipos do Almoxarifado Central (`unidCentral`/`estoqueCentral.totalItens`), unidades e tipos por obra (`r.totalUnidades`/`r.totalItens`). Valores que vêm do banco como STRING (`SUM()`/`COUNT()`) blindados com `Number(... || 0)` antes do format. Monetários (R$) já formatados, intactos. ZERO ALTER/DROP/DELETE. Validação: HMR ok; architect. Detalhe: `shared/changelog.ts`.
