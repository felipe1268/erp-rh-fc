...
- **Rev. 1798**: **R-013 · Auto-sync de NOMES com o orçamento (sem botão) — no save E ao abrir o Diagnóstico (corrige base existente)**. User: "nome E código devem ser EXATAMENTE idênticos" / "não quero botão, quero que seja automático" / "os já importados foram resolvido tbm?". **(A)** Em `salvarAtividades` (planejamento.ts L969-1052): se projeto tem orçamento, sobrescreve `r.nome` com `orcIt.descricao` (texto EXATO) sempre que houver qualquer diferença; EAP que NÃO existe no orçamento aborta com TRPCError listando até 20 itens. **(B)** Novo proc `autoSincronizarNomesComOrcamento` (planejamento.ts L3685+) + `useEffect` em `DiagnosticoEapOrcCron.tsx` que dispara automaticamente quando `descDiverge > 0`: corrige base já salva no banco, banner verde "N nome(s) sincronizado(s) automaticamente". Tenant-isolated, idempotente. Indiretas/Marcos/Grupos/Disabled isentos.
