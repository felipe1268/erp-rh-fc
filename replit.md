...
- **Rev. 4606** — **UI: EMISSÃO DE CRACHÁS — NOVO MODELO DE ARTE (CLT/PJ/TERCEIROS).** BadgePreview redesenhado conforme arte enviada pelo usuário: frente com cabeçalho na cor do tipo + curva dupla (faixa laranja da marca `ACCENT #F49D1F` + recorte branco), foto circular sobreposta, nome caixa alta + ponto laranja + função entre filetes, linhas de dados com ícone/rótulo/valor sublinhadas (só aparecem se houver dado), QR pequeno + ID e marca d'água de construção; verso com QR emoldurado em laranja, slogan "Grandes obras começam com grandes pessoas." e rodapé na cor do tipo com telefone da empresa (fallback (12) 3133-5504). Cor por tipo segue personalizável; cartão 340×540, download/impressão inalterados. Arquivo: `client/src/pages/terceiros/Crachas.tsx`. ZERO schema/server change.
