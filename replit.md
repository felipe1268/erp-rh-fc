...
- **Rev. 3700** — **CONCILIAÇÃO · CAMPO DE BUSCA NA SEÇÃO "JÁ CONCILIADOS". 100% FRONTEND · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** Seção "Já conciliados" tinha centenas de linhas sem forma de filtrar. Input de busca adicionado logo abaixo do header colapsável (ícone lupa + botão ✕ para limpar). Filtra por descrição, fornecedor/lançamento vinculado, data e valor; normaliza acentos e caixa. Mensagem "Nenhum resultado" quando busca não encontra nada. Filtros Todos/Entrada/Saída funcionam em conjunto. Arquivo: `FinanceiroConciliacao.tsx`. Detalhe: `shared/changelog.ts`.
