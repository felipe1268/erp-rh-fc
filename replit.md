...
- **Rev. 3761** — **FINANCEIRO · DRE — DEMONSTRATIVO DE RESULTADO · CADA LINHA DO DRE FICOU CLICÁVEL (DRILL-DOWN): AO CLICAR ABRE UM DIÁLOGO COM OS VALORES QUE COMPÕEM A LINHA — LINHAS-FOLHA (RECEITA BRUTA, RECEITAS FINANCEIRAS, CUSTOS DE OBRA, DESPESAS FIXAS/VARIÁVEIS, DESPESAS FINANCEIRAS, IMPOSTOS) MOSTRAM OS LANÇAMENTOS REAIS AGRUPADOS POR CATEGORIA + LISTA; LINHAS-TOTAL (RECEITA LÍQUIDA, LUCRO BRUTO, EBITDA, RESULTADO FINANCEIRO, LAIR, LUCRO LÍQUIDO) MOSTRAM A FÓRMULA; MARGENS MOSTRAM A DIVISÃO. O TOTAL DO DIÁLOGO SEMPRE FECHA COM A LINHA. BACKEND READ-ONLY · ZERO SCHEMA/ALTER/DROP/DELETE.** Pedido do usuário (tela "DRE"): "poder clicar e ver os valores pertinentes". Fonte única p/ garantir paridade: extraí os 7 predicados SQL p/ `dreLinhaPredicate(linha)` e REFATOREI `calcularDRE` p/ usá-lo nos 7 `FILTER(...)`; nova `calcularDRELinhaDetalhe(companyId,periodo,tipoPeriodo,linha)` reusa a MESMA CTE base + retorna `{total,qtdTotal,porConta[],itens[] top 1000,itensTruncados}` (linha `impostos` mescla `origem='guia_tributaria'` + `financial_tax_obligations`, espelhando `calcularDRE`). tRPC `getDRELinhaDetalhe` (mesmo `_assertFinanceiroCompanyAccess` do `getDRE`). Front: `DRERow.drill` (leaf/composicao/ratio/info), linhas clicáveis (cursor/hover/chevron/teclado, `Info` faz stopPropagation), `DrillBody` renderiza cada tipo com skeleton/erro/empty e `break-words`. `tsc` limpo (filtrado); HTTP 200. Arquivos: `client/src/pages/financeiro/FinanceiroDRE.tsx`, `server/services/financialKpiService.ts`, `server/routers/financial.ts`. Detalhe: `shared/changelog.ts`.
