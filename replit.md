...
- **Rev. 2151** — **UX POLISH · Dialog "Novo Termo de Recebimento" repaginado com a identidade FC (Regra de Ouro).** User: "Melhore o layout conforme nossas regras de ouro". Modal estava com estilo padrão shadcn (cinza, lista chapada), destoando do resto do app. **Mudanças em `client/src/components/controleDocumentos/TermosResponsabilidadePanel.tsx` (só o dialog ~L419-510):** (1) `DialogContent` com `p-0 gap-0 overflow-hidden` p/ faixa full-width; (2) faixa azul `#1B2A4A` border-b-2 white + `printColorAdjust:exact` inline + título uppercase `letterSpacing 3px` (Regra de Ouro) + ícone Plus + subtítulo `indigo-100`; (3) corpo `bg-slate-50` com input `bg-white` + linha de meta "COLABORADORES ATIVOS" + badge contador; (4) lista com avatar circular 40px (iniciais em fundo #1B2A4A), nome semibold (→indigo-900 hover), meta CPF mono + função uppercase, chevron com translate-x e cor indigo no hover, bg `indigo-50/60`; (5) empty state com ícone; (6) footer `bg-white border-t`. **Zero mudanças funcionais. Backend:** zero. **R-001/R-007/R-010:** OK.
...
- ~~Rev. 2147~~ — HOTFIX UX · Tabs de Controle de Documentos com grid responsivo `grid-cols-2 sm:3 md:5 lg:9` (depois refinado na Rev. 2148 trocando `lg:` por `xl:`). Ver `shared/changelog.ts`.
