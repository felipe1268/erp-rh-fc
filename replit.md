...
- **Rev. 2610** — **CADASTRO · NOVA PÁGINA/MENU "GERENCIADORAS" — O CADASTRO DE GERENCIADORAS (NOME + LOGO + CNPJ/TELEFONE/E-MAIL) DEIXA DE EXISTIR SÓ COMO MINI-MODAL DENTRO DO FORM DE OBRA E GANHA UMA PÁGINA DEDICADA NO MENU LATERAL "CADASTRO", LOGO ABAIXO DE "CLIENTES".** Pedido (usuário, no Canvas, sobre o menu CADASTRO): "coloca aqui tbm o campo para cadastro da gerenciadora". A tabela `gerenciadoras` e o router tRPC (list/criar/atualizar/excluir) já existiam desde a Rev. 2606, mas só pelo combobox "+ Cadastrar" dentro de "Nova Obra" — não havia tela central. Implementação (SÓ CLIENT; ZERO SERVER/SCHEMA/ALTER/DROP/DELETE — reaproveita o router): `client/src/pages/Gerenciadoras.tsx` (NOVO, mesmo padrão de `Clientes.tsx`: busca, contador "Total", grid de cards com logo/CNPJ/telefone/e-mail, botões Editar/Excluir já com visibilidade no touch, modal com upload de logo via FileReader→dataURL máx. 2MB); `client/src/App.tsx` (lazy import + rota `/gerenciadoras`, RouteGuard `route="/empresas"`); `client/src/components/DashboardLayout.tsx` (item de menu "Gerenciadoras", ícone `Network`, na seção "Cadastro" abaixo de "Clientes"). Validado: esbuild client (exit 0) + workflow reiniciado. Detalhe: `shared/changelog.ts`.
