...
- **Rev. 3203** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · CORREÇÃO DE BUG CRÍTICO: A TELA MOSTRAVA "NÃO FOI POSSÍVEL CARREGAR A CONCILIAÇÃO · DB code=42703 | column e.comprovante_beneficiario does not exist". AS COLUNAS `comprovante_*` (Rev. 3193) NUNCA FORAM GARANTIDAS POR SELF-HEAL, ENTÃO O BANCO DE PRODUÇÃO FICOU SEM ELAS E TODA A CONCILIAÇÃO QUEBRAVA.** PEDIDO (piloto FC): "fiz a importação porém os dados não estão aparecendo mais pq?" + print do erro vermelho. DIAGNÓSTICO: a Rev. 3193 declarou 6 colunas (`comprovante_beneficiario/documento/txid/valor/data/extraido_em`) em `drizzle/schema.ts` dizendo "via self-heal", MAS o `ALTER TABLE financial_entries ADD COLUMN IF NOT EXISTS …` nunca foi escrito; em DBs evoluídos por self-heal aditivo (FC) as colunas ficaram faltando, e `getConciliacaoReport`/`sugerirConciliacao` (que fazem `SELECT e.comprovante_beneficiario …`) estouravam 42703. (O sumiço de dados da "Caixa Guaratinguetá" era SEPARADO: as 121 linhas foram apagadas via "Limpar extrato" — soft-delete; a importação é aditiva e não apaga.) SOLUÇÃO (`server/_core/index.ts`): bloco self-heal Rev. 3203 garante as 6 colunas via `ADD COLUMN IF NOT EXISTS` (TEXT/VARCHAR(20)/VARCHAR(140)/NUMERIC(15,2)/DATE/TIMESTAMP); colunas também aplicadas na hora no banco da FC. Pós-fix as queries do report rodam OK (Santander 216 linhas; Guaratinguetá 0 pois foi limpa). ZERO ALTER/DROP/DELETE destrutivo (só ADD COLUMN IF NOT EXISTS). Detalhe: `shared/changelog.ts`.
