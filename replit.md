...
- **Rev. 1795**: **Compras · fix DEFINITIVO race condition de SC — extensão do advisory lock para EPIs e Frotas**. Bug do user (toast '3 tentativas — uq_compras_solicitacoes_numero' MESMO depois da Rev. 1790). Causa raiz: a Rev. 1790 colocou `pg_advisory_xact_lock` em `criarSolicitacao`/`duplicarSolicitacao` de `compras.ts`, MAS havia 3 outros pontos inserindo direto em `compras_solicitacoes` SEM lock e usando `COUNT(*)+1` (subestima quando há SCs deletadas + race): `epis.ts:501` (reposição auto), `epis.ts:1945` (gerar SC EPIs em lote), `frotas.ts:4520` (manutenção→SC). Quando uma SC manual rodava em paralelo com qualquer dessas, ambas geravam o mesmo `numero_sc` e a manual estourava `uq_compras_solicitacoes_numero` mesmo com seu lock funcionando. Fix: `lockEGerarNumeroSc` exportado de `compras.ts`; os 3 pontos passaram a envolver o INSERT em `db.transaction(async tx => { numeroSc = await lockEGerarNumeroSc(tx, companyId); tx.insert(...) })`. Mesmo lock = serialização global por (empresa, ano).
