...
- **Rev. 2551** — **RH & DP · NOVA FEATURE "CONVENÇÃO COLETIVA (IA)".** Aba nova (seção Inteligência Artificial) onde o RH sobe o PDF da CCT/circular do sindicato → a IA lê e extrai TODAS as mudanças (% reajuste, piso, VA/VR/VT, cesta/café, seguro de vida, adicionais, contribuição assistencial, data-base, sindicato, nº CCT, vigência) → gera um relatório/diff REVISÁVEL → simulação por funcionário (desmarcando pessoas/campos) → aplicação em massa em 1 clique, com auditoria por funcionário/campo, status (analisado→aplicado), sem reaplicar e histórico por ano. IA: helper `extrairCctComIA` (`invokeAnthropicVision` com PDF base64 → fallback `invokeGeminiVision`; SEM fallback silencioso — falha das duas grava status 'erro'). SALÁRIO aplicado pelo MOTOR DE DISSÍDIO existente (cria/reusa `dissidios` do ano + auditoria em `dissidio_funcionarios`, respeitando Art. 468 não-regressão); BENEFÍCIOS pelas colunas de `employees` (va→vaValor, vr→vrBeneficio, vt→vtValorDiario, seguroVida→seguroVida, auxFarmacia→auxFarmaciaValor). Auditoria em `convencao_analise_itens`. Guardas: `aplicar`/`excluir` só `admin_master`; análise 'aplicado' é imutável; efetivo = Ativo & não-PJ. SCHEMA: 2 tabelas novas (`convencao_analises` + `convencao_analise_itens`) criadas via SyncSchema+ `CREATE TABLE IF NOT EXISTS` (`server/_core/index.ts`) — ZERO ALTER/DROP/DELETE. Backend `server/routers/convencaoIA.ts` (namespace `trpc.convencaoIA`, distinto do CRUD legado `trpc.sprint1.convencao`); client `ConvencaoColetivaIA.tsx`; wiring App/DashboardLayout/ModuleContext/modules/modulePages. Detalhe: `shared/changelog.ts`.
- **Rev. 2550** — **ALMOXARIFADO · INVENTÁRIO SEMANAL · REMOÇÃO DA BUSCA/SCANNER DE CÓDIGO DE BARRAS — "VOLTA COMO ESTAVA" (PRÉ-REV. 2530).** User: "cancela a opção de código de barras para o módulo de inventário, volta como estava — quero analisar esta questão melhor depois." Removida por completo a caixa de busca/leitor (scanner) introduzida na Rev. 2530 (a Rev. 2549 já tinha tirado só a parte de código de barras). FIX (client puro, `Inventario.tsx`): (1) UI — removido todo o bloco `{session.status === "em_andamento" && (...)}` da caixa de busca (input com `ScanLine`, botão limpar com `X`, legenda de contagem); a tela volta a listar pendentes/finalizados sem filtro. (2) ESTADO — removidos `[busca,setBusca]` e `buscaRef`. (3) LÓGICA — `pendentes`/`finalizados` voltam a derivar direto de `sessionItems.filter(...)` (sem `pendentesAll`/`finalizadosAll`, `norm`, `filterFn`, `qNorm` nem os `useMemo`). (4) IMPORTS — limpos órfãos: `useMemo`/`useRef` (react), `Search`/`ScanLine`/`X` (lucide). Rules of Hooks OK: early return de `loadingSession` segue depois de todos os hooks. Zero schema. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
