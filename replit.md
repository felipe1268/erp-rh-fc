...
- **Rev. 3709** — **SEFAZ NF-e · BACKOFF PROGRESSIVO PARA cStat=656 CONSECUTIVOS (1h→2h→4h). BACKEND PONTUAL + FRONTEND · ZERO SCHEMA/ALTER/DROP/DELETE.** Após duplo disparo de 24/06 (pré-Rev.3707), o SEFAZ bloqueou o CNPJ. O sistema tentava a cada 1h e cada tentativa retornava 656 — podendo prolongar o bloqueio. Fix: `consecutiveRlCount` lido de `last_sync_result.rateLimitConsecutive`; multiplicador 1× (1 RL) → 2× (2–3 RL) → 4× (4+ RL) aplicado em AMBOS os gates (check CNPJ + UPDATE atômico). `resultPayload` salva `rateLimitConsecutive` (incrementa em 656, zera em sucesso). Frontend: cronômetro e anel de progresso multiplicados pelo mesmo fator; banner mostra "⚠️ SEFAZ bloqueou N× seguidas — backoff Mx". Arquivos: `server/routers/sefaz.ts`, `FinanceiroNotasFiscais.tsx`. Detalhe: `shared/changelog.ts`.
