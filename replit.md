...
- **Rev. 3021** — **FINANCEIRO → "ANÁLISE DE CUSTOS": OS RÓTULOS DAS BARRAS ("CUSTO POR CATEGORIA" E "CUSTO POR CENTRO DE CUSTO") DEIXAM DE USAR O FORMATO COMPACTO (`R$ 2,0 mi` / `R$ 770 mil`) E PASSAM A MOSTRAR O VALOR EM REAIS POR EXTENSO (`R$ 2.000.000,00`).** PEDIDO (com print): "Só quero valores em reais." Nas barras os valores apareciam compactos via `BRLk`; o usuário quer o número completo pt-BR, igual aos KPIs (Rev. 3018). SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE, ZERO backend/schema, ZERO mudança de dado; 100% client-side — só a formatação dos rótulos muda) em `FinanceiroAnaliseCustos.tsx`: os dois `<LabelList>` dos gráficos de barras (Categoria + Centro de Custo) passam de `formatter={BRLk}` p/ `formatter={formatBRL}` (número completo pt-BR); pra caber à direita da barra sem cortar, a `margin.right` dos dois `<BarChart>` subiu de 78 p/ 118 px. Os EIXOS X seguem em `BRLk` DE PROPÓSITO (número completo no eixo poluiria as marcações — mesma decisão da Rev. 3018). REPUBLICAR (só front). Detalhe: `shared/changelog.ts`.
- **Rev. 3020** — **FINANCEIRO → "ANÁLISE DE CUSTOS": O GRÁFICO "CUSTO POR CATEGORIA" DEIXA DE SER PIZZA/ROSCA E VIRA BARRAS HORIZONTAIS COM O VALOR (R$) INDICADO EM CADA BARRA.** PEDIDO (com print da rosca): "Mude o gráfico para outro modelo, está confuso, acho que o barras seria melhor com indicadores." A rosca (donut) com 9 fatias + legenda dificultava comparar categorias e não mostrava o valor direto no gráfico. SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE, ZERO backend/schema, ZERO mudança de dado; 100% client-side — só a renderização muda) em `FinanceiroAnaliseCustos.tsx`: o `useMemo` `pizzaCategoria` (top 8 + "Outros") virou `barCategoria` = `porCategoria.slice(0,12)` (sem balde "Outros"); o `<PieChart>/<Pie>` foi trocado por `<BarChart layout="vertical">` no MESMO padrão do gráfico "Custo por Centro de Custo" ao lado (altura dinâmica ~46px/barra, eixo em `BRLk`, `YAxis` truncado a 22 chars, cor por categoria via `<Cell>`+`PIE_COLORS`, `<LabelList position="right" formatter={BRLk}>` mostrando o VALOR à direita). Clique mantido (`irParaDetalhe("categoria",nome)`); título de `PieIcon`→`BarChart2`; imports `PieChart/Pie/PieIcon` removidos (`Legend`/`Cell` seguem em uso). Nenhum cálculo/dado mudou — só a visualização do mesmo `porCategoria`. REPUBLICAR (só front). Detalhe: `shared/changelog.ts`.
