...
- **Rev. 3249** — **FINANCEIRO / CONTROLE DE CARTÃO DE CRÉDITO · O DIÁLOGO "NOVO CARTÃO / EDITAR CARTÃO" GANHOU UM LAYOUT MODERNO (CABEÇALHO AZUL COM ÍCONE, SEÇÕES "IDENTIFICAÇÃO" E "DATAS & LIMITE", RODAPÉ DESTACADO) E O CAMPO "LIMITE" PASSOU A USAR MÁSCARA DE MOEDA BRL AUTOMÁTICA — DIGITANDO OS NÚMEROS, O VALOR JÁ APARECE "R$ 10.000,00" (PONTO P/ MILHAR, VÍRGULA P/ CENTAVOS), FACILITANDO O LANÇAMENTO.** PEDIDO (piloto FC): "quero um layout moderno e todos os valores separados por ponto e vírgula para facilitar o lançamento". SOLUÇÃO (FRONT, `client/src/pages/financeiro/FinanceiroCartaoCredito.tsx`): novos helpers locais `maskBRL`/`parseMaskBRL` (motor "digita centavos → 1.234,56" já usado em Contas a Receber) — `onChange` do Limite passa por `maskBRL`, `salvarCartao` converte com `parseMaskBRL(...) || null`, `abrirEditarCartao` formata o valor do banco via `maskBRL(String(Math.round(Number(c.limite)*100)))`; diálogo redesenhado (`DialogContent` sem padding, faixa azul `#1B2A4A` + ícone, corpo seccionado com rótulos uppercase, input do Limite com prefixo "R$" embutido + `tabular-nums`/`inputMode`, rodapé destacado). ZERO BACKEND · ZERO SCHEMA/ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.

- **Rev. 3248** — **FINANCEIRO / DASHBOARDS · OS 5 PAINÉIS VISUAIS (RECEBER, PAGAR, CONCILIAÇÃO, CHEQUES, CARTÃO) FORAM ENRIQUECIDOS: (1) TUDO EM BRL — A CONCILIAÇÃO, QUE SÓ MOSTRAVA CONTAGEM DE LINHAS, AGORA EXIBE VALOR MOVIMENTADO/CONCILIADO EM REAIS; (2) CLICAR EM QUALQUER GRÁFICO/KPI ABRE UM DIÁLOGO (DRILL-DOWN) COM TODOS OS LANÇAMENTOS PERTINENTES NUMA TABELA MAXIMIZÁVEL; (3) MAIS KPIs/GRÁFICOS (TICKET MÉDIO, AGING, CENTRO DE CUSTO, FATURADO×COMPRAS); (4) NOVA TABELA COMPARATIVA EM TODOS — MÊS A MÊS E ANO A ANO COM SETAS ↑/↓ E % (VERDE/VERMELHO CONFORME RECEITA OU CUSTO). 100% READ-ONLY.** PEDIDO (piloto FC): "melhore os 5 dashboards: tudo em BRL; clicar no gráfico abre diálogo com todos os dados; mais KPIs/gráficos; tabela comparativa mês a mês e ano a ano (subiu/desceu)". SOLUÇÃO (FRONT, kit `client/src/pages/financeiro/dashboards/_kit.tsx`): novos `formatPct`/`variacaoPct`, `DeltaBadge({curr,prev,goodWhen})` (seta+% colorido; up=receita/down=custo), `DetailDialog` (tabela genérica `DetailColumn` com `brl`/`format` + total + "Abrir tela", usa DialogContent maximizável da Rev. 3237), `ComparativoAnual` (12 meses: atual|anterior|Δ a/a|Δ m/m + KPIs + total, drill por mês via `onOpenMes`). Cada painel ganhou 2ª query do ano-1, cliques (barra→`activeLabel`, fatia→`payload`, KPI) abrindo o `DetailDialog` filtrado, e KPIs/gráficos extras — tudo `useMemo` client-side. BACK (`server/routers/financial.ts`, READ-ONLY): `getBankAccountsConciliacaoStatus` agora retorna `valorTotal`/`valorConciliado` (SUM(ABS(valor))) + NOVA `getConciliacaoResumoMensal` ({companyId,ano}) p/ o comparativo mensal; ambas com tenant guard. ZERO SCHEMA/ALTER/DROP/DELETE · ZERO mutação. Detalhe: `shared/changelog.ts`.
