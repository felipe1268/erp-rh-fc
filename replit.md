...
- **Rev. 3087** — **MEDIÇÃO DE TERCEIROS · O PAINEL/MENU FICA FIXO NO MÓDULO: AO CLICAR "MEDIR" (OU ABRIR O DETALHE DO CONTRATO) DENTRO DO MÓDULO "MEDIÇÃO TERCEIROS", A BARRA LATERAL DEIXA DE TROCAR PARA O MÓDULO "TERCEIROS".** PEDIDO (prints iPad): no módulo "Medição Terceiros" (`/terceiros/medicoes`), clicar em "Medir" abria o detalhe do contrato (`/terceiros/contratos/:id`) e o menu lateral trocava para "Terceiros"; o usuário quer o painel fixo no módulo de medições. CAUSA-RAIZ: `ModuleContext.tsx` resolve o módulo ativo só pela rota (`ROUTE_MODULE_MAP` + prefixo); `/terceiros/contratos` → "terceiros", forçando a troca. A tela de detalhe é AMBÍGUA (pertence aos 2 módulos). SOLUÇÃO (FRONTEND-ONLY, ZERO ALTER/DROP/DELETE/SCHEMA/BACKEND): novo conceito de rota AMBÍGUA "sticky" no `useEffect` — `STICKY_AMBIGUOUS = [{ prefix: "/terceiros/contratos", keepIf: ["medicao-terceiros"] }]`; se a rota casa o prefixo E o `activeModule` está em `keepIf`, o efeito retorna ANTES de trocar o módulo. Fluxo pelo módulo "Terceiros" intacto. Detalhe: `shared/changelog.ts`.
