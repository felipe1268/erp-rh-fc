...
- **Rev. 3672** — **NFS-e EMITIDAS · BUGFIX PARSER SIAP GEO 0 NOTAS — PATH .NET ASMX + CDATA INNER XML + LOG DIAGNÓSTICO. BACKEND PONTUAL · ZERO ALTER/DROP/DELETE.** `parseAbrasrResponse` só conhecia paths ABRASF direto; SIAP GEO (.NET ASMX) retorna `ConsultarNfseResponse.ConsultarNfseResult`. Fix: 3 camadas + log diagnóstico body keys. Arquivo: `server/routers/nfseEmitidas.ts`. Detalhe: `shared/changelog.ts`.
