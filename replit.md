...
- **Rev. 4616** — **FEAT: DOSSIÊ — SAI ADVERTÊNCIA (INTERNA), ENTRAM DOCUMENTOS PESSOAIS.** Dossiê = pacote COMPLETO de integração pra indústria/cliente: ASO + Treinamentos + Integrações + Documentos pessoais (employee_documents: RG, CNH, CTPS, PIS, foto 3x4, diploma etc.). Documentos internos (advertência, atestado, contrato/rescisão) FORA — exclusão feita no servidor (painel e ZIP), não só na UI. `painelDossie` troca advertências por `documentos`; DossiePanel: coluna "Documentos" (chip azul) + bloco expandido com tipo/nome/link; ZIP ganha pasta Documentos (TipoLabel_Nome.ext). Arquivos: `server/routers/controleDocumentos.ts`, `server/routers/downloadDossie.ts`, `client/src/pages/ControleDocumentos.tsx`. ZERO schema change.
- **Rev. 4615** — **FEAT: DOSSIÊ — SAI ATESTADO (INTERNO), ENTRA INTEGRAÇÃO (TELA + ZIP).** Pedido do usuário: dossiê é o pacote de INTEGRAÇÃO que vai pro cliente (ASO, treinamentos, integrações); atestado é interno e não deve nem aparecer nem ir no ZIP. `painelDossie` troca atestados por `integracoes` (employee_integrations, companyFilter) + `integracaoVigente`; pendências ganham "Sem integração"/"Integração vencida" (entra no filtro ❌). DossiePanel: coluna "Integração" com chip ✓ N / ❌ Vencida / — Sem + bloco expandido com cliente, evidência e vigência. ZIP: pasta Atestados removida; pasta Integracoes com evidências (só `/uploads/` interno — anti-SSRF). Arquivos: `server/routers/controleDocumentos.ts`, `server/routers/downloadDossie.ts`, `client/src/pages/ControleDocumentos.tsx`. ZERO schema change.
### 5 one-liners
...
- **Rev. 4612** — **UI: EMISSÃO DE CRACHÁS — FRENTE COM MODO COMPACTO (TUDO CABE NOS 540px).** 3 níveis proporcionais (normal/compact/denso) + bloco de linhas `flex-1 + justify-evenly`: impossível estourar em qualquer combinação; vale p/ o PNG baixado. Detalhe em `shared/changelog.ts`. ZERO schema/server change.
...
Ver `replit-history.md` para revisões Rev. 4608 e anteriores.
