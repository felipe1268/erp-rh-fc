...
- **Rev. 2765** — **PLANEJAMENTO · "% PREVISTO": A CURVA AGORA É CONGELADA NO CADASTRO DO CRONOGRAMA E O UPLOAD SEMANAL (AVANÇO) NUNCA MAIS A REGENERA — ELIMINA A DERIVA DE ±1% EM RELAÇÃO AO MS PROJECT EM SEMANAS AVANÇADAS.** Sintoma (Felipe): o "% Previsto" do ERP divergia ±1% do MSP e a diferença CRESCIA nas semanas avançadas. Decisão do usuário: aba CRONOGRAMA = cadastro inicial (lê a previsão semana-a-semana da baseline FIXA do XML); aba AVANÇO SEMANAL = SÓ % Concluída (realizado), NUNCA toca o previsto. Causa-raiz (SÓ SERVER): a curva (`previsto_semanas_json`, Caminho B) era gerada no cadastro, MAS desde a Rev. 2646 `salvarMetadadosMSProject` a REGENERAVA em TODO upload com `calendarioJson` — inclusive o SEMANAL — re-rodando o motor com o calendário daquela semana → oscilação ±1%. Investigação do XML (`_-_CADASTRO_`/`_-_BASE_LINE_` + semanais 00–05): NÃO existe coluna literal "previsto semana a semana"; o MSP guarda só a baseline (`<Baseline Number=0>` com hora) + calendário + Texto10 único na raiz → a curva por semana é derivada do motor, logo precisa ser gerada 1x (cadastro) e CONGELADA. Fix (SERVER+CLIENT; ZERO SCHEMA — R-001/R-007/R-010): `salvarMetadadosMSProject` ganhou input `origem: "cadastro"|"avanco"` (default cadastro); em `"avanco"` NÃO regenera o previsto (gate `origem !== "avanco"`) E mescla o `calendarioJson` preservando do cadastro `previstoMspSnapshot`+calendário (só atualiza realizado/StatusDate). `importarDoMSProject` (aba Avanço) passa `origem:"avanco"`; `gravarMetadadosMSP` (cadastro) passa `origem:"cadastro"`. O cliente já lia a curva congelada (`previstoCurva.raizAt`) como fonte primária. Validação: servidor sobe limpo (tsx, zero erro de compilação); architect. Detalhe: `shared/changelog.ts`.
- **Rev. 2764** — **RH/DP · RECONTRATAÇÃO (FILA DE APROVAÇÃO): OS MODAIS "LIBERAR RECONTRATAÇÃO" E "RECUSAR RECONTRATAÇÃO" FORAM MODERNIZADOS E PERDERAM A BARRA DE ROLAGEM HORIZONTAL — CABEÇALHO EM FAIXA COLORIDA, CARD DE COLABORADOR LIMPO, ALERTA DE EXPERIÊNCIA EM CAIXA E RODAPÉ FIXO.** Pedido (Felipe, print): o modal "Liberar recontratação" (página `RecontratacoesPendentes`) aparecia com barra de rolagem horizontal e layout pouco amigável; queria algo moderno e sem a barra. Fix (SÓ CLIENT/UI; ZERO SCHEMA/SERVER — R-001/R-007/R-010) em `client/src/pages/RecontratacoesPendentes.tsx` (os DOIS modais — "Liberar" e "Recusar"): `DialogContent` ganhou `resizable={false}` (remove handles de resize que induziam overflow) + `p-0 gap-0 overflow-x-hidden w-[calc(100vw-2rem)] max-w-md`; cabeçalho virou FAIXA com gradiente (emerald→lime no Liberar, red→rose no Recusar) com ícone em chip; corpo com card do colaborador (`break-words`), aviso "sem experiência" virou caixa âmbar com ícone (antes concatenado na descrição); rodapé fixo `border-t bg-muted/30`. Sem mudança de lógica (states/mutations/validações intactos). Validação: `tsc --noEmit` limpo; client-only. Detalhe: `shared/changelog.ts`.
