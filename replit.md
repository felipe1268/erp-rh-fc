...
- **Rev. 3236** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · DEMONSTRATIVOS DE PAGAMENTO · OS SLOTS "COMPROVANTES DE PIX" E "COMPROVANTES DE BOLETOS" AGORA ACEITAM VÁRIOS PDFs DE UMA VEZ (ANTES ERA 1 PDF POR TIPO QUE SUBSTITUÍA O ANTERIOR) — DÁ PRA SUBIR TODOS OS COMPROVANTES DO MÊS NUMA TACADA. O FLUXO "ANEXAR → LER COM IA" GANHOU UMA BARRA DE PROGRESSO REAL (0→100%) COM RÓTULO DETALHADO ("ENVIANDO ARQUIVO 2 DE 5", "LENDO COM IA ARQUIVO 3 DE 7", "SALVANDO LEITURA"), NO LUGAR DA BARRA ASSINTÓTICA FALSA. CADA ARQUIVO APARECE NA LISTA COM "ABRIR"/"REMOVER" INDIVIDUAIS. TUDO READ-ONLY.** PEDIDO (piloto FC): "deixar anexar vários PDFs de uma vez (não só um) na parte de Demonstrativos de pagamento da Conciliação e mostrar uma barra de progresso de verdade, em %, conforme processa". CAUSA-RAIZ: o slot guardava só 1 URL/nome por tipo (`pix_url`/`boleto_url`) e cada upload SUBSTITUÍA o anterior; a leitura por IA era um `lerDemonstrativoComIA` single-shot (sem progresso real) → barra assintótica/fake até 92%. SOLUÇÃO BACK (`financial.ts`): novas colunas-array `pix_arquivos_json`/`boleto_arquivos_json` (TEXT JSON `[{url,nome}]`); helper `_carregarDemoArquivos` com FALLBACK p/ a coluna legacy; `getConciliacaoDemonstrativos` devolve `pixArquivos`/`boletoArquivos`; `salvarConciliacaoDemonstrativo` aceita `arquivos[]` (APPEND+dedup por url, espelha 1º no legacy, zera extração); `removerConciliacaoDemonstrativo` ganhou `indice?` (remove 1 ou tudo); novas procedures `lerDemonstrativoArquivoIA` (lê 1 por índice, SEM gravar — url do DB=SSRF-safe) e `salvarDemonstrativoExtraido` (persiste lista combinada re-sanitizada). SELF-HEAL (`_core/index.ts`): `ADD COLUMN IF NOT EXISTS` das 2 colunas TEXT. FRONT (`FinanceiroConciliacao.tsx`): input `multiple`; `onDemoFile` faz upload em loop (1→44%) → salvar (append) → `lerDemoIA` por arquivo (46→100%); novo estado `demoProg{kind,pct,label}` substitui `demoBusy`/`lerBusy`/`lerProgress`; `_sleep(300)` de pacing (free-tier Gemini); UI lista todos os arquivos (Abrir/Remover por índice) + "Anexar mais PDFs" + "Ler/Reler com IA" + barra real. ZERO conciliação/baixa automática · ZERO SCHEMA destrutivo. Detalhe: `shared/changelog.ts`.
