...
- **Rev. 2605** — **PLANEJAMENTO · REFIS · O "PREVISTO ACUMULADO" DO RELATÓRIO PASSA A LER A MESMA CURVA CAMINHO B DA BARRA "AVANÇO FÍSICO" DO TOPO — ANTES RECALCULAVA O PV NO CLIENT E DIVERGIA (REFIS 3,13% vs TOPO 3,00% NO PROJETO 35 REVTE-CIVIL), GERANDO DUPLA INFORMAÇÃO PRA O ENGENHEIRO.** Pedido (usuário): "os valores do refis não estão padronizados com a barra do topo… garanta que todas as informações leem a mesma coisa, sem dupla informação". Diagnóstico: o REALIZADO já espelhava o topo (Rev. 2272 via prop `avancoAtual`); o PREVISTO não — o useMemo `avancoPrevisto` recalculava `pctRaizMSP(semanaFimRefis, proj.dataInicio, proj.dataTerminoContratual)` sobre o ENVELOPE CONTRATUAL, enquanto o topo (`avancoPrevistoDia`) LÊ a curva CAMINHO B (`previsto_semanas_json`), gerada sobre a BASELINE (min BL_Start/max BL_Finish, Rev. 2603). A data já era a mesma (`topRefStr`, Rev. 2248); só a FÓRMULA divergia. FIX (SÓ CLIENT — `PlanejamentoDetalhe.tsx`; ZERO SERVER/SCHEMA/ALTER/DROP/DELETE): a prop `previstoCurva` passa a ser propagada ao `<Refis>`; `avancoPrevisto` e `avancoPrevAntes` leem `previstoCurva.raizAt(...)` primeiro (clamp 0–100), com `pctRaizMSP`/ponderado como FALLBACK só sem curva. REFIS = topo = curva = MSP. Modo "comIndiretas" intocado. Validado: esbuild client (exit 0) + workflow reiniciado. Detalhe: `shared/changelog.ts`.
- **Rev. 2604** — **PLANEJAMENTO · AVANÇO SEMANAL · O REALIZADO IMPORTADO DO MS PROJECT PASSA A LER SOMENTE A COLUNA "% CONCLUÍDA" (PercentComplete). ANTES O IMPORT PRIORIZAVA "%REALI AUX" (Texto7) E "DURAÇÃO REAL" (AD/(AD+RD)), GRAVANDO VALORES FRACIONADOS (EX.: 48,89% / 9,97% / 22,51%) DIVERGENTES DO QUE O ENGENHEIRO CADASTRA NO PROJECT.** Sintoma (usuário, projeto 35 REVTE-CIVIL): "esses não foram os valores que cadastramos". Diagnóstico (Neon, READ-ONLY): avanços com casas quebradas (48,8889 / 9,9715 / 22,5071) — impossíveis pra "% Concluída" (INTEIRA no MSP) → vinham do Texto7/Duração Real, contrariando a REGRA DE OURO (Rev. 2533+: realizado = PercentComplete). FIX (SÓ CLIENT — `PlanejamentoDetalhe.tsx` `importarDoMSProject`; ZERO SERVER/SCHEMA/ALTER/DROP/DELETE): removida a cascata Texto7→ActualDuration→PercentComplete; lê EXCLUSIVAMENTE `PercentComplete` (clamp 0–100). Removidas `srcTexto7`/`srcDurNat` e o helper `parseDurMin`; breakdown do toast vira "fonte: N %Concluído". Dados legados (projeto 35 etc.) só corrigem ao REIMPORTAR cada XML semanal (ERP não recalcula retroativo). Validado: esbuild client (exit 0) + workflow reiniciado. Detalhe: `shared/changelog.ts`.
