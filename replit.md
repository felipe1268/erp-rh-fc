...
- **Rev. 1657**: **Controle de Documentos → Integrações: filtro por função + badge de função na linha**. Adicionado `Select` "Todas as funções" ao lado do filtro de clientes (popula dinamicamente com as funções presentes nas integrações listadas, ordenadas pt-BR). Coluna Colaborador agora exibe a função do funcionário como badge cinza ao lado do nome (mesma fonte `i.funcao` já retornada pela API `integracoes.listar`). Search global passou a casar também por função. `client/src/pages/ControleDocumentos.tsx` ~L62-138 (state+filtragem) e ~L177-183 (Select) e ~L242-254 (badge na linha).
- **Rev. 1656.4**: **Aderência alinhada ao SPI clássico (EV/PV acumulados)**. O card "Aderência" da sub-linha da Semana usava DELTA semanal (`real/prev` = PMBOK SV), enquanto a Variação usava acumulado — gerava contradição direta no MESMO projeto: Var **-0,16% (atrasado/vermelho)** vs Aderência **118% (adiantado/verde)**. Agora `aderencia = (realAcum / previstoAcumulado) × 100` — mesma janela cobrável (cutoff p/ semana corrente, semFim p/ passada/futura) usada pelo top bar (Rev. 1656.1) e card "PREVISTO (SEMANA)". Coerência absoluta: Var < 0 ⇔ Aderência < 100%. `previstoRealizadoSemana` agora também expõe `realizadoAcumulado` (sum de peso × acumAtual). `PlanejamentoDetalhe.tsx` ~L4975 (realAcum) e ~L5018-5044 (Aderência).
- **Rev. 1656.3**: **REFIS — "Previsto Acumulado" alinhado ao top bar via clip de cutoff**. O BLOCO 2 do REFIS (Evolução Física Global → Previsto Acumulado) usava `semanaFimRefis = semana + 7d` SEM clip, divergindo do top bar (Rev. 1656.1) que clipa no cutoff. Em REVTE-CIVIL Sem 1 (04/05-10/05) com cutoff Qui (07/05): top bar 1,41% × REFIS 2,21%. Agora `semanaFimRefis` aplica a MESMA regra: se cutoff in [semana, semFim) → refStr=cutoff; senão → semFim. Refis recebe `dataCorteInfo` como prop. `PlanejamentoDetalhe.tsx` ~L1019 (render Refis) e ~L10763 (semanaFimRefis).
- **Rev. 1656.2**: **Hotfix — `calMSP is not defined` na aba REFIS**. A função `prevIndRef` (definida no Refis ~L10731) usa `calMSP` por closure, mas o componente Refis nunca o declarava — qualquer render que avaliasse `prevIndRef` quebrava com ReferenceError (latente, exposto agora pela navegação entre abas). Adicionado `const calMSP = useMemo(() => parseCalendarioJson(proj?.calendarioJson), [proj])` no topo do Refis, espelhando o padrão do AvancoSemanal (Rev. 1645). `PlanejamentoDetalhe.tsx` ~L10600.
- **Rev. 1656.1**: **Avanço Físico — top card acompanha a semana selecionada COM clipping do cutoff**. Restaura Rev. 1649 sem perder a Opção A do Rev. 1655. Top bar segue a semana selecionada na aba Avanço Semanal aplicando a MESMA regra de clipping do card "PREVISTO (SEMANA)": semana CORRENTE (contém cutoff) → `refStr = cutoff` (PV exigível PMBOK 7ª); semanas passadas/futuras → `refStr = semFim`. Paridade absoluta: Sem 1 (04/05-10/05) com cutoff Qui (07/05) = 1,41% nos dois; Sem 3 futura = 5,56% nos dois. Sem semana selecionada, mantém lock no cutoff oficial (Rev. 1655). EV volta a usar `avancosMapSemana`. `PlanejamentoDetalhe.tsx` ~L490-560.
- **Rev. 1655**: **Avanço Físico — 3 displays do %Previsto convergem no cutoff oficial (Opção A)**. Antes: top card 2,08% (Live + extrapolação semVis+7d Rev.1649), card "PREVISTO (SEMANA)" 1,41% (cutoff), sub-linha "Semana 1 — Previsto: 1,06%" (DELTA pvMacro). 3 valores pro mesmo projeto. Agora: (1) Top card `avancoAtual` E `avancoPrevistoDia` travam `refStr = min(refDateStr, cutoffOficial)` — EV e PV usam o MESMO refStr (badge Avanço/Atraso compara mesmo horizonte). `avancoAtual` agora usa `avancosMap` em vez de `avancosMapSemana` — independe de `semanaVisualizacao`. (2) `previstoRealizadoSemana` expõe `previstoAcumulado` (pvMacro ou fallback linear, ambos clipados no cutoff) — usado na sub-linha; Aderência/débito mantêm DELTA (PMBOK 7ª SV). (3) Fonte do cutoff migrada de `proj.dataCorteAtual` para `dataCorteInfo.dataCorteOficial` (refetched por mutations) em todos os cálculos — preserva responsividade Rev. 1654. (4) Mutations setDiaCorte/consolidarCutoff/fecharSemana invalidam `getProjetoById` (sincroniza `cutoffIso` da Programação Semanal). (5) Backend `fecharSemana` persiste `dataCorteIso = ${novoCorte}T17:00:00` (paridade com setDiaCorte). Resultado: top = card = sub-linha = 1,41% quando cutoff Qui; todas as telas respondem juntas à troca de premissa. `PlanejamentoDetalhe.tsx` ~L488-584 e ~L4970-5070; `server/routers/planejamento.ts` ~L1567-1583.
- **Rev. 1654**: **Premissa de cutoff redefine a leitura oficial agora (Opção A com lock)**. Antes, trocar Qui ↔ Sex ↔ Qua só mudava o "molde" das semanas — a `dataCorteAtual` continuava travada e o PV não respondia. Agora, ao trocar a premissa enquanto NÃO consolidado, recalcula `dataCorteAtual = ultimoDiaSemanaAte(today, novoDia)`. Qua = 3 du = ~1,06%; Qui = 4 du = 1,41%; Sex = 5 du = ~1,76%. Após **Consolidar**, lock one-way preserva auditoria. `setDiaCorte` (`server/routers/planejamento.ts` ~L1478-1497) atualiza 4 colunas + audit log com ambas as mudanças.
- **Rev. 1653**: **Programação Semanal — preserva a navegação do usuário entre re-renders**. Antes, qualquer mudança em `semanas` (cutoff, atividades, query reanimada) jogava o usuário de volta pra Semana Atual — quebrava simulação de semana futura/passada. Agora: F5 → Semana Atual (default); clique → memoriza ini ISO em `userSelectedIniRef`; `semanas` muda → busca essa semana no novo array; se sumiu, fallback pro currentWeekIdx. `ProgramacaoSemanal.tsx` ~L389-419.
- **Rev. 1652**: **Confirmações do cutoff usam modal interno**. Os 3 popups do header do Planejamento (Mudar dia, Consolidar, Fechar semana) usavam `window.confirm` — que renderizava "xxx.replit.dev diz" no topo (poluído, dificultava leitura). Substituídos por `AlertDialog` shadcn via helper `askConfirm({ title, description, onConfirm })`. Demais `confirm()` do arquivo (recalcular, desconsolidar, IA) preservados.
