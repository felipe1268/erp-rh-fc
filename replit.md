...
- **Rev. 2506** — **DASHBOARD PERFIL POR TEMPO DE CASA · UX — Barra de progresso 0→100% no card "Analisando perfis dos funcionários...".** User (iPad): "Na tela de carregamento, quero de 0 a 100%" + screenshot do dashboard "Análise de Perfil por Tempo de Casa" com botão "Analisando..." sem feedback visual de quanto falta. Causa: chamada ao Claude (`dashboards.analiseIAPerfil`) leva 15-40s sem indicador → usuário acha que travou. Anthropic API não expõe progresso real, então solução padrão: progresso simulado assintótico que sobe rápido no início, desacelera perto de 95%, completa em 100% no `onSuccess`. Fix em `client/src/pages/dashboards/DashPerfilTempoCasa.tsx`: novo state `iaProgress` + `iaIntervalRef`; `onMutate` inicia interval de 250ms com curva `step = max(0.4, (95 - prev) * 0.04)` (~3pts/s no começo, satura em 95%); helper `stopIaProgress(100)` no `onSuccess` fixa em 100% e reseta pra 0 após 800ms; `onError` zera imediato; `useEffect` cleanup limpa interval no unmount. UI: bloco novo dentro do card roxo com header "Progresso" + valor `tabular-nums` (não pula) + `<Progress h-2 bg-purple-100>`. Imports novos: `Progress`, `useEffect`, `useRef`. Outros dashboards IA (`DashAvaliacaoFuncionarios`, `DashSinistralidade`) podem usar o mesmo padrão — fica como follow-up. Detalhe: `shared/changelog.ts`.
