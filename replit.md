...
- **Rev. 3666** — **PORTAL NACIONAL NFS-e + SEFAZ NF-e · BUGFIX PROBE "INESPERADO" + CRON LOG SILENCIOSO. BACKEND PONTUAL · ZERO SCHEMA/ALTER/DROP/DELETE.** (1) Probe retornava "inesperado" mesmo com cert válido: HTTP 403 sem cert, HTTP 404 com cert = endpoint alcançado → `authOk=true`. Try/catch separado captura apenas erros reais (HTTP 401/403); null (404) = auth OK implícito. Mensagem agora: "Certificado A1 aceito ✓". (2) Cron SEFAZ só logava quando havia empresas elegíveis; com empresa em cooldown (58min) o cron disparava sem nenhum log, parecendo "parado". Fix: log sempre, inclusive "0 elegíveis". Arquivos: `server/routers/nfseEmitidas.ts`, `server/routers/sefaz.ts`. Detalhe: `shared/changelog.ts`.

- **Rev. 3665** — **NOTAS FISCAIS · REDESIGN EMITIDAS + RECEBIDAS NFS-e — FONTE ÚNICA, SEM DUPLICATAS. 100% FRONTEND · ZERO BACKEND/SCHEMA.** Remove cronômetro NFS-e municipal + "REGRAS DE CONSULTA" (fontes mortas retornando 0). Banner limpo na aba Emitidas: histórico 2018–2025 importado, 2026+ → Importar PDF. `pdfInputRef` input movido para fora de condicionais (sempre no DOM). Header Recebidas NFS-e Serviços: botão vira "Importar PDF" (amber). Referências a `nfseInputRef` (nunca definida) substituídas por `pdfInputRef`. Detalhe: `shared/changelog.ts`.
