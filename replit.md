...
- **Rev. 3134** — **FINANCEIRO / ANÁLISE DE CUSTOS · O GRÁFICO "CUSTO POR MÊS" (E SEU DRILL-DOWN) PASSOU A BUCKETIZAR OS LANÇAMENTOS PELA DATA DE PAGAMENTO (REGIME DE CAIXA), NÃO MAIS PELA COMPETÊNCIA — UM ITEM DE COMPETÊNCIA 12/2025 PAGO EM 01/2026 AGORA CAI EM JANEIRO, NÃO EM "DEZEMBRO/2026".** PEDIDO (iPad, 2 prints): "o gráfico deve ler as datas de pagamentos, não de competência; se for competência não é dezembro de 2026, é de 2025." CAUSA-RAIZ: `mesNumDe` lia `dataCompetencia||dataVencimento` e extraía só o MÊS (ignorando o ANO), enquanto o dataset vinha de `getContasAPagarByYear` filtrado por ANO DO VENCIMENTO → dois critérios desalinhados → vazamento cross-ano (competência-dez de OUTRO ano caía no balde "Dez" do ano do gráfico). CORREÇÃO (caixa ponta a ponta; ZERO ALTER/DROP/DELETE/SCHEMA): (1) backend `getContasAPagarByYear` ganhou param OPCIONAL `baseData:"vencimento"|"caixa"` — default "vencimento" INTACTO p/ Contas a Pagar/Fluxo de Caixa; com "caixa" o filtro de ano vira `(status='pago' AND year(data_pagamento)=ano) OR (status<>'pago' AND year(COALESCE(venc,created))=ano)` (ATENÇÃO: `dbExecute` liga placeholders por ORDEM DE APARIÇÃO → empilha `ano` 2×); (2) Análise de Custos + drill-down passam `baseData:"caixa"` e `mesNumDe` usa DATA EFETIVA (`dataEfetivaDe`: pago→pagamento, em aberto→vencimento), alinhando dataset e balde — o vazamento some em TODOS os agregados que passam por `mesNumDe` (porMes, KPIs, tabela mensal, filtro de mês, categorias/fornecedores). RESSALVA: pago com pagamento e vencimento em anos distintos passa a aparecer no ANO DO PAGAMENTO (intencional). Detalhe: `shared/changelog.ts`.
