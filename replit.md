...
- **Rev. 3718** — **PACOTE CONTADOR + PLANILHA XLSX · BUGFIX "COLUMN DOES NOT EXIST" — codigo_servico → cd_lista_servico + fe.numero_nf → JOIN fiscal_notes. 100% BACKEND PONTUAL · ZERO SCHEMA/ALTER/DROP/DELETE.** `downloadPacoteContador.ts`: query NFS-e usava `codigo_servico` (inexistente); corrigido para `cd_lista_servico`. Query de cartão usava `fe.numero_nf` (inexistente em `financial_entries`); substituído por LEFT JOIN `fiscal_notes ON stmt_line_id`. `downloadContabilidadeXlsx.ts`: JOIN financial_entries usava `fe.numero_nf` e `fe.fornecedor_cnpj` (inexistentes); substituídos por LEFT JOIN `fiscal_notes ON stmt_line_id` com COALESCE dos campos corretos. Arquivos: `downloadPacoteContador.ts`, `downloadContabilidadeXlsx.ts`. Detalhe: `shared/changelog.ts`.
...
- **Rev. 3716** — **PACOTE CONTADOR · REESTRUTURADO NO PADRÃO PRONUS — XLSX BANCÁRIO + ESPELHOS HTML NFS-e + CARTÃO + PASTAS CORRETAS. BACKEND PONTUAL · ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
...
- **Rev. 3710** — **SEFAZ NF-e · BUGFIX "UNEXPECTED END OF JSON INPUT" — LIMITE 3 PÁGINAS NO syncNow + REMOVE AUTO-SYNC APÓS RESET NSU. BACKEND PONTUAL + FRONTEND · ZERO SCHEMA/ALTER/DROP/DELETE.** NSU foi resetado para `000000000000000`; ao clicar "Sincronizar SEFAZ" com backoff expirado, `syncNow` sem limite de páginas tentava baixar 200+ páginas (~1000s) → proxy da Replit cortava a conexão → body HTTP vazio → "Unexpected end of JSON input" no browser. Fix: `executarSyncNFe` recebe `maxPaginas` (default 20 para cron); `syncNow` chama com `{ maxPaginas: 3 }` (≤150 docs, ~15s máx). Frontend: `sefazResetNsuMut.onSuccess` não chama mais `sefazSyncMut` automaticamente (perigoso com NSU=0); novo toast "parcial" explica que o cron continua. Arquivos: `server/routers/sefaz.ts`, `FinanceiroNotasFiscais.tsx`. Detalhe: `shared/changelog.ts`.

- **Rev. 3709** — **SEFAZ NF-e · BACKOFF PROGRESSIVO PARA cStat=656 CONSECUTIVOS (1h→2h→4h). BACKEND PONTUAL + FRONTEND · ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.

- **Rev. 3708** — **CONCILIAÇÃO BANCÁRIA · BUGFIX ITENS VOLTANDO COMO PENDENTES APÓS CONCILIAR — refetchReport() FALTANDO EM 7 MUTATIONS. 100% FRONTEND · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** A query `getConciliacaoReport` tem `staleTime: Infinity` por design (Rev. 3478), portanto toda mutation que altera estado de conciliação precisa chamar `refetchReport()` explicitamente. O principal bug: `conciliarSugMut.onSuccess` (conciliação em lote via sugestões) não chamava `refetchReport()` — após conciliar N pares via IA, os lançamentos do ERP (coluna direita "No ERP, sem extrato") continuavam aparecendo como pendentes até o usuário clicar "Atualizar" manualmente. O mesmo problema afetava outros 6 `onSuccess`: `consolidarMut`, `desconsolidarMut`, `desconciliarMut`, `executarBulkDesconciliar`, `excluirLinhaMut`, `limparMut`, `limparExtratoMesMut`. Fix: adicionado `refetchReport()` + `setReportStale(false)` em todos os 7. Arquivo: `client/src/pages/financeiro/FinanceiroConciliacao.tsx`. Detalhe: `shared/changelog.ts`.

- **Rev. 3707** — **SEFAZ NF-e · BUGFIX RACE CONDITION — GATE ATÔMICO (UPDATE…RETURNING) SUBSTITUI SELECT→UPDATE SEPARADOS. BACKEND PONTUAL · ZERO SCHEMA/ALTER/DROP/DELETE.** Log mostrava entradas duplicadas no mesmo segundo (`24/06 07:00:00 × 2`): cron e startup run coincidiam quando o servidor reiniciava perto de um marco de :15min (ex.: restart 06:57 → cron 07:00, startup run 07:00 = mesmo instante). Ambos passavam pelo SELECT antes de qualquer UPDATE, gerando duas chamadas reais ao SEFAZ — a segunda retornava cStat=656 e bloqueava syncs por ~1h. Fix: substituída a sequência SELECT+UPDATE por `UPDATE … WHERE last_sync_at IS NULL OR last_sync_at < NOW()-cooldown RETURNING company_id`. Postgres garante que apenas 1 dos 2 processos simultâneos terá rowCount=1. Arquivo: `server/routers/sefaz.ts`. Detalhe: `shared/changelog.ts`.

- **Rev. 3706** — **MANIFESTAÇÃO DO DESTINATÁRIO · BUGFIX ENDPOINT ERRADO — MD-e DEVE IR AO AMBIENTE NACIONAL (AN), NÃO AO SVRS. BACKEND PONTUAL · ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.

- **Rev. 3705** — **MANIFESTAÇÃO DO DESTINATÁRIO · BUGFIX SEFAZ 242 "MENSAGEM SOAP INVÁLIDA" — 4 CORREÇÕES: node:crypto + indSinc + CÓDIGOS TROCADOS + cUF. BACKEND PONTUAL · ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
...
- **Rev. 3691** — **NFS-e EMITIDAS · BOTÃO "CONCILIAR MÊS" — MARCA TODAS AS NOTAS DO MÊS COMO CONCILIADA. BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.

- **Rev. 3690** — **SEFAZ NF-e · HORÁRIO HH:MM CONFIGURÁVEL + CRON A CADA 15 MIN. BACKEND ADITIVO + SCHEMA + FRONTEND · ZERO DROP/DELETE.** Nova coluna `sync_minuto`. Cron 30 min → 15 min. Gate HH:MM BRT. Detalhe: `shared/changelog.ts`.
...
- **Rev. 3683** — **DASHBOARD NF-e · ANÁLISE TRIBUTÁRIA COMPLETA (ISS/INSS/IRRF/PIS/COFINS/CSLL) + PERFIL DE ENTRADAS NF-e. BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** Novo endpoint `fiscalNotes.getAnalyseTributaria` agrega em paralelo (Promise.all): impostos NFS-e (iss_retido, inss, irrf, csll, pis+cofins+pis_cofins unificados, carga efetiva %), KPIs NF-e (ticket médio, fornecedores únicos, pendentes, com lançamento, menor/maior), evolução mensal de retenções (GROUP BY MONTH). Dois novos cards no DashNotasFiscais: "Carga Tributária NFS-e" (6 KPI cards coloridos + fluxo bruto→deduções→base ISS→retenções→líquido + barras distribuição + BarChart stacked mensal) e "Perfil das Entradas NF-e" (ticket médio, fornecedores únicos, extremos, barras de status). Arquivos: `server/routers/fiscalNotes.ts`, `client/src/pages/financeiro/dashboards/DashNotasFiscais.tsx`. Detalhe: `shared/changelog.ts`.

- **Rev. 3682** — **DASHBOARD NF-e · CARD "EVOLUÇÃO 5 ANOS — ENTRADAS × SAÍDAS" COM BARCHART AGRUPADO + TABELA a/a. BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** Novo endpoint `fiscalNotes.getMultiYearSeries` agrega totais anuais com SUM CASE WHEN por origem + GROUP BY EXTRACT(YEAR). Card no DashNotasFiscais: BarChart agrupado (verde=NF-e Recebidas, violeta=NFS-e Emitidas) + tabela Ano|NF-e+Δ%|NFS-e+Δ%|Saldo com rodapé "Total período" + destaque do ano corrente. Arquivos: `server/routers/fiscalNotes.ts`, `client/src/pages/financeiro/dashboards/DashNotasFiscais.tsx`. Detalhe: `shared/changelog.ts`.
...
- **Rev. 3680** — **NFS-e EMITIDAS · IMPORTAÇÃO HISTÓRICA XML + BUGFIX DEDUP ANO 2026. BACKEND PONTUAL + SCRIPT ONE-SHOT · ZERO ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
...
- **Rev. 3674** — **NFS-e EMITIDAS · IMPORTAR XML SIAP GEO EXPORT — MÚLTIPLAS NOTAS POR ARQUIVO (<nfse>/<nf>). BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** Novo `parseSiapGeoExportXml`, `importNfseXmlManual` com fall-through SIAP GEO → ABRASF. Detalhe: `shared/changelog.ts`.

- **Rev. 3673** — **NFS-e EMITIDAS · BUGFIX CAP 2025-12-31 SIAP GEO — PORTAL TEM NOTAS 2026 + RESET ROWS TRAVADAS + LOOP ATÉ ANO ATUAL. BACKEND PONTUAL + FRONTEND · ZERO ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.

- **Rev. 3671** — **NFS-e EMITIDAS · PROGRESSO 0–100% NO HISTÓRICO + CRONÔMETRO RATE-LIMIT + BUGFIX HTTP 404 SIAP GEO. BACKEND PONTUAL + FRONTEND · ZERO ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
...
- **Rev. 3622** — **PANORAMA FISCAL · MODO "ANO TODO" FUNCIONAL + BUGFIX ÍNDICE GERAL 17% FALSO. BACKEND PONTUAL + FRONTEND · ZERO ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
...
- **Rev. 3617** — **NF-e RECEBIDAS · DIALOG "ESPELHO FIEL" COMPLETO — SEÇÃO "DADOS DA NF-e" + "EVENTOS E SERVIÇOS" + DigestValue. BACKEND PONTUAL + FRONTEND · ZERO SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.

- **Rev. 3616** — **NFS-e EMITIDAS · UX PORTAL INDISPONÍVEL — BANNER VERMELHO + CTA "IMPORTAR PDF" NO ESTADO VAZIO. 100% FRONTEND · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** Detalhe: `shared/changelog.ts`.
...
- **Rev. 3607** — **NF-e RECEBIDAS · BUGFIX BOTÃO "HISTÓRICO COMPLETO" NÃO ABRIA DIALOG. 100% FRONTEND · ZERO BACKEND/SCHEMA/ALTER/DROP/DELETE.** `AlertDialog` estava dentro do bloco `{pageTab === "emitidas" && <>...</>}` → na aba Recebidas o dialog não existia no DOM, clicar mudava o estado mas nada aparecia. Fix: movido para fora de qualquer condicional de aba. Detalhe: `shared/changelog.ts`.

- **Rev. 3606** — **NF-e RECEBIDAS · BUGFIX CRÍTICO TELA VAZIA — COLUNA `origem` FALTAVA NO SCHEMA DRIZZLE + GATE DE TEMPO GERAL NO SEFAZ SYNC. BACKEND PONTUAL + SCHEMA · ZERO ALTER/DROP/DELETE.** (1) `fiscalNotes.origem` era `undefined` no schema Drizzle → `notInArray(...)` gerava `syntax error at or near "in"` em toda chamada de `fiscalNotes.list` → aba NFS-e Emitidas crashava; 115 notas recebidas existiam no banco mas a query morria antes de renderizar. Fix: 5 colunas adicionadas ao schema Drizzle (`origem`, `emitenteCnpj`, `emitenteNome`, `nsuSefaz`, `xmlPayload`). (2) Gate de tempo geral (58 min via `last_sync_at`) substitui gate restrito ao rate-limit anterior — bloqueia "Sincronizar Agora" mesmo sem cStat=656 prévio, evitando queimar cota SEFAZ por clicks repetidos. Detalhe: `shared/changelog.ts`.

- **Rev. 3605** — **NF-e RECEBIDAS · BACKFILL AUTOMÁTICO DE XML + PASSO A PASSO DA RECUSA. BACKEND ADITIVO + FRONTEND · ZERO ALTER/DROP/DELETE.** (1) Fix no sync NSU: nota já existente com `xml_payload=NULL` recebe UPDATE quando chega `nfeProc` (em vez de ser ignorada). (2) Novo endpoint `recuperarXmlsBackfill` usa `consChNFe` para buscar XMLs individualmente. (3) Botão âmbar "N notas sem XML — Recuperar" aparece no rodapé da tabela só quando há pendências. (4) `listNFeRecebidas` retorna `{ semXml, items }`. (5) Guia passo a passo na recusa (4 passos numerados + aviso A1 + botão renomeado). Detalhe: `shared/changelog.ts`.
