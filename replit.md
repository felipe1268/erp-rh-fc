...
- **Rev. 3184** — **FINANCEIRO / CONCILIAÇÃO BANCÁRIA · CORREÇÃO DE UX: O DROPDOWN "TOLERÂNCIA (DIAS)" AGORA ABRE SEMPRE LOGO ABAIXO DO CAMPO, EM VEZ DE "SALTAR" PRO MEIO/TOPO DA TELA.** PEDIDO (piloto FC FEV/2026): "revisa a caixa/diálogo dos dias a considerar — está abrindo no meio da tela e deveria ficar logo abaixo do campo. Arrume de vez." CAUSA-RAIZ: o `<Select>` de Tolerância usa o `SelectContent` shadcn/Radix com `avoidCollisions` ligado (default); como o campo fica no rodapé do card "Sugestões Automáticas de Conciliação" (perto do fim do viewport), sem espaço abaixo o Radix VIRAVA a lista de 9 opções (0,1,2,3,5,7,10,15,28) pra CIMA, subindo até o meio/topo da tela. SOLUÇÃO (FRONTEND-ONLY, ZERO BACKEND): nos 3 seletores de Tolerância (`FinanceiroConciliacao.tsx`, `FinanceiroConciliacaoWorkspace.tsx`, `FinanceiroConciliacaoPainel.tsx`) o `SelectContent` passou a fixar `position="popper" side="bottom" sideOffset={4} align="start" avoidCollisions={false}` — ancora SEMPRE logo abaixo e à esquerda do campo; sem espaço, a própria lista rola (já tem `max-h`+overflow). Isolado nos 3 selects (NÃO mexe no `select.tsx` global). ZERO SCHEMA/ALTER/DROP/DELETE · ZERO BACKEND. Detalhe: `shared/changelog.ts`.
