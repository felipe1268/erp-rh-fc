...
- **Rev. 3082** — **MEDIÇÕES · FECHAMENTO DO SESSION_PLAN (T003 + T007): (A) HISTÓRICO "JÁ MEDIDO" ACUMULADO POR CONTRATO NO LEVANTAMENTO DE CAMPO (CINZA, REFERÊNCIA P/ NÃO REMEDIR O MESMO ITEM) E (B) A ABA "MEDIÇÕES" DO CONTRATO DE TERCEIROS VIRA ESPELHO SÓ-LEITURA POR PADRÃO, COM TOGGLE "EDITAR NESTA ABA" E CTA P/ O MÓDULO DEDICADO `/terceiros/medicoes`.** Fecha as 2 tarefas adiadas na Rev. 3080. ZERO ALTER/DROP/DELETE/SCHEMA — T003 é só query READ-ONLY (sem DDL), T007 é FRONTEND-ONLY. **T003** `server/routers/medicao.ts`: nova query `getHistoricoQuantidades({contratoId, companyId, excluirCampoId})` soma `quantidade` dos contornos vinculados a item do orçamento em TODOS os OUTROS campos do MESMO contrato (tenant-guard por companyId em todas as subconsultas) → `{orcamentoItemId, quantidade}[]`. FE `MedicaoLevantamento.tsx`: `jaMedidoMap`/`jaMedidoLista`; dropdown de vínculo anexa " · já medido: <qtd> <un>" cinza; NOVO bloco lateral "Já medido neste contrato". OVERLAY visual cinza na planta DESCARTADO (cada campo tem PDFs próprios/pdfId distinto → sobreposição geométrica não confiável). **T007** `ContratoDetalhe.tsx` (MedicoesTab): state `modoEdicao` (default false; lê `?edit=1`); banner Eye/Pencil com "Abrir módulo de Medições" + toggle "Editar nesta aba"; gating por `modoEdicao` de Gerar Medição, botões de aprovação/rejeição/excluir, edição inline do % (`editavel`), coluna remover (`mostrarRemover`), `RetencoesSec` e `FdMedicaoPanel` (prop `readOnly`). Detalhe: `shared/changelog.ts`.

- **Rev. 3081** — **HUB DE MÓDULOS (HOME) · O CARD ÚNICO "MEDIÇÃO" FOI DESDOBRADO NOS DOIS MÓDULOS DEDICADOS: "MEDIÇÃO CLIENTE" (A RECEBER, % AUTO DO AVANÇO FÍSICO → `/medicao`) E "MEDIÇÃO TERCEIROS" (A PAGAR, LEVANTAMENTO + DIVERGÊNCIA + FD + APROVAÇÃO 3 NÍVEIS → `/terceiros/medicoes`).** PEDIDO (print iPad, home, Admin Master): "quero que apareça aqui os novos módulos de medições" / "tem a medição para terceiros e medição do cliente". A home `ModuleHub.tsx` mostrava só 1 card "Medição". IMPLEMENTAÇÃO (FRONTEND-ONLY, ZERO ALTER/DROP/DELETE/SCHEMA/BACKEND) em `client/src/pages/ModuleHub.tsx`: card "medicao" renomeado p/ "Medição Cliente · A Receber" (Ruler, teal); NOVO card "medicao-terceiros" → `/terceiros/medicoes` (Receipt, laranja). Novo campo opcional `permId?` no type `Module` — o id do card ("medicao-terceiros") difere do módulo de PERMISSÃO ("terceiros"), então o filtro resolve `permId = m.permId ?? m.id` e usa-o em `MODULE_DEFINITIONS.find` (gate de grupo + visibilidade) e em `canAccessModule`; `hubToConfigKey["medicao-terceiros"]="terceiros"` p/ o toggle de empresa. Admin Master vê ambos. Detalhe: `shared/changelog.ts`.
