...
- **Rev. 1826**: **Fechamento de Ponto · DIXI — botão verde "Substituir DIXI" (SEGURO) + diagnóstico ghost-records**. User (15/05/2026, screenshot do FullScreenDialog "Dados já Existentes" mostrando 7/7 funcionárias `Já importado` SEM nenhum import prévio): (a) bug — aviso "Já existem registros DIXI" aparece sem motivo; (b) feature — pediu 3ª tag "SUBSTITUIR TUDO, EXCETO MARCAÇÕES MANUAIS E APONTAMENTOS". **Investigação**: SQL read-only nos 2 ambientes (PROD+DEV) → ZERO rows em `time_records` pra esses 7 nomes/CPFs em qualquer mesReferencia. Screenshot deve ser de instância separada (repl local) ou réplica de leitura defasada — sem dado pra forensics direto. **Decisão (interview 3 perguntas, opção A)**: NÃO criar 3ª tag — `replace_all` ATUAL já preserva manuais (deleta só `fonte='dixi'` em dixiPonto L569 + insert pula dia se `fonte!='dixi'` em L606-609). Bug real era de UX: botão vermelho "Substituir Tudo" + ícone ArrowRightLeft + texto pequeno tranquilizador embaixo dava medo no RH. **Implementação**: (1) `client/src/pages/FechamentoPonto.tsx` L4533-4552 — card re-skin emerald: `border-red-200`→`border-emerald-200`, `ArrowRightLeft text-red-600`→`ShieldCheck text-emerald-600`, "Substituir Tudo"→"**Substituir DIXI** + Badge SEGURO", subtítulo agora EXPLÍCITO "Preserva marcações manuais e apontamentos de campo". (2) L4659-4732 — modal de confirmação repintado: gradient `from-red-700`→`from-emerald-700`, título "Confirmação de Segurança / Ação destrutiva"→"Substituir DIXI — Confirmação / Ação segura", bullets vermelhos alarmistas ("PERDER TODOS os ajustes")→bullets emerald ("O que acontece ao confirmar": substitui DIXI antigo, mantém manuais, mantém apontamentos, preenche dias em branco), card amber pequeno explicando que senha é dupla-checagem. Botão final `bg-red-600`→`bg-emerald-600` + ícone ShieldCheck. Senha **MANTIDA** (proteção contra clique acidental). (3) L27 — `ShieldCheck` adicionado ao import único de lucide-react (R-007). (4) `server/routers/fechamentoPonto.ts` L847-895 — diagnóstico extra no payload `previewDixi`: cada `registrosDetalhe[i]` agora inclui `obraId`+`obraNome`+`createdAt` (já tinha em mãos via `obrasList`/`timeRecords.createdAt`, sem query nova, 0 round-trips). Frontend ainda não mostra visualmente — fica disponível pra próxima iter mostrar "criado em DD/MM HH:MM via obra X" no expand do funcionário, permitindo identificar de qual import vieram batidas fantasma. Backend modes (`replace_all`/`selective`) e proteção `fonte!='dixi'` PRESERVADOS. Reversível em 3 edits. Zero schema/migration/DELETE. R-001/R-006/R-007/R-008/R-010 OK.
