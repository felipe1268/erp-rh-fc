...
- **Rev. 3159** — **USUÁRIOS E PERMISSÕES · NOVO CONTROLE DE ACESSO "ATIVO / DESLIGADO" POR USUÁRIO — DESLIGAR BLOQUEIA O LOGIN E DERRUBA A SESSÃO ABERTA NA HORA, SEM EXCLUIR O CADASTRO (DIFERENTE DA LIXEIRA/EXCLUIR).** PEDIDO (IMG_2099, iPad): "coloca um campo de status, ativo/desligado para não ter acesso ao sistema, para controle". O ERP já tinha EXCLUIR (lixeira via `users.deletedAt`), mas faltava REVOGAR acesso temporariamente mantendo usuário/permissões/vínculos/histórico p/ reativar. MUDANÇA: (1) SCHEMA `drizzle/schema.ts` — `users.status varchar(20) DEFAULT 'ativo'` + self-heal `[SyncSchema+]` `ADD COLUMN IF NOT EXISTS` (todos nascem 'ativo'); (2) LOGIN `routers.ts`·`loginLocal` — senha ok mas `status==='desligado'` → UNAUTHORIZED "Acesso desativado..."; (3) SESSÃO `server/_core/context.ts` — sessão aberta de desligado vira `user=null` (toda protectedProcedure rejeita → front volta pro /login, vale na hora); (4) BACKEND nova mutation `userManagement.setUserStatus` (admin only, não desativa a si mesmo, admin não mexe em admin_master, auditado); (5) FRONTEND `Usuarios.tsx` — badge "Desligado" na lista + `Switch` "Acesso ativo/desligado" no detalhe (confirma ao desligar). ZERO DROP/DELETE; só `ADD COLUMN IF NOT EXISTS`. Detalhe: `shared/changelog.ts`.

- **Rev. 3158** — **RH / COLABORADORES · OS CARDS DE RESUMO GANHARAM UM CARD "Sócio" — ANTES "Ativos" NÃO FECHAVA COM "CLT" + "PJ" PORQUE OS SÓCIOS (tipoContrato='Socio') FICAVAM INVISÍVEIS.** PEDIDO (IMG_2098, iPad): "verifique a quantidade total de funcionários nos ativos tá diferente se somar os CLT, PJ, férias...". Na FC (60002): Ativos=106, mas CLT(94)+PJ(9)=103 → faltavam 3, que são ATIVOS com `tipoContrato='Socio'` (já tinham filtro/SelectItem "Socio", mas sem card no resumo). ("Na Empresa"=124=312−163−25=Ativos+Férias+Afastados+Aviso+Reclusos+Licença já batia.) MUDANÇA: (1) BACKEND `server/db.ts`·`getEmployeeStats` — novo campo `socio` (e no early-return), do MESMO agrupamento `tipoContrato` já restrito a `status='Ativo'` que faz `clt`/`pj`; (2) FRONTEND `Colaboradores.tsx` — card "Sócio" (ícone `Award`, ciano) após "PJ", `filter:"Socio"` (já tratado em `displayEmployees`). Agora Ativos = CLT + PJ + Sócio (106=94+9+3). ZERO SCHEMA/ALTER/DROP/DELETE (só leitura agregada). Detalhe: `shared/changelog.ts`.
