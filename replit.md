...
- **Rev. 2152** — **UX/CLEANUP · Sessões FCSign canceladas deixam de poluir a Timeline Cronológica do Raio-X.** User: "Tão usando meu usuário para teste, zere. Minha timeline" (3 sessões canceladas de Termo de Responsabilidade geravam 4 eventos cada — 17 eventos pra 3 testes). Fix idiomático sem violar R-010: em `server/routers/controleDocumentos.ts` (~L2037), adicionado early-return `if (s.status==="cancelado") return;` no `fcsignRows.forEach` que popula timeline + removido bloco que empurrava evento "FCSign · Cancelado". Array `fcsignSessions` retornado segue incluindo canceladas (auditoria), mas a timeline passa a ignorá-las. **Dados em prod:** zero mudanças — sessões continuam no banco com status='cancelado'. **R-001/R-007/R-010:** OK.
