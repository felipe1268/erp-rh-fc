...
- **Rev. 3131** — **RH / COLETA DE CAMPO · QUANDO A COLETA ATINGE 100% DOS FUNCIONÁRIOS A COLETAR ELA AGORA SE FINALIZA SOZINHA (DÁ COMO "CONCLUÍDO" + FECHA O LINK / `ativo=0`) MESMO QUANDO A CONCLUSÃO NÃO VEIO DO ÚLTIMO ENVIO PÚBLICO.** PEDIDO (iPad, com print da tela "Coleta — VITRA/REVTE-CIVIL/..."): "Revise isso de forma que quando acabar tudo, fecha a coleta e dê como concluído e finalize a coleta." CAUSA-RAIZ: duas regras de conclusão desalinhadas em `server/routers/coletaRh.ts` — o badge "Concluído" de `listarSessoes` é DERIVADO ao vivo (`coletados >= totalAlocados`, universo = ativos-alocados ∩ respostas), mas o auto-close (`SET ativo=0`) só existia em `enviarResposta` e só dispara no ÚLTIMO ENVIO do link público. Se a coleta batia 100% por outro caminho (ex.: funcionário DESALOCADO/DESLIGADO encolhe o universo), o badge virava "Concluído" mas o link seguia ATIVO. CORREÇÃO (BACKEND-ONLY; ZERO SCHEMA/ALTER/DROP/DELETE): `listarSessoes` se AUTO-CURA em 3 etapas (payload nunca diverge do banco) — (1) 1ª passada calcula `concluida`/progresso e junta `finalizarIds` (`concluida` && `ativo=1`); (2) UM `UPDATE ... SET ativo=0 WHERE id IN (...) AND companyId IN (...) AND ativo=1 RETURNING id` (best-effort try/catch, idempotente, tenant-safe); (3) só os IDs RETORNADOS rebaixam `ativo:0` no payload. Se o UPDATE falhar, volta o `ativo` real e re-tenta no próximo list. O auto-close de `enviarResposta` (Rev. 2902) segue intacto. Detalhe: `shared/changelog.ts`.
