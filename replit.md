...
- **Rev. 2224** — **FIX/PARSER · Contrato puxava "R$ 3,20" quando salário base era "3.200" (formato BR de milhar).** Lilian: "está puxando 3,20 por hora, o correto é 3.200,00 p/mês". Cadastro: Salário Base "3.200" + Valor Hora "14,55". Bug em `client/src/lib/numeroExtenso.ts:17-37` `parseValor`: quando string tem APENAS ponto sem vírgula (`"3.200"`), assumia formato US → `Number("3.200")` = 3.2. Fix com novo ramo `else if (hasDot)`: heurística pt-BR — se o ÚLTIMO grupo após o último ponto tem **exatamente 3 dígitos**, é milhar BR (strip pontos); senão é decimal US. Casos: `"3.200"→3200`, `"3.20"→3.20`, `"3.5"→3.5`, `"1.234.567"→1234567`, `"1234.56"→1234.56`. Impacto: TODO doc client-side via `formatBRL`/`valorPorExtenso` (contratos, advertências, comunicados com valor monetário) passa a renderizar valor correto. **R-001/R-007/R-010:** N/A (só parser client-side).
