...
- **Rev. 2556** — **RH & DP · AVISO PRÉVIO / CUSTO DE DEMISSÃO EM MASSA · FÉRIAS VENCIDAS JÁ PAGAS NÃO INFLAM MAIS O CUSTO POR FUNCIONÁRIO.** User: no dashboard "Custo de Demissão em Massa" (CDM) o custo de cada encarregado contava FÉRIAS VENCIDAS QUE JÁ FORAM PAGAS como se estivessem em aberto, inflando o "Custo Total". CAUSA: a contagem de períodos vencidos usada no `periodosVencidosOverride` de `calcularRescisaoCompleta` filtrava só `status NOT IN ('concluida','cancelada','em_gozo') AND periodoAquisitivoFim < <cutoff>`, mas NÃO excluía os JÁ PAGOS — uma férias paga cujo status não migrou p/ concluida/em_gozo (ainda 'pendente'/'agendada'/'vencida'/'aguardando_pagamento', mas com `dataPagamento`) seguia contada como vencida em aberto, contradizendo a definição de "pago" da Rev. 1879 (status concluida/em_gozo OU `dataPagamento <= hoje`). FIX (não-destrutivo, só SQL de leitura): adicionado `AND ("dataPagamento" IS NULL OR "dataPagamento" > <cutoff>)` (mesma data de ref de cada query) nos 8 sites de contagem/listagem de vencidas — 1× em `dashboards.ts` (`vpCountByEmp`, cutoff `dataRef`) e 7× em `avisoPrevioFerias.ts` (batch LEFT JOIN `p.data_fim`; listagem `dataFimParaCalculo`; detalhe `dataFimAviso`; preview `input.dataDesligamento`; criação/atualização/reconciliação 3× `dataFim`) — preservando a paridade canônica CDM × modal Aviso Prévio (1:1). Override segue sempre do banco (BUG-001 Rev. 716). Zero schema. Zero ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
