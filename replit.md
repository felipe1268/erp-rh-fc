...
- **Rev. 1639**: **Baixa da Rescisão Complementar** no modal "Dar Baixa no Aviso Prévio". Antes só existiam 2 baixas (Rescisão oficial + Multa FGTS); funcionários como GIVALDO (que recebem complemento salarial "por fora", calculado em `previsaoRescisaoComplementar`) ficavam sem campo dedicado pra confirmar o pagamento dessa rubrica interna. (1) Schema: 4 colunas novas em `termination_notices` — `baixa_complementar_valor`, `baixa_complementar_data`, `baixa_complementar_por`, `baixa_complementar_obs` — garantidas via ColFix `v1639`. (2) Backend `darBaixa` agora aceita `tipo: 'rescisao' | 'fgts' | 'complementar'`, valida que a previsão complementar > 0 antes de permitir, persiste as 4 colunas, e a regra de **conclusão** considera as 3 baixas (`rescisaoOk && fgtsOk && complOk`, onde `complOk = !temComplementar || jáFeita`). `revertConcluido` e `revertAllConcluidos` limpam as 4 novas colunas. Select da `list` retorna os 4 campos novos. (3) UI: `darBaixaForm.tipo` virou união de 3 valores; modal mostra **3 cards no grid** (`grid-cols-3`) quando `previsaoRescisaoComplementar.total > 0`, ou 2 cards quando não há complementar. Card violeta com label "Rescisão Complementar — Uso interno — pago 'por fora'" e estimado já pré-calculado. Bloco "Baixas já registradas" mostra a 3ª linha com data/valor/responsável. Botão "Usar Estimado" e label do campo "Valor efetivo da..." se adaptam ao tipo selecionado. Footer "Confirmar Baixa Rescisão Complementar" idem. (4) `handleDarBaixa` define o `defaultTipo` como a primeira pendência na ordem rescisão → FGTS → complementar.
