...
- **Rev. 2665** — **PAINEL RH (`/painel-rh`) · CARD "FÉRIAS — PERÍODO AQUISITIVO": A FRASE PARA DE DAR A IMPRESSÃO DE "FÉRIAS VENCIDAS" — DEIXA CLARO QUE É O FUNCIONÁRIO COMPLETANDO MAIS UM ANO DE EMPRESA E ABRINDO UM NOVO PERÍODO DE FÉRIAS, NÃO PENDÊNCIA/ATRASO.** Pedido (usuário, print image_1780282815146): "3º período aquisitivo · 1d para vencer" passava a impressão de 3 férias vencidas; na verdade só indica que o colaborador completa 3 anos de empresa e o 3º período está prestes a ABRIR. Confirmado no `server/routers/homeData.ts` (bloco "ALERTAS DE FÉRIAS"): `periodoAquisitivo` = anos completos + 1 (ciclo a abrir) e `diasParaVencer` = dias até o aniversário de empresa (abertura do novo período / direito de 30 dias) — NÃO é prazo concessivo expirando nem férias atrasadas. Fix (SÓ CLIENT/UI; ZERO SCHEMA; ZERO SERVER; R-001/R-007/R-010): CLIENT `PainelRH.tsx` em 5 pontos do mesmo dado `feriasAlerta` — (1) card expandido: sublinha "{funcao} · {N}º período aquisitivo"→"{funcao} · vai abrir o {N}º período de férias (completa {N} ano(s) de empresa)", badge "{d}d para vencer"/"VENCIDO!"→"abre em {d}d"/"abre hoje"; (2) card compacto: chip "{N}º período"→"abre {N}º período" com `title` explicativo, contador "VENCIDO"→"abre hoje"; (3) sino de alertas: título "Férias VENCIDAS/Vencendo"→"Novo período de férias", descrição reescrita ("Vai abrir o {N}º período aquisitivo … Abre hoje./Em N dias."); (4) KPI card: "Férias a Vencer"→"Períodos de Férias a Abrir" + `extra` "Período vencido!/Vence em Nd"→"Abre … o {N}º período"; (5) badge da seção "{n} pendente(s)"→"{n} a abrir" (4 e 5 após review do architect). Cálculo, cores/urgência e link `/ferias` inalterados. esbuild EXIT 0 (`pnpm build`/`tsc` estouram OOM). Detalhe: `shared/changelog.ts`.
