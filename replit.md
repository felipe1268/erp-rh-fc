...
- **Rev. 2628** — **ANÁLISE DE EXPERIÊNCIA · TRANSPARÊNCIA DO CARTÃO DE PONTO: O MODAL "ANÁLISE" AGORA AVISA QUANDO O CARTÃO DE PONTO NÃO FOI IMPORTADO/FECHADO NO PERÍODO (EM VEZ DE EXIBIR "0 FALTAS / 100% ASSIDUIDADE" ENGANOSO), EXPLICA O CRITÉRIO USADO E MOSTRA OS DADOS BRUTOS DO CARTÃO QUE O ERP ANALISOU — TUDO RESPONSIVO.** Pedido (usuário): a análise parecia errada porque dizia "0 faltas/100%" para quem ainda não teve o ponto fechado; pediu transparência sobre o critério, poder VER os dados brutos do cartão analisado e uma tela responsiva pra validar. Fix ADITIVO (SOMENTE SELECT/LEITURA — ZERO ALTER/DROP/DELETE; R-001/R-007/R-010). CAUSA-RAIZ: em `employees.analiseExperiencia`, faltas/assiduidade só enxergam `time_records`; sem nenhuma linha (mês não fechado) faltas=0 e assiduidade cai no DEFAULT 100% — ausência de dados, não presença real. Impl.: `server/routers.ts` (`analiseExperiencia`) — novo objeto `cartao` no retorno {totalRegistros, semCartao, diasTrabalhados, diasComFalta, primeiro/último registro, `mesesNaJanela` (loop mês a mês início→hoje), `mesesComRegistro`, `mesesSemRegistro`, `detalhe` (linhas de ponto da janela ordenadas asc)} + `assiduidade.verificada`; motivos do veredito ganham alerta honesto quando sem cartão / meses faltando, e os positivos de assiduidade só disparam com cartão. `client/src/components/AnaliseExperiencia.tsx` — cards "Assiduidade"/"Faltas" mostram "N/D"+sub "sem cartão" (não 100%/0 em verde) quando não verificado; banner âmbar honesto; caixa azul explicando o CRITÉRIO; NOVA seção "Cartão de Ponto (dados analisados)" (tabela sm+ / cards no mobile, dias com falta em vermelho, empty-state); helper `fmtMes`. Validado: servidor reiniciado e recompilou limpo (tsx watch); Neon conectado. Detalhe: `shared/changelog.ts`.
- **Rev. 2627** — **DASHBOARD DE FUNCIONÁRIOS · NOVO PAINEL "TOTAL DE FUNCIONÁRIOS POR ANO": MOSTRA O QUADRO ATIVO AO FIM DE CADA ANO DESDE A FUNDAÇÃO DA EMPRESA (GRÁFICO DE BARRAS + CARDS POR ANO) E, AO CLICAR EM UM ANO, ABRE A TELA COM A LISTA COMPLETA DE QUEM ESTAVA ATIVO NAQUELE ANO — COM NOME E FOTO DE CADA UM.** Pedido (usuário): "Quero o total de funcionários de cada ano desde quando a empresa foi aberta e, ao clicar, aparecer a tela com todas as informações pertinentes para análise, com nome dos funcionários e fotos". Fix ADITIVO (SOMENTE SELECT/LEITURA — ZERO ALTER/DROP/DELETE; R-001/R-007/R-010). SEMÂNTICA: "ativo ao fim do ano" = ponto-no-tempo por DATAS (admitido até 31/12 do ano E sem demissão OU demitido só depois); range vai do 1º ano com admissão (MIN) até o ano corrente; o painel INDEPENDE do seletor "Ano de análise" (série histórica sempre completa). PARIDADE: o nº de cada ano usa a MESMA régua do drill `ativosAno` → a contagem do card casa com o tamanho da lista. Impl.: `server/routers/dashboards.ts` (novo `getDashFuncionariosHeadcountAnual(companyId, companyIds?)` com CTE `bounds`+`generate_series`; nova procedure `dashboards.funcionariosHeadcountAnual`; novo case `ativosAno` em `getDrillDown` com `make_date(ano,12,31)`+guard, adicionado a `HISTORICOS_MES`); `client/src/components/HeadcountAnualFuncionarios.tsx` (NOVO — `DashChart` de barras + grid responsivo de cards por ano, tudo clicável); `client/src/pages/dashboards/DashFuncionarios.tsx` (query + handler `openAnoDrill` não-gated → reusa `DrillDownModal` com nome+foto→Raio-X). Validado: servidor reiniciado e recompilou limpo (tsx watch); Neon conectado. Detalhe: `shared/changelog.ts`.
