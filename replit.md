...
- **Rev. 2597** — **PLANEJAMENTO · ABA "EFETIVO × IA" · PLANO DE ATAQUE ENXUTO: FICA SÓ O GUIA PASSO A PASSO + O PLANO TÁTICO + A LINHA DE BALANÇO; AS DEMAIS SEÇÕES NARRATIVAS DA "MESA DE GUERRA" SAEM DA TELA.** User: "só deixa o guia passo a passo" — em duas confirmações definiu o alcance: MANTER também a Linha de Balanço e o Plano Tático e REMOVER o resto. O "Plano de Ataque" do Simulador tinha acumulado ~16 seções (Rev. 2590–2596) e ficou poluído demais para o engenheiro. FIX (SÓ CLIENT — UI; ZERO SERVER/SCHEMA/ALTER/DROP/DELETE; prompt da IA em `iaCronograma.ts` INTOCADO — segue gerando os campos, só não são mais renderizados): `client/src/pages/planejamento/AnaliseEfetivoIA.tsx` (componente `PlanoAtaque`) — MANTÉM cabeçalho/Missão + badge de veredito, "Guia passo a passo", "Plano Tático" e a "Linha de Balanço" (gráfico por pavimento `LinhaBalancoPavimentoChart` + geral `LinhaBalancoChart`); REMOVE da renderização Centro de Gravidade + Princípio Guia, Frentes Críticas, Mesa de Guerra, Manobras, Realocação de equipes, Assertividade global, "Novo plano de Linha de Balanço" (texto), Processos/Automações, Cenários não óbvios, Absorção das férias, KPIs e Condições de vitória/Se piorar. Consts órfãos removidos e `temAlgo` recalculado só com as seções mantidas. Simulações antigas não precisam ser refeitas (campos seguem no JSON salvo, só não aparecem). Validado via esbuild client (exit 0) + grep zero refs órfãs. Detalhe: `shared/changelog.ts`.
