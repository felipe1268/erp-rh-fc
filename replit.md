...
- **Rev. 4607** — **FIX: QR DO CRACHÁ — "VERIFICAR APTIDÃO" CALCULA AO VIVO + LISTA DE DOCUMENTOS (LGPD-SAFE).** O QR mostrava "PENDENTE"/tudo vermelho p/ funcionário com documentação em dia (ex.: Anderson Braga Silva): `portalExterno.verificar.funcionario` lia a snapshot `employee_aptidao` (só populada por recálculo manual, defasada). Agora calcula AO VIVO de `asos`/`trainings` com as mesmas regras do recálculo oficial (deletedAt IS NULL + companyId do funcionário, validade >= hoje). Página pública passa a exibir cards ASO (tipo/vigência/validade) e Treinamentos (norma/nome/vigente/validade) — sem RG, CPF completo, nascimento ou atestados (LGPD). Arquivos: `server/routers/portalExterno.ts`, `client/src/pages/VerificarAptidao.tsx`. ZERO schema change.
- **Rev. 4606** — **UI: EMISSÃO DE CRACHÁS — RÉPLICA FIEL DA ARTE ENVIADA (CLT/PJ/TERCEIROS).** BadgePreview refeito como réplica da arte: paleta FIXA extraída da própria arte (navy `#0A1E3C` + laranja `#EE9803`) p/ todos os tipos; logos recortados da arte (`attached_assets/cracha_logo_navy.png` frente / `cracha_logo_white.png` verso, via `@assets`). Frente: header navy + curva com faixa laranja, foto circular sobreposta, nome caixa alta + ponto laranja + função entre filetes, linhas de dados ícone/rótulo/valor sublinhadas, marca d'água de construção — SEM QR/selo na frente (a arte não tem). Verso: logo + sublinhado laranja, QR emoldurado, slogan, rodapé navy com telefone (fallback (12) 3133-5504). `formatDateBR` sem `new Date` (fuso). Cartão 340×540; download/impressão inalterados. Arquivo: `client/src/pages/terceiros/Crachas.tsx`. ZERO schema/server change.
### 5 one-liners

- **Rev. 4605** — **FIX: TRAVAS ANTI-DUPLICIDADE — CHEQUE RE-CONCILIADO NÃO CRIA GÊMEO + ÍNDICE ÚNICO NAS PROJEÇÕES.** Trava nível 2 no `createEntry` (cheque nº limpo + valor + não-cancelado → CONFLICT c/ override); estorno cura órfãos; ColFix dedupa projeções e cria `uq_fin_entries_projecao` parcial. Detalhe em `shared/changelog.ts`. ZERO schema change (só índice).
- **Rev. 4604** — **FEAT: CONTRATO DE PRESTAÇÃO DE SERVIÇOS — FLUXO DE DUAS MEDIÇÕES EXPLÍCITO.** Cláusula 13ª declara as 2 medições do mês (NF até dia 10 → pgto dia 15; NF até corte 25 → pgto 5 dias após aprovação da NF); placeholders resolvidos nos 3 pontos; Neon template id 8 Vigente (v15). Detalhe em `shared/changelog.ts`. ZERO schema change.
...
Ver `replit-history.md` para revisões Rev. 4599 e anteriores.
