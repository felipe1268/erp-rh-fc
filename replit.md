...
- **Rev. 4578** — **UX: FLUXO DE CAIXA — REDESIGN DIDÁTICO NA ESTRUTURA DA LITERATURA.** Rewrite completo do render (computações preservadas): matriz na ordem do regime de caixa — Saldo Inicial do Mês → (+) Entradas → (−) Saídas (fixas/variáveis) → (=) Geração de Caixa → (=) Saldo Final do Mês (dark; vira o Inicial do próximo) → Margem % → linha âmbar de cheques (informativa). 5 KPI cards numerados contando a história (Começou com/Entrou/Saiu/Geração/Termina com), guia colapsável "Como ler?", insights DETERMINÍSTICOS (sem IA; Poka-Yoke nível 1 contra leitura errada: saldo final vermelho/azul, meses no vermelho, pior/melhor mês, fixas ≥50%, float, gap previsto×recebido >10%). `saldoIniMes/saldoFimMes` substituem o "Saldo Acumulado" (ini[0] = abertura das contas bancárias). Arquivo: `FinanceiroFluxoCaixa.tsx`. Review architect: Pass. ZERO schema/server change.
### 5 one-liners
...
Ver `replit-history.md` para revisões Rev. 4572 e anteriores.
