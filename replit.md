...
- **Rev. 1669**: **Programação Semanal FC — Realizado/Aderência da Curva S alinhados ao cutoff (Sex→Qui)**. Reportado: card "Semana 1" mostrava Previsto 1,41% / Realizado 0,00% / Aderência —, mesmo com linhas de atividade exibindo Real% 28%. Causa: `evmSemana` em `ProgramacaoSemanal.tsx` usava `acumAt(curvaRealizada, semIniStr)` onde semIniStr=Sex 01/05 (após Rev. 1667). Curvas são indexadas por Segunda; Mon 04/05 > Sex 01/05 → lookup `<= semIni` não capturava o ponto → realizado=0. Fix: trocado `semIniStr`/`semAntStr` por `semFimStr`/`semAntFimStr` (Quinta cutoff). Monday key sempre cai dentro de Sex→Qui, então `<= semFim` captura corretamente. Aderência volta a calcular. `client/src/pages/planejamento/ProgramacaoSemanal.tsx` ~L557-585.
