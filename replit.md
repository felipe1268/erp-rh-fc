...
- **Rev. 3212** — **FINANCEIRO / CONTROLE DE CHEQUES · OS 3 CARDS DE RESUMO (TOTAL DE CHEQUES / COMPENSADOS / FALTAM COMPENSAR) AGORA FICAM FIXOS TAMBÉM AO CLICAR EM "ANO TODO", MOSTRANDO O AGREGADO DO ANO INTEIRO (LABEL "RESUMO DE 2026 (ANO TODO)") EM VEZ DE SUMIREM (ANTES SÓ APARECIAM COM UM MÊS SELECIONADO).** PEDIDO (piloto FC): "quero estes cards fixos tbm.. quando eu clicar na opção Ano todo" + print. SOLUÇÃO — FRONT (`FinanceiroCheques.tsx`): bloco dos 3 cards deixou de ser gated por `mesSel != null` e renderiza SEMPRE; derivado `cardTotais` escolhe a fonte conforme a régua — mês selecionado usa `totaisMes` (resumo do mês); "Ano todo" usa `totais` (resumo do ANO, query `cheques.resumo({companyId,ano})` SEM mês, já existente p/ os cards do topo), normalizando a forma `{qtd,total,map}` (resumo do ano expunha `qtdGeral`/`totalGeral`). Título via `cardTitulo` + sufixo das labels via `cardEscopo` ("do mês"/"do ano"). Cards seguem sendo botões que filtram a lista por status (Rev. 3210). ZERO BACKEND · ZERO SCHEMA/ALTER/DROP/DELETE · só UI. Detalhe: `shared/changelog.ts`.

- **Rev. 3211** — **FINANCEIRO · NOVO MÓDULO "CONTROLE DE CARTÃO DE CRÉDITO" (CADASTRO DE CARTÕES + IMPORTAÇÃO DE FATURA LIDA POR IA + CLASSIFICAÇÃO POR OBRA/CENTRO DE CUSTO/CATEGORIA) E O GANCHO "FORMA DE PAGAMENTO = CARTÃO DE CRÉDITO" NO FORM DE LANÇAMENTOS. CARTÃO NÃO VIRA LANÇAMENTO (IGUAL CHEQUE).** Decisões FC: IOF/anuidade/encargos → CC "Administrativo/Financeiro" (sem obra); cartões PF NÃO viram FC auto (ALERTA regularização quando `tipoPessoa="PF"`). BACK (`server/routers/cartao.ts`, novo): CRUD cartões (`alertaPessoal` derivado), faturas (`resumoMensal` p/ régua + `excluirFatura`/`reverterLote`), itens (`classificarItem`), import IA (`importarPreview` dry-run + `importarConfirmar`) via `invokeGeminiVision`+`SCHEMA_FATURA` (fallback Anthropic); `assertCompanyAccess` + `assertAiModuleEnabled(companyId,"financeiro")`; registrado em `routers.ts`. GANCHO: `financial_entries` +3 colunas ADITIVAS (`cartao_id`/`cartao_parcelas`/`cartao_estabelecimento`) (schema + self-heal `ADD COLUMN IF NOT EXISTS`); `createEntry` grava só se `formaPagamento==="cartao_credito"`; `FinanceiroLancamentos.tsx` mostra bloco condicional. FRONT: tela `FinanceiroCartaoCredito.tsx` (abas Cartões/Faturas) + menu `financeiro-cartao` (`modules.ts`) + `App.tsx`. Tabelas `financial_cartoes`/`_faturas`/`_itens` já criadas inertes na Rev. 3210. ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
