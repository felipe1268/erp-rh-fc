...
- **Rev. 2610** — **CADASTRO · NOVA PÁGINA/MENU "GERENCIADORAS" — O CADASTRO DE GERENCIADORAS (NOME + LOGO + CNPJ/TELEFONE/E-MAIL) DEIXA DE EXISTIR SÓ COMO MINI-MODAL DENTRO DO FORM DE OBRA E GANHA UMA PÁGINA DEDICADA NO MENU LATERAL "CADASTRO", LOGO ABAIXO DE "CLIENTES".** Pedido (usuário, no Canvas, sobre o menu CADASTRO): "coloca aqui tbm o campo para cadastro da gerenciadora". A tabela `gerenciadoras` e o router tRPC (list/criar/atualizar/excluir) já existiam desde a Rev. 2606, mas só pelo combobox "+ Cadastrar" dentro de "Nova Obra" — não havia tela central. Implementação: `client/src/pages/Gerenciadoras.tsx` (NOVO, mesmo padrão de `Clientes.tsx`); `client/src/App.tsx` (lazy import + rota `/gerenciadoras`); `client/src/components/DashboardLayout.tsx` (item de menu "Gerenciadoras", ícone `Network`, seção "Cadastro"); permissão em `shared/modules.ts` (feature `cadastro-gerenciadoras`) + `sharedPaths` (visível p/ não-master). Validado: esbuild client (exit 0) + workflow reiniciado. Detalhe: `shared/changelog.ts`.
