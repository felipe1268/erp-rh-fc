...
- **Rev. 3129** — **PORTAL DO CLIENTE / AVALIAÇÕES (NPS) · "EXCLUIR SELECIONADOS" (EXCLUSÃO EM LOTE DE LINKS DE AVALIAÇÃO) PAROU DE FALHAR COM ERRO DE SQL — `ANY($2, $3::text[])` INVÁLIDO.** PEDIDO (iPad, com print): ao marcar 2 links e clicar "Excluir selecionados (2)" aparecia "Failed query: UPDATE cliente_avaliacao_shortlink SET deletado_em = NOW() WHERE company_id = $1 AND deletado_em IS NULL AND codigo = ANY($2, $3::text[]) RETURNING codigo". CAUSA-RAIZ: em `server/routers/portalExterno.ts` o `excluirLinksAvaliacao` usava `AND codigo = ANY(${input.codigos}::text[])`; o `sql` template do Drizzle EXPANDE um array JS em placeholders separados por vírgula (`$2, $3`), então virava o SQL INVÁLIDO `ANY($2, $3::text[])` (`ANY()` aceita UM array, não uma lista). A exclusão individual (`excluirLinkAvaliacao`, 1 código só) nunca quebrou. CORREÇÃO (BACKEND-ONLY; ZERO SCHEMA/ALTER/DROP/DELETE — exclusão lógica via `deletado_em`): trocado para `codigo IN (${input.codigos})`, que é EXATAMENTE o formato que a expansão de array do Drizzle gera (`IN ($2, $3)`) → SQL válido. Guard de tenancy (company_id) e gate Admin Master inalterados. Detalhe: `shared/changelog.ts`.

- **Rev. 3128** — **CONTROLE DE DOCUMENTOS / ABA "MAPEAMENTO" · A LEITURA DE ASOs POR IA DEIXOU DE "FALHAR EM MASSA" (ERAM ~73 FALHAS P/ 6 SUCESSOS) — AGORA RESISTE AOS ERROS TRANSITÓRIOS DA API DO GEMINI (429 COTA DO FREE-TIER / 503 "HIGH DEMAND") COM RETRY + RITMO (PACING) + SALVAGE DE JSON TRUNCADO.** PEDIDO (iPad, com print "Processando 10 de 94 · 1 sucesso · 8 falhas"): "está tendo muita falha na leitura dos ASOs, como melhorar para a leitura ser correta e coesa?" DIAGNÓSTICO (lendo os `erro_msg` reais de `aso_extracao_ia` no Neon): de 73 falhas, ~95% eram TRANSITÓRIAS — `429` (cota do free-tier, métrica `generate_content_free_tier_requests`) e `503` ("high demand"/UNAVAILABLE); + 1 `Unterminated string in JSON` (truncado por `maxTokens:2048`). CAUSA: (a) os 94 ASOs disparavam em RAJADA (`runBatch` sem pausa) estourando o limite/minuto; (b) `invokeGeminiVision` só dava retry em `429` (503/500 morriam na 1ª); (c) backoff ignorava o `retryDelay` da API; (d) maxTokens baixo. CORREÇÃO (FRONTEND + BACKEND DE IA; ZERO SCHEMA/ALTER/DROP/DELETE): `server/_core/llm.ts` retry agora cobre 429/500/502/503/504, `MAX_RETRIES` 3→5 e espera `max(retryDelay sugerido, backoff)` (helper `extrairRetryDelayMs`); `server/routers/controleDocumentos.ts` maxTokens 2048→4096 + `parseAsoIaLoose` com SALVAGE de JSON truncado (extração parcial revisável em vez de "Falha"); `client/.../ControleDocumentos.tsx` `runBatch` ganhou RITMO (~3,5s entre leituras) + até 3 tentativas/ASO em erro transitório. RESSALVA: a chave Gemini está no FREE-TIER — retry+pacing reduzem muito, mas cota DIÁRIA esgotada exige chave paga (ou ANTHROPIC/OPENAI). Detalhe: `shared/changelog.ts`.
