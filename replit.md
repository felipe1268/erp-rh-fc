...
- **Rev. 3100** — **MEDIÇÃO / LEVANTAMENTO DE CAMPO · A ÁREA DE DESENHO SOBRE A PLANTA (PDF) GANHA "OSNAP" ESTILO AUTOCAD (OBJECT SNAP): AO MARCAR/CONECTAR PONTOS, ELES "GRUDAM" NAS GEOMETRIAS NOTÁVEIS DOS CONTORNOS JÁ DESENHADOS — EXTREMIDADE, PONTO MÉDIO, INTERSEÇÃO, NÓ/CENTRO, PERPENDICULAR E PRÓXIMO (SOBRE A LINHA).** PEDIDO: "criar funções de OSnap como no AutoCAD para os pontos grudarem nas geometrias notáveis dos contornos existentes". SOLUÇÃO (FRONTEND-ONLY, ZERO BACKEND/ALTER/DROP/DELETE/SCHEMA): `MedicaoLevantamento.tsx` ganha motor de Object Snap — helpers de módulo (`SnapKind`, `OSNAP_DEFS`, `SNAP_PRIO`, `OSNAP_TODOS`, `toolUsaSnap`, `projetarNoSegmento`, `interseccaoSegmentos`); `useMemo snapData` coleta candidatos dos contornos da página + referência (Rev. 3093) + rascunho (vértices→extremidade, médios→ponto médio, contagem→nó, interseções cap ≤240 segs O(n²)); `applySnap(raw,fromPt?)` escolhe o melhor dentro de tolerância de **14px de TELA** (rect do `overlayRef` → alcance visual constante em qualquer zoom), Perpendicular usa último vértice como pé, Próximo projeta no segmento. Snap aplicado no `onTap` (ponto-a-ponto + calibração) e nos 2 cantos do Retângulo; hover do mouse mostra marcador SVG (#16a34a, `non-scaling-stroke`) por tipo. UI: botão "OSnap" (ímã) + Popover/Checkbox de modos; atalho **F3** alterna. Pontos seguem normalizados [0..1]; geometria/persistência inalteradas. RESSALVA/DRIFT: nenhuma — só a ENTRADA de pontos ganha o "imã". Detalhe: `shared/changelog.ts`.
