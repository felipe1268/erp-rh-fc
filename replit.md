...
- **Rev. 2697** — **FINANCEIRO · PAINEL FINANCEIRO / DASHBOARD CFO (`/financeiro` → "Painel Financeiro") · CORRIGIDO O DASHBOARD QUE APARECIA TODO ZERADO ("SALDO CONSOLIDADO R$ 0,00", "NENHUMA CONTA BANCÁRIA CADASTRADA", KPIs ZERADOS) MESMO HAVENDO 3 CONTAS BANCÁRIAS ATIVAS E LANÇAMENTOS.** Pedido (usuário, prints do Painel zerado + tela "Contas Bancárias" com 3 contas ativas): "está mostrando que não tem contas cadastradas mas já tem cadastro sim, faça a verificação". Causa-raiz: em `getDashboardExecutivo` (`server/routers/financial.ts`) DUAS das ~13 queries do `Promise.all` (topDespesas e receitaPorObra) estavam com o placeholder `$1` FALTANDO após `AND TO_CHAR(data_competencia,'YYYY-MM')=` (sobrou `=` + espaços + `GROUP BY`) → erro de sintaxe Postgres (42601); como todas rodam num único `Promise.all`, UMA rejeição derrubava o endpoint inteiro → `data` undefined no cliente → `bancos ?? []` + `kpis ?? 0` zeravam TUDO. A página "Contas Bancárias" (router folha, query independente) listava as 3 normalmente. Fix (SÓ SERVER; ZERO SCHEMA/CLIENT — R-001/R-007/R-010): recolocado `$1` nas duas queries. Validado contra Neon real (companyId 60002): bancos=3, queries OK. esbuild EXIT 0. Detalhe: `shared/changelog.ts`.
