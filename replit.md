...
- **Rev. 2480** — **EFETIVO DA OBRA (PLANEJAMENTO) · botão TRANSFERIR + REMOVER por linha — gerenciamento de alocação direto da aba do engenheiro.** User (IMG_1290): "Preciso poder fazer a transferência de efetivo por aqui... preciso ver tudo para definir as frentes de atividades". Status real (Aviso/Férias/Afastado/Licença) e badge CIPA já estavam OK após Rev. 2479 — os "0 Aviso/Férias/Afastados" do IMG_1290 são dado real dessa obra (não bug). Faltava a **ação inline** — antes o engenheiro tinha que sair pra `/obras/efetivo` pra mover funcionários. **Mudanças** (todas em `client/src/pages/planejamento/PlanejamentoDetalhe.tsx`, componente `EfetivoObraView`): (1) Nova coluna "Ações" na tabela "Lista de Funcionários" com 2 botões por linha CLT/PJ — **Transferir** (azul, `ArrowRightLeft`) abre modal com `<select>` de obra destino + motivo; e **Remover** (vermelho, `Trash2`) via `confirm()`. Terceiros ficam "—" (gerenciados no módulo Terceiros). (2) Modal de Transferência reusa `obras.allocateEmployee` (server já fecha alocação ativa anterior automaticamente — mesmo fluxo de `/obras/efetivo`). Query `obras.listActive` é **lazy** (enabled só com modal aberto). (3) Props expandidas em `EfetivoObraView` (`companyId?`, `obraId?`, `obraNome?`) vindas de `EfetivoObraTab` via `proj.obraId` / `proj.obraNome`. (4) onSuccess invalida `equipeObra` + `efetivoPorObra` + `funcionarios`. R-001/R-007/R-010 OK — só mutations existentes que já gravam histórico em `obra_funcionarios_eventos`. Detalhe: `shared/changelog.ts`.
...
- **Rev. 2474** — PAINEL RH · "Avisos Prévios em Andamento" ganham foto real ao lado do nome. Ver `shared/changelog.ts`.
