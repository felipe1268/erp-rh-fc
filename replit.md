...
- **Rev. 2768** — **FOLHA DE PAGAMENTO · "LISTA POR BANCO" + "GERAR REMESSA CNAB": AGORA AGRUPAM PELA CONTA DA EMPRESA PARA PAGAMENTO (CONTA SALÁRIO), NÃO MAIS PELO BANCO PESSOAL DO FUNCIONÁRIO.** Sintoma (usuário): na aba "Cálculo do Pagamento" → "Por Banco", a lista era separada pelo banco da conta PESSOAL do funcionário (`employees.banco`) e as colunas Agência/Conta mostravam a conta pessoal — mas a empresa paga todos por uma CONTA SALÁRIO específica (ex.: Caixa ag 0306 / cc 1596-0). Regras (confirmadas): (1) agrupar pela CONTA DA EMPRESA (`employees.contaBancariaEmpresaId` → `company_bank_accounts`), nunca a pessoal, mesmo sem conta-salário individual; (2) sem conta-empresa → grupo "Sem conta definida" (sem botão CNAB); (3) agrupar pela conta ESPECÍFICA (id → ag/cc), pois a empresa pode ter 2 contas no mesmo banco. Fix (SERVER+CLIENT; ZERO SCHEMA — coluna já existe): `simularPagamento` (`server/routers/payrollEngine.ts`) traz `contaBancariaEmpresaId` + carrega `company_bank_accounts` num Map e anexa `contaEmpresaId/Banco/CodigoBanco/Agencia/Conta/Tipo/Apelido` a cada funcionário do `pagamentoResultJson`; a subview `por_banco` (`client/src/pages/FolhaPagamento.tsx`) agrupa por `contaEmpresaId`, exibe rótulo apelido/banco + "Ag • Cc", colunas mostram a conta-empresa, grupo "Sem conta definida" sem botão, e o botão envia `gerarRemessaCnab({contaBancariaId, codigoBanco})`; `gerarRemessaCnab` filtra por `contaBancariaEmpresaId===contaBancariaId` e usa a conta da empresa no favorecido (identificação por CPF), com fallback legado por banco pessoal. HARDENING (architect): `gerarRemessaCnab` ganhou guard de tenancy (`getCompaniesForUser`+`FORBIDDEN`, antes era IDOR), o branch por `contaBancariaId` agora exige `ativo=1 AND deletedAt IS NULL`, e o cliente envia `Number(meta.id)` (guard `Number.isFinite`). RESSALVA: meses já simulados antes desta rev. caem em "Sem conta definida" até resimular. Validação: servidor sobe limpo; HMR ok; architect PASS (`tsc` full estoura memória no ambiente). Detalhe: `shared/changelog.ts`.
