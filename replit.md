...
- **Rev. 2680** — **FUNCIONÁRIOS TERCEIROS (`/funcionarios-terceiros`) · AGORA DÁ PRA CLICAR NO FUNCIONÁRIO (NOME/CARD OU BOTÃO "RAIO-X") E VER UM RAIO-X COMPLETO READ-ONLY COM TODA A DOCUMENTAÇÃO — SEM PRECISAR ENTRAR NO "EDITAR".** Pedido (usuário, print IMG_1485): "clicar no terceiro e ver o raio-x completo dele, com toda documentação, sem precisar clicar no editar". Antes só dava pra ver os docs abrindo Editar → aba Documentos. Diagnóstico: `terceiros.funcionarios.list` faz `select().from(funcionariosTerceiros)` (todas as colunas) → cada item já traz os campos de doc + `documentosExtras`, então o Raio-X read-only é montado no cliente sem query nova. Fix (SÓ CLIENT/UI; ZERO SCHEMA/SERVER): `client/src/pages/terceiros/FuncionariosTerceiros.tsx` — extraí as seções pra helper `getSecoesTerceiro()` (fonte única, reusada no editor E no Raio-X); novo `RaioXTerceiroDialog` (FullScreenDialog read-only) com cabeçalho do funcionário + painel de Status de Integração (% conformidade/vencidos/a vencer) + seções com links "Ver documento" e validade em texto, sem upload/editar/remover; estado `viewFunc` + abertura por clique no nome do card (role=button + Enter/Espaço) e por botão "Raio-X" (ícone Eye); botão "Editar" interno faz ponte pro `openEdit`. Import add `Eye`. esbuild EXIT 0. Detalhe: `shared/changelog.ts`.
- **Rev. 2679** — **CONTROLE DE DOCUMENTOS (`/controle-documentos` → aba "ASO") · A LISTA DE ASOs PASSA A SER AGRUPADA POR FUNCIONÁRIO: O EXAME VIGENTE EM DESTAQUE E OS ANTERIORES (SUBSTITUÍDOS) VIRAM UM "HISTÓRICO" RECOLHÍVEL — EM VEZ DE REPETIR O MESMO FUNCIONÁRIO VÁRIAS VEZES.** Pedido (usuário): "agrupa tudo no mesmo funcionário mas, com controle de histórico". Antes os substituídos apareciam como linhas soltas (mesmo funcionário ocupava várias linhas). Fix (SÓ CLIENT/UI; ZERO SCHEMA/SERVER): `client/src/pages/ControleDocumentos.tsx` — nova memo `groupedAso` (agrupa `filteredAso` por `employeeId` em `{atuais, historicos}` via `isHistorico`), estado `expandedAsoEmps`/`toggleAsoEmp` por funcionário, `tbody` itera os grupos com helper `renderAsoRow`; última linha "atual" ganha botão "Ver histórico (N)"/"Ocultar" (chevrons); históricos vêm `opacity-60`, rótulo "Histórico", sem foto/nome, numeração só na 1ª linha do grupo. Imports add `ChevronDown`/`ChevronRight` + `Fragment`. esbuild EXIT 0. Detalhe: `shared/changelog.ts`.
