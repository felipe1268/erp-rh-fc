...
- **Rev. 2648** — **RH & DP / GERAL · A FOTO AMPLIADA (LIGHTBOX DO `PersonPhoto`) PARA DE SER CORTADA AO ABRIR — CABEÇA E PÉS VOLTAM A APARECER INTEIROS, EM TODA TELA QUE TEM FOTO DE PESSOA (FÉRIAS, COLABORADORES, OBRA EFETIVO, SST, ETC.).** Pedido (usuário): "ajuste a foto, tá cortando quando aumenta e não pode" (prints da tela Controle de Férias). CAUSA-RAIZ: o lightbox usa `position: fixed; inset-0`, mas era renderizado DENTRO da árvore da página; quando QUALQUER ancestral tem `transform`/`filter`/`backdrop-filter`/`will-change`/`contain` (ex.: wrappers com `animate-in`, blur de dialog), esse ancestral vira o "containing block" do `fixed` → o overlay deixa de ser relativo à viewport e fica PRESO/RECORTADO na caixa do ancestral (foto clipada no topo/base, faixas escuras nas laterais). Fix (SÓ CLIENT/UI; R-001/R-007/R-010 — ZERO SCHEMA/SERVER): `client/src/components/PersonPhoto.tsx` — overlay renderizado via `createPortal(..., document.body)` (react-dom), devolvendo o `fixed` à VIEWPORT (escapa de ancestrais com transform/filter) → foto COMPLETA (object-contain, sem corte). Validado (estático): `pnpm build` exit 0. Detalhe: `shared/changelog.ts`.
