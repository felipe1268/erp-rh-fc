...
- **Rev. 2903** — **COLETA DE CAMPO (RH) — O FORMULÁRIO PÚBLICO SÓ DEIXA ENVIAR QUANDO TODOS OS DADOS SOLICITADOS ESTÃO PREENCHIDOS.** Pedido: "Só pode deixar salvar a coleta quando o usuário preencher todos os dados solicitados, se não tiver preenchido não pode deixar salvar." Antes o botão "Enviar" liberava com QUALQUER campo (ou só foto) → chegavam coletas pela metade. REGRA: só envia com TODOS os campos dos grupos habilitados + TODOS os itens custom + foto (se grupo "foto" ativo) preenchidos; ÚNICA exceção `complemento` (opcional); nome do auxiliar segue opcional. SOLUÇÃO (ADITIVA — ZERO schema/ALTER/DROP/DELETE): `shared/coletaCampos.ts` vira fonte única (`CAMPOS_COLETA_OPCIONAIS`, `LABEL_CAMPO_COLETA`, `camposObrigatorios`, `camposFaltantesColeta`, `coletaCompleta`); `client/src/pages/portal/ColetaCampoPublica.tsx` `podeEnviar` exige `camposFaltantesColeta(...).length===0` + aviso âmbar listando o que falta; backend `enviarResposta` troca o check "≥1 campo ou foto" pela MESMA validação (rejeita "Preencha TODOS os dados solicitados antes de enviar."). ZERO ALTER/DROP/DELETE. Detalhe: `shared/changelog.ts`.
