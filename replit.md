...
- **Filtro de desligados em Efetivo por Obra (May 2026)**: `getEfetivoPorObra` e `getObraFuncionarios` (server/db.ts) agora filtram funcionários com `status IN ('Desligado', 'Lista_Negra', 'Inativo')` ou `deletedAt NOT NULL`. Antes, se o `obra_funcionarios.isActive` não fosse desativado junto com a demissão, o funcionário continuava aparecendo na contagem da obra e nos cards de status. Agora a exclusão é garantida na query de leitura, independente do estado da alocação. `getFuncionariosSemObra` já filtrava corretamente.
