...
- **Rev. 2162** — **NOVO CAMPO · Vincular Categoria ao Plano de Contas (contábil) via `conta_pai_id`.** User: "preciso ter um campo para vincular a CATEGORIA ao PLANO DE CONTAS". Schema: zero ALTER — coluna `conta_pai_id` já existia (self-FK, usada antes só pra hierarquia interna do plano), reaproveitada. **Backend** (`financial.ts`) — `createAccount` já passava `contaPaiId` no INSERT; `updateAccount` ganha `contaPaiId: number.nullable.optional` no Zod + branch `parts.push("conta_pai_id=$N")`. **Frontend** (`FinanceiroCategorias.tsx`) — `Categoria` type + `FormState` + `INITIAL_FORM` ganham `contaPaiId`; nova `useQuery getAccounts({escopo:"plano", ativo:true})` pra popular select; `<select>` "Plano de Contas (opcional)" no dialog (options indentadas por `nivel`, ordenadas por `ordem`); `openEdit` carrega valor existente; `handleSave` envia em create+update; lista exibe badge indigo "› 4.4.2 · Aluguel de Veículos" abaixo do nome quando vinculado. **R-001/R-007/R-010:** OK.
