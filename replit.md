...
- **Rev. 3106** — **FINANCEIRO / ANÁLISE DE CUSTOS · O MODAL "EDITAR LANÇAMENTO" (DRILL "POR CENTRO DE CUSTO") FOI REDESENHADO — MAIS MODERNO E FÁCIL DE PREENCHER — E PAROU DE CORTAR/SOBREPOR O TEXTO DO DROPDOWN DE CATEGORIA/CENTRO DE CUSTO (QUE APARECIA CLIPADO À ESQUERDA E SOBRE OS BOTÕES EM iPad ~768px).** PEDIDO: "melhore este layout... não pode ter textos cortados ou sobrepostos" (print IMG_2036). CAUSA: Categoria e Centro de Custo num `grid-cols-2` (selects estreitos); o `SelectContent` do shadcn usa `align="center"` + `min-w-[8rem]` sem truncar/quebrar o texto dos itens → nomes longos estouravam a largura, o popper centralizava sobre o gatilho estreito e vazava p/ a ESQUERDA (cortado pelo viewport) e por baixo (cobrindo o rodapé). SOLUÇÃO (FRONTEND-ONLY, ZERO ALTER/DROP/DELETE/SCHEMA/BACKEND) em `client/src/pages/financeiro/FinanceiroAnaliseCustosDetalhe.tsx`: (1) Categoria/Centro saem do grid e ocupam LARGURA TOTAL, empilhados num cartão "Classificação" (`rounded-xl border bg-gray-50/60`) — gatilho largo = popper largo = nomes longos cabem; datas (Competência/Vencimento) seguem em 2 colunas. (2) `SelectContent` ganha `align="start"` (ancora à esquerda, não vaza) + `max-w-[calc(100vw-2rem)]` + `max-h-72`; cada `SelectItem` ganha `whitespace-normal leading-snug` → nomes longos QUEBRAM em 2 linhas em vez de cortar. (3) Polimento moderno: alturas h-9→h-10, labels `font-medium text-gray-600`, placeholders de exemplo e `DialogDescription` no cabeçalho. O componente `ui/select.tsx` NÃO foi tocado (align/whitespace por instância). NENHUMA mudança na lógica de gravação. Detalhe: `shared/changelog.ts`.
