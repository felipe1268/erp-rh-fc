...
- **Rev. 2753** — **CONFIGURAÇÕES · CENTRAL DE DOCUMENTOS: A GERAÇÃO POR IA AGORA MOSTRA BARRA DE PROGRESSO 0–100% E NUNCA PASSA DE ~1 MINUTO.** Pedido (Felipe): "precisa ter o percentual de 0 a 100% para saber como está a evolução... quero tbm que não demore mais de 1 minuto". A geração por IA (modal "Novo Documento" — abas assunto/PDF — e editor dos 7 fixos) só tinha spinner indefinido e nenhum teto de tempo. Fix (SERVER+CLIENT; ZERO SCHEMA; ZERO ALTER/DROP/DELETE — R-001/R-007/R-010): (1) `server/routers/systemDocumentTemplates.ts` — NOVO `withTimeout(promise, 58_000)` corre a IA contra timeout de 58s (folga p/ o limite de 60s do proxy) e rejeita com "A IA demorou mais de 1 minuto..."; aplicado em `iaGerarDoZero` (invokeLLM) e `iaLerPdfSugerir` (invokeAnthropicVision); `iaGerarDoZero` passou a usar `fast: true` (caminho rápido Gemini Flash, fallback Claude) p/ cortar latência. (2) `client/.../TemplatesDocsTab.tsx` — NOVO hook `useIaProgress(active)` + componente `IaProgressBar` (shadcn `Progress`): como a mutation tRPC NÃO é streaming, anima ESTIMATIVA rumo a 95% em ~55s (ease-out) e crava 100% ao terminar (sucesso/erro); aparece no modal e no editor com rótulo dinâmico. Ressalva: o progresso é estimativa visual (sem streaming real); o garantido é o TETO de ~1 min. Validação: esbuild parse client+server EXIT 0; `vitest server/rescisao.test.ts` 41/41 verde. Detalhe: `shared/changelog.ts`.
