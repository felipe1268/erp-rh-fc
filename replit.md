...
- **Rev. 2979** — **PORTAL DO CLIENTE → NPS → "LINK DE AVALIAÇÃO (SEM LOGIN)" → BOTÃO "WHATSAPP" — O LINK QUE CHEGAVA NO WHATSAPP NÃO ERA IGUAL AO LINK COPIÁVEL ("não está vinculado a uma obra"); CORRIGIDO REMOVENDO EMOJI/MARKDOWN DA MENSAGEM E PONDO O URL COMO ÚLTIMA LINHA.** SINTOMA (usuário): "o link que vai no WhatsApp não está igual ao link que posso copiar… ele diz que não tá vinculado". DIAGNÓSTICO: copiar e WhatsApp usam a MESMA string (`url`); a diferença é o detector de links do WhatsApp — emoji (😊/🤝 = pares surrogados UTF-16) e `*markdown*` ANTES do URL desalinham o cálculo de offset e o app captura um token TRUNCADO/deslocado → `jwt.verify` falha e o backend não acha `obraId` → "não vinculado". SOLUÇÃO (FRONT-only, ZERO ALTER/DROP/DELETE, `ClientesPortalAdmin.tsx`): mensagem reescrita em texto PLANO (sem emoji, sem `*`) e com o URL na ÚLTIMA linha (nada depois dele) → o link compartilhado fica idêntico ao copiável. Detalhe: `shared/changelog.ts`.
