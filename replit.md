...
- **Rev. 2132** — **HOTFIX FCSign · `pendingForCurrentUser` retornava zero silenciosamente: `sql\`... = ANY(${'${array}'})\`` no Drizzle não serializa `number[]` como PG array.** User: "O alerta não tá funcionando, recarreguei e ele não apareceu". Mesmo com Rev. 2128/2130/2131 (server por PAPEL + gate relaxado + modal), Felipe não recebia o alerta da sessão Lilian. **Causa:** template `sql\`${'${signatureSessions.companyId}'} = ANY(${'${allowedCompanyIds}'})\`` vira placeholders quebrados → zero matches. Outros routers (processosCivis, smo, purchaseRouter) usam `inArray()` corretamente. **Fix em `server/routers/signatures.ts`:** import `inArray`, 3 trocas (companyId/sessionId/id) + `status IN (...)` → `inArray(status, [...])` + `IS NULL` → `isNull()` + log diagnóstico temporário `[FCSign.pendingForCurrentUser]` p/ confirmar via workflow logs. **R-001/R-007/R-010:** OK — só refator de query, zero DDL.
- **Rev. 2131** — **FCSign · alerta global agora é POPUP MODAL bloqueante que reabre a cada navegação enquanto houver pendência (substitui/complementa o toast).** User: "Liliam assinou mas não recebi Alerta quando entrei na tela... precisa ter popup na hora que entrar pra não deixar passar nada" + "em qualquer tela de acesso ao ERP". **Causa UX:** toast Sonner no canto é discreto demais — Felipe não viu o aviso após Lilian assinar como 1ª. **Fix em `FCSignPendingAlertGlobal.tsx`:** adicionado `<Dialog>` shadcn que abre automaticamente quando `data.length > 0`, reabre a cada navegação (`useLocation` wouter) se o user fechou em "Lembrar mais tarde". `dismissedAtLocationRef` evita reabrir na mesma rota a cada refetch de 60s. Lista com botão "Assinar agora" por doc (nova aba). Toast Sonner mantido como redundância. **R-001/R-007/R-010:** OK — só client-side.
