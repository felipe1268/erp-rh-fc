...
- **Rev. 2143** — **Termo de Responsabilidade · novo campo "Quantidade" por item entregue.** User: "Falta colocar a quantidade do produto aqui" (screenshot do dialog com Descrição+Estado). Faz sentido — colaborador pode receber 2 capacetes, 5 conjuntos EPI, etc. **Mudanças em `client/src/components/TermoResponsabilidadeDialog.tsx`:** tipo `ItemEntregue` ganhou `quantidade: number`; `iniciarCompose`+`addItem` inicializam com `1`; UI grid passou de `[1fr_200px]` → `[1fr_90px_200px]` (Input type=number min=1 step=1 inputMode=numeric com clamp ≥1); `validar()` rejeita quantidade não-finita ou <1; tabela HTML do termo FCSign agora tem 4 colunas (#, Descrição, **Qtd.** 60px centro, Estado) e o `colspan` da linha de fotos foi ajustado 3→4. **R-001/R-007/R-010:** OK — só client-side.
