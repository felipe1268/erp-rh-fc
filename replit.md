...
- **Rev. 2618** — **DASHBOARD DE FUNCIONÁRIOS · SELETOR DE "ANO DE ANÁLISE" + NOVA TABELA COMPARATIVA ANUAL DE CONTRATAÇÕES x DESLIGAMENTOS (TRIMESTRE, SEMESTRE E ANOS ANTERIORES).** Pedido (usuário): "quero poder escolher o ano de análise e ter uma tabela comparativa dos anos anteriores… principalmente na tabela de contratações e demissão, comparada com o trimestre, semestre e anos anteriores". Fix ADITIVO (ZERO ALTER/DROP/DELETE — só SELECT; R-001/R-007/R-010): `DashFuncionarios.tsx` ganha state `anoAnalise` (default ano atual) + `<Select>` "Ano de análise" (últimos 7 anos) no cabeçalho, que alimenta a query mensal existente (`funcionariosComparativo`) E a nova query anual; título da tabela mês-a-mês passa a refletir o ano selecionado (antes preso ao ano corrente). NOVO procedure tRPC `dashboards.funcionariosAnual` + `getDashFuncionariosAnual` (`server/routers/dashboards.ts`): 1 query `UNION ALL` (adm/dem) agregando `EXTRACT(YEAR/QUARTER FROM dataAdmissao|dataDemissao)` p/ o ano-ref + 4 anteriores → T1–T4, 1º/2º semestre, total anual (respeita multi-empresa `_companyList` + `deletedAt IS NULL`). NOVO componente `client/src/components/ComparativoAnosFuncionarios.tsx`: cards-resumo (Contratações/Desligamentos/Saldo c/ variação % vs ano anterior) + 2 tabelas (Admissões e Demissões) Ano·T1·T2·T3·T4·1ºSem·2ºSem·Ano·var; cor da variação por `lowerIsBetter` (adm=false, dem=true); ano-ref destacado. Validado: esbuild client+server (exit 0); servidor recompilou limpo. Detalhe: `shared/changelog.ts`.
