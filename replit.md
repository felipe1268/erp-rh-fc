...
8. **Compras** (Rev.819) — 25 DB tables, 40+ tRPC endpoints, 8+ páginas: SC emergencial, cotações (portal fornecedor), OC com numeração configurável, recebimentos, AP integrada, realocação de verba, comissões, bridge financeiro. **Compras Inteligentes** (Rev.819): SC via EAP com explosão de insumos (2 modos: Via EAP Inteligente / Manual), saldo orçamentário em tempo real, consolidação automática de insumos, Meta×Real no mapa de cotação, histórico de preços por insumo, rastreabilidade SC→Cotação→OC, entregas programadas na OC. New schema fields on `comprasSolicitacoesItens` (insumoCodigo, composicaoCodigo, precoMeta, quantidadeServico, coeficiente, origemEap). New table: `compras_entregas_programadas`.
