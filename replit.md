...
- **Rev. 3095** — **MEDIÇÃO DE TERCEIROS · A LISTA "MEDIÇÕES REGISTRADAS" PASSA A SER ORGANIZADA POR MÊS E ANO, COM NAVEGADOR DE ANO (`< 2026 >`) E 12 CHIPS (JAN–DEZ), CADA UM COM UM "DOT" DE STATUS (AZUL = COM LANÇAMENTO · VERDE = CONSOLIDADO · CINZA = SEM DADOS), IGUAL À LEGENDA DO CONTAS A PAGAR.** PEDIDO: na tela "Medições de Terceiros", as "Medições registradas" vinham como lista plana sem noção de período; o usuário quer enxergar/filtrar por MÊS e ANO com a mesma legenda visual do financeiro. CAUSA-RAIZ: `client/src/pages/terceiros/Medicoes.tsx` só tinha `<Select>` de status + lista plana (`filtradas = filtroStatus === "todos" ? medicoes : ...`); cada medição já carrega `m.periodo` ("YYYY-MM") e `m.status`, mas nada agrupava por período. SOLUÇÃO (FRONTEND-ONLY, ZERO BACKEND/ALTER/DROP/DELETE/SCHEMA): (1) helper `periodoDe(m)` extrai `{ano,mes}` de `m.periodo` e, na falta, de `m.dataReferencia`; (2) estados `ano`/`mesSel` (default = mês/ano atuais) + `mesesStatus` (useMemo) calcula status do mês: "consolidado" (verde) = tem medição aprovada/paga; "lancamento" (azul) = tem medição não-consolidada; ausente (cinza) = sem dados; (3) `filtradas` (useMemo) filtra por mês/ano + status; (4) UI com navegador de ano (ChevronLeft/Right), legenda de 3 dots e grid `grid-cols-6 sm:grid-cols-12` de 12 chips — espelha o padrão inline de `FinanceiroContasAPagar.tsx` (o `MonthSelector.tsx` é só prev/next, sem legenda). RESSALVA/DRIFT: contadores globais (aguardando/divergência) seguem somando TODAS as medições (visão de fila), não escopados ao mês; medição sem período válido não cai em nenhum mês. Detalhe: `shared/changelog.ts`.
