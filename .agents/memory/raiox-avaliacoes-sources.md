...
- Obras geridas = `obras.responsavelId == employeeId` (não há FK de gestor em `clienteAvaliacoes`).
- Match = `obraId ∈ obrasGeridas` **OU** `ilike(gestorNome, nomeCompleto)` — `gestorNome` é um
  SNAPSHOT textual gravado no momento da avaliação. Use ilike SEM wildcard (igualdade
  case-insensitive) para não agregar homônimos por match parcial.
