...
- Rev. 4689: envio gera **2 lançamentos** quando a multa FGTS se aplica (tipo `empregador_*`/`rescisao_indireta`/`acordo_mutuo` 20%): RESCISÃO (`aviso_previo`, valor = total salvo MENOS multaFGTS do JSON) + FGTS (`aviso_previo_fgts`, link `financeiro_fgts_entry_id`, índice único parcial `uq_fin_entries_aviso_previo_fgts`). Multa sugerida usa `fgts_real` quando informado. Conclusão automática exige TODOS os lançamentos ativos quitados (fail-safe: erro na checagem = NÃO conclui). Justa causa/pedido de demissão = só RESCISÃO.
