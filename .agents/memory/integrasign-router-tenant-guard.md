...
**Update (ago/2026):** `getEnvelope` (expõe TOKENS de assinatura) e `reenviarNotificacao` (rotaciona token) agora chamam `assertIntegraSignCompanyAccess`. A rota pública `gerarBoletimPdfPublico` valida token + expiração + estados terminais (espelha `getDocumentoPublico`) antes de servir o PDF. Regra: toda rota pública por token deve replicar TODOS os gates do getDocumentoPublico, e `reenviarNotificacao` ROTACIONA o token — telas que exibem link precisam refetch após reenviar.
