...
- `_puxarFdAutomatico` (helper) cria 1 lançamento origem "auto" capado em `min(pendente, valorMedido − jáLançadoNaMedição)`; transação + advisory xact lock (478003, medicaoId) — idempotente. Chamado AUTOMATICAMENTE em gerarMedicao e recalcularMedicao (usuário não clica; exigência ago/2026) e pela mutation `puxarFdPendente` (fallback UI). Medido=0 → no-op (puxa no recálculo). Restante fica pendente pras próximas medições.
