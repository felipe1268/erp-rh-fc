...
- Receitas no Fluxo de Caixa: desde Rev. 3759 vêm de `financial.getContasAReceberByYear` (livro REAL de Contas a Receber, irmão de `getContasAPagarByYear`), NÃO mais da matriz `getContasReceberMatrix`. O endpoint já aplica `sqlNotProjecao()` quando a trava está ligada, e o memo de Receitas espelha o de Despesas (agrupa por `dataVencimento`, split client-side por `isProjecaoOrigem(origemModulo)`, sub-linha "já recebido em caixa" = `COALESCE(valorRealizado,valorPrevisto)` p/ status recebido*/pago). Motivo da troca: Despesas refletiam dinheiro real e Receitas só forecast (~R$200k vs ~R$2,59mi reais) → caixa não fechava com extrato. NOTA: a matriz `getContasReceberMatrix` (previsão de faturamento cronograma×orçamento) ainda existe e alimenta OUTRAS telas — não confundir.
