...
**Conciliação Bancária usa a MESMA engine read-only** (`_agruparConciliacao` em `financial.ts`, mapa `GRUP`): Frota (`frota_abastecimento`/`frota_manutencao` → frotaFornecedor), Parceiro (`parceiro_lancamento` → `lancamentos_parceiros`→`parceiros_conveniados`, coluna `parceiroFornecedor`) E **PJ (`pagamento_pj` → tipo "pj", a partir da Rev. 3261)** agrupam por FORNECEDOR/PRESTADOR + MÊS (`<tipo>|<forn>|<YYYY-MM>`), pois no extrato paga-se só o total mensal. PJ resolve o nome via 3 LEFT JOIN (`pj_payments`→`employees`/`pj_contracts`, coluna `pjFornecedor` = `nomeCompleto` || `razaoSocialPrestador`), rótulo "Pagamento PJ · NOME" (pílula índigo), fallback "Pagamento PJ (sem prestador)". Conciliação é N:1 (grupo inteiro × 1 linha do extrato via `itensIds`); entries seguem individuais. NÃO assume PIX — forma de pagamento é a definida pelo usuário.

**ATENÇÃO — esta nota já esteve ERRADA:** antes da Rev. 3261 ela afirmava que PJ JÁ agrupava na Conciliação, mas o ground-truth do mapa `GRUP` NÃO tinha `pagamento_pj` (cada pagamento caía solto no passthrough). Sempre confira o `GRUP` real antes de assumir que uma origem agrupa.

**REGRA (tenant-binding em getConciliacaoReport):** as JOINs por `origem_id` (`lp`/`pc`/`ff`/`fm`/`pjp`/`pjemp`/`pjc`) NÃO podem confiar só no `e.company_id=$1` do `financial_entries` — todo novo LEFT JOIN por `origem_id` deve amarrar `<tab>."companyId" = e.company_id` (tabelas parceiro/PJ usam `companyId` camelCase quoted; `financial_entries` usa `company_id` snake) ou um `origem_id` forjado/inconsistente vaza nome de fornecedor de outro tenant.
