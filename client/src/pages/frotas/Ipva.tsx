...
import {
  Receipt, Plus, Pencil, Trash2, CheckCircle2, XCircle, AlertTriangle,
  DollarSign, Car, Calendar, TrendingUp, Ban
} from "lucide-react";
...
export default function Ipva() {
  ...
  const batchMut = trpc.frotas.batchUpdateIpvaStatus.useMutation({
    onSuccess: (data) => {
      ipva.refetch();
  ...
  const filteredData = useMemo(() => {
    const data = ipva.data || [];
    if (filterStatus === "all") return data;
    return data.filter((r: any) => r.status === filterStatus);
  }, [ipva.data, filterStatus]);

  const stats = useMemo(() => {
    const data = ipva.data || [];
    const total = data.reduce((s: number, r: any) => s + parseFloat(r.valor_total || 0), 0);
    const pendente = data.filter((r: any) => r.status === "pendente");
    const totalPendente = pendente.reduce((s: number, r: any) => s + parseFloat(r.valor_total || 0), 0);
    const pago = data.filter((r: any) => r.status === "pago");
    const totalPago = pago.reduce((s: number, r: any) => s + parseFloat(r.valor_total || 0), 0);
    const isento = data.filter((r: any) => r.status === "isento");
    return { total, totalPendente, totalPago, qtdPendente: pendente.length, qtdPago: pago.length, qtdIsento: isento.length, qtd: data.length };
  }, [ipva.data]);
  ...
  const statusConfig: Record<string, { color: string; bg: string; icon: any }> = {
    ...
    isento: { color: "text-slate-500", bg: "bg-slate-50 border-slate-200", icon: Ban },
  };
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-2xl font-bold flex items-center gap-3">
                ...
                IPVA — Gestão Completa
              </h1>
              <p className="text-blue-100 mt-1 text-sm">Imposto sobre Propriedade de Veículos Automotores</p>
            </div>
            <Button onClick={openNew} className="bg-white text-[#1e3a5f] hover:bg-blue-50 font-semibold shadow-lg">
              <Plus className="h-4 w-4 mr-2" /> Novo IPVA
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card className="border-l-4 border-l-slate-400 shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium">Isentos</p>
                  <p className="text-xl font-bold mt-1 text-slate-500">{stats.qtdIsento}</p>
                </div>
                <div className="bg-slate-50 rounded-full p-2.5"><Ban className="h-5 w-5 text-slate-400" /></div>
              </div>
              <p className="text-xs text-muted-foreground mt-2">veículo(s) isento(s)</p>
            </CardContent>
          </Card>
        </div>
        ...
        {ipva.isLoading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-muted-foreground font-medium">Nenhum IPVA encontrado</p>
              <p className="text-sm text-muted-foreground/60 mt-1">Clique em "Novo IPVA" para cadastrar</p>
            </CardContent>
          </Card>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b">
                    ...
                    <th className="text-center p-3 font-semibold text-slate-600">Parcelas</th>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3 text-center">
                          <Badge variant="outline" className="font-bold">{r.ano_referencia}</Badge>
                        </td>
                        <td className="p-3 text-right font-bold text-slate-800">{fmt(r.valor_total)}</td>
                        <td className="p-3 text-center">
                        ...
                        <td className="p-3 text-center">
                          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-semibold ${sc.bg} ${sc.color}`}>
                            ...
                            {r.status === "pago" ? "Pago" : r.status === "isento" ? "Isento" : "Pendente"}
                          </div>
                        </td>
                        <td className="p-3 text-center font-medium">{r.parcelas || "—"}</td>
                        <td className="p-3 text-right" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-blue-50" onClick={() => openEdit(r)}>
                              <Pencil className="h-4 w-4 text-blue-600" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-red-50" onClick={() => { if (confirm("Excluir este IPVA?")) deleteMut.mutate({ id: r.id, companyId: cId }); }}>
        ...
        <Dialog open={batchDialogOpen} onOpenChange={setBatchDialogOpen}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {batchAction === "pago" ? (
                  <><CheckCircle2 className="h-5 w-5 text-emerald-600" /> Dar Baixa em {selectedIds.length} IPVA(s)</>
                ) : (
                  <><XCircle className="h-5 w-5 text-amber-600" /> Cancelar Baixa de {selectedIds.length} IPVA(s)</>
                )}
              </DialogTitle>
            </DialogHeader>
            {batchAction === "pago" && (
            ...
            <p className="text-sm text-muted-foreground">
              {batchAction === "pago"
                ...
                : "Os registros selecionados voltarão para o status PENDENTE e a data de pagamento será removida."}
            </p>
            <DialogFooter>
