...
import {
  ...
  TrendingDown, TrendingUp, BarChart3, Clock, Loader2, Car,
} from "lucide-react";
      ...
      <div className="space-y-6 pb-10">
        ...
        {consolidation.isLoading ? (
          ...
          <>
            ...
            {(pendingReg.data || []).length > 0 && !existing && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-amber-800 mb-1">
                        Atenção: {(pendingReg.data || []).length} veículo(s) com cadastro incompleto
                      </h3>
                      <p className="text-sm text-amber-700 mb-3">
                        A consolidação será bloqueada se algum destes veículos tiver lançamentos no período. Complete o cadastro em Frotas → Veículos.
                      </p>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                        {(pendingReg.data || []).map((v: any) => (
                          <div key={v.id} className="bg-white border border-amber-200 rounded-lg px-3 py-2 text-sm">
                            <div className="flex items-center gap-2">
                              <Car className="h-4 w-4 text-amber-600" />
                              <span className="font-medium text-gray-800">{v.placa}</span>
                              <span className="text-gray-500 text-xs">— {v.modelo}</span>
                            </div>
                            <p className="text-xs text-amber-600 mt-1">Falta: {v.camposFaltantes.join(", ")}</p>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <Card>
            <CardContent className="p-5">
              <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
                ...
                Postos Próximos à Região {pc?.regiaoLabel ? `(${pc.regiaoLabel})` : ""}
              </h3>
              {priceComparison.isLoading ? (
                <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-gray-400" /></div>
