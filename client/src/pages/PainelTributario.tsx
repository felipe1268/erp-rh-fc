...
const ESFERA_LABELS: Record<string, string> = {
  judicial: "Judicial", administrativo: "Administrativo",
};
...
export default function PainelTributario() {
  ...
  const { data: dashData, isLoading: dashLoading } = trpc.dashboards.tributario.useQuery(
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">Gestão de Processos Tributários e Fiscais — KPIs, Tributos e Audiências</p>
              </div>
            </div>
          </div>
          ...
          <>
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                <KpiCard title="Total de Processos" value={fmtNum(dashData.resumo.totalProcessos)} icon={Receipt} color="teal" onClick={() => navigate("/processos-tributarios")} />
                <KpiCard title="Processos Ativos" value={fmtNum(dashData.resumo.processosAtivos)} icon={AlertTriangle} color="amber" onClick={() => navigate("/processos-tributarios")} />
                <KpiCard title="Encerrados" value={fmtNum(dashData.resumo.processosEncerrados)} icon={FileText} color="green" onClick={() => navigate("/processos-tributarios")} />
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                ...
                <KpiCard title="Auto Infração" value={fmtBRL(dashData.resumo.totalAutoInfracao)} icon={ShieldAlert} color="amber" isMonetary />
                        ...
                        return (
                          <div key={i} className={`px-2.5 py-1.5 rounded-lg border text-xs ${urgente ? "bg-red-50 border-red-200" : "bg-muted/30 border-border/50"}`}>
                            <div className="flex items-center justify-between gap-1">
                              <span className="font-semibold truncate flex-1">{a.contribuinte}</span>
                            ...
                            <div className="flex items-center justify-between mt-0.5">
                              ...
                              <Badge className={`text-[9px] px-1 py-0 ${a.risco === "critico" || a.risco === "alto" ? "bg-red-100 text-red-700" : a.risco === "medio" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}>
                                {TRIBUTO_LABELS[a.tributo] || a.tributo}
                              </Badge>
                            </div>
                          </div>
                        );
                ...
                <Card>
                  ...
                  <CardContent className="px-4 pb-3">
                    <div className="space-y-2">
                      {dashData.porTributo.sort((a: any, b: any) => b.value - a.value).map((t: any, i: number) => {
                        const total = dashData.resumo.totalProcessos;
                        const pct = total > 0 ? Math.round((t.value / total) * 100) : 0;
                        return (
                          <div key={t.label}>
                            <div className="flex items-center justify-between mb-0.5">
                              <span className="text-xs text-muted-foreground">{TRIBUTO_LABELS[t.label] || t.label}</span>
                ...
                <Card>
                  ...
                  <CardContent className="px-4 pb-3">
                    <div className="flex items-center gap-4">
                      <div className="shrink-0">
                        <MiniDoughnut
                          data={dashData.porEsfera.map((e: any) => e.value)}
                          colors={[CHART_PALETTE[2], CHART_PALETTE[5]]}
                          size={72}
                        />
                      </div>
                      <div className="flex-1 space-y-2">
                        {dashData.porEsfera.map((e: any, i: number) => (
                          <div key={e.label} className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                              <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: i === 0 ? CHART_PALETTE[2] : CHART_PALETTE[5] }} />
                              <span className="text-xs text-muted-foreground">{ESFERA_LABELS[e.label] || e.label}</span>
                            </div>
            ...
            <Card>
              <CardHeader className="pb-2 pt-3 px-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" className="h-6 text-[10px] gap-0.5" onClick={() => navigate("/processos-tributarios")}>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b text-left">
                          ...
                          <th className="py-1.5 pr-2 font-medium text-muted-foreground hidden sm:table-cell">Contribuinte</th>
                          <th className="py-1.5 pr-2 font-medium text-muted-foreground">Tributo</th>
                      ...
                      <tbody>
                        {processosRecentes.map((p: any) => {
                          ...
                          return (
                            <tr key={p.id} className="border-b border-border/30 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => navigate("/processos-tributarios")}>
                              <td className="py-1.5 pr-2">
                                <span className="font-mono text-[10px]">{p.numeroProcesso}</span>
                                <span className="block sm:hidden text-[10px] text-muted-foreground truncate max-w-[120px]">{p.contribuinte}</span>
                              </td>
                              <td className="py-1.5 pr-2 hidden sm:table-cell">
                                <span className="font-medium truncate max-w-[150px] block">{p.contribuinte}</span>
                              </td>
                              <td className="py-1.5 pr-2">
                                <Badge className="text-[9px] px-1.5 py-0 bg-teal-100 text-teal-700 border-0">{TRIBUTO_LABELS[p.tipoTributo] || p.tipoTributo}</Badge>
                              </td>
                              <td className="py-1.5 pr-2">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  { label: "Processos Tributários", icon: Receipt, path: "/processos-tributarios", color: "text-teal-600", bg: "bg-teal-50" },
                  { label: "Dashboard Tributário", icon: BarChart3, path: "/dashboards/tributario", color: "text-purple-600", bg: "bg-purple-50" },
