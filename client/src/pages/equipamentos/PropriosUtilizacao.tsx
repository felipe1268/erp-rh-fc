...
// Análise de ociosidade, ciclos de saída/retorno, rankings e custo de depreciação parado.
...
import {
  ...
  Package, TrendingDown, DollarSign, Activity,
  BadgeDollarSign, Hourglass, Zap, ThumbsDown, Bell,
  Timer, Sun, Sunset, Sunrise, X, ExternalLink, HardHat, Wrench,
  Anchor, Check,
} from "lucide-react";
import { Link } from "wouter";
import { toast } from "sonner";
import { sugereFixo } from "./LocadosUtilizacao";
...
function Avatar({ nome, size = "md" }: { nome?: string | null; size?: "sm" | "md" }) {
  ...
  return (
    <span className={`rounded-full bg-blue-700 text-white font-bold flex items-center justify-center shrink-0 ${sz}`}>
      {initials(nome)}
    </span>
  );
}
      ...
      <div className="px-4 py-3 border-b bg-slate-50">
        <div className="relative">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Filtrar por nome…"
            className="w-full pl-9 pr-3 py-2 text-sm border rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
      </div>
...
function KpiCard({ icon, label, value, sub, tone, big = false, onClick }: {
  ...
  return (
    <div
      onClick={onClick}
      className={`relative ${colors.bg} border rounded-xl p-4 flex flex-col gap-2 transition
        ${isClickable ? "cursor-pointer hover:shadow-md active:scale-[0.98]" : ""}`}
    >
      {isClickable && <ExternalLink className="absolute top-3 right-3 h-3.5 w-3.5 text-slate-300" />}
      <div className={`h-9 w-9 rounded-lg flex items-center justify-center ${colors.icon}`}>{icon}</div>
      <div className={`${big ? "text-xl" : "text-2xl"} font-black tabular-nums leading-none ${colors.val}`}>{value}</div>
      <div className="text-[11px] font-semibold text-slate-600 uppercase tracking-wide leading-tight">{label}</div>
  ...
  return (
    <div className="bg-white border rounded-xl shadow-sm p-4">
      <h3 className="font-semibold text-slate-800 text-sm mb-3 flex items-center gap-2">
        ...
        {topQuem.length > 0 && <span className="ml-auto text-[10px] text-slate-400">{topQuem.length} pessoa{topQuem.length !== 1 ? "s" : ""}</span>}
      </h3>
      {topQuem.length === 0
          ...
          <ul className="space-y-2">
            {visivel.map((p, i) => (
              <li key={p.nome} className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-blue-700 tabular-nums shrink-0">{p.count}×</span>
              </li>
            ))}
          </ul>
          {topQuem.length > 5 && (
            <button onClick={() => setExpandAll(v => !v)}
              className="mt-3 w-full text-center text-xs text-slate-500 hover:text-slate-700 flex items-center justify-center gap-1">
              {expandAll ? <><ChevronUp className="h-3 w-3" /> Mostrar menos</> : <><ChevronDown className="h-3 w-3" /> Ver todos ({topQuem.length})</>}
            </button>
          )}
        </>
      }
...
export default function PropriosUtilizacao() {
  ...
  const regimeLote = trpc.equipamentos.regimeUsoAtualizarLote.useMutation({
    onSuccess: (r) => {
      ...
      utils.equipamentos.propriosUtilizacao.invalidate();
  ...
  // pendentes = saídas sem devolução (ciclos com devolvidoEm null)
  const pendentes = useMemo(() =>
    ciclos.filter(c => c.devolvidoEm === null).sort((a, b) => b.horasFora - a.horasFora),
  [ciclos]);

  const atrasados = useMemo(() =>
    pendentes.filter(c => c.horasFora > 16),
  [pendentes]);
  ...
  const ciclosFiltrados = useMemo(() => {
    ...
    return ciclos.filter(c =>
      ...
      c.codigoPatrimonio?.toLowerCase().includes(q)
    );
  }, [ciclos, busca]);
  ...
  const barColorHora = (h: number) =>
    h < 12 ? "#3b82f6" : h < 17 ? "#f59e0b" : "#6366f1";

  // Para drill "campo": usa emCampo (active loans); fallback: pendentes dos ciclos
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              ...
              Utilização — Equipamentos Próprios
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Ciclos de saída e retorno ao almox · Equipamentos parados geram depreciação sem retorno.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <KpiCard icon={<HardHat className="h-5 w-5" />} label="Em obra agora"
            value={isLoading ? "…" : (stats?.emCampoCount ?? 0).toLocaleString("pt-BR")}
            sub="toque para ver itens" tone="blue"
            onClick={() => setDrill("campo")} />
          <KpiCard icon={<Boxes className="h-5 w-5" />} label="Disponível (ocioso)"
          ...
          <KpiCard icon={<BadgeDollarSign className="h-5 w-5" />} label="Custo de ociosidade"
            ...
            sub="depreciação acumulada (parado)" tone="red" big
        ...
        {drill === "campo" && (
          <DrillModal
            titulo="Em obra agora"
            subtitulo={`${drillCampoItens.length} item${drillCampoItens.length !== 1 ? "s" : ""} fora do almox`}
            onClose={() => setDrill(null)}
          >
            {drillCampoItens.length === 0 ? (
              <div className="py-10 text-center text-sm text-slate-400">Nenhum item em obra no momento.</div>
            ) : drillCampoItens.map((c: any) => (
              <DrillItemCampo key={c.id} item={c} />
            ))}
          </DrillModal>
        )}

        {/* ── Drill-down: Disponível / Custo ── */}
        {(drill === "almox" || drill === "custo") && (
          <DrillModal
            titulo={drill === "custo" ? "Custo de ociosidade — ranking" : "Disponível (ocioso)"}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {topEquip[0] ? (
              <>
                ...
                <div className="mt-auto flex items-center justify-between">
                  ...
                  <span className="text-2xl font-black text-blue-600 tabular-nums">{topEquip[0].count}×</span>
                </div>
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Retornar hoje</span>
              {atrasados.length > 0 && (
            ...
            {atrasados[0] ? (
              ...
              <div className="text-xs text-amber-700 py-2">{pendentes.length} item(s) em campo, dentro do prazo.</div>
            ) : (
              <div className="text-xs text-blue-700 py-3 text-center font-medium">✓ Nenhum pendente!</div>
            )}
          </div>
        </div>
          ...
          <section className="bg-white border border-amber-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-3 bg-amber-50 border-b border-amber-100 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-800 text-sm">Pendentes de retorno ao almox</span>
                <span className="text-[11px] bg-amber-100 text-amber-700 ring-1 ring-amber-200 rounded-full px-2 py-0.5 font-semibold">
                  {pendentes.length}
                </span>
              </div>
              {atrasados.length > 0 && (
                <span className="text-xs text-red-600 font-semibold shrink-0">{atrasados.length} passou de 1 dia</span>
              )}
            </div>
                  ...
                  <li key={c.id} className={`flex items-center gap-3 px-5 py-3 ${atrasado ? "bg-red-50/30" : ""}`}>
                    ...
                    <div className="text-right shrink-0">
                      <div className={`font-black text-sm tabular-nums flex items-center gap-1 ${atrasado ? "text-red-600" : "text-amber-600"}`}>
                        <Clock className="h-3.5 w-3.5" />{fmtHoras(c.horasFora)}
                      </div>
            ...
            {pendentes.length > 5 && (
              <div className="border-t border-amber-100 px-5 py-2">
                <button onClick={() => setExpandPendentes(v => !v)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white border rounded-xl shadow-sm p-5">
            <h3 className="font-semibold text-slate-800 text-sm mb-1 flex items-center gap-2">
              <CalendarDays className="h-4 w-4 text-blue-600" /> Saídas por dia da semana
            </h3>
                  ...
                  <BarChart data={porDiaSemana} margin={{ top: 4, right: 4, left: -20, bottom: 4 }}>
                    ...
                    <Bar dataKey="count" radius={[4, 4, 0, 0]} maxBarSize={40}>
                      {porDiaSemana.map((d, i) => (
                        <Cell key={i} fill={d.count === maxDia ? "#2563eb" : "#dbeafe"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 bg-white border rounded-xl shadow-sm p-5">
            <h3 className="font-semibold text-slate-800 text-sm mb-4 flex items-center gap-2">
              <RotateCcw className="h-4 w-4 text-blue-600" /> Saídas por mês
            </h3>
            {mensal.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={mensal} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
                  ...
                  <RechTooltip formatter={(v: any) => [v, "Saídas"]} contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                  <Bar dataKey="count" radius={[4, 4, 0, 0]} fill="#2563eb" maxBarSize={48} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
                ...
                : <ul className="space-y-2">
                  {topEquip.slice(0, 5).map((e, i) => (
                    <li key={e.descricao} className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-blue-700 tabular-nums">{e.count}×</span>
                    </li>
                  ))}
                </ul>
              }
          ...
          <section className="bg-white border border-amber-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-3 bg-amber-50/80 border-b border-amber-100 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-semibold text-slate-900 text-sm">Depreciando parado no almox</div>
                      ...
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <div className="text-[11px] text-slate-500 tabular-nums">
                            {item.custoDiario > 0 ? `${fmtMoeda(item.custoDiario)}/dia` : "sem custo cadast."}
                          </div>
            ...
            <div className="divide-y divide-indigo-100/60">
              {instalados.map((item: any) => (
                <div key={item.id} className="flex items-center gap-3 px-5 py-3 hover:bg-indigo-50/40 transition">
                  ...
                  <div className="text-right shrink-0">
                    {item.custoMensalDepreciacao > 0 && (
                      <div className="font-bold text-slate-800 text-sm tabular-nums">{fmtMoeda(item.custoMensalDepreciacao)}/mês</div>
                    )}
                    <button
                      onClick={() => {
                        if (!confirm(`Voltar "${item.descricao}" para o regime rotativo? Ele voltará a contar na ociosidade.`)) return;
                        regimeLote.mutate({ companyId, tipo: "proprio", ids: [item.id], regimeUso: "rotativo" });
                      }}
              ...
              <>
                ...
                <div className="sticky bottom-0 bg-white border-t px-4 py-3 flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    onClick={() => regimeLote.mutate({ companyId, tipo: "proprio", ids: [...triagemSel], regimeUso: "fixo" })}
        ...
        {!isLoading && emAlmox.length === 0 && stats && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl px-5 py-4 flex items-center gap-3">
            <TrendingDown className="h-5 w-5 text-blue-600 shrink-0" />
            <div>
              <div className="font-semibold text-blue-900 text-sm">Nenhum equipamento parado!</div>
              <div className="text-xs text-blue-700 mt-0.5">Todos os equipamentos ativos estão em obra ou manutenção.</div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b flex items-center justify-between gap-3 flex-wrap">
            <h3 className="font-semibold text-slate-800 text-sm flex items-center gap-2">
              ...
              Histórico de ciclos
              {ciclos.length > 0 && (
                <span className="text-[11px] bg-blue-100 text-blue-700 rounded-full px-2 py-0.5 font-medium">{ciclos.length}</span>
              )}
            </h3>
                    ...
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {c.codigoPatrimonio && (
                            <span className="text-[10px] text-slate-500 border border-slate-200 rounded px-1.5 py-0.5 font-mono">
                              {c.codigoPatrimonio}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        {c.devolvidoEm ? (
                          <span className="inline-flex bg-blue-100 text-blue-700 rounded-full px-2 py-0.5 text-[10px] font-semibold mt-1">Devolvido</span>
                        ) : (
          ...
          {ciclosFiltrados.length > 10 && (
            <div className="border-t px-5 py-2">
              <button onClick={() => setExpandCiclos(v => !v)}
                className="w-full text-center py-1 text-xs font-medium text-blue-700 hover:bg-blue-50 rounded transition flex items-center justify-center gap-1">
                {expandCiclos
                  ? <><ChevronUp className="h-3 w-3" /> Mostrar menos</>
...
function DrillItemCampo({ item }: { item: any }) {
  const q = useDrillFilter();
  const nome = (item.descricao ?? "").toLowerCase();
  const quem = (item.quemTem ?? item.quemSaiu ?? "").toLowerCase();
  if (q && !nome.includes(q) && !quem.includes(q)) return null;
  const atrasado = (item.horasFora ?? 0) > 16;
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b last:border-0 hover:bg-slate-50">
      ...
      <div className="text-right shrink-0">
        <span className={`font-black text-sm tabular-nums ${atrasado ? "text-red-600" : "text-amber-600"}`}>
          {fmtHoras(item.horasFora ?? 0)}
        </span>
        <div className={`text-[10px] font-semibold mt-0.5 ${atrasado ? "text-red-500" : "text-amber-500"}`}>
          {atrasado ? "Atrasado" : "Em obra"}
        </div>
      </div>
    </div>
  );
...
function DrillItemAlmox({ item, idx }: { item: any; idx: number }) {
  const q = useDrillFilter();
  const nome = (item.descricao ?? "").toLowerCase();
  if (q && !nome.includes(q)) return null;
