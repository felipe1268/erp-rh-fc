      ...
      <Card className="border shadow-sm">
        <CardHeader className="pb-2 pt-4 px-4">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-[10px] font-normal text-gray-400 ml-1">(lançamentos realizados no financeiro)</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
          <div className="space-y-2">
            {[
              { label: "Receita Realizada", val: financeiro.receitaRealizada, icon: <TrendingUp className="w-3.5 h-3.5 text-green-500" />, color: "text-green-700" },
              { label: "Custo Realizado", val: financeiro.custoRealizado, icon: <TrendingDown className="w-3.5 h-3.5 text-red-500" />, color: "text-red-700" },
            ].map((row, i) => (
              <div key={i} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
                ...
                <span className={`text-sm font-bold ${row.color}`}>{fmt(row.val)}</span>
              </div>
            ))}
            <div className="flex items-center justify-between pt-2 border-t border-gray-200 mt-1">
              ...
              <span className={`text-base font-black ${financeiro.lucroRealizado >= 0 ? "text-green-600" : "text-red-600"}`}>
                {fmt(financeiro.lucroRealizado)}
              </span>
            </div>
            {financeiro.lucroRealizado < 0 && (
              <p className="text-[10px] text-red-500 flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" />Obra com resultado negativo — bônus não pode ser calculado sobre lucro negativo.
              </p>
            )}
          </div>
        </CardContent>
