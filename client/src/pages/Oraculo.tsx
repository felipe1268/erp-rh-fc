...
function OrbAnimated({ state }: { state: "idle" | "listening" | "thinking" | "speaking" }) {
  ...
  const ringClass  = state === "speaking" ? "ring-4 ring-fuchsia-500/50"
    : state === "listening" ? "ring-4 ring-pink-500/50"
    : state === "thinking" ? "ring-4 ring-indigo-500/40" : "";
  const emoji = state === "listening" ? "👂" : state === "thinking" ? "⏳" : state === "speaking" ? "🔊" : "🔮";

  return (
    <div className={`relative w-16 h-16 rounded-full bg-gradient-to-br ${colors[state] ?? colors.idle} ${pulseClass} ${ringClass} shadow-2xl shadow-violet-900/60 flex items-center justify-center transition-all duration-500`}>
...
export default function Oraculo() {
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#0e0a1a]">
  ...
  return (
    <div className="flex h-screen bg-[#0e0a1a] text-white overflow-hidden">

      {/* ── Sidebar de sessões ────────────────────────────────── */}
      <div className={`${sidebarOpen ? "w-64" : "w-0"} shrink-0 transition-all duration-300 overflow-hidden border-r border-violet-900/30 bg-[#130d24] flex flex-col`}>
        <div className="p-4 border-b border-violet-900/30">
          <div className="flex items-center gap-2 mb-3">
            <button
              onClick={() => setLocation("/")}
              className="text-violet-500 hover:text-violet-300 transition-colors p-1"
              title="Voltar ao Hub"
            >
        ...
        <div className="flex-1 overflow-y-auto py-2 px-2 space-y-1">
          {sessions.length === 0 && !listSessions.isLoading && (
            <p className="text-xs text-violet-500/60 text-center py-8 px-4">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center gap-3 px-4 h-12 border-b border-violet-900/30 bg-[#0e0a1a]/80 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-violet-500">
              {orbState === "listening" ? "Ouvindo..."
                : orbState === "thinking" ? "Analisando dados..."
                : orbState === "speaking" ? "Respondendo..."
          ...
          <button
            onClick={() => setLocation("/")}
            className="text-violet-600 hover:text-violet-400 transition-colors ml-1"
            title="Voltar ao Hub"
          >
            <ArrowLeft className="w-4 h-4" />
          </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-2xl w-full">
                {suggestions.map((s, i) => (
                  <button
                    ...
                    className="text-left p-3 rounded-xl bg-violet-900/20 border border-violet-800/30 text-violet-300/80 text-xs hover:bg-violet-900/40 hover:border-violet-700/50 hover:text-violet-200 transition-all"
                  >
                    <Sparkles className="w-3 h-3 mb-1.5 text-violet-500" />
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-3 items-end">
              ...
              <div className="flex-1 relative">
                <Textarea
                  ...
                  rows={1}
                  className="min-h-[44px] max-h-32 resize-none bg-violet-950/40 border-violet-800/50 text-violet-100 placeholder:text-violet-600/50 focus:border-violet-600 rounded-xl text-sm"
