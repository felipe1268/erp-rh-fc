...
const BATCH = parseInt(process.argv[2] || '40');
...
async function main(){
  const obras=await pool.query(`
    ...
    ORDER BY (SELECT COUNT(*) FROM diario_obra_relatorios r WHERE r.obra_id=o.id) ASC, o.id
  `,[COMPANY_ID]);
  ...
  for(const obra of obras.rows){
    ...
    if(!Array.isArray(allRels)||allRels.length===0)continue;

    if(Number(obra.db_count)>=allRels.length)continue;
    ...
    totalImp+=imp;
    const newCount=Number(obra.db_count)+imp;
    console.log(obra.nome+': +'+imp+(err?' err:'+err:'')+' | '+newCount+'/'+allRels.length);
    if(totalImp>=BATCH)break;
  }

  const total=await pool.query('SELECT COUNT(*) as c FROM diario_obra_relatorios');
  console.log('TOTAL:'+total.rows[0].c+' +'+totalImp);
  await pool.end();
}
